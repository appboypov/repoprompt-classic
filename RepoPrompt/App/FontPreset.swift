...
extension FontScalePreset {
	...
	/// Call `refreshCachedPreset()` to update after a Settings change.
	private static var _cachedCurrent: FontScalePreset = {
		let stored = UserDefaults.standard.double(forKey: SettingKeys.fontPresetBodySize)
		return FontScalePreset(rawValue: stored) ?? .normal
	}()
	...
	static var isDefaultPreset: Bool { _cachedCurrent == .normal }
	
	/// Refreshes the cached preset.  Call from Settings when the user picks a
	/// different body font size.
