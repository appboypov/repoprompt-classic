...
struct WindowSessionEntry: Codable, Sendable {
	...
	/// Per-window UI mode at capture time. Missing in legacy snapshots; use global fallback then.
	var uiMode: WindowUIMode?
	/// Sticky instance number for this workspace in this window (for deterministic restore)
	var workspaceInstanceNumber: Int?
}

struct WindowSessionCaptureCandidate: Sendable {
	let windowID: Int
...
class WindowStatesManager: ObservableObject {
	...
	private init() {
		...
		// Observe changes to keyboard shortcuts setting
		NotificationCenter.default.addObserver(
			self,
			selector: #selector(keyboardShortcutsSettingChanged),
	...
	@objc private func keyboardShortcutsSettingChanged() {
	...
	/// Applies the post-onboarding auto-restore default only when the user has never
	/// explicitly picked a value for this setting.
	static func applyAutoRestoreDefaultIfUnset() {
	...
	/// Workspace IDs that were present in the loaded snapshot but had no valid instance numbers.
	...
	func loadWindowRestoreSessionIfNeeded() {
		...
		Task { [weak self] in
			...
			await MainActor.run {
				...
				self.applyDeferredGlobalUIModeFallbackToUnassignedWindowsIfNeeded()
				#if DEBUG
				if let applyStartMS {
					WorkspaceRestorePerfLog.log(
	...
	private func preseedInstanceNumberState(from snapshot: WindowSessionSnapshot?) {
		...
		workspaceIDsMissingNumbersInSnapshot.removeAll()
		...
		for entry in snapshot.windows {
			...
			// Track all workspace IDs in snapshot
		...
		// Track workspace IDs that were in snapshot but had no valid instance numbers (v1 migration)
		let workspacesWithNumbers = Set(restoredInstanceNumbersByWorkspace.keys)
		workspaceIDsMissingNumbersInSnapshot = allWorkspaceIDs.subtracting(workspacesWithNumbers)
	}
	...
	private func applyNextRestoreEntryIfAvailable(to state: WindowState) {
		...
		#if DEBUG
		WorkspaceRestorePerfLog.log(
			"restore.entry assign windowID=\(state.windowID) workspaceID=\(WorkspaceRestorePerfLog.shortID(entry.workspaceID)) workspaceName=\(entry.workspaceName ?? "nil") isSystem=\(entry.isSystemWorkspace) uiMode=\(entry.uiMode?.rawValue ?? "legacy") remainingQueue=\(restoreQueue.count) registeredWindows=\(allWindows.count)"
		)
		#endif
	...
	private func applyDeferredGlobalUIModeFallbackToUnassignedWindowsIfNeeded() {
		guard restoreQueue.isEmpty else { return }
		for state in allWindows {
			state.applyDeferredGlobalUIModeFallbackIfNeeded()
		}
	}
	...
	func registerWindowState(_ state: WindowState) {
		...
		#if DEBUG
		WorkspaceRestorePerfLog.log(
			"restore.window registered windowID=\(state.windowID) registeredWindows=\(allWindows.count) pendingRestoreEntries=\(restoreQueue.count) initialMode=\(state.uiMode.rawValue)"
		)
		#endif
		...
		state.onFocusChanged = { [weak self] isFocused in
			guard let self = self else { return }
			self.updateKeyboardShortcutsState()
			self.persistWindowSession(reason: "focusChanged:\(isFocused)")
		}
		...
		state.workspaceManager.addWorkspaceDidSwitchListener(label: "windowStateManager") { [weak self, weak state] newWorkspace in
			...
			let number = self.recordWorkspaceSwitch(forWindowID: state.windowID, to: newWorkspace)
	...
	private func updateKeyboardShortcutsState() {
		...
		let shortcutsEnabled = UserDefaults.standard.bool(forKey: "enableKeyboardShortcuts") || !UserDefaults.standard.objectIsForced(forKey: "enableKeyboardShortcuts")
	...
	func mcpEnabledWindowIDs() -> [Int] {
		allWindows.filter { $0.mcpServer.windowToolsEnabled }.map(\.windowID)
	}
		...
		await withTaskGroup(of: Void.self) { group in
			for windowID in windowIDs {
				group.addTask { @MainActor [weak self] in
					guard let self, let ws = self.window(withID: windowID) else { return }
	...
	func recordWorkspaceSwitch(forWindowID windowID: Int, to workspace: WorkspaceModel?) -> Int? {
		...
		} else if workspaceIDsMissingNumbersInSnapshot.contains(wsID) {
			// 4) Workspace was in v1 snapshot without instance numbers - migrate by starting at 1
			assignedNumber = 1
			nextInstanceNumberByWorkspace[wsID] = 2
			workspaceIDsMissingNumbersInSnapshot.remove(wsID)
		} else {
			// 5) Completely new workspace (fresh install or newly created)
