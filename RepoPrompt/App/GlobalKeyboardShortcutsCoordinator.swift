...
final class GlobalKeyboardShortcutsCoordinator {
	...
	private func createNewComposeTab() {
		guard let win = guardedFocusedWindowState() else { return }
		Task { await win.promptManager.createBlankComposeTab() }
	}
	...
	private func focusAdjacentComposeTab(forward: Bool) {
		...
		Task { await win.promptManager.focusAdjacentComposeTab(forward: forward) }
	}
	...
	private func registerFileTreeShortcuts() {
		register(.cmdOptionC) { [weak self] in self?.toggleFileTreeCollapseAll() }
	}

	private func toggleFileTreeCollapseAll() {
		guard let win = guardedFocusedWindowState() else { return }
		NotificationCenter.default.post(
			name: .toggleFileTreeCollapseAll,
			object: win.fileManager,
			userInfo: ["windowID": win.windowID]
		)
	}
	...
	private func registerAgentAndIDEShortcuts() {
		register(.agentNewChat) { [weak self] in self?.startNewAgentSessionFromShortcut() }
		register(.toggleAgentIDEMode) { [weak self] in self?.toggleAgentIDEModeFromShortcut() }
		register(.toggleNavigationSidebar) { [weak self] in self?.toggleNavigationSidebarFromShortcut() }
	}

	private func toggleAgentIDEModeFromShortcut() {
		guard let win = guardedFocusedWindowState() else { return }
		win.toggleUIModeFromGlobalShortcut()
	}
