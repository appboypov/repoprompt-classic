...
class ApplicationSecurity {
	...
	private var lastBundleVerificationContinuous: Double? = nil
	...
	public static func enableAntiDebugging() {
#if !DEBUG
		_ = shared
		shared.stateQueue.async {
	...
	public static func softRestrictionsActive() -> Bool {
#if !DEBUG
		_ = shared
	...
	/// Exit immediately after a local security violation.
	private func terminateForSecurityViolation() -> Never {
		exit(0)
	}
		...
		verificationQueue.async { [weak self] in
			...
			guard isValid else {
				self.stateQueue.async {
					self.terminateForSecurityViolation()
				}
				return
			}
		}
