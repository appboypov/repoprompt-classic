...
final class FontScaleManager: ObservableObject {
    ...
    private init(store: GlobalSettingsStore = .shared) {
        self.store = store
	...
	func applyPersistedRawValueFromAppSettings(
		_ rawValue: Double,
	...
	func reloadFromPersistentStoreAfterExternalChange() {
		guard let rawValue = store.reloadFontScaleBodySizeFromDiskUpdatingLegacyMirror() else {
#if DEBUG
...
#endif
		reloadFromPersistentStoreAfterExternalChange()
	}
