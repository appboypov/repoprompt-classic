...
class NotificationService: NSObject {
	...
	/// Send a notification when discovery completes and tab is renamed
	/// - Parameters:
	///   - tabName: The new name of the tab
	///   - fallbackToDockBounce: Whether to fall back to dock icon bounce if notifications aren't authorized
	func notifyDiscoveryComplete(tabName: String, fallbackToDockBounce: Bool = true) {
		// Only notify if app is not active
		guard !NSApp.isActive else { return }
		
		if isAuthorized {
			sendDiscoveryCompleteNotification(tabName: tabName)
		} else if fallbackToDockBounce {
			// Fallback to dock icon bounce
			NSApp.requestUserAttention(.informationalRequest)
		}
	...
	/// Send the actual discovery complete notification
	private func sendDiscoveryCompleteNotification(tabName: String) {
		let content = UNMutableNotificationContent()
		content.title = "Discovery Complete"
