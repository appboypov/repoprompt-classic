...
extension Notification.Name {
	...
	/// Toggle "collapse/expand all" in the file tree for a specific window.
	/// `userInfo["windowID"]` should be the target window ID.
	/// `object` should be the window's `RepoFileManagerViewModel`.
	static let toggleFileTreeCollapseAll = Notification.Name("toggleFileTreeCollapseAll")
	...
	/// Posted when a delegate edit agent uses an MCP tool
	static let delegateEditToolUsed = Notification.Name("delegateEditToolUsed")
	...
	/// Posted when the UI should show the Prompts popover.
	/// userInfo: ["windowID": Int]
	static let showPromptsPopover = Notification.Name("showPromptsPopover")
	...
	/// Toggle the navigation sidebar for the focused window (IDE file tree or Agent sessions list).
	/// `userInfo["windowID"]` should be the target window ID.
