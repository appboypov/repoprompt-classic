...
class AppDelegate: NSObject, ObservableObject, NSApplicationDelegate {
	...
	func applicationDidFinishLaunching(_ notification: Notification) {
		...
		AppearanceController.shared.applyFromUserDefaults()
		
		// One-time MCP migration cleanup (clears stale connections from older versions)
		MCPFilesystemConstants.performMigrationCleanupIfNeeded()
		
		// One-time migration: consolidate legacy mcpPlanningModel into planningModel
		APISettingsViewModel.migrateLegacyMCPPlanningModel()
		...
		Task { [weak self] in
			...
			do {
				...
				guard isValid else {
					self.showSecurityViolationAlert(message: "Application integrity check failed. Please reinstall.")
			...
			// Only proceed with local security monitoring after verification succeeds
	...
	func applicationShouldTerminateAfterLastWindowClosed(_ sender: NSApplication) -> Bool {
		...
		return !MCPBackgroundModeCoordinator.shared.isBackgrounded
	}
