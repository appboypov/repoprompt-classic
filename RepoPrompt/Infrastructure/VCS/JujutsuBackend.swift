...
public actor JujutsuBackend: VCSBackendWithWarnings {
	...
	public nonisolated func normalizeCompareSpecWithWarning(_ spec: GitDiffCompareSpec) -> NormalizedCompareResult {
		...
		case .uncommitted(let base):
			...
			return NormalizedCompareResult(spec: .uncommitted(base: base), warning: nil)

		case .uncommittedMergeBase(let base):
			return NormalizedCompareResult(
		...
		case .revspec(let raw):
			// Keep revspec as-is and handle translation at execution time.
			return NormalizedCompareResult(spec: .revspec(raw), warning: nil)
		}
	}
	...
	public func getCommitInfo(ref: String, at repoURL: URL) async throws -> VCSCommitInfo {
		let trimmed = ref.trimmingCharacters(in: .whitespacesAndNewlines)
		guard !trimmed.isEmpty else { throw VCSError.parseError(message: "Empty ref") }
	...
	private func resolveDiffRefs(for compare: GitDiffCompareSpec, repoURL: URL) throws -> ResolvedRefs {
		switch compare {
		case .uncommitted(let base), .uncommittedMergeBase(let base), .staged(let base), .stagedMergeBase(let base):
	...
	private struct BookmarkListResult: Sendable {
		let names: [String]
		let isCacheable: Bool
	}

	private struct BookmarkSnapshot: Sendable {
		let all: [String]
		let current: [String]
	}
	...
	private func bookmarkSnapshot(repoURL: URL) async throws -> BookmarkSnapshot {
		...
		let task = Task { [weak self] in
			...
			let bookmarks = try await self.listBookmarks(repoURL: repoURL, mode: .all)
			let current = try await self.currentBookmarksPointingAtAt(repoURL: repoURL, bookmarks: bookmarks)
	...
	private func parseJjDiffStat(_ output: String) -> [String: (additions: Int, deletions: Int)] {
		...
		for raw in lines {
			...
			let additions = rhs.filter { $0 == "+" }.count
			let deletions = rhs.filter { $0 == "-" }.count
	...
	private func parseShowStat(_ output: String) -> (filesChanged: Int, insertions: Int, deletions: Int)? {
		...
		let filesChanged = extractInt(before: "file", in: lowered)
			...
			?? output.split(separator: "\n", omittingEmptySubsequences: true).filter { $0.contains("|") }.count
	...
	private func firstHexLikeToken(in text: String) -> String? {
		...
		for token in tokens {
			...
			if t.count >= 6, t.count <= 64, t.allSatisfy({ $0.isHexDigit }) {
				return t
			}
		}
		return nil
