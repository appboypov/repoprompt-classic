...
public enum VCSError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .notARepository(let path):
            return "Not a version control repository: \(path)"
        case .commandFailed(let command, let message):
            return "\(command) failed: \(message)"
        case .unsupportedOperation(let operation, let backend):
            return "Operation '\(operation)' is not supported by \(backend.rawValue) backend"
        case .parseError(let message):
            return "Failed to parse VCS output: \(message)"
        case .executableNotFound(let name):
            return "VCS executable not found: \(name)"
        }
    }
}
