...
struct GitDiffSnapshotStore {
	...
	struct GitDiffSnapshotRef: Hashable, Sendable {
	...
	struct LegacyPurgeResult: Sendable {
	...
	public func readManifest(workspaceDirectory: URL, snapshotID: String) throws -> GitDiffSnapshotManifest? {
	...
	private func listSnapshotEntries(in root: URL, allowLegacyPaths: Bool, repoKey: String? = nil) throws -> [SnapshotEntry] {
		...
		for case let fileURL as URL in enumerator {
			...
			guard let data = try? Data(contentsOf: fileURL),
				let manifest = try? decoder.decode(GitDiffSnapshotManifest.self, from: data) else {
				continue
			}
	...
	public static func encodeGitPathForPatchFileName(_ gitPath: String) -> String {
	...
	public static func normalizeSnapshotID(_ raw: String) -> String? {
		let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
		guard !trimmed.isEmpty else { return nil }
...
struct GitDiffMapBuilder {
	...
	static func build(_ inputs: Inputs) -> String {
		...
		let omittedCount = inputs.files.filter { $0.patchPath == nil }.count
		let binaryCount = inputs.files.filter { $0.additions == nil && $0.deletions == nil }.count
	...
	static func modeDetails(for mode: GitDiffPublishMode) -> String {
		switch mode {
		case .quick:
			return "quick: no patch text, summary-only artifacts"
		case .standard:
			return "standard: full diff patches (all.patch + per-file when size allows)"
		case .deep:
			return "deep: standard patches + hunk and changed-line indexes"
		}
	}
