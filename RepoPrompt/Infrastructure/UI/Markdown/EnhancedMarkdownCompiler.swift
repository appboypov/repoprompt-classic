...
struct EnhancedMarkdownCompiler: Markdown.MarkupVisitor {
	...
	private static let maxTextTableCharactersForInlineLayout = 50_000
	...
	private func attributes(font: NSFont? = nil,
							paragraphStyle: NSParagraphStyle? = nil,
							additional: [NSAttributedString.Key: Any] = [:]) -> [NSAttributedString.Key: Any] {
    ...
    mutating func visitParagraph(_ paragraph: Markdown.Paragraph) -> NSAttributedString {
		...
		if paragraph.hasSuccessorConsideringBlockElements {
			...
			let lf = isInListItem ? "\n" : "\n\n"
			result.append(NSAttributedString(string: lf,
    ...
    mutating func visitCodeBlock(_ codeBlock: Markdown.CodeBlock) -> NSAttributedString {
		...
		let font           = NSFont.monospacedSystemFont(ofSize: fontSize - 1,
															weight: .regular)
		...
		body.addAttribute(.codeBlockSource,
							value: code,
	...
	mutating func visitBlockQuote(_ blockQuote: Markdown.BlockQuote) -> NSAttributedString {
		let result = NSMutableAttributedString()
		let originalFontSize = self.fontSize
		// self.fontSize *= 0.9 // Optionally slightly smaller font for quotes

		let originalForceTextColor = self.forceTextColor
		...
		self.forceTextColor = originalForceTextColor

		if blockQuote.hasSuccessorConsideringBlockElements {
			result.append(NSAttributedString(string: "\n\n", attributes: attributes(font: NSFont.systemFont(ofSize: self.fontSize).rounded())))
		}
		return result
    }
    ...
    mutating func visitHeading(_ heading: Markdown.Heading) -> NSAttributedString {
		...
		case 1: headingFontSize = fontSize * 1.8
		case 2: headingFontSize = fontSize * 1.5
		case 3: headingFontSize = fontSize * 1.3
		case 4: headingFontSize = fontSize * 1.15
		case 5: headingFontSize = fontSize * 1.05
		default: headingFontSize = fontSize
		}
    ...
    mutating func visitHTMLBlock(_ html: Markdown.HTMLBlock) -> NSAttributedString {
		...
		return NSAttributedString(string: html.rawHTML, attributes: attributes(font: NSFont.monospacedSystemFont(ofSize: fontSize - 2, weight: .regular)))
    }
...
// MARK: - Ordered List ---------------------------------------------------
    mutating func visitOrderedList(_ orderedList: Markdown.OrderedList) -> NSAttributedString {
		...
		let numeralColumnWidth = ceil(
			NSAttributedString(string: "\(highestNumber).",
...
// MARK: - Unordered List -------------------------------------------------
    mutating func visitUnorderedList(_ unorderedList: Markdown.UnorderedList) -> NSAttributedString {
		...
		for case let listItem as Markdown.ListItem in unorderedList.children {
			...
			body.insert(NSAttributedString(string: "\t•\t", attributes: listItemAttributes),
						at: 0)
			result.append(body)
        }
    ...
    mutating func visitCustomInline(_ customInline: Markdown.CustomInline) -> NSAttributedString {
		return NSAttributedString(string: customInline.plainText, attributes: attributes())
    }
    ...
    mutating func visitInlineHTML(_ inlineHTML: Markdown.InlineHTML) -> NSAttributedString {
		return NSAttributedString(string: inlineHTML.rawHTML, attributes: attributes(font: NSFont.monospacedSystemFont(ofSize: fontSize - 2, weight: .regular)))
    }
	...
	private func tableCellAttributedString(
		...
		if original.length > 0 {
			cell = original.mutableCopy() as! NSMutableAttributedString
		} else {
			cell = NSMutableAttributedString(string: "\u{00a0}", attributes: baseAttributes)
		}
	...
	mutating func visitTableCell(_ tableCell: Markdown.Table.Cell) -> NSAttributedString {
		...
		let originalUseMonospaced = self.useMonospaced
		self.useMonospaced = false
		let firstPass = trimmedTableCell(defaultVisit(tableCell))
		self.useMonospaced = originalUseMonospaced
	...
	mutating func visitInlineAttributes(_ attributesNode: Markdown.InlineAttributes) -> NSAttributedString {
		...
		return defaultVisit(attributesNode)
    }
...
extension Markdown.OrderedList {
	...
	var isContainedInList: Bool { (parent as? Markdown.ListItemContainer) != nil }
}

extension Markdown.ListItemContainer {
	var listDepth: Int {
		var depth = 0
		var current: Markdown.Markup? = self.parent
		while let c = current {
...
extension NSFont {
	...
	func rounded() -> NSFont {
		let descriptor = self.fontDescriptor.withDesign(.rounded) ?? self.fontDescriptor
