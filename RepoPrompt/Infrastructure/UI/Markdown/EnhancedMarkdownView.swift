...
final class CodeBlockTextView: NSTextView {
	...
	private(set) var lastMeasuredSize: CGSize = CGSize(width: 0, height: 20)
	...
	override func setFrameSize(_ newSize: NSSize) {
		...
		abs(container.containerSize.width - newSize.width) > 0.5 {
			container.containerSize = NSSize(width: newSize.width,
			...
			lastMeasuredSize = CGSize(width: newSize.width,
	...
	func measuredHeight(constrainedTo width: CGFloat) -> CGFloat {
		...
		textContainer?.containerSize = NSSize(width: measureWidth,
												height: .greatestFiniteMagnitude)
		if let container = textContainer {
...
final class MarkdownTextViewCoordinator: NSObject, NSTextViewDelegate {
	...
	private func fileLinkTarget(in textView: NSTextView, link: Any, charIndex: Int) -> MarkdownFileLinkTarget? {
		...
		if let storage = textView.textStorage, charIndex >= 0, charIndex < storage.length {
			rawDestination = storage.attribute(.markdownRawLink, at: charIndex, effectiveRange: nil) as? String
		} else {
			rawDestination = nil
		}
...
struct AttributedTextView: NSViewRepresentable {
	...
	func sizeThatFits(
		...
		guard let resolvedWidth else {
			...
			return CGSize(width: proposedWidth ?? textView.lastMeasuredSize.width,
...
struct EnhancedCodeBlock: View {
	...
	private var copyButtonView: some View {
		Button(action: copyCode) {
			Image(systemName: "doc.on.clipboard")
				.foregroundColor(isCopyHovering ? BubbleColors.copyIconHover
    ...
    private var codeContent: some View {
        if let highlightedCode = highlightedCode, isFinalized {
			// ① final + highlighted
            AttributedTextView(
	...
	private var plainAttributedCode: NSAttributedString {
		let font  = NSFont.monospacedSystemFont(ofSize: scaledCodeFontSize,
												weight: .regular)
    ...
    private func applySyntaxHighlighting(to code: String, language: String?) -> NSAttributedString {
		...
		return CodeHighlightCache.shared.highlighted(code, language: language, fontPointSize: scaledCodeFontSize)
    }
}
