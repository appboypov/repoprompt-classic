...
final class TooltipOverlayController {
              ...
              preset     : FontScalePreset)
    {
        ...
        let bubbleSize = self.bubbleSize(for: text, preset: preset)
        guard let win = self.win else { return }
    ...
    private func prepareWindowIfNeeded(owner: NSWindow,
                                       preset: FontScalePreset)
    {
    ...
    private func bubbleSize(for text: String, preset: FontScalePreset) -> CGSize {
        ...
        let box = (text as NSString).boundingRect(
            with: CGSize(width: maxWidth - padding,
        ...
        return CGSize(width : ceil(min(box.width + padding + widthBuffer, maxWidth)),
