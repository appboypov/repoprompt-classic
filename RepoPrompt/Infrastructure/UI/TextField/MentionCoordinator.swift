...
func mentionQueryChanged(_ q: String, parent: MentionSuggestion?) {
	...
	func mentionQueryChanged(_ q: String,
	...
	// ------------------------------------------------------------------
	private func runQuery(_ q: String,
							parent: MentionSuggestion?,
							preserveIndex: Bool)
	{
		...
		let desired = preserveIndex
			? (self.highlightStack.last ?? 0)
			: 0
		// Clamp in case the list shrunk; avoid negative when list is empty
		self.highlighted = items.isEmpty ? 0 : min(max(desired, 0), items.count - 1)

		// Sync per-level cache
		if !self.highlightStack.isEmpty {
			self.highlightStack[self.highlightStack.count - 1] = self.highlighted
		}

		self.overlay.update(items: items, highlighted: self.highlighted)
	...
	private func scheduleOverlayReanchor() {
		...
		pendingReanchorTask = Task { @MainActor [weak self] in
			...
			guard generation == self.reanchorGeneration else { return }
			guard !self.textView.hasMarkedText() else {
				self.overlay.hide()
				return
			}
			guard self.textView.window != nil else {
				self.overlay.hide()
				return
			}
			let selection = self.textView.clampSelectionToCurrentString()
			let caretRect = self.textView.firstRect(
