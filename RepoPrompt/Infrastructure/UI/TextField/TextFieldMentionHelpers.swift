...
final class FileTagMentionHelper {
	...
	private weak var fileTagManager: RepoFileManagerViewModel?
	...
	func configure(
		...
		guard enabled else {
			let hadState = (service != nil || fileTagManager != nil || triggerRange != nil || refreshTask != nil || suggestionTask != nil)
			service = nil
			fileTagManager = nil
			if hadState {
				dismiss()
		...
		if service == nil {
			service = AgentFileTagSuggestionService(fileManager: fileManager, maxResults: 5)
			fileTagManager = fileManager
			shouldRefreshNow = true
		} else if fileManager !== fileTagManager {
			service?.updateFileManager(fileManager)
			fileTagManager = fileManager
			shouldRefreshNow = true
		}
		if shouldRefreshNow {
	...
	func scheduleRefresh(
		...
		refreshTask = Task { @MainActor [weak self, weak textView] in
			...
			self.refreshSuggestions(for: textView, enabled: enabled)
		}
	}
	...
	private func refreshSuggestions(for textView: NSTextView, enabled: Bool) {
		...
		suggestionTask = Task { @MainActor [weak self, weak textView] in
			...
			guard !Task.isCancelled,
				self.suggestionRequestID == requestID
			else { return }
			...
			guard let latestTrigger = self.detectTrigger(in: textView),
				latestTrigger.query == requestedQuery,
				latestTrigger.range == expectedRange
			else { return }

			self.suggestions = freshSuggestions
			self.highlightedIndex = 0
			self.triggerRange = latestTrigger.range
			...
			if overlayWasVisible {
				self.overlay.update(items: self.suggestions, highlighted: self.highlightedIndex)
				self.overlay.repositionRoot(to: caretRect)
			} else {
				self.overlay.show(at: caretRect, owner: ownerWindow, items: self.suggestions)
				self.overlay.update(items: self.suggestions, highlighted: self.highlightedIndex)
			}
		}
	}
	...
	private func deferCaretPlacementAfterCommit(in textView: NSTextView, insertionPoint: Int) {
		...
		commitFinalizationTask = Task { @MainActor [weak self, weak textView] in
			...
			self.positionCaretAfterCommittedReplacement(in: textView, insertionPoint: insertionPoint)
			await Self.waitForNextMainRunLoopTurn()
			guard !Task.isCancelled else { return }
			self.positionCaretAfterCommittedReplacement(in: textView, insertionPoint: insertionPoint)
			self.isFinalizingCommittedMention = false
			self.commitFinalizationTask = nil
		}
	}
