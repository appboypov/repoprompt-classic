...
final class MentionTextView: NSTextView {
	...
	internal func repairTextKitIfNeeded() {
		...
		let extras = containers.enumerated().filter { idx, c in
    ...
    override func keyDown(with event: NSEvent) {
		...
		if let _ = mentionStartLocation {
			...
			case 53:   // esc
                ...
                return
			case 125:  // ↓
				mentionDelegate?.mentionNavigate(.down); return
			case 126:  // ↑
				mentionDelegate?.mentionNavigate(.up);   return
			case 123:  // ←
				mentionDelegate?.mentionNavigate(.left); return
			case 124:  // →
				mentionDelegate?.mentionNavigate(.right);return
	...
	// MARK: – Insert token ---------------------------------------------------
	internal func insertMentionToken(_ suggestion: MentionSuggestion) {
        ...
        let attr = NSMutableAttributedString(string: tokenText,
        ...
        attr.addAttribute(.mentionToken,
                          value: payload,
    ...
    private func deleteTokenIfNeeded(isForwardDelete: Bool) -> Bool {
        ...
        guard let payload = ts.attribute(.mentionToken,
                                         at: indexToInspect,
                                         effectiveRange: &effective) as? MentionTokenPayload
        else { return false }
	...
	private func notifyCaretPosition() {
		...
		let selection = clampSelectionToCurrentString()
		let caret = firstRect(forCharacterRange: selection,
								actualRange: nil)
