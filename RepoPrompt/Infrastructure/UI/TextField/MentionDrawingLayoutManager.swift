								 ...
								 at origin: CGPoint)
	{
		...
		let container: NSTextContainer? = {
			if glyphsToShow.location != NSNotFound, glyphsToShow.length > 0,
			   let resolved = textContainer(forGlyphAt: glyphsToShow.location,
												effectiveRange: nil) {
				return resolved
			}
		...
		let charRangeToDraw = characterRange(forGlyphRange: glyphsToShow,
											 actualGlyphRange: &actualGlyphRange)

		textStorage.enumerateAttribute(.mentionToken,
									   in: charRangeToDraw,
									   options: []) { value, attrCharRange, _ in
			guard value != nil else { return }

			// Convert the attribute's character range back into glyph space.
			let attrGlyphRange = self.glyphRange(forCharacterRange: attrCharRange,
