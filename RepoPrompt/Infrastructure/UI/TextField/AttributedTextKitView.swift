...
struct AttributedTextKitView: NSViewRepresentable {
	...
	private static var tokenRegex: NSRegularExpression = {
		MentionAssets.shared.tokenRegex
	}()
	...
	class Coordinator: NSObject, NSTextViewDelegate {
        ...
        init(_ parent: AttributedTextKitView) {
			...
			self.wasEmpty = parent.text.isEmpty
		...
		func textDidChange(_ notification: Notification) {
			...
			if isEmpty != wasEmpty {
				wasEmpty = isEmpty
				scheduleLayoutStabilization(for: textView,
        ...
        func undoManager(for view: NSTextView) -> UndoManager? { undoMgr }
		...
		fileprivate func scheduleLayoutStabilization(for textView: NSTextView,
    ...
    func makeNSView(context: Context) -> NSScrollView {
		...
		// ------------------------------------------------------------------
		let svc = MentionSuggestionService(fileManager: fileManager,
											maxResults: 5)
        ...
        if textView.string != text {
			...
			if wasFirstResponder && !shouldForce {
				...
				if shouldForce {
					...
					context.coordinator.scheduleLayoutStabilization(for: textView,
...
// NEW: ensure we unhook delegates and cancel pending tasks
	...
	private func ensureDocumentViewFillsVisibleHeight(_ scrollView: NSScrollView,
														textView: NSTextView)
	{
...
private func applyMentionAttributes(on tv: NSTextView,
	...
	coordinator.pendingAttrTask = Task.detached(priority: .userInitiated) { [weak tv] in
		...
		await MainActor.run { [weak tv, weak fm] in
			...
			for (range, rawPath) in capturedSpans {
				...
				tv.textStorage?.addAttribute(.mentionToken,
											value: payload,
											range: safeRange)
			...
			if let lm = tv.layoutManager {
				lm.invalidateLayout(forCharacterRange: fullRange,
