...
struct MCPServerToggleView: View {
	...
	init(windowState: WindowState, showPopover: Binding<Bool>) {
		...
		self.server = server
		self.promptViewModel = windowState.promptManager
		self.windowID = windowState.windowID
...
struct MCPServerPopoverContent: View {
	...
	private var isServerRunning: Bool { server.isRunning }
	private var windowToolsEnabled: Bool { server.windowToolsEnabled }
	@ObservedObject private var fontScale = FontScaleManager.shared
	private var fontPreset: FontScalePreset { fontScale.preset }
	private var popoverWidth: CGFloat { fontPreset.scaledClamped(380, min: 380, max: 500) }
	private var popoverMaxHeight: CGFloat { fontPreset.scaledClamped(580, min: 580, max: 760) }
	private var contentPadding: CGFloat { fontPreset.scaledClamped(20, min: 20, max: 28) }
	private var sectionSpacing: CGFloat { fontPreset.scaledClamped(16, min: 16, max: 24) }
	private var subsectionSpacing: CGFloat { fontPreset.scaledClamped(10, min: 10, max: 16) }
	private var rowSpacing: CGFloat { fontPreset.scaledClamped(12, min: 12, max: 18) }
	private var compactRowSpacing: CGFloat { fontPreset.scaledClamped(8, min: 8, max: 12) }
	private var buttonMinHeight: CGFloat { fontPreset.scaledClamped(28, min: 28, max: 38) }
	...
	@AppStorage(SettingKeys.mcpShowModelPresets) private var showModelPresets = false
	@AppStorage("mcpTemporarilyDisablePresets") private var mcpTemporarilyDisablePresets = false
    ...
    private var serverControlSection: some View {
		VStack(alignment: .leading, spacing: rowSpacing) {
			...
			HStack {
				Toggle("Auto-Start", isOn: $autoStartServer)
    ...
    private var modelPresetsSection: some View {
		VStack(alignment: .leading, spacing: subsectionSpacing) {
			...
			Toggle("Use Oracle Model Presets for MCP", isOn: $showModelPresets)
				.font(fontPreset.font)
                        ...
                        if mcpTemporarilyDisablePresets {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text("Presets hidden by Setup Wizard.")
                                    ...
                                    .foregroundColor(.secondary)
                                Button("Show presets") {
                                    mcpTemporarilyDisablePresets = false
                                    DispatchQueue.main.async {
                                        NotificationCenter.default.post(name: .recommendationsDidApply, object: nil)
                                    }
                                }
			...
			HStack(spacing: subsectionSpacing) {
                Menu {
					Section("MCP Server") {
						...
						Button("Gemini CLI") {
							let result = MCPIntegrationHelper.installInGemini()
							if result.success {
								showMCPFeedback(result.wasAlreadyPresent ? "Gemini already configured" : "Gemini configured")
							} else {
								showMCPFeedback("Gemini config failed", isError: true)
							}
						}
						Button("OpenCode") {
					...
					Section {
						...
						Menu("Per-project") {
							Button("MCP skills") {
								...
								let count = MCPIntegrationHelper.installClaudeCodeCommands(workspacePaths: workspacePaths, useCLIVariant: false)
								showMCPFeedback(count > 0 ? "Skills installed" : "Install failed", isError: count == 0)
							}
							Button("rp-cli skills") {
								let workspacePaths = promptViewModel.fileManager.visibleRootFolders.map(\.fullPath)
								...
								let count = MCPIntegrationHelper.installClaudeCodeCommands(workspacePaths: workspacePaths, useCLIVariant: true)
								showMCPFeedback(count > 0 ? "Skills installed" : "Install failed", isError: count == 0)
							}
							Divider()
							Button("Uninstall") {
								let workspacePaths = promptViewModel.fileManager.visibleRootFolders.map(\.fullPath)
								let count = MCPIntegrationHelper.uninstallClaudeCodeCommands(workspacePaths: workspacePaths, useCLIVariant: false)
								let cliCount = MCPIntegrationHelper.uninstallClaudeCodeCommands(workspacePaths: workspacePaths, useCLIVariant: true)
								showMCPFeedback(count + cliCount > 0 ? "Skills removed" : "Nothing to remove")
							}
						}
						.disabled(promptViewModel.fileManager.visibleRootFolders.isEmpty)
	...
	private func updateActiveToolCount() {
		activeToolCount = toolStore.advertisedTools.filter { toolStore.isEnabled($0.name) }.count
    }
...
fileprivate struct StatusButtonStyle: ButtonStyle {
