...
struct MCPStatusView: View {
    ...
    private func connectionStateBadge(_ state: ConnectionStateSummary) -> some View {
        let (color, text): (Color, String) = {
            ...
            case .setup: return (.orange, "Setup")
            case .waiting: return (.yellow, "Waiting")
            case .failed: return (.red, "Failed")
            case .cancelled: return (.gray, "Cancelled")
    ...
    private func formatIdleTime(_ seconds: TimeInterval) -> String {
        if seconds < 60 {
            return "\(Int(seconds))s idle"
        } else if seconds < 3600 {
            return "\(Int(seconds / 60))m idle"
        } else {
            return "\(Int(seconds / 3600))h idle"
        }
    }
