...
public enum RegexToolkit {
    ...
    public static func validate(_ pattern: String) throws {
        ...
        while let (i, ch) = iterator.next() {
            ...
            if ch == "[" && !inClass        { bracket += 1; inClass = true; continue }
            if ch == "]" &&  inClass        { bracket -= 1; if bracket == 0 { inClass = false }; continue }
            if ch == "]" && !inClass        { throw SearchPatternError.unmatchedBrackets(pattern) }

            if !inClass {          // outside [...]
                if ch == "(" { paren += 1 }
                if ch == ")" { paren -= 1; if paren < 0 { throw SearchPatternError.unmatchedParentheses(pattern) } }
                ...
                if ch == "{" {
                    ...
                    while idx < pattern.endIndex, pattern[idx].isNumber {
                        firstDigits.append(pattern[idx]); idx = pattern.index(after: idx)
                    }
                    var hasComma = false
                    var secondDigits = ""
                    if idx < pattern.endIndex, pattern[idx] == "," {
                        hasComma = true; idx = pattern.index(after: idx)
                        while idx < pattern.endIndex, pattern[idx].isNumber {
                            secondDigits.append(pattern[idx]); idx = pattern.index(after: idx)
                        }
                    }
                    ...
                    for _ in 0..<distance { _ = iterator.next() }
                    continue
                }
            }
        }
    ...
    public static func validateComplexity(_ pattern: String, isRegex: Bool) throws {
        ...
        let maxPatternLength = 2_000   // characters
...
internal enum SearchPatternErrorFormatter {
    internal static func parts(for pattern: String, isRegex: Bool, error: SearchPatternError) -> (issue: String, suggestion: String?) {
        ...
        case .invalidRegex(_, let details):
...
public enum SearchPatternError: Error, LocalizedError, RegexPatternFailure {
    ...
    public var errorDescription: String? {
        switch self {
        case .unmatchedParentheses(let pattern):
            return "Unmatched parentheses in pattern: '\(pattern)'. Check that all '(' have matching ')'."
        case .unmatchedBrackets(let pattern):
            return "Unmatched brackets in pattern: '\(pattern)'. Check that all '[' have matching ']'."
        case .invalidEscape(let pattern):
            return "Invalid escape sequence in pattern: '\(pattern)'. Use '\\\\' for literal backslash."
        case .invalidQuantifier(let pattern):
            return "Invalid quantifier in pattern: '\(pattern)'. Check syntax like {n,m} or *+?."
        case .invalidRegex(let pattern, let details):
            return "Invalid regex pattern: '\(pattern)'. \(details)"
        case .emptyAlternative(let pattern):
            return "Empty alternative in pattern: '\(pattern)'. Remove extra '|' characters."
        }
    }
}
