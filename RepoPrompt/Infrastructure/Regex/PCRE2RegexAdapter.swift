...
enum RepoPromptPCRE2MatchPolicy {
	...
	static var fileSearchLine: PCRE2MatchLimits? {
		guard RepoPromptRegexRuntime.pcre2SearchMatchLimitsEnabled else { return nil }
		return PCRE2MatchLimits(matchLimit: 1_000_000, depthLimit: 10_000, heapLimitKiB: 16 * 1024)
	}

	static var pathSearchShortSubject: PCRE2MatchLimits? {
		guard RepoPromptRegexRuntime.pcre2SearchMatchLimitsEnabled else { return nil }
		return PCRE2MatchLimits(matchLimit: 100_000, depthLimit: 1_000, heapLimitKiB: 4 * 1024)
	}
}
...
enum RepoPromptPCRE2Adapter {
	...
	static func asciiWholeWordLiteralPlan(
		...
		guard !pattern.isEmpty,
			pattern.utf8.allSatisfy({ ($0 >= 65 && $0 <= 90) || ($0 >= 97 && $0 <= 122) || ($0 >= 48 && $0 <= 57) || $0 == 95 }) else {
			return nil
		}
