...
/// Pro editing override options
enum ProEditingOverride: String, Codable, Sendable {
    case useDefault = "default"  // Use current setting
    case forceOn = "on"         // Force pro editing on
    case forceOff = "off"       // Force pro editing off
}

struct ModelPreset: Codable, Identifiable, Equatable, Sendable {
    let id: UUID
    let name: String
    let modelString: String  // Store as string for Codable
    let description: String?
    let supportedModes: SupportedModes?
    let proEditingOverride: ProEditingOverride  // Controls pro editing behavior
    ...
    init(id: UUID = UUID(), name: String, model: AIModel, description: String? = nil, supportedModes: SupportedModes? = nil, proEditingOverride: ProEditingOverride = .useDefault, chatPresetMappings: ChatPresetMappings? = nil) {
        ...
        self.modelString = model.rawValue
...
struct SupportedModes: Codable, Equatable, Sendable {
    let chat: Bool
    let plan: Bool
    let edit: Bool
    let review: Bool

    init(chat: Bool = true, plan: Bool = true, edit: Bool = true, review: Bool = true) {
        self.chat = chat
        self.plan = plan
        self.edit = edit
        self.review = review
    ...
    init(from decoder: Decoder) throws {
        ...
        self.chat = try container.decodeIfPresent(Bool.self, forKey: .chat) ?? true
        self.plan = try container.decodeIfPresent(Bool.self, forKey: .plan) ?? true
        self.edit = try container.decodeIfPresent(Bool.self, forKey: .edit) ?? true
        self.review = try container.decodeIfPresent(Bool.self, forKey: .review) ?? true
    }

    /// Returns true if at least one mode is enabled
    var hasEnabledModes: Bool {
        chat || plan || edit || review
    }
...
struct ChatPresetMappings: Codable, Equatable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.chatPresetID = try container.decodeIfPresent(UUID.self, forKey: .chatPresetID)
        self.planPresetID = try container.decodeIfPresent(UUID.self, forKey: .planPresetID)
        self.editPresetID = try container.decodeIfPresent(UUID.self, forKey: .editPresetID)
        self.reviewPresetID = try container.decodeIfPresent(UUID.self, forKey: .reviewPresetID)
    }

    /// Get the chat preset ID for a specific mode
    func presetID(for mode: String) -> UUID? {
        switch mode.lowercased() {
        case "chat": return chatPresetID
        case "plan": return planPresetID
        case "edit": return editPresetID
        case "review": return reviewPresetID
        default: return nil
        }
    }
...
class ModelPresetsManager: ObservableObject {
    ...
    /// Loads presets from Application Support JSON with legacy UserDefaults migration/recovery support.
    private func loadPresets() {
        presets = presetFileStore.loadModelPresets().modelPresets
    }
    
    /// Saves presets to Application Support JSON and legacy UserDefaults mirrors.
    ...
    func getAllPresets() -> [ModelPreset] {
