...
enum ApplyPromptFormat: String, CaseIterable, Codable {
	case diff      = "Diff"
	case whole     = "Whole"
...
enum SystemPromptFlavor: String, Codable {
...
class SystemPromptService {
    ...
    /// Returns a system prompt string for a given flavor.
    /// Architect/code-edit prompts are supported.
    /// Legacy flavors (review/mcp*) return empty; Discover is supported via hardcoded prompt.
    static func systemPrompt(for flavor: SystemPromptFlavor, language: String = "Swift") -> String {
        switch flavor {
        case .architectPlan:
            return architectPrompt(language: language)
        case .codeEditDiff:
            // Diff-oriented editing (allowRewrite defaults to true for system-level prompt)
            return codeEditPrompt(format: .diff, allowRewrite: true, language: language)
        case .codeEditWhole:
            return codeEditPrompt(format: .whole, allowRewrite: true, language: language)
        case .mcpDiscover:
            return mcpDiscoverPrompt()
		case .mcpAgent:
			return mcpAgentPrompt()
		case .mcpPairProgram:
			return mcpPairProgramPrompt()
		case .mcpBuilder:
			return mcpBuilderPrompt()
		case .review, .mcpPairPlan:
            return ""
        }
    }
	...
	static func discoverPrompt(tokenBudget: Int? = nil, agentKind: DiscoverAgentKind? = nil, enhancementMode: PromptEnhancementMode = .fullRewrite, allowClarifyingQuestions: Bool = false, responseType: String? = nil, instructions: String? = nil, questionTimeoutSeconds: TimeInterval = ContextBuilderDefaults.questionTimeoutSeconds) -> String {
		return mcpDiscoverPrompt(tokenBudget: tokenBudget, agentKind: agentKind, enhancementMode: enhancementMode, allowClarifyingQuestions: allowClarifyingQuestions, responseType: responseType, instructions: instructions, questionTimeoutSeconds: questionTimeoutSeconds)
	}
	...
	/// - Parameter responseType: Optional response type for context_builder (e.g., "review" for code review context).
	/// - Parameter instructions: Optional discovery instructions, used for hidden review hotword detection in clarify mode.
	private static func mcpDiscoverPrompt(tokenBudget: Int? = nil, agentKind: DiscoverAgentKind? = nil, enhancementMode: PromptEnhancementMode = .fullRewrite, allowClarifyingQuestions: Bool = false, responseType: String? = nil, instructions: String? = nil, questionTimeoutSeconds: TimeInterval = ContextBuilderDefaults.questionTimeoutSeconds) -> String {
		...
		let coverageLine = {
			...
			return langs.isEmpty
				? ""
				: "**Codemap coverage:** " + langs.map { $0.displayName }.joined(separator: ", ")
		}()

		// Gemini-specific: Tool name note for duplicate read_file variants.
		let geminiToolNote = agentKind == .gemini ? """

**Tool Name Note for Gemini:** A duplicate read tool may appear as `RepoPrompt__read_file` in your tool list. Prefer that tool when available. Example phrasing: "Reading more core logic files using the RepoPrompt__read_file tool."
""" : ""
...
- **Batch related clarifications** into one `questions` array instead of making repeated `ask_user` calls
- Give every question a stable, unique `id` so you can read the structured answers by key
- **Provide options** when the choices are clear (the user can still type a custom answer unless `allows_custom` is false)
- **Limit questions** — one or two well-crafted questions are better than many

**Example usage:**
```json
{"tool":"ask_user","args":{"title":"Clarifying questions","context":"I found two authentication implementations and need to know which one is relevant to your task.","questions":[{"id":"auth_scope","header":"Scope","question":"Which authentication approach should I focus on?","options":[{"label":"JWT-based auth in AuthService.swift","description":"Focus on token-based login and refresh flows"},{"label":"Session-based auth in SessionManager.swift","description":"Focus on cookie/session persistence"},{"label":"Both approaches","description":"Compare both implementations"}],"allows_multiple":false,"allows_custom":true}]}}
```

**Response handling:**
- Responses are structured as `answers` keyed by question ID; read `answers.<id>.answers`, `selected_options`, `custom_response`, and `skipped`
- If the user skips or times out, proceed with your best judgment; timeout responses may include partial draft answers
...
1. **Initial selection**: Add all relevant files/directories as full files. `op=set` with `mode=full` is a complete replacement; after that, use `op=add` / `op=remove` for incremental changes. For mixed full-file + slice additions, use `op=add` with both `paths` and `slices`.
    ```json
    {"tool":"manage_selection","args":{"op":"set","paths":["Root/src/auth","Root/src/models/User.swift"]}}
    ```
...
3. **If over budget, optimize in this order:**
	...
	c) **If still over budget, convert large files to slices** (keep as many full files as possible). `op=set` with `mode=slices` only replaces slices for the listed files and preserves unrelated full files/slices.
       ```json
...
"""
		...
		let afterCompletingTask: String
		if agentKind == .codexExec {
			afterCompletingTask = """
		...
		let providerReadPolicyGuidance: String
		switch agentKind {
		case .claudeCode, .claudeCodeGLM, .kimiCode, .customClaudeCompatible:
			providerReadPolicyGuidance = """
...
**Read policy (important):**
**Tool disambiguation (important):**
Some tools exist as both Gemini's native `read_file` tool and RepoPrompt MCP tools. Use the correct version:
- `read_file` — Use Gemini's native `read_file` tool for binary assets (images, screenshots, PDFs, other binary files). Use MCP `RepoPrompt__read_file` for text-based reads (source code, configs, docs, logs). Prefer the MCP version for text so line ranges and path behavior stay consistent in RepoPrompt.
- `ask_user` — ALWAYS use the MCP version (`mcp__RepoPrompt__ask_user`). It routes through RepoPrompt's UI and supports structured options. Do not use native `ask_user`.
- If the user message includes media references like `@path/to/file.png` (or other `@path` binary assets), ALWAYS open those paths with native `read_file`.
"""
		default:
			providerReadPolicyGuidance = ""
		}
...
*User Interaction:*
- `ask_user` - Ask structured clarification questions with a top-level `questions` array and stable question IDs; responses return `answers` keyed by ID\(setStatusInList)\(codexToolPriorityGuidance)\(progressUpdatesGuidance)
...
2. Use `ask_oracle` for planning, editing, review, and follow-up turns.
3. Use `apply_edits` / `file_actions` only for small mechanical deltas when necessary.
...
## Core loop: Plan → Edit → Verify → Mend
...
### 3) Edit
Let the oracle execute the implementation step:
```json
{"tool":"ask_oracle","args":{
	"message":"Edit: Implement the preferences model and wire it into the selected files. Existing UserDefaults keys must be preserved or migrated. Determine the best structure and implementation details.",
	"new_chat":false,
	"mode":"edit"
}}
```
...
```json
{"tool":"ask_oracle","args":{
	"message":"Mend: Preferences do not persist after restart, and malformed UserDefaults data causes a crash. Fix persistence and corrupted-data handling.",
	"new_chat":false,
	"mode":"edit"
}}
```
Use `apply_edits` only for clearly mechanical fixes, then summarize the delta back into the same oracle thread.
...
"""
	...
	/// Uses ClaudeCodeCommands.rpBuildCore for shared content, adds copy preset-specific preamble.
...
\(ClaudeCodeCommands.rpBuildCore)
"""
	}
	...
	static func predominantLanguage(from files: [FileViewModel]) -> String
	{
		...
		for file in files {
			if let ext = file.fileExtension?.lowercased() {
	...
	@MainActor
	static func getPredominantLanguage(from fileManager: RepoFileManagerViewModel) -> (language: String, fileExtension: String) {
	...
	/// Convenience for the picker → prompt lookup.
	static func applyPrompt(for format: ApplyPromptFormat,
				allowRewrite: Bool = true,
				language: String) -> String {
		switch format {
		case .diff:      return getApplyInstructions(format: .diff,
												allowRewrite: allowRewrite,
												language: language)
		case .whole:     return getApplyInstructions(format: .whole,
												allowRewrite: allowRewrite,
												language: language)
		case .architect: return getArchitectApplyPrompt(language: language)
		}
	}
	
	static func getArchitectApplyPrompt(language: String) -> String {
		// Derive file extension from language for backward compatibility
		let fileExt = fileExtension(for: language)
		
		let config = PromptConfig(
			role: .architect,
			canCreate: true,
			canRewrite: false,
			canSearchReplace: false,
			canDelete: true,
			canDelegateEdit: true,
			supportsRename: true,
			language: language,
			fileExtension: fileExt,
			codeBlockFence: applyCodeFence,
			includeIndentationEncoding: false,
			includeEscapingRules: true,
			includeApplyOutputWrappingNotes: true
		)
		
		return PromptFactory.buildPrompt(with: config)
	}
	
	static func getCodeEditPromptFor(format: DiffViewModel.PromptFormat = .diff, allowRewrite: Bool = true, language: String) -> String {
		// Derive file extension from language
		let fileExt = fileExtension(for: language)
		
		switch format {
		case .diff:
			// Construct a PromptConfig emphasizing partial modifications (search/replace)
			let diffConfig = PromptConfig(
				role: .codeAssistant,
				canCreate: true,
				canRewrite: allowRewrite,
				canSearchReplace: true,
				canDelete: true,
				canDelegateEdit: false,
				supportsRename: true,
				language: language,
				fileExtension: fileExt,
				codeBlockFence: applyCodeFence,          // or use "===" if needed
				includeIndentationEncoding: false,
				includeEscapingRules: true
			)
			return PromptFactory.buildPrompt(with: diffConfig)
			
		case .whole:
			// Construct a PromptConfig emphasizing a full-file rewrite
			let rewriteConfig = PromptConfig(
				role: .codeAssistant,
				canCreate: true,
				canRewrite: true,
				canSearchReplace: false,
			...
			return PromptFactory.buildPrompt(with: rewriteConfig)
		}
	}
	
	static func getApplyInstructions(format: DiffViewModel.PromptFormat = .diff, allowRewrite: Bool = true, language: String) -> String {
		// Derive file extension from language
		let fileExt = fileExtension(for: language)
	...
	/*
	static func getXMLInstructions() -> String{
		return chatSearchReplacePromptNoIndent
	}
	
	static func getFullXMLInstructions() -> String{
		return wholeClipboard
	}
	
	static func getChatInstructions() -> String{
		return chatSearchReplacePrompt
	}
	
	static func getFullChatInstructions() -> String{
		return chatPromptFull//architectPrompt
	}
	
	static func getDirectDiffGenerationPrompt() -> String {
		return directDiffPrompt
	}
	*/
	
    // getArchitectPropmt removed (legacy). Use architectPrompt(language:) instead.
	
	static func getFileEditPromptFor(format: DiffViewModel.PromptFormat = .diff, allowRewrite: Bool = true, language: String) -> String {
		// Derive file extension from language
		let fileExt = fileExtension(for: language)
	...
	/*
	 static func getFileEditPrompt() -> String {
	 return fileEditPrompt
	 }
	 
	 static func getDiffFileEditPrompt() -> String {
	 return fileEditDiffPromptIndent
	 }
	 */
	// MARK: - Architect Apply Prompt
	
	static func architectPrompt(language: String) -> String
	{
		// Derive file extension from language
		let fileExt = fileExtension(for: language)
		...
		let cfg = PromptConfig(
			role: .codeAssistant,
			canCreate: true, canRewrite: false, canSearchReplace: false,
			canDelete: true, canDelegateEdit: true, supportsRename: true,
			language: language, fileExtension: fileExt,
			codeBlockFence: applyCodeFence,
			includeIndentationEncoding: false,
			includeEscapingRules: true
		)
		return PromptFactory.buildPrompt(with: cfg)
	}

	static func codeEditPrompt(format: DiffViewModel.PromptFormat = .diff,
								allowRewrite: Bool = true,
								language: String) -> String
	{
		// Derive file extension from language
		...
		case .diff:
			let cfg = PromptConfig(
				role: .codeAssistant,
				canCreate: true, canRewrite: allowRewrite, canSearchReplace: true,
				canDelete: true, canDelegateEdit: false, supportsRename: true,
				language: language, fileExtension: fileExt,
				codeBlockFence: applyCodeFence,
				includeIndentationEncoding: false,
				includeEscapingRules: true
		...
		case .whole:
			let cfg = PromptConfig(
				role: .codeAssistant,
				canCreate: true, canRewrite: true, canSearchReplace: false,
				canDelete: true, canDelegateEdit: false, supportsRename: true,
				language: language, fileExtension: fileExt,
				codeBlockFence: applyCodeFence,
				includeIndentationEncoding: false,
...
extension SystemPromptService {
	...
	static func mcpDelegateEditPrompt(filePath: String,
									  changes: [DelegateEditItem.Change],
									  retryingAfterNoChanges: Bool = false,
									  agentKind: DiscoverAgentKind = .claudeCode) -> String {
		// Format changes using existing DelegateEditItem formatter to keep consistency
		let item = DelegateEditItem(filePath: filePath, changes: changes)
		let formattedChanges = item.formattedString()

		let retryNotice = retryingAfterNoChanges
			? """

**RETRY CONTEXT – ACTION REQUIRED**
- You last tried to complete this delegate edit but did not apply any changes.
- There are definitely changes to make and the user is explicitly requesting you try again right now.

"""
			: ""

		// Gemini-specific: MCP tools get prefixed with "RepoPrompt__" when there are built-in tools with the same name
		let readFileToolName = (agentKind == .gemini) ? "read_file OR RepoPrompt__read_file" : DelegateEditToolNames.readFile
		let editToolName = DelegateEditToolNames.editFile
		let geminiToolNote = (agentKind == .gemini)
			? "\n- Note: A duplicate read tool may appear as 'RepoPrompt__read_file'. Prefer it when available. Example phrasing: \"Reading more core logic files using the RepoPrompt__read_file tool.\""
			: ""

		// Build detailed instructions for single-file, tools-only edits
		return """
You are an MCP-enabled agent assigned to edit exactly one file.

**CRITICAL: Tool Restrictions**
- Use ONLY these RepoPrompt MCP tools: \(DelegateEditToolNames.fileSearch), \(readFileToolName), and \(editToolName)
- Use \(editToolName) for every edit.
- You may read and edit EXACTLY this file: \(filePath)
- All operations must reference this exact path
- Attempts to access any other path will fail\(geminiToolNote)

\(retryNotice)**Your Task:**
<file_to_edit path="\(filePath)"/>
<changes_to_apply>
\(formattedChanges)
</changes_to_apply>

**CRITICAL: You MUST implement ALL the changes listed above using \(editToolName).**

The changes above are NOT suggestions or recommendations - they are REQUIRED modifications you must apply to the file. Your primary objective is to ensure every single change is implemented accurately.

**Mandatory Workflow:**

1) **Locate the code that needs editing**
	- FIRST, use \(DelegateEditToolNames.fileSearch) to find the specific symbols, functions, or patterns mentioned in the changes
   - Search for each key element that needs to be modified
   - This helps you identify the exact locations and understand what currently exists
   - Example: if changes mention "updateUser function", search for "updateUser" to find it

2) **Read and understand the context**
	- Use \(DelegateEditToolNames.readFile) with targeted line ranges around the areas you found
   - Understand the surrounding code and structure
   - Verify you've found the correct locations for all changes

3) **Implement ALL the prescribed changes**
	- You MUST use \(editToolName) to make every change listed in <changes_to_apply>
   - For focused edits: use search/replace with "all":true if replacing all occurrences
   - For multiple related changes: use the "edits" array to batch them together
   - For major restructuring: use "rewrite" with the complete new file content
   - Always include "verbose":true to see confirmation of what changed
   - DO NOT skip any changes - all changes must be applied

4) **Verify ALL changes were applied**
	- Use \(DelegateEditToolNames.fileSearch) to confirm old patterns are gone and new patterns exist
	- Use \(DelegateEditToolNames.readFile) to verify the changes look correct in context
   - If any change failed or was missed, apply it immediately

**Success Criteria:**
✅ All requested changes implemented accurately
✅ File remains syntactically valid after modifications
✅ Changes are focused and minimal (no unrelated modifications)

**Anti-patterns to Avoid:**
- 🚫 **CRITICAL:** Skipping or ignoring any of the prescribed changes
- 🚫 **CRITICAL:** Not using \(editToolName) to implement the changes
- 🚫 **CRITICAL:** Treating the changes as optional or as suggestions
- 🚫 Attempting to access any file besides \(filePath)
- 🚫 Using tools other than the three allowed MCP tools
- 🚫 Making unrelated changes beyond the requested modifications
- 🚫 Forgetting to set "all":true when you want to replace all occurrences
- 🚫 Using "rewrite" for small changes (prefer targeted search/replace)
- 🚫 Stopping before all changes are applied and verified
"""
	}
}
