...
"""
	...
	init(systemPrompt: String, userMessage: String, temperature: Double? = nil, disableTemperatureOverrides: Bool = false) {
		...
		self.promptSectionsOrder = PromptAssemblyBuilder.defaultSectionOrder
		self.disabledPromptSections = []
		self.duplicateUserInstructionsAtTop = false
	}
	...
	func buildTail(embedSystemPrompt: Bool) -> String {
		...
		for section in promptSectionsOrder where !disabledPromptSections.contains(section) {
			...
			case .diffFormatting, .userInstructions:
				// These are user‑authored blocks, never auto‑prepended.
				continue
			}
		}
	...
	func effectiveTemperature(for model: AIModel) -> Double? {
		...
		if let global = self.temperature, global != 0.0 {
			return global
		}
...
struct AIResponse: Identifiable, Equatable {
	...
	static func == (lhs: AIResponse, rhs: AIResponse) -> Bool {
		...
		lhs.appliedChanges == rhs.appliedChanges &&
		lhs.rejectedChanges == rhs.rejectedChanges
	}
}
...
struct FileChanges: Identifiable {
	let id: UUID
	var path: String
	var changes: [FileChange]
	...
	enum CodingKeys: String, CodingKey {
		case id
		case relativePath = "file_path"
		case changes
		case action
	}
	
	init(id: UUID = UUID(), relativePath: String, changes: [FileChange], action: FileAction) {
		self.id = id
	...
	init(id: UUID = UUID(), relativePath: String, changes: [FileChange], action: FileAction, contentItem: ContentItem) {
		self.id = id
		self.path = relativePath
		self.changes = changes
		self.action = action
	...
	// New function to print all lines in all file changes
	func printAllChanges() {
		print("File Changes ID: \(id)")
		print("Relative Path: \(path)")
		print("Action: \(action)")
		print("Changes:")
		for (index, change) in changes.enumerated() {
			print("Change \(index + 1):")
			change.printAllLines()
		}
		print("") // Empty line for better readability
	}
}
...
actor FileChangesAccumulator {
	private var accumulatedFileChanges: [String: FileChanges] = [:]
	
	func add(_ fileChanges: FileChanges) {
		if var existing = accumulatedFileChanges[fileChanges.path] {
			let uniqueChanges = fileChanges.changes.filter { change in
				!existing.changes.contains { $0.id == change.id }
			}
			existing.changes.append(contentsOf: uniqueChanges)
			accumulatedFileChanges[fileChanges.path] = existing
		} else {
			accumulatedFileChanges[fileChanges.path] = fileChanges
		}
	}
	
	func getAll() -> [FileChanges] {
		Array(accumulatedFileChanges.values)
	}
}