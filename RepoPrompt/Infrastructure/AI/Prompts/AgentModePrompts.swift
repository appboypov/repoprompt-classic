...
enum AgentModePrompts {
	...
	///
	/// The explore agent has a minimal RepoPrompt MCP toolset (file_search, get_file_tree,
	/// get_code_structure, read_file, git, ask_user, set_status) enforced at the advertisement
	/// level. Native provider shell/Bash may also be available for read-only inspection, but the
	/// prompt keeps exploration non-mutating and gives the (typically smaller) model clear workflow
	/// guidance for rapid exploration.
	...
	static func explorePrompt(
		...
		let shellReadOnlyPolicy = Fragments.providerShellReadOnlyPolicy(agentKind: agentKind)
...
\(readPolicy)\(shellReadOnlyPolicy)
**Conversation Style**
...
- Using shell/Bash commands that mutate files, move/delete content, format code, install dependencies, change git state, or start long-running services
...
"""
		...
		static func exportDelegationGuidance(
			...
			switch audience {
			case .agentRunOnly:
				return agentRunExportGuidance
			case .agentExploreOnly:
				return agentExploreExportGuidance
			case .both:
				return agentBothExportGuidance
			case .none:
				return ""
			}
		}

		/// Provider-specific shell guidance for read-only explore agents.
		static func providerShellReadOnlyPolicy(agentKind: DiscoverAgentKind?) -> String {
			let shellName: String
			switch agentKind {
			case .codexExec:
				shellName = "native shell/Bash"
			case .claudeCode, .claudeCodeGLM, .kimiCode, .customClaudeCompatible:
				shellName = "Claude's native `Bash` tool"
			default:
				return ""
			}
...
**Shell use (read-only only):**
- You may use \(shellName) for inspection: listing files, searching, printing environment/version info, reading logs, or running safe read-only diagnostics.
- Prefer RepoPrompt MCP tools for normal code search/read because they are context-efficient, multi-root aware, and respect workspace ignore rules.
- Do not use shell commands that create, modify, delete, move, format, patch, install dependencies, change git state, or start long-running services.
"""
		}

		/// Provider-specific read policy guidance.
		static func providerReadPolicy(agentKind: DiscoverAgentKind?) -> String {
			switch agentKind {
			case .claudeCode, .claudeCodeGLM, .kimiCode, .customClaudeCompatible:
				return """
...
**Read policy (important):**
- For non-text assets (images, screenshots, PDFs, other binary files), use Gemini's native `read_file` tool.
...
"""
		...
		static func codexQualifiedToolReferences(_ prompt: String, agentKind: DiscoverAgentKind?) -> String {
		...
		static func setStatusToolListItem(agentKind: DiscoverAgentKind?) -> String {
		...
		static func setStatusStartSentence(agentKind: DiscoverAgentKind?) -> String {
			if agentKind == .codexExec {
		...
		static func setStatusStartupBullet(agentKind: DiscoverAgentKind?) -> String {
			if agentKind == .codexExec {
		...
		static func setStatusTitleOnlyBullet(agentKind: DiscoverAgentKind?) -> String {
			if agentKind == .codexExec {
