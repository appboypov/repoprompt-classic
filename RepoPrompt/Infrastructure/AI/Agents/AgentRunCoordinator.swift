...
/// This centralises policy installation and provider orchestration so different run types
/// (e.g., Discover or Delegate‑Edit) can reuse the same plumbing.
final class AgentRunCoordinator {
    ...
    private static let defaultRoutingTimeoutMs = 10_000
	...
	func makeProvider(
		...
		return DiscoverAgentService.shared.makeProvider(
			for: agentKind,
    ...
    func runStream(provider: HeadlessAgentProvider, message: AgentMessage, runID: UUID?) async throws -> AsyncThrowingStream<AIStreamResult, Error> {
        return try await provider.streamAgentMessage(message, runID: runID)
    }
    
    /// Cleanup ownership:
    /// For delegate‑edit runs, this method owns both the per‑run client policy cleanup and the provider lifecycle.
    ...
    /// Discovery runs perform their own policy cleanup in DiscoverAgentViewModel (runDiscoverAgent) to coordinate
