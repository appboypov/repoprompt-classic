...
public class AIQueriesService {
	private weak var viewModel: AIResponseViewModel?
	private let taskManager = TaskManager()
	private let chunkSizeThreshold = 8_000          // e.g. 8KB
	private let timeThreshold: TimeInterval = 0.7    // 0.4 seconds
    private let providerPool: DisposableProviderPool
    private let keyManager: KeyManager
    private var currentModel: AIModel
    
    init(viewModel: AIResponseViewModel, keyManager: KeyManager) {
        self.currentModel = .claude4Sonnet // Default model
        self.viewModel = viewModel
        self.keyManager = keyManager
        self.providerPool = DisposableProviderPool(keyManager: keyManager)
    }

	init(keyManager: KeyManager) {
		self.currentModel = .claude4Sonnet
		self.viewModel = nil
		self.keyManager = keyManager
		self.providerPool = DisposableProviderPool(keyManager: keyManager)
	}
	...
	func testOpenAIAPI(with apiKey: String? = nil, baseURL: String? = nil) async throws -> Bool {
		...
		if let apiKey = apiKey {
			...
			key = try await keyManager.getAPIKey(for: .openAI)
		}
    ...
    func testGeminiAPI(with apiKey: String? = nil) async throws -> Bool {
        ...
        if let apiKey {
            provider = try await AIProviderFactory.createProvider(for: .gemini, key: apiKey) as! GeminiProvider
        } else {
            provider = try await AIProviderFactory.createProvider(for: .gemini, keyManager: keyManager) as! GeminiProvider
        }
		return try await withTimeout(seconds: 30) { try await provider.testAPIKey() }
    }
    ...
    func testOpenRouterAPI(with apiKey: String? = nil, model: AIModel = .openrouterGpt5) async throws -> Bool {
        ...
        if let apiKey {
            provider = try await AIProviderFactory.createProvider(for: .openRouter, key: apiKey) as! OpenRouterProvider
        } else {
            provider = try await AIProviderFactory.createProvider(for: .openRouter, keyManager: keyManager) as! OpenRouterProvider
        }
	...
	func testDeepSeekAPI(with apiKey: String? = nil) async throws -> Bool {
		...
		if let apiKey {
			provider = try await AIProviderFactory.createProvider(for: .deepseek, key: apiKey) as! DeepSeekProvider
		} else {
			provider = try await AIProviderFactory.createProvider(for: .deepseek, keyManager: keyManager) as! DeepSeekProvider
		}
	...
	func testFireworksAPI(with apiKey: String? = nil) async throws -> Bool {
		...
		if let apiKey {
			provider = try await AIProviderFactory.createProvider(for: .fireworks, key: apiKey) as! FireworksProvider
		} else {
			provider = try await AIProviderFactory.createProvider(for: .fireworks, keyManager: keyManager) as! FireworksProvider
		}
		return try await withTimeout(seconds: 30) { try await provider.testAPIKey() }
	}
	...
	func testGrokAPI(with apiKey: String? = nil) async throws -> Bool { // NEW
		...
		if let apiKey {
			provider = try await AIProviderFactory.createProvider(for: .grok, key: apiKey) as! GrokProvider
		} else {
			provider = try await AIProviderFactory.createProvider(for: .grok, keyManager: keyManager) as! GrokProvider
		}
	...
	func testGroqAPI(with apiKey: String? = nil) async throws -> Bool { // NEW
		...
		if let apiKey {
			provider = try await AIProviderFactory.createProvider(for: .groq, key: apiKey) as! GroqProvider
		} else {
			provider = try await AIProviderFactory.createProvider(for: .groq, keyManager: keyManager) as! GroqProvider
		}
	...
	func testZAIAPI(with apiKey: String? = nil) async throws -> Bool {
		...
		if let apiKey {
			provider = try await AIProviderFactory.createProvider(for: .zAI, key: apiKey) as! ZAIProvider
		} else {
			provider = try await AIProviderFactory.createProvider(for: .zAI, keyManager: keyManager) as! ZAIProvider
		}
		return try await withTimeout(seconds: 30) { try await provider.testAPIKey() }
	}
