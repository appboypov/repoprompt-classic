...
final class ClaudeCodeAgentProvider: HeadlessAgentProvider {
	...
	func buildArguments(
		...
		var args: [String] = [
			"-p",
			"--verbose",  // required for stream-json mode to emit events
			"--output-format", "stream-json"
		]
		
		// If resuming a session, use --resume. RepoPrompt-generated instructions
		// are XML-injected into stdin, not passed via prompt flags.
		...
		// Add model argument if specified (and not "default")
		if let model = ClaudeModelSpecifier(raw: context.launchEnvironment?.effectiveModel).runtimeModelParam {
		...
		// Headless runs (discovery + delegate edit) skip all permission prompts.
		// This is safe because native tools are blocked via --disallowedTools
		// and MCP tools have their own policy layer.
		...
		// Disable Claude's built-in tools so the agent uses MCP tools instead
		// MCP tool restrictions are handled via ServerNetworkManager connection policies
		...
		// Don't append userMessage as positional arg - it will be passed via stdin
		if enableDebugLogging {
				...
				await withTaskCancellationHandler(operation: {
					...
					let context = try await self.prepare(runID: runID)
						try await AsyncScope.withCleanup({}, cleanup: {
							...
							let userMessage = nativePromptMode.sendsRepoPromptAsUserMessage
								? ClaudeCodePromptDelivery.decoratedUserMessage(
									message.userMessage,
