...
struct CustomProviderConfiguration: Codable {
	...
	init(from decoder: Decoder) throws {
		...
		self.enabledModels = try container.decode(Set<String>.self, forKey: .enabledModels)
		self.maxTokens = try container.decodeIfPresent(Int.self, forKey: .maxTokens)
		self.userPreferredModel = try container.decodeIfPresent(String.self, forKey: .userPreferredModel)
		// Default to false for backwards compatibility
		self.includeContentTypeHeader = try container.decodeIfPresent(Bool.self, forKey: .includeContentTypeHeader) ?? false
		self.apiVersion = try container.decodeIfPresent(String.self, forKey: .apiVersion) // may be nil for legacy
	}
