...
class CustomOpenAIProvider: AIProvider, AIModelGetter {
	...
	struct ModelsResponse: Codable {
		...
		init(from decoder: Decoder) throws {
			...
			if let arrayData = try? container.decode([Model].self) {
				...
				self.data = try? keyedContainer.decode([Model].self, forKey: .data)
				self.models = try? keyedContainer.decode([Model].self, forKey: .models)
			}
		}
						...
						guard httpResponse.statusCode == 200 else {
							...
							do {
								for try await byte in bytes {
									...
									if errorData.count > 10_000 { break }
								}
							} catch {
	...
	private func shouldSkipTemperature(for model: String) -> Bool {
		...
		// Skip temperature for provider model IDs that reject it.
	...
	private func createCompletionRequest(
		...
		let openAIMessages = messages.map { message -> ChatMessage in
			...
			case .contentArray(let array):
				content = array.compactMap { chunk -> String? in
					if case .text(let part) = chunk {
						return part
					}
					return nil
				}.joined(separator: "\n")
		...
		let maxTokensToUse = (finalMaxTokens == 2_048) ? nil : finalMaxTokens
