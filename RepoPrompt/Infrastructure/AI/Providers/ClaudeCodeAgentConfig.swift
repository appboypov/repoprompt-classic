...
struct ClaudeCodeAgentConfig {
	...
	var processEnvironmentOverrides: [String: String] {
		...
		case .agentRun, .terminal:
			return MCPIntegrationHelper.claudeProcessEnvironmentOverrides
		case .discoverRun, .delegateEdit, .promptOnly:
			return [:]
		}
	}
	...
	// MARK: - Discovery (headless, restricted read-only toolset)

	/// Config for headless discovery runs.
	/// Permissions are skipped via --dangerously-skip-permissions at the CLI level.
	/// Enables native Bash for read-only inspection while keeping edit/file/task tools restricted.
	...
	// MARK: - Delegate Edit (headless, single-file sandbox)

	/// Config for headless delegate-edit runs.
	/// Permissions are skipped via --dangerously-skip-permissions at the CLI level.
	/// Never reads from agent mode user preferences.
	static func delegateEdit(
		commandName: String? = nil,
		modelString: String? = nil,
		runtimeVariant: ClaudeCodeRuntimeVariant = .standard,
		enableDebugLogging: Bool = false
	...
	private init(
		...
		self.additionalPathHints = CLIPathHints.claudeCode
