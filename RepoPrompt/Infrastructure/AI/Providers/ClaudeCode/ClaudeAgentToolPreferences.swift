...
struct ClaudeAgentToolPreferences {
	enum AgentModePromptDelivery: String, CaseIterable, Sendable {
		case userMessageXML
		...
		var displayName: String {
			switch self {
			case .nativeSystemPrompt:
				return "Replace System Prompt"
			case .userMessageXMLWithEmptySystemPrompt:
				return "User Message (No Native)"
			case .userMessageXML:
				return "User Message (Keep Native)"
			}
		}
		...
		var detailText: String {
			switch self {
			case .nativeSystemPrompt:
				return "RepoPrompt instructions replace Claude Code's native system prompt."
			case .userMessageXMLWithEmptySystemPrompt:
				return "RepoPrompt instructions are added to the user message, and Claude Code's native system prompt is removed."
			case .userMessageXML:
				return "RepoPrompt instructions are added to the user message, and Claude Code keeps its native prompt."
			}
		}
	...
	enum PermissionLevel: String, CaseIterable, Sendable {
		...
		var detailText: String {
			switch self {
			case .requireApproval:
				return "Claude prompts for approval before running tools."
			case .autoApproveEdits:
				return "Claude auto-approves file edits and common filesystem commands."
			case .auto:
				return "Claude auto-approves tool calls that pass background safety checks. Research preview — behavior may change."
			case .fullAccess:
				return "Claude runs every tool without prompting. Use only in trusted environments."
			}
		}
	...
	enum UnsupportedAutoPermissionFallback: Sendable, Equatable {
		case autoApproveEdits
		case fullAccess
	}

	struct PermissionModeResolution: Sendable, Equatable {
	...
	static func storedEffortLevel(
		...
		guard includeLegacyFallback,
			...
			isEffort(legacyEffort, supportedForModelRaw: concreteModelRaw, agentKind: agentKind) else {
			return nil
		}
		return legacyEffort
	}
	...
	private static func concreteEffortModelRaw(_ modelRaw: String?) -> String? {
		...
		guard let trimmed,
			!trimmed.isEmpty,
			trimmed.caseInsensitiveCompare(AgentModel.defaultModel.rawValue) != .orderedSame else {
			return nil
		}
		return trimmed
	}
...
enum ClaudeCodeEffortLevel: String, CaseIterable, Sendable {
