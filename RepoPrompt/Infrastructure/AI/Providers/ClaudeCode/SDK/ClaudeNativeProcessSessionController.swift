...
protocol ClaudeSessionControlling: Actor {
	var hasActiveSession: Bool { get }
	var hasTurnInFlight: Bool { get }
	var events: AsyncStream<ClaudeNativeProcessSessionController.Event> { get }
	...
	/// Sends a reasoned interrupt control_request to Claude Code.
...
final actor ClaudeNativeProcessSessionController {
	...
	struct RuntimeInitStatus: Sendable, Equatable {
		struct InitializeResponseSnapshot: Sendable, Equatable {
	...
	enum InterruptOutcome: Sendable, Equatable {
	...
	enum ControllerError: Error, LocalizedError {
		...
		var errorDescription: String? {
			...
			case .initializationFailed(let message):
				return "Claude initialization failed: \(message)"
			case .invalidControlResponse(let message):
				return "Claude control response failed: \(message)"
			case .inputWriteFailed(let message):
				return "Failed writing to Claude process stdin: \(message)"
			case .controlRequestTimedOut(let requestID):
				return "Claude control request timed out: \(requestID)"
			}
		}
	}
	...
	private var turnInFlight: Bool { pendingTurnIDHead < pendingTurnIDBuffer.count }
	...
	var events: AsyncStream<Event> { eventsStream }
	var hasActiveSession: Bool { process != nil }
	var hasTurnInFlight: Bool { pendingTurnIDHead < pendingTurnIDBuffer.count }
	...
	init(
		...
		self.rawEventLogFileSessionID = nil
		self.translator = ClaudeSDKNDJSONTranslator(enableDebugLogging: config.enableDebugLogging)
	...
	private func routeInboundMessage(_ inbound: ClaudeSDKProtocolCodec.InboundMessage) {
		switch inbound {
		case .streamPayload(let payload):
	...
	static func parseInitializeResponseSnapshot(
		...
		let commands: [RuntimeInitStatus.InitializeResponseSnapshot.Command]
		...
		let agents: [RuntimeInitStatus.InitializeResponseSnapshot.Agent]
		...
		let account: RuntimeInitStatus.InitializeResponseSnapshot.Account?
	...
	private func permissionResponsePayload(
		...
		switch decision {
		case .accept:
			return Self.allowPermissionResponsePayload(
	...
	private func registerExpectedAgentPIDIfNeeded(_ pid: pid_t) async {
		guard config.toolContext == .agentRun,
			let clientName = config.runtimeVariant.agentKind.mcpClientNameHint else {
			return
		}
	...
	private static func isRawEventFileLoggingEnabled() -> Bool {
		...
		true
		#else
		UserDefaults.standard.bool(forKey: rawEventLoggingEnabledKey)
		#endif
	}
	...
	private static func makeRawEventLogFileURL(workspacePath: String?, sessionID: String) -> URL? {
		...
		let baseDirectory: URL = {
			...
			if let workspacePath = workspacePath?.trimmingCharacters(in: .whitespacesAndNewlines),
				...
				// Keep debug traces easy to discover beside the active workspace.
	...
	private var hasPendingTurnIDs: Bool { pendingTurnIDHead < pendingTurnIDBuffer.count }
...
extension ClaudeNativeProcessSessionController: ClaudeSessionControlling {}
