...
enum ClaudeCodeIntegrationConfiguration {
	...
	struct BatchInstallResult {
		...
		var success: Bool { successCount > 0 }
	}

	/// Claude Code tools to disallow during Agent Mode runs.
	/// Keep native Read and Skill enabled in Agent Mode so Claude can still
	/// discover and invoke workspace/user skills from `.claude/skills`.
	/// Bash can be opt-in enabled for trusted runs, including explore sessions.
	...
	/// Native file/edit/task tools stay blocked; Bash entries remain in the base list for callers
	/// that do not opt in. `ClaudeCodeAgentConfig.discovery()` opts into Bash for read-only inspection.
	...
	/// Claude Code tools to disallow during delegate-edit runs.
	/// Everything blocked — delegate edit is a focused single-file sandbox
	/// that only needs MCP read_file and apply_edits.
	private static let delegateEditDisallowedTools: [String] = [
		"Bash",
		"Read",
		"Write",
		"Edit",
	...
	static func disallowedTools(
		...
		switch context {
		case .agentRun:
			base = agentDisallowedTools
		case .discoverRun:
			base = discoverDisallowedTools
		case .delegateEdit:
			base = delegateEditDisallowedTools
		case .promptOnly:
			base = agentDisallowedTools
		case .terminal:
			base = terminalDisallowedTools
		}
		let bashToolNames = ["Bash", "BashOutput", "KillShell"]
		if allowNativeBashTool {
			let bashToolNameSet = Set(bashToolNames)
			return base.filter { !bashToolNameSet.contains($0) }
		}
		return appendingMissingDisallowedTools(bashToolNames, to: base)
	}

	private static func appendingMissingDisallowedTools(_ tools: [String], to base: [String]) -> [String] {
		var result = base
		var seen = Set(base)
	...
	static func installInClaudeCode(
		...
		do {
			...
			let message: String
			if let runnerError = error as? CLIProcessRunnerError, case .commandNotFound = runnerError {
				message = "Claude Code CLI not found. Install it from claude.ai/download"
			} else {
				message = "Install error: \(error.localizedDescription)"
			}
	...
	private static func friendlyClaudeCodeError(_ stderr: String) -> String {
		...
		if !stderr.isEmpty {
			...
			let firstLine = stderr.components(separatedBy: .newlines).first(where: { !$0.isEmpty }) ?? stderr
			return firstLine
		}
