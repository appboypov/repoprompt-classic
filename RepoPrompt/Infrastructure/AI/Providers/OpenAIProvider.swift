...
class OpenAIProvider: AIProvider {
	...
	open func getService() -> OpenAIService {
		...
		if let baseURL = cachedBaseURL {
			...
			configuration.timeoutIntervalForRequest = 21_600 // 6 hours
			configuration.timeoutIntervalForResource = 21_600 // 6 hours
	...
	func testAPIKey(model: AIModel = .gpt54Mini) async throws -> Bool {
		...
		do {
			let response = try await self.callAppropriateCompletion(for: model, message: testMessage, nil)
		...
		for fallback in fallbacks {
			do {
				...
				let response = try await self.callAppropriateCompletion(for: fallback, message: testMessage, nil)
				if response.lowercased().contains("hello") { return true }
			} catch {
