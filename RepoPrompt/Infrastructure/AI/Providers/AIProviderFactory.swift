...
class AIProviderFactory {
	static func createProvider(
		...
		if providerType == .azure, let config = azureConfiguration {
			return try await createProvider(for: providerType, key: "", ollamaURL: ollamaURL, azureConfiguration: config)
		}
		
		// CLI providers don't need API keys - they leverage existing authentication
		if providerType == .claudeCode || providerType == .codex || providerType == .geminiCli || providerType == .openCode || providerType == .cursor {
			return try await createProvider(for: providerType, key: "", ollamaURL: ollamaURL, azureConfiguration: azureConfiguration)
		}
		
		guard let key = try await keyManager.getAPIKey(for: providerType) else {
			throw AIProviderError.missingAPIKey
		}
		return try await createProvider(for: providerType, key: key, ollamaURL: ollamaURL, azureConfiguration: azureConfiguration)
	}
...
extension AIProviderType {
	...
	var displayName: String {
		return Self.displayName(for: self)
	}

	static func pickerSortPrecedes(_ lhs: AIProviderType, _ rhs: AIProviderType) -> Bool {
		ModelPickerStringOrdering.precedes(displayName(for: lhs), displayName(for: rhs))
	}
}
