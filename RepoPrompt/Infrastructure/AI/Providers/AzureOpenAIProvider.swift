...
public struct AzureOpenAIConfiguration: Codable, Equatable {
	...
	public init(from decoder: Decoder) throws {
		...
		self.models = try container.decodeIfPresent([ModelDescriptor].self, forKey: .models) ?? []
	...
	static func inferredEndpoint(for modelID: String, baseModelID: String? = nil) -> ModelDescriptor.EndpointKind {
		let candidates = [baseModelID, modelID].compactMap { $0 }
				...
				do {
					...
					while !Task.isCancelled {
						...
						if response.status != lastStatus {
							...
							case .queued:
								statusMessage = "Job queued, waiting for processing...\n"
							case .inProgress:
								statusMessage = "Processing started...\n"
							case .completed, .failed, .incomplete, .cancelled, .none:
								statusMessage = "" // Will be handled below
							}
...
final class AzureOpenAIProvider: AIProvider {
	...
	private lazy var urlSessionConfiguration: URLSessionConfiguration = {
		...
		config.timeoutIntervalForRequest = 21_600 // 6 hours
		config.timeoutIntervalForResource = 21_600 // 6 hours
		return config
	}()
	private lazy var azureService: OpenAIService = {
	...
	private func resolveBaseModelAndEffort(for model: AIModel) -> (AIModel, String?) {
		...
		case .gpt5Pro:
			return (.gpt5Pro, "high")
		case .gpt5ProXHigh:
			return (.gpt5Pro, "xhigh")
		case .gpt54Pro:
			return (.gpt54Pro, "high")
		case .gpt54ProXHigh:
			return (.gpt54Pro, "xhigh")
		...
		case .gpt5CodexXHigh:
			return (.gpt5CodexMed, "xhigh")
		case .gpt5CodexHigh:
			return (.gpt5CodexMed, "high")
		case .gpt5CodexMed:
			return (.gpt5CodexMed, "medium")
		case .gpt5CodexLow:
			return (.gpt5CodexMed, "low")
	...
	private func resolvedMaxTokens(for model: AIModel?, override: Int?) -> Int? {
		...
		return defaultValue == 2_048 ? nil : defaultValue
	}
...
extension AzureOpenAIProvider {
	...
	private static func normalizedDescriptor(_ descriptor: AzureOpenAIConfiguration.ModelDescriptor) -> AzureOpenAIConfiguration.ModelDescriptor {
		...
		let resolvedBaseModelID: String
		if let explicitBase = descriptor.baseModelID, !explicitBase.isEmpty {
			resolvedBaseModelID = explicitBase
		} else if let openAIModel {
			resolvedBaseModelID = openAIModel.rawValue
		} else {
			resolvedBaseModelID = normalizedID
		}
