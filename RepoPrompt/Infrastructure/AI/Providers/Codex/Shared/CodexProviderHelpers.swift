...
struct CodexProviderHelpers {
	...
	private static func codexExecutableUserMessage(
		...
		case .available:
			return ""
		case .notFound:
			return "Codex CLI executable was not found. Install Codex CLI and ensure `codex` is available in your login shell PATH. RepoPrompt searched your login-shell PATH plus common Homebrew, npm/pnpm/yarn/Volta, Bun, Cargo, version-manager shim, and Codex.app locations."
		case .missingResolvedPath:
			return "Codex CLI resolved to `\(resolvedCommand)`, but that file does not exist. Reinstall Codex CLI or fix your shell PATH."
		case .resolvedToDirectory:
			return "Codex CLI resolved to `\(resolvedCommand)`, but that path is a directory. Fix your shell PATH so `codex` points to the executable."
		case .notExecutable:
			return "Codex CLI resolved to `\(resolvedCommand)`, but it is not executable. Check file permissions or reinstall Codex CLI."
		}
	}
	...
	private static func isInsideInlineCodeSpan(_ text: String) -> Bool {
		let backtickCount = text.replacingOccurrences(of: "```", with: "").filter { $0 == "`" }.count
