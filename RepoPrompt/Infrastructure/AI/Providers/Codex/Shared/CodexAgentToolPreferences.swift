...
struct CodexAgentToolPreferences {
	enum AppServerRequestValueStyle: CaseIterable, Sendable {
		case configStyle
		case camelCase
	}

	enum ApprovalPolicy: CaseIterable, Sendable {
		case onRequest
		case onFailure
		case unlessTrusted
		case never
	...
	enum SandboxMode: CaseIterable, Sendable {
		case readOnly
		case workspaceWrite
	...
	enum ApprovalReviewer: CaseIterable, Sendable {
		case user
		case autoReview
	...
	enum PermissionLevel: String, CaseIterable, Sendable {
		...
		var displayName: String {
			...
			case .defaultPermission: return "Default"
		...
		var iconName: String {
			...
			case .defaultPermission: return "shield"
			case .autoReview: return "checkmark.shield"
			case .fullAccess: return "exclamationmark.shield.fill"
			}
		}
		...
		var approvalPolicy: ApprovalPolicy {
			switch self {
			case .readOnly, .defaultPermission, .autoReview: return .onRequest
		...
		var sandboxMode: SandboxMode {
			...
			case .defaultPermission, .autoReview: return .workspaceWrite
		...
		var approvalReviewer: ApprovalReviewer {
			switch self {
			case .autoReview: return .autoReview
			case .readOnly, .defaultPermission, .fullAccess: return .user
			}
		}
		...
		static func from(sandbox: SandboxMode, approvalReviewer: ApprovalReviewer) -> PermissionLevel {
			switch sandbox {
			case .readOnly: return .readOnly
			case .workspaceWrite: return approvalReviewer == .autoReview ? .autoReview : .defaultPermission
			case .dangerFullAccess: return .fullAccess
			}
		}
	}
	...
	static func approvalPolicy(
		...
		guard let raw = defaults.string(forKey: approvalPolicyKey),
			  let policy = ApprovalPolicy(storedValue: raw) else {
			return .onRequest
		}
		return policy
	}
	...
	static func sandboxMode(
		...
		guard let raw = defaults.string(forKey: sandboxModeKey),
			  let mode = SandboxMode(storedValue: raw) else {
			return .workspaceWrite
		}
		return mode
	}
	...
	static func approvalReviewer(
		...
		guard let raw = defaults.string(forKey: approvalReviewerKey),
			  let reviewer = ApprovalReviewer(storedValue: raw) else {
			return .user
		}
		return reviewer
	}
	...
	static func lastUsedReasoningEffort(defaults: UserDefaults = .standard) -> CodexReasoningEffort? {
		...
		guard let legacyRaw = defaults.string(forKey: legacyLastUsedReasoningEffortKey),
			let legacyEffort = CodexReasoningEffort.parse(legacyRaw) else {
			return nil
		}
	...
	static func mcpServerEnabled(
		...
		let serverActive = storedMCPServerToggles(defaults: defaults)[key] ?? false
		return serverActive
	}
