...
final class CodexExecAgentProvider: HeadlessAgentProvider {
	...
	private static let codexMCPClientID = DiscoverAgentKind.codexMCPClientID
					...
					while attemptNumber < maxAttempts {
						...
						do {
							...
							if self.enableDebugLogging {
								...
								print("[DEBUG] CodexExec: MCP servers detected: \(serverEntries.map { $0.normalizedName })")
							...
							try await AsyncScope.withCleanup({}, cleanup: {
								...
								if status != 0 || timedOut {
									...
									switch retryAction {
									case .modelFallback(let fallbackModel):
									...
									case .brokenServer(let brokenServer):
