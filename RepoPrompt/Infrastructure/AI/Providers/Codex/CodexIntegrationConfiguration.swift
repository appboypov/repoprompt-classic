...
enum CodexIntegrationConfiguration {
	...
	static let desiredToolOutputTokenLimit = 25_000
	...
	private static var serverCommand: String { repoPromptMCPConfiguration.command }
	...
	static func configOverrides(for context: AgentCLIToolContext) -> [String] {
		...
		switch context {
		case .agentRun:
			toolPolicy = CodexOverrides.ToolPolicy(
	...
	static func configContainsRepoPrompt() -> Bool {
		mcpServerEntries().contains {
			$0.normalizedName.compare(repoPromptMCPServerName, options: .caseInsensitive) == .orderedSame
		}
	}
	...
	private static func ensureRepoPromptServer(
	...
	private static func ensureKey(
		...
		if let first = indices.first {
			...
			if let afterKey = afterKey,
				...
				insertionIndex = blockRange.start + 1
			}
