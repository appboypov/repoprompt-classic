...
"""
	...
	func streamMessage(_ aiMessage: AIMessage, model: AIModel, maxTokens _: Int? = nil) async throws -> AsyncThrowingStream<AIStreamResult, Error> {
		...
		return AsyncThrowingStream { continuation in
			...
			let bridgeTask = Task { [weak self] in
				...
				do {
					try await self.streamViaAppServer(
			...
			try await withTaskCancellationHandler(operation: {
				...
				eventLoop: for await event in controller.events {
					...
					case .errorNotification(let error):
						if error.willRetry || Self.isRetriableStreamErrorMessage(error.message) {
							continuation.yield(AIStreamResult(type: "status", text: error.message))
							continue
						}
						throw AIProviderError.invalidConfiguration(detail: error.message)
	...
	private func makeInteractiveSessionController(
		...
		let options = CodexNativeSessionController.Options(
			...
			configOverridesProvider: { [weak self] in
				...
				return self.interactiveConfigOverrides(excludeServers: excludeServers)
			},
	...
	private func modelIdentifier(for model: AIModel) -> String? {
		...
		default:
			return model.providerType == .codex ? model.modelName : nil
		}
	}
