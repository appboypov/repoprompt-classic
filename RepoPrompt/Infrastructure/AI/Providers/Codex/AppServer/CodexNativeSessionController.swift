...
final class CodexNativeSessionController {
	...
	private static let maxRunningAggregatedOutputCharacters = 24_000
	...
	struct CommandExecutionRunningUpdate: Sendable {
	...
	struct ReasoningDeltaPayload: Sendable, Equatable {
	...
	private enum CommandExecutionEventFamily: Sendable {
		case raw
		case normalized
	}
	...
	struct ChatgptAuthTokensRefreshRequest: Sendable, Equatable {
	...
	struct ChatgptAuthTokensRefreshResponse: Sendable, Equatable {
	...
	struct ServerRequestIssue: Sendable, Equatable {
	...
	struct LivenessActivity: Sendable, Equatable {
		enum Kind: String, Sendable, Equatable {
	...
	struct ErrorNotification: Sendable, Equatable {
		let message: String
		let willRetry: Bool
		let threadID: String?
		let turnID: String?
	...
	enum ThreadGoalStatus: String, Sendable, Equatable {
		case active
		case paused
		case budgetLimited
		case complete
	...
	struct ThreadGoal: Sendable, Equatable {
		let threadID: String
	...
	enum ClientShutdownBehavior: Sendable {
		case none
		case stopOnShutdown
	}
	...
	private enum BufferedInbound: Sendable {
		case notification(CodexAppServerClient.Notification)
		case serverRequest(CodexAppServerClient.ServerRequest)
	}

	private enum BufferedInboundRetention: Sendable {
		case critical
		case normal
	}

	private enum InboundStreamKind: Sendable {
		case notifications
		case serverRequests
	}
	...
	private var bufferedInboundOverflowCount = 0
	private var bufferedInboundDroppedNormalCount = 0
	private var bufferedInboundCriticalSoftCapExceededCount = 0
	...
	private func requestWithCompatibleAppServerRequestValueStyle(
		...
		do {
			return try await client.request(
				method: method,
				params: await paramsBuilder(attemptedStyle),
				timeout: timeout
			)
		} catch {
			...
			let result = try await client.request(
				method: method,
				params: await paramsBuilder(fallbackStyle),
				timeout: timeout
			)
	...
	init(
		...
		self.rawEventFileLoggingEnabled = Self.isRawEventFileLoggingEnabled()
		self.rawEventLogFileURL = nil
		self.rawEventLogFileThreadID = nil
	...
	func getThreadGoal() async throws -> ThreadGoal? {
		...
		let result = try await client.request(
			...
			params: [
				"threadId": try resolvedThreadID(nil)
			],
			timeout: options.requestTimeout
		)
	...
	func cancelCurrentTurn() async {
		guard let initialThreadID = threadID?.trimmingCharacters(in: .whitespacesAndNewlines),
			!initialThreadID.isEmpty
		else { return }
		let refreshedTurnID = await refreshActiveTurnForInterruptIfPossible()
		let resolvedThreadID = threadID?.trimmingCharacters(in: .whitespacesAndNewlines)
		let interruptThreadID = (resolvedThreadID?.isEmpty == false ? resolvedThreadID : nil) ?? initialThreadID
		guard let turnID = (refreshedTurnID ?? currentTurnID)?.trimmingCharacters(in: .whitespacesAndNewlines),
			!turnID.isEmpty
		else { return }
		do {
			try await interruptTurn(threadID: interruptThreadID, turnID: turnID)
		} catch {
			...
			do {
				try await interruptTurn(threadID: interruptThreadID, turnID: actualTurnID)
			} catch {
				if let retryActualTurnID = Self.activeTurnMismatchActualTurnID(from: error), retryActualTurnID != actualTurnID {
	...
	private func reconcileActiveTurnRoutingState(from snapshot: ThreadSnapshot) {
		...
		var orderedActiveTurnIDs: [String] = []
		for turnID in snapshot.activeTurnIDs {
			let trimmed = turnID.trimmingCharacters(in: .whitespacesAndNewlines)
			guard !trimmed.isEmpty, !orderedActiveTurnIDs.contains(trimmed) else { continue }
			orderedActiveTurnIDs.append(trimmed)
		}
		let snapshotCurrentTurnID = snapshot.currentTurnID?.trimmingCharacters(in: .whitespacesAndNewlines)
		if let snapshotCurrentTurnID, !snapshotCurrentTurnID.isEmpty, !orderedActiveTurnIDs.contains(snapshotCurrentTurnID) {
			orderedActiveTurnIDs.append(snapshotCurrentTurnID)
		}
		currentTurnID = snapshotCurrentTurnID?.isEmpty == false ? snapshotCurrentTurnID : orderedActiveTurnIDs.last
		activeTurnIDs = Set(orderedActiveTurnIDs)
		activeTurnOrder = orderedActiveTurnIDs
		activeTurnIDsWithObservedActivity = Set(orderedActiveTurnIDs)
	}

	private func refreshActiveTurnForInterruptIfPossible() async -> String? {
		...
		do {
			...
			reconcileActiveTurnRoutingState(from: snapshot)
			return snapshot.currentTurnID?.trimmingCharacters(in: .whitespacesAndNewlines)
		} catch {
	...
	static func activeTurnMismatchActualTurnID(fromErrorDescription description: String) -> String? {
		let expectedPrefix = "expected active turn id `"
		guard let expectedRange = description.range(of: expectedPrefix) else { return nil }
		let afterExpected = description[expectedRange.upperBound...]
		let separator = "` but found `"
		guard let separatorRange = afterExpected.range(of: separator) else { return nil }
		let afterSeparator = afterExpected[separatorRange.upperBound...]
		guard let endRange = afterSeparator.range(of: "`") else { return nil }
		let actualTurnID = afterSeparator[..<endRange.lowerBound]
			.trimmingCharacters(in: .whitespacesAndNewlines)
		return actualTurnID.isEmpty ? nil : actualTurnID
	}
	...
	private func updateClientProcessFeaturePolicy() async {
		await client.updateProcessFeaturePolicy(await currentFeaturePolicy())
	}
	...
	private func startNotificationStreamIfNeeded() async {
		...
		notificationTask = Task { [weak self] in
			...
			guard let self else { return }
			await self.handleInboundStreamDidExit(kind: .notifications, source: "notifications")
		}
	}
	...
	private func startServerRequestStreamIfNeeded() async {
		...
		serverRequestTask = Task { [weak self] in
			...
			guard let self else { return }
			await self.handleInboundStreamDidExit(kind: .serverRequests, source: "serverRequests")
		}
	}
	...
	private func appendBufferedInbound(_ inbound: BufferedInbound) {
		guard bufferedInbound.count >= maxBufferedInbound else {
			bufferedInbound.append(inbound)
			return
		}

		bufferedInboundOverflowCount += 1
		switch bufferedInboundRetention(for: inbound) {
		case .critical:
			if let normalIndex = bufferedInbound.firstIndex(where: { bufferedInboundRetention(for: $0) == .normal }) {
				bufferedInbound.remove(at: normalIndex)
			} else {
				bufferedInboundCriticalSoftCapExceededCount += 1
				Self.logCodexDebug("[CodexNativeController] pre-bind buffer soft cap exceeded by critical-only inbound events count=\(bufferedInbound.count)")
			}
			bufferedInbound.append(inbound)
		case .normal:
			if let normalIndex = bufferedInbound.firstIndex(where: { bufferedInboundRetention(for: $0) == .normal }) {
				...
				bufferedInboundDroppedNormalCount += 1
				Self.logCodexDebug("[CodexNativeController] dropping normal pre-bind inbound event because buffer contains only critical events")
			}
		}
	}

	private func bufferedInboundRetention(for inbound: BufferedInbound) -> BufferedInboundRetention {
		switch inbound {
		case .serverRequest:
			return .critical
		case .notification(let notification):
			return Self.bufferedNotificationRetention(method: notification.method)
		}
	}

	private static func bufferedNotificationRetention(method: String) -> BufferedInboundRetention {
		switch method {
		case "error",
			...
			return .critical
		default:
			return isItemLifecycleNotificationMethod(method) ? .critical : .normal
		}
	}
	...
	private func handleNotification(_ notification: CodexAppServerClient.Notification) async {
		...
		// Pre-register observed turn IDs before the routing drop decision only when
		// local tracking is uncertain or the notification has strong streaming-item
		// correlation. This prevents cascade drops without making stale generic
		// liveness/control notifications appear active.
		if Self.isTurnOrItemScopedNotificationMethod(notification.method) {
			...
			let trackingIsUncertain = activeTurnIDs.isEmpty && currentTurnID == nil
			let hasStrongStreamingCorrelation = Self.isStreamingItemRelatedNotification(
				method: notification.method,
				params: params
			) && Self.hasStrongStreamingItemCorrelation(method: notification.method, params: params)
			if threadMatches,
				trackingIsUncertain || hasStrongStreamingCorrelation,
				let observedTurnID = Self.notificationTurnID(from: params) {
		...
		case "item/reasoning/textDelta":
			...
			case let method where Self.isItemLifecycleNotificationMethod(method):
				if let toolEvent = parseToolLifecycleEvent(method: method, params: params) {
					switch toolEvent {
					case .call(let name, let invocationID, let argsJSON, let dedupKey):
					...
					case .result(let name, let invocationID, let argsJSON, let resultJSON, let isError, let dedupKey):
			...
			case "item/fileChange/outputDelta",
				...
				if let patchUpdate = parseFileChangeOutputDeltaEvent(params: params),
					case .result(let name, let invocationID, let argsJSON, let resultJSON, let isError, let dedupKey) = patchUpdate {
					let emitted = markToolEventEmitted(key: "result:\(dedupKey)")
	...
	private static func livenessActivityMessage(from params: [String: Any]) -> String? {
		firstString(
			in: params,
	...
	private static func notificationThreadID(from params: [String: Any]) -> String? {
		...
		for candidate in notificationEnvelopeDictionaries(from: params) {
			if let threadID = firstString(in: candidate, keys: threadIDKeys) {
				return threadID
			}
			if let threadID = stringScalarValue(from: candidate["thread"]) {
				return threadID
			}
			if let turn = candidate["turn"] as? [String: Any],
				let threadID = firstString(in: turn, keys: threadIDKeys) {
				return threadID
			}
			if let thread = candidate["thread"] as? [String: Any],
				let threadID = firstString(in: thread, keys: ["id"] + threadIDKeys) {
				return threadID
			}
		}
		return nil
	...
	private static func notificationTurnID(from params: [String: Any]) -> String? {
		...
		for candidate in notificationEnvelopeDictionaries(from: params) {
			if let turnID = firstString(in: candidate, keys: turnIDKeys) {
				return turnID
			}
			if let turnID = stringScalarValue(from: candidate["turn"]) {
				return turnID
			}
			if let turn = candidate["turn"] as? [String: Any],
				let turnID = firstString(in: turn, keys: ["id"] + turnIDKeys) {
				return turnID
			}
		}
	...
	private static func notificationItemID(from params: [String: Any]) -> String? {
		...
		for candidate in notificationEnvelopeDictionaries(from: params) {
			...
			if let itemID = stringScalarValue(from: candidate["item"]) {
				return itemID
			}
		}
	...
	static func test_parseLivenessActivity(method: String, params: [String: Any]) -> LivenessActivity? {
	...
	func test_parseToolLifecycleEvent(method: String, params: [String: Any]) -> TestToolLifecycleEvent? {
		...
		switch event {
		case .call(let name, let invocationID, let argsJSON, _):
		...
		case .result(let name, let invocationID, let argsJSON, let resultJSON, let isError, _):
			return TestToolLifecycleEvent(
				kind: "result",
				name: name,
				invocationID: invocationID,
	...
	private func buildApprovalRequest(
		...
		return Self.parseApprovalRequest(
			requestID: id,
			method: method,
			params: params,
			activeThreadID: threadID,
	...
	private func computerUseMCPElicitationAutoAcceptResult(params: [String: Any]) async -> [String: Any]? {
		...
		guard computerUseEnabled,
			...
			Self.isComputerUseMCPElicitationRequest(params: params) else {
			return nil
		}
		return [
			"action": "accept",
		...
		for preferredKind in ["allow_once", "allow_always"] {
			for option in options {
				guard let kind = stringScalarValue(from: option["kind"])?.lowercased(),
					kind == preferredKind,
					let optionID = stringScalarValue(from: option["optionId"]) else {
					continue
				}
				return [
					"outcome": [
	...
	private static func debugEventSummary(_ event: Event) -> String {
		...
		case .livenessActivity(let activity):
			return "livenessActivity kind=\(activity.kind.rawValue) method=\(activity.method) threadID=\(activity.threadID ?? "nil") turnID=\(activity.turnID ?? "nil") flags=\(activity.activeFlags.joined(separator: ","))"
		case .errorNotification(let error):
			return "errorNotification willRetry=\(error.willRetry) threadID=\(error.threadID ?? "nil") turnID=\(error.turnID ?? "nil") message=\(debugPreview(error.message))"
		case .error(let message):
			return "error \(debugPreview(message))"
		case .system(let message):
			return "system \(debugPreview(message))"
		}
	}
	...
	static func applyCommandExecutionRunningUpdate(
		...
		guard let target else {
			logCodexDebug("[CodexNativeController] runningUpdateMiss invocationID=\(update.invocationID?.uuidString ?? "nil") processID=\(update.processID ?? "nil") outputChars=\(update.appendedOutput?.count ?? 0) bashCount=\(items.filter { normalizedExternalToolName($0.toolName) == "bash" }.count)")
			return false
		}
...
enum CodexSessionControllerError: LocalizedError {
	...
	var errorDescription: String? {
		switch self {
		case .imageAttachmentsUnsupported:
			return "This Codex session controller does not support image attachments."
		case .emptyUserTurn:
			return "Cannot send an empty user turn."
		case .invalidLifecycleState(let description):
			return "This Codex session controller cannot be started because it is \(description). Create a new controller instance."
		}
	}
}
