...
actor CodexAppServerClient {
	struct RemoteReasoningEffort: Sendable, Hashable {
		let reasoningEffort: String
		let description: String
	}

	struct RemoteModel: Sendable, Hashable {
		let id: String
		let model: String
	...
	enum TransportTerminationReason: Sendable, Equatable {
	...
	struct ExpectedAgentPIDRegistration: Sendable, Equatable {
		let clientName: String
		let runID: UUID
	}

	struct ExpectedAgentPIDRegistrar: Sendable {
				...
				let supportedReasoningEfforts = (entry["supportedReasoningEfforts"] as? [[String: Any]] ?? [])
					.compactMap { effortEntry -> RemoteReasoningEffort? in
						guard let reasoningEffort = effortEntry["reasoningEffort"] as? String,
							!reasoningEffort.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
							return nil
						}
	...
	private static func errnoValue(from error: Error) -> Int32? {
		if let preflightError = error as? ReadSourceFDPreflightError {
			...
			case .invalidFileDescriptor:
				return EBADF
			case .descriptorCheckFailed(_, _, let errnoValue):
				return errnoValue
			}
		}
