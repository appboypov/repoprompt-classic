...
struct CodexOverrides {
	...
	enum ReasoningSummary: String, Sendable {
		case auto
		case concise
		case detailed
		case none
	...
	struct FeaturePolicy: Sendable, Equatable {
	...
	struct ToolPolicy: Sendable {
		...
		var multiAgentEnabled: Bool? = nil
		var experimentalSteeringEnabled: Bool? = nil
	...
	private static func mcpServerOverrideEntries(
		...
		switch policy {
		case .disableAll(let exceptBroken):
		...
		case .enableOnlyRepoPrompt(let repoPromptName, let exceptBroken):
			var didEnableRepoPrompt = false
			for entry in entries {
				if entry.normalizedName.isEmpty {
					continue
				}
				if entry.normalizedName.compare(repoPromptName, options: .caseInsensitive) == .orderedSame {
		...
		case .enableSelected(let enabledNames, let repoPromptName, let exceptBroken):
			let normalizedEnabled = Set(enabledNames.map { $0.lowercased() })
			let normalizedBroken = Set(exceptBroken.map { $0.lowercased() })
			let normalizedRepoPrompt = repoPromptName.lowercased()
			var didEnableRepoPrompt = false
			...
			for entry in entries {
				...
				let normalizedName = entry.normalizedName.lowercased()
