...
struct IgnoreCacheStore {
	static let finalIgnoreCacheCapacity = 50_000
	...
	mutating func isIgnoredPrefixCheck(
		...
		for (i, comp) in components.enumerated() {
			...
			if let cached = prefixIgnoreCache[key] {
				...
				#endif
				ignored = ignoreRules.isIgnored(relativePath: pathSoFar,
													 isDirectory: dirFlag)
	...
	static func isIgnored(
		...
		let ignored = ignoreRules.isIgnored(relativePath: relPath,
	...
	static func isIgnored(
		...
		#endif
		let ignored = ignoreRules.isIgnored(relativePathComponents: components,
											isDirectory: isDirectory)
		localCache[key] = ignored
		return ignored
	}
