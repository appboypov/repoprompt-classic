...
struct DiffApplicator {
	...
	static func myersDiff(a: [String], b: [String]) throws -> [EditOperation] {
		...
		for d in 0...max {
			for k in stride(from: -d, through: d, by: 2) {
				var x: Int
				if k == -d || (k != d && furthestReachingPaths[k - 1, default: -1] < furthestReachingPaths[k + 1, default: -1]) {
					x = furthestReachingPaths[k + 1, default: 0]
				} else {
					x = furthestReachingPaths[k - 1, default: -1] + 1
				}
	...
	private static func backtrack(trace: [[Int: Int]], a: [String], b: [String]) -> [EditOperation] {
		...
		for d in (0..<trace.count).reversed() {
			...
			let prevK: Int
			if k == -d || (k != d && v[k - 1, default: -1] < v[k + 1, default: -1]) {
				prevK = k + 1
			} else {
				prevK = k - 1
			}
