...
enum UnifiedDiffGenerator {
	...
	public static func stats(from chunks: [DiffChunk]) -> (linesChanged: Int, chunks: Int) {
		let linesChanged = chunks.reduce(0) { sum, chunk in
			let adds = chunk.lines.filter { $0.type == .addition }.count
			let rems = chunk.lines.filter { $0.type == .removal }.count
	...
	public static func buildFromEditChunks(
		...
		for chunk in sorted {
			let baseOldStart: Int
			switch startLineBase {
			case .zeroBased: baseOldStart = chunk.startLine + 1
			case .oneBased: baseOldStart = chunk.startLine
			}
    ...
    private static func hunkHasChange(_ lines: [DiffLine]) -> Bool {
        return lines.contains { $0.type != .context }
    }
    ...
    private static func generateRewriteDiff(
        ...
        func flushHunk() {
            ...
            hunks.append(DiffChunk(lines: currentLines,
