...
class ChangeManager {
	...
	func revertChange(_ change: FileChange) -> (updatedContent: [String], appliedChangeIds: Set<UUID>, error: Error?) {
		guard let index = appliedChanges.firstIndex(where: { $0.id == change.id }) else {
			return (fileContent, Set(appliedChanges.map { $0.id }), DiffApplicationError.changeNotApplied)
		}
		...
		do {
			...
			return (fileContent, Set(appliedChanges.map { $0.id }), nil)
		} catch {
			return (fileContent, Set(appliedChanges.map { $0.id }), error)
		}
	}
	...
	func getChangeGroups(threshold: Int = 3) -> [ChangeGroup] {
