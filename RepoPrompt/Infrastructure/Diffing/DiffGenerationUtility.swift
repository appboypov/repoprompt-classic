...
extension DiffGenerationError: LocalizedError {
    var errorDescription: String? {
        switch self {
        case .emptyContent:
            return "file content is empty – nothing to edit"
        case .invalidSelector:
            return "search block is empty or invalid; provide non-empty search text"
        case .noMatchFound:
            return "search block not found in file (matches are exact, including whitespace/indentation)"
        ...
        case .invalidRange(let start, let end):
            return "invalid search range (start: \(start), end: \(end))"
        case .timeout:
            return "diff generation timed out; try a shorter search block or smaller scope"
        }
    }
}
...
extension DiffGenerationError: CustomNSError {
    static var errorDomain: String { "RepoPrompt.DiffGenerationError" }
    var errorCode: Int {
        switch self {
        case .emptyContent: return 1
        case .invalidSelector: return 2
        case .noMatchFound: return 3
        case .ambiguousMatch: return 4
        case .invalidRange: return 5
...
// 🚩 Public façade
	...
	static func generateRewriteDiff(fileContent: [String], newContent: [String]) -> [DiffChunk] {
		...
		let linesToRemove = diffLines[..<matchedLineIndex].filter { $0.type == .context }.count
...
// 🚩 Offset-aware search-block diff
		...
		for line in diffLines {
			...
			case .context:
				...
				if contextLineBuffer.count >= 3 {
					...
					currentStartLine += currentChunkLines.filter { $0.type != .addition }.count + contextLineBuffer.count
	...
	static func findBestMatchUsingNGrams(selector: [LineData], in content: [LineData], lineIndexMap: [String: [Int]], mcpAmbiguityCheck: Bool = false) async throws -> Int {
		...
		let quickIndex: Int?
		if mcpAmbiguityCheck {
			quickIndex = try matchSelectorFastWithAmbiguityCheck(selector: selector, content: content, lineIndex: lineIndexMap)
		} else {
			quickIndex = try matchSelectorFast(selector: selector, content: content, lineIndex: lineIndexMap)
		}
		
		if let quickIndex = quickIndex {
			if enableDetailedLogging {
		...
		let selectorNGrams = createNGrams(from: selector.map { $0.removedTags })
		var bestMatch = -1
		var bestScore = 0.0
		...
		for i in stride(from: 0, through: maxIndex, by: step) {
			...
			let contentNGrams = createNGrams(from: contentSlice.map { $0.removedTags })
	...
	private static func calculateMatchScore(selector: [LineData], content: [LineData]) -> Double {
		return zip(selector, content).reduce(0.0) { score, pair in
			let (selectorLine, contentLine) = pair
			let similarity = selectorLine.cleaned.similarity(to: contentLine.cleaned)
	...
	static func collapseSeparatorRuns(_ s: String) -> String {
		let pattern = #"[-_–—─━═]{2,}"#
		return s.replacingOccurrences(of: pattern,
									  with: "-",
	...
	private static func normalized(_ raw: String) -> String {
		...
		for q in ["public", "private", "internal", "fileprivate",
				  "open", "final", "static", "class", "override"] where s.hasPrefix(q + " ") {
	...
	static func processLine(_ raw: String,
							precision: DiffPrecision = .normal) -> LineData
	{
		guard let base = canonicalKey(raw) else {
			return LineData(original        : raw,
							cleaned         : "",
							removedTags     : "",
							removedTagsHigh : "")
		}
	...
	private static func adaptiveDiceThreshold(forShortestLine len: Int) -> Double {
		...
		case ...12:  return 0.50
		case ...20:  return 0.65
		case ...40:  return 0.70   // ← NEW band: be less strict for 21‑40‑char lines
		default:     return 0.80
		}
	}
	...
	static func matchSelectorFast(
		...
		let fuzzyThresh: Double = {
			if selCount >= 6 {                 // large blocks – keep caller-supplied
				...
				let minLen = selector.map { $0.removedTagsHigh.count }.min() ?? 0
		...
		for s in starts {
			...
			var head = 0
			for o in 0..<headLen
			where content[s+o].removedTagsHigh == selector[o].removedTagsHigh { head += 1 }
			...
			let passes: Bool
			if isLong {
				passes = (head == headLen && tail == tailLen)
			} else if selCount < 3 {
				passes = (head == selCount)                 // 1–2 lines → perfect head
			} else {
				passes = (head >= requiredHeadForMedium)    // 3–5 lines → ≥ 3 head lines
			}
	...
	static func matchSelectorFastWithAmbiguityCheck(
		...
		for s in starts {
			...
			if isLong {
				...
				passes = (head == selCount)
			} else {
				passes = (head >= requiredHeadForMedium)
			}
		...
		if validMatches.count > 1 {
			let indices = validMatches.map { $0.idx }.sorted()
