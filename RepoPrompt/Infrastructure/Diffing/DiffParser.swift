...
struct DiffParserUtils {
	...
	static let commentStyles: [String: String] = [
		"swift": "//",
		"js": "//",
		"javascript": "//",
		"ts": "//",
	...
	static func scopeMarkerPattern(for language: String) -> String {
	...
	static func parseScopeBasedDelegateEdit(_ content: String, language: String = "swift") -> [(scopeNumber: Int, content: String, description: String)] {
	...
	static func extractScopeContent(_ content: String, language: String = "swift") -> [String] {
	...
	static func extractScopeInfo(_ content: String, language: String = "swift") -> [(scopeNumber: Int, content: String, description: String)] {
	...
	static func trimAtSiblingBoundary(_ text: String,
	...
	static func extractContent(from input: String,
							   tag: String,
...
"""
	...
	static func parseChanges(
		...
		for (index, changeRaw) in changeBlocks.enumerated() {
			...
			// ——— description ———
			let description =
			DiffParserUtils.extractContent(from: changeRaw,
										   tag: "description") ??
			...
			switch fileAction {
			case .delete:       changeType = .remove
			case .modify, .rewrite:
				changeType = fileExists ? .modify : .add
			case .delegateEdit: continue        // no direct changes
	...
	static func extractLenientContent(from input: String,
										tag: String) -> String? {
		guard let openRange =
...
class DiffParser {
	...
	private func dbg(_ msg: @autoclosure () -> String) { dprint(msg()) }
	
	#if DEBUG
	init(fileManager: RepoFileManagerViewModel, debugConfig: DebugConfig? = nil) {
	...
	func parse(_ rawInput: String) async throws -> [ParsedFile] {
		...
		for (filePathRaw, actionRaw, fileBody) in fileEntries {
			...
			if actionString.lowercased() == "rename" {
				await handleRename(oldPath: filePath,
								   fileBody: fileBody,
			...
			// ---- Skip delegateEdit files entirely -----------------------------
			if originalAction == .delegateEdit {
				dbg("⏭️  Skipping <file> with action='delegateEdit' at path '\(filePath)'")
				continue
			}
			...
			if let location = await fileManager.getFileSystemServiceForRelativePath(filePath, exactMatchOnly: isCreate || isDelete) {
				...
				if isCreate {
					...
					let candCount: Int
					if filePath.hasPrefix("/") {            // absolute input
						candCount = candidateCanonical
							.split(separator: "/")
							.filter { !$0.isEmpty }
							.count
					} else {                                // relative input
						candCount = location.correctedPath
							.split(separator: "/")
							.filter { !$0.isEmpty }
							.count
					}
				...
				if useCandidate {
					...
					} else if let baselineContent = await fileManager.getBaselineContent(forPath: location.correctedPath,
																							rootIdentifier: location.rootIdentifier) {
			...
			if let existing = parsedFileMap[canonicalPath] {
				...
				if effectiveAction == .create || effectiveAction == .rewrite {
					initialFileContent = resolveMainContent()
				} else {
					initialFileContent = contentFromChanges(changes)
				}
	...
	private func handleRename(oldPath: String,
							  fileBody: String,
							  parsedFileMap: inout [String: ParsedFile],
							  errors: inout [ParserError]) async {
		...
		guard
			let m = newRegex.firstMatch(in: fileBody,
										options: [],
		...
		if let locationOld = await fileManager.getFileSystemServiceForRelativePath(oldPath, exactMatchOnly: true) {
			...
			if let file = await fileManager.findFile(atPath: locationOld.correctedPath,
														rootIdentifier: locationOld.rootIdentifier) {
				if let data = await file.latestContent {
					loadedOld = data
				}
			} else if let baselineContent = await fileManager.getBaselineContent(forPath: locationOld.correctedPath,
																					rootIdentifier: locationOld.rootIdentifier) {
				// Use baseline content if available (e.g., in benchmark mode)
...
struct Change: Identifiable {
	...
	func printDetails() {
		...
		if let endSelector = endSelector {
			print("      End Selector:")
...
enum ChangeType: String {
	...
	func printDetails() {
		...
		print("  Display String: \(self.displayString)")
