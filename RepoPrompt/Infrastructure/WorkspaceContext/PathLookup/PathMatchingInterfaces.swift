...
public struct FrozenFileRecord: FileRecord {
    ...
    init(from vm: FileViewModel) {
        self.name = vm.name
        self.relativePath = vm.relativePath
        self.fullPath = vm.standardizedFullPath
        self.rootFolderPath = vm.standardizedRootFolderPath
    }
}
...
public struct FrozenFolderRecord: FolderRecord {
	...
	init(from vm: FolderViewModel) {
		self.name = vm.name
		self.displayName = vm.name
		self.relativePath = vm.relativePath
		self.fullPath = vm.standardizedFullPath
		self.rootPath = (vm.rootPath as NSString).standardizingPath
	}
}
