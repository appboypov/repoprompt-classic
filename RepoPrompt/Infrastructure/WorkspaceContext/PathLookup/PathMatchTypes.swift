...
struct PathMatchLocation: Sendable {
...
struct StaticPathMatchData: Sendable {
    ...
    init(
        ...
        self.filesByLowerFullPath = filesByLower
        ...
        self.foldersByLowerFullPath = foldersByLower
...
/// All references to UI/ViewModel types are stripped; uses frozen records only.
struct PathMatchSnapshot: Sendable {
    let filesByFullPath: [String: FileRecord]
    let foldersByFullPath: [String: FolderRecord]
    let rootFolders: [FolderRecord]
    ...
    init(
        ...
        self.filesByFullPath        = staticData.filesByFullPath
        self.foldersByFullPath      = staticData.foldersByFullPath
        self.rootFolders            = staticData.rootFolders
        self.filesByLowerFullPath   = staticData.filesByLowerFullPath
        self.foldersByLowerFullPath = staticData.foldersByLowerFullPath
        self.selectedFileFullPaths  = selectedFileFullPaths
        self.storedIndexes          = indexes
        self.caseSensitive          = staticData.caseSensitive
    }
    ...
    init(
        ...
        self.filesByLowerFullPath = filesLower
        ...
        self.foldersByLowerFullPath = foldersLower
        ...
        self.caseSensitive = policyCaseSensitive
        
        // Build indexes synchronously (tests / legacy path)
        self.storedIndexes = PathMatchIndexes.build(
...
struct PathMatchIndexes: Sendable {
    ...
    static func canonical(_ s: String, caseSensitive: Bool) -> String {
        ...
        var isASCII = true
        for b in s.utf8 { if b >= 0x80 { isASCII = false; break } }
...
enum PathLocateProfile: Sendable {
	case uiAssisted
	case mcpRead
	case mcpSelection
	case mcpSearchScope
...
struct PathLocateOptions: Sendable, Equatable {
...
struct FileCreationResult: Sendable {
...
enum CreationResolutionMode: Sendable {
...
enum FileCreationResolution: Sendable, Equatable {
...
enum AnyItem {
    ...
    var rootPath: String {
        switch self {
        case .folder(let f): return f.rootPath
        case .file(let f): return f.rootFolderPath
        }
    }
    
    var relativePath: String {
        switch self {
        case .folder(let f): return f.relativePath
