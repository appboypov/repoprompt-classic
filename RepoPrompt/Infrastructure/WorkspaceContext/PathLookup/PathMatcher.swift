...
enum PathMatcher {
    ...
    @inline(__always)
    private static func firstAlnumLowercasedByte<S: StringProtocol>(_ s: S) -> UInt8? {
        for b in s.utf8 {
            // '0'..'9' or 'A'..'Z' or 'a'..'z'
            ...
            return withUnsafeTemporaryAllocation(of: UInt8.self, capacity: aByteCount) { aTmp in
                withUnsafeTemporaryAllocation(of: UInt8.self, capacity: bByteCount) { bTmp in
                    ...
                    if t >= 1.0 {
                        ...
                        for i in 0..<aLen where aTmp[i] != bTmp[i] { return 0.0 }
                        return 1.0
                    }
    ...
    private static func levenshteinDistanceCapped<T: Equatable>(
        ...
        if maxDist == 0 {
            ...
            for i in 0..<lenA where a[i] != b[i] { return big }
            return 0
        }
        ...
        return withUnsafeTemporaryAllocation(of: Int.self, capacity: lenB + 1) { prev in
            withUnsafeTemporaryAllocation(of: Int.self, capacity: lenB + 1) { curr in
                // init to big
                for j in 0...lenB { prev[j] = big; curr[j] = big }
                prev[0] = 0

                let hi0 = min(lenB, maxDist)
                if hi0 >= 1 {
                    for j in 1...hi0 { prev[j] = j }
                }
    ...
    /// Main entry point - equivalent to getFileSystemServiceForRelativePath
    static func locate(
        userPath: String,
        options: PathLocateOptions,
        snapshot: PathMatchSnapshot
        ...
        for (variantComps, biasRoot) in variantComponentLists {
            ...
            if let biasRoot {
                ...
                if let hit = snapshot.fileRecord(forStandardizedFullPath: abs) {
                    return PathMatchLocation(rootPath: hit.rootFolderPath,
        ...
        for (variantComps, biasRoot) in variantComponentLists {
            ...
            for root in orderedRoots {
                ...
                if snapshot.folderRecord(forStandardizedFullPath: folderAbs) != nil {
                    let expectedFilePath = appendStandardizedRelativePath(rootPath: folderAbs, relativePath: fileName)
                    if let fileVM = snapshot.fileRecord(forStandardizedFullPath: expectedFilePath) {
                        return PathMatchLocation(rootPath: fileVM.rootFolderPath,
                                                 correctedPath: fileVM.relativePath)
                    }
                }
            }
    ...
    private static func folderExists(_ fullPath: String,
                              snapshot: PathMatchSnapshot) -> Bool {
    ...
    static func findCreationPath(
		...
		let aliasRootCandidatePaths = Set(aliasRootCandidateRoots.map { $0.fullPath })
		let rootsWithSel = rootsWithSelection(snapshot)
        ...
        if Self.isLoggingEnabled {
            print("\nAvailable roots: \(snapshot.rootFolders.map { $0.fullPath })")
        }
        ...
        let _ = snapshot.rootFolders.contains { otherRoot in
    ...
    static func resolveCreationPath(
        ...
        if tiedCandidates.count > 1 {
            ...
            let rootPaths = tiedCandidates.map { $0.root.fullPath }
    ...
    private static func candidatesFor(
        ...
        func addFiles(_ arr: [FileRecord]?) {
            guard let arr = arr, !arr.isEmpty else { return }
            for f in arr {
        ...
        if let last = relevantComps.last {
            ...
            if !ext.isEmpty, let extGroup = snapshot.indexes.byExtension[ext] {
                if fileMap.isEmpty {
                    ...
                    let extPaths = Set(extGroup.map { $0.fullPath })
    ...
    private static func makeLocation(folder: FolderRecord?, file: FileRecord?) -> PathMatchLocation? {
        if let f = file {
            return PathMatchLocation(rootPath: f.rootFolderPath,
                                   correctedPath: f.relativePath)
        }
        if let f = folder {
            return PathMatchLocation(rootPath: f.rootPath,
                                   correctedPath: f.relativePath)
        }
        return nil
    }
    ...
    private static func findSingleComponentMatch(
        ...
        if !ext.isEmpty, let byExt = snapshot.indexes.byExtension[ext] {
            ...
            for (k, group) in snapshot.indexes.byFileName {
                ...
                added += group.count
                if added >= 12_000 { break } // cap to avoid pathological growth
            }
        }
    ...
    private static func cleaned(_ str: String) -> String {
        ...
        var isASCII = true
        for b in str.utf8 { if b >= 0x80 { isASCII = false; break } }
    ...
    private static func findStrictSuffixMatch(
        ...
        if let last = userComponents.last {
            ...
            if let arr = snapshot.indexes.byFileName[k1] {
                for f in arr { candMap[f.fullPath] = f }
            }
            if userComponents.count >= 2 {
                ...
                if let arr2 = snapshot.indexes.byLastTwo[k2] {
                    for f in arr2 { candMap[f.fullPath] = f }
                }
            }
        }
        ...
        for item in candidates {
            if let rawScore = computeWeightedMatchScorePrecleaned(
                ...
                let adjustedScore: Double = {
                    switch item {
                    case .file(let f): return rawScore + (rootsWithSel.contains(f.rootFolderPath) ? rootSelectionBonus : 0.0)
                    case .folder(let f): return rawScore + (rootsWithSel.contains(f.rootPath) ? rootSelectionBonus : 0.0)
                    }
                }()
                ...
                if adjustedScore > bestScore {
                    ...
                    switch item {
                    case .folder(let f): bestItem = (f, nil)
                    case .file(let f):   bestItem = (nil, f)
                    }
                    ...
                    // Tie-breaker: prefer selected files
                    if case .file(let currentFile) = item,
    ...
    private static func computeWeightedMatchScore(
        ...
        return computeWeightedMatchScorePrecleaned(
            item: item,
    ...
    private static func findBestMatchWithOneMissingComponent(
        ...
        for skipIndex in 0..<userComponents.count {
            ...
            for item in allItems {
                guard case .file(let file) = item else { continue }
...
extension PathMatchSnapshot {
    fileprivate func file(_ full: String) -> FileRecord? {
        fileRecord(forFullPath: full)
    }
    
    fileprivate func folder(_ full: String) -> FolderRecord? {
