...
struct TokenInfo: Identifiable, Equatable {
	...
	init(count: Int, fullCount: Int? = nil, codemapCount: Int = 0, totalTokens: Int) {
		...
		self.formatted = String(format: "%.2fk", Double(count) / 1000.0)
		self.percentage = totalTokens > 0 ? Double(count) / Double(totalTokens) : 0
	}
}
...
struct TokenComponentBreakdown {
	...
	var promptDisplay: Int { prompt + duplicatePrompt }
	var other: Int { formatting + metadata }
	var totalNonFile: Int { prompt + duplicatePrompt + instructions + formatting + fileTree + gitDiff + metadata }
}
...
actor TokenCalculationService {
	...
	/// Return the files that actually require a content load for this calculation pass.
	private static func filesNeedingLoad(from entries: [PromptFileEntry]) -> [FileViewModel] {
		...
		for entry in entries {
			...
			// Only load when the cached token count is missing so we can reuse cached values.
			if (entry.file.cachedTokenCount ?? 0) == 0 {
				candidates.append(entry.file)
			}
		}
		...
		var seen = Set<UUID>()
		var deduped: [FileViewModel] = []
		deduped.reserveCapacity(candidates.count)
		
		for file in candidates where seen.insert(file.id).inserted {
			deduped.append(file)
		}
	...
	static func calculateComponentBreakdown(
		...
		let formattingTokens = includeDiffFormatting ? estimateTokens(for: xmlFormattingPrompt) : 0
	...
	func calculatePromptStats(
		...
		currentCalculationTask = Task.detached {
			...
			case .none:
				""
			case .rendered(let content):
				content
			case .snapshot(let treeSnapshot):
			...
			let components = Self.calculateComponentBreakdown(
				...
				includeDiffFormatting: snapshot.includeDiffFormatting,
				xmlFormattingPrompt: snapshot.xmlFormattingPrompt,
