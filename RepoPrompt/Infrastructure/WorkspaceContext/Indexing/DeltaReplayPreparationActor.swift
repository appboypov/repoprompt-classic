...
struct PreparedFileSystemDelta: Sendable {
...
struct PreparedFolderRenameTransfer: Sendable, Equatable {
	let oldAbsolutePath: String
	let newAbsolutePath: String
}

struct PreparedFileSystemReplayChunkSummary: Sendable, Equatable {
...
struct PreparedFileSystemReplayChunk: Sendable, Equatable {
	let range: Range<Int>
	let deltaCount: Int
...
struct PreparedFileSystemReplayBatch: Sendable {
	let rootKey: String
...
enum FileSystemDeltaPreparation {
	...
	static func rawRelativePath(for delta: FileSystemDelta) -> String {
		switch delta {
		case .fileAdded(let rel), .fileRemoved(let rel),
				.folderAdded(let rel), .folderRemoved(let rel),
				.fileModified(let rel, _), .folderModified(let rel, _):
			return rel
		}
	}
