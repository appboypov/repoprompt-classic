...
private struct ImmediatePreparedIngressReservation: Sendable, Equatable {
	let id: UInt64
	let rootKey: String
}

struct PreparedImmediateReplay: Sendable {
...
enum DeferredReplayIngressResult: Sendable {
...
struct DeferredReplayPendingWorkSnapshot: Sendable, Equatable {
...
#if DEBUG
struct DeferredReplayBufferDiagnostics: Sendable, Equatable {
	let pendingRootCount: Int
	let pendingDeltaCount: Int
	let overflowedRootCount: Int
	let routingVersion: UInt64
...
actor DeferredReplayBufferActor {
	...
	func ingestLiveDeltas(
		...
		return await routeIngress(deltas, forRootKey: rootKey, rootGeneration: nil)
	}
	...
	private func routeIngress(
		...
		let preparedBatch = await preparationActor.prepare(
			rootKey: rootKey,
			deltas: deltas,
			chunkSize: max(immediateReplayChunkSizeOverride ?? deltas.count, 1)
		)
