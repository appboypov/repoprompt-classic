...
private func commandPathResolverLog(_ message: @autoclosure () -> String) {
	guard commandPathResolverDebugLoggingEnabled else { return }
	print("[PathResolver] \(message())")
}
#else
private func commandPathResolverLog(_ message: @autoclosure () -> String) {}
#endif
...
enum CommandPathResolver {
	...
	private static func expandEnvironmentVariables(in path: String, environment: [String: String]) -> String {
		...
		for match in matches {
			...
			if match.range(at: 1).location != NSNotFound {
				keyRange = match.range(at: 1)
			} else {
				keyRange = match.range(at: 2)
			}
	...
	private static func lookupViaInteractiveShell(
		...
		for query in queries {
			...
			for rawLine in lines {
				...
				if line == sentinelBegin { betweenSentinels = true; continue }
				if line == sentinelEnd   { betweenSentinels = false; break }
	...
	static func sanitizedExecutableOutput(_ rawLine: String, originalCommand: String, preferredBasenames: [String]? = nil) -> (path: String, isAliasTarget: Bool)? {
		...
		if let aliasRangeColon = line.range(of: ": aliased to ") {
			payload = line[aliasRangeColon.upperBound...]; isAliasTarget = true
		} else if let aliasRange = line.range(of: " is aliased to ") {
			payload = line[aliasRange.upperBound...]; isAliasTarget = true
		} else if line.hasPrefix("alias ") {
			let afterAlias = line.dropFirst("alias ".count)
			if let eqIndex = afterAlias.firstIndex(of: "=") {
				payload = afterAlias[afterAlias.index(after: eqIndex)...]; isAliasTarget = true
			}
		} else if let arrowRange = line.range(of: " -> ") {
			payload = line[arrowRange.upperBound...]; isAliasTarget = true
		}
		...
		guard !tokens.isEmpty,
			  var candidate = selectExecutableToken(from: tokens, originalCommand: originalCommand, isAliasTarget: isAliasTarget, preferredBasenames: Set((preferredBasenames ?? [originalCommand]).filter { !$0.isEmpty })) else {
			return nil
		}
