...
enum ShellEnvironmentSource: Sendable, Equatable {
...
struct CLIEnvironmentSnapshot: Sendable, Equatable {
	let environment: [String: String]
	let source: ShellEnvironmentSource
}

enum ProcessLaunchPurpose: Sendable, Equatable {
	case cliRunner
	case codexAppServer
	case codexPreflight
	case claudeNative
...
struct ProcessEnvironmentRequest: Sendable, Equatable {
...
struct ProcessEnvironmentResult: Sendable, Equatable {
	let environment: [String: String]
...
enum ProcessEnvironmentBuilder {
	...
	static func build(
		...
		let baseSnapshot: CLIEnvironmentSnapshot
		if shouldUseInheritedEnvironment(
			purpose: request.purpose,
			launchContext: launchContext,
			forceRefreshShellEnvironment: request.forceRefreshShellEnvironment
		) {
			baseSnapshot = CLIEnvironmentSnapshot(
				environment: request.inheritedEnvironment,
				source: .inheritedRichEnvironment
			)
		} else {
			baseSnapshot = await shellEnvironmentProvider(
