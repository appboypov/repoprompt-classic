...
""",
				...
				returnsValue: { [weak self] args in
					...
					return try await self.handle(args)
				}
			)
		]
	}
	...
	private func handle(_ args: [String: Value]) async throws -> Value {
		guard let rawOp = args["op"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(),
			let op = AppSettingsOperation(rawValue: rawOp) else {
	...
	private func options(_ args: [String: Value]) async throws -> Value {
		...
		if let requestedAgent {
			...
			agent = await MainActor.run { resolver(store) }
		} else {
			agent = nil
		}
		let detailed = try parseOptionalBool(args["detailed"], parameter: "detailed") ?? true
	...
	private func parseOptionalAgent(_ value: Value?, parameter: String) throws -> DiscoverAgentKind? {
		guard let trimmed = try parseOptionalString(value, parameter: parameter) else { return nil }
		if let match = DiscoverAgentKind(rawValue: trimmed) {
			return match
		}
		let loweredTarget = trimmed.lowercased()
		if let match = DiscoverAgentKind.allCases.first(where: { $0.rawValue.lowercased() == loweredTarget }) {
			return match
		}
		let allowed = DiscoverAgentKind.allCases.map(\.rawValue).joined(separator: ", ")
...
private enum AppSettingValueType: String, Sendable {
	case boolean
	case string
...
private struct AppSettingCandidateRequest: Sendable {
	let key: String
	let agentFilter: DiscoverAgentKind?
	let detailed: Bool
	let limit: Int
	let availability: AgentModelCatalog.AvailabilityContext
}

private struct AppSettingCandidate: Sendable {
	let value: Value
	let label: String?
...
private struct AppSettingCandidatesResult: Sendable {
	let source: String
	let exhaustive: Bool
	let totalCount: Int
...
private struct AppSettingDefinition: @unchecked Sendable {
	...
	let defaultOptionsAgent: (@MainActor (GlobalSettingsStore) -> DiscoverAgentKind?)?
	...
	init(
		...
		defaultOptionsAgent: (@MainActor (GlobalSettingsStore) -> DiscoverAgentKind?)? = nil
	) {
		self.key = key
		self.group = group
...
private enum AppSettingsMCPRegistry {
	...
	private static let freeformStringMaxLength = 20_000
	private static let globalIgnoreDefaultsMaxLength = 20_000
	private static let promptSectionsOrderMaxLength = 10_000
	private static let debugDefaultsStringMaxLength = 4_096
	...
	static let definitions: [AppSettingDefinition] = [
		...
		stringEnumSetting(
			...
			write: { store, value in store.setAppearanceModeRaw(try requiredString(from: value)) },
		...
		boolSetting(
			...
			write: { $0.setShowTooltips(try requiredBool(from: $1)) }
		),
		boolSetting(
			...
			write: { $0.setEnableKeyboardShortcuts(try requiredBool(from: $1)) }
		),
		AppSettingDefinition(
			...
			write: { store, value in store.setFontScaleBodySize(try requiredDouble(from: value)) },
			afterWrite: nil,
		...
		AppSettingDefinition(
			...
			write: { store, value in store.setPromptSectionsOrderRaw(try requiredString(from: value)) },
			afterWrite: nil
		),
		boolSetting(
			...
			write: { $0.setDuplicateUserInstructionsAtTop(try requiredBool(from: $1)) }
		),
		stringEnumSetting(
			...
			write: { $0.setFilePathDisplayOptionRaw(try requiredString(from: $1)) }
		),
		stringEnumSetting(
			...
			write: { $0.setSelectedFilesSortMethodRaw(try requiredString(from: $1)) }
		),
		boolSetting(
			...
			write: { $0.setIncludeDatetimeInUserInstructions(try requiredBool(from: $1)) }
		),
		...
		optionalModelRawSetting(
			...
			write: { $0.setPreferredComposeModelRaw(
				try optionalString(from: $1),
		...
		boolSetting(
			...
			write: {
				$0.setSyncChatModelWithOracle(
					try requiredBool(from: $1),
		...
		numberSetting(
			...
			write: { $0.setModelTemperature(try requiredDouble(from: $1)) }
		),
		boolSetting(
			...
			write: { $0.setShouldSetModelTemperature(try requiredBool(from: $1)) }
		),
		freeformStringSetting(
			...
			write: { $0.setCustomPlanningPrompt(try requiredString(from: $1)) }
		),

		// Context Builder discovery agent/model selection. Uses the global
		// discover-agent slot only; legacy workspace-scoped context builder
		...
		stringEnumSetting(
			...
			allowedValues: DiscoverAgentKind.allCases.map(\.rawValue),
			read: { .string($0.globalDiscoverAgentSelection().agentRaw ?? DiscoverAgentKind.claudeCode.rawValue) },
			write: { store, value in
				let agentRaw = try requiredString(from: value)
				let kind = DiscoverAgentKind(rawValue: agentRaw) ?? .claudeCode
				let rememberedModelRaw = store.globalDiscoverRememberedModelRaw(for: kind.rawValue)
		...
		optionalModelRawSetting(
			...
			read: { store in
				let agentRaw = store.globalDiscoverAgentSelection().agentRaw
				return stringOrNull(agentRaw.flatMap { store.globalDiscoverRememberedModelRaw(for: $0) })
			},
			write: { store, value in
				let currentAgentRaw = store.globalDiscoverAgentSelection().agentRaw ?? DiscoverAgentKind.claudeCode.rawValue
				store.setGlobalDiscoverAgentSelection(
					agentRaw: currentAgentRaw,
					modelRaw: try optionalString(from: value),
					markUserDefined: true,
			...
			defaultOptionsAgent: { store in
				store.globalDiscoverAgentSelection().agentRaw.flatMap(DiscoverAgentKind.init(rawValue:))
			}
		),
		...
		boolSetting(
			...
			write: { $0.setMCPShowModelPresets(try requiredBool(from: $1)) },
			afterWrite: postRecommendationsDidApply
		),
		...
		boolSetting(
			...
			write: { $0.setCodeMapsGloballyDisabled(try requiredBool(from: $1)) }
		),
		...
		boolSetting(
			...
			write: { $0.setShowBuiltInWorkflowCleanupGuidance(try requiredBool(from: $1)) }
		),
		boolSetting(
			...
			write: { $0.setCodexGoalSupportEnabled(try requiredBool(from: $1)) }
		),
		...
		boolSetting(
			...
			write: { $0.setRespectGitignore(try requiredBool(from: $1)) },
		...
		boolSetting(
			...
			write: { $0.setRespectRepoIgnore(try requiredBool(from: $1)) },
		...
		boolSetting(
			...
			write: { $0.setRespectCursorignore(try requiredBool(from: $1)) },
		...
		rawTextSetting(
			...
			write: { $0.setGlobalIgnoreDefaults(try requiredString(from: $1)) },
		...
		boolSetting(
			...
			write: { $0.setEnableHierarchicalIgnores(try requiredBool(from: $1)) },
		...
		boolSetting(
			...
			write: { $0.setSkipSymlinks(try requiredBool(from: $1)) },
		...
		boolSetting(
			...
			write: { $0.setShowEmptyFolders(try requiredBool(from: $1)) },
	...
	private static let debugDefinitions: [AppSettingDefinition] = [
		boolSetting(
			...
			description: "DEBUG-only toggle for raw Claude Code event JSONL capture. Writes UserDefaults key 'claudeRawEventLoggingEnabled'.",
			read: { .bool($0.claudeRawEventLoggingEnabled()) },
			write: { $0.setClaudeRawEventLoggingEnabled(try requiredBool(from: $1)) }
		),
		rawTextSetting(
			key: "agent_mode.claude_raw_event_log_file_path",
			group: "agent_mode",
			label: "Claude Raw Event Log Directory",
			description: "DEBUG-only directory override for raw Claude Code event JSONL files. Empty string clears the override and writes UserDefaults key 'claudeRawEventLogFilePath'.",
			maxLength: debugDefaultsStringMaxLength,
			allowEmpty: true,
			read: { .string($0.claudeRawEventLogFilePath()) },
			write: { $0.setClaudeRawEventLogFilePath(try requiredString(from: $1)) }
		),
		boolSetting(
			...
			write: { $0.setAgentModePerfDiagnosticsEnabled(try requiredBool(from: $1)) }
		),
		boolSetting(
			...
			write: { $0.setAgentModePerfDiagnosticsOSLogEnabled(try requiredBool(from: $1)) }
		),
		boolSetting(
			key: "mcp.file_search_perf_diagnostics_enabled",
			group: "mcp",
			label: "MCP File Search Perf Diagnostics",
			description: "DEBUG-only toggle for MCP file_search performance diagnostics. Writes UserDefaults key 'enableMCPFileSearchPerfDiagnostics'.",
			read: { .bool($0.mcpFileSearchPerfDiagnosticsEnabled()) },
			write: { $0.setMCPFileSearchPerfDiagnosticsEnabled(try requiredBool(from: $1)) }
		)
	]
	#else
	private static let debugDefinitions: [AppSettingDefinition] = []
	...
	/// `DiscoverAgentViewModel`, `PromptViewModel`, `RecommendationWizardViewModel`,
	...
	private static func postModelRawDidWrite(
		...
		if store.syncChatModelWithOracle() {
			...
			let siblingCurrent: String?
			switch siblingKey {
			case "models.planning_model":
				siblingCurrent = store.planningModelRaw()
			case "models.preferred_compose_model":
				siblingCurrent = store.preferredComposeModelRaw()
			default:
				siblingCurrent = nil
			}
	...
	private static func aiProviderType(for agent: DiscoverAgentKind) -> AIProviderType? {
		switch agent {
	...
	private static func providerRawValue(_ provider: AIProviderType) -> String {
		...
		case .customProvider: return "customProvider"
	...
	static func agentModelRawCandidates(
		...
		let filteredAgents: [AgentModelCatalog.DiscoveryAgent]
		if let agentFilter = request.agentFilter {
			filteredAgents = discoveryAgents.filter { $0.agent == agentFilter && $0.available }
		} else {
			filteredAgents = discoveryAgents.filter { $0.available }
		}
