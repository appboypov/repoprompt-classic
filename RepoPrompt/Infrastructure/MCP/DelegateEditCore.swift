...
/// Shared module for delegate edit argument normalization.
/// Used by MCPConnectionManager and apply_edits request builders to ensure consistent
/// handling of malformed payloads and tool-name wrappers.

enum DelegateEditToolNames {
	static let editFile = "edit_file"
	static let readFile = "read_file"
	static let fileSearch = "file_search"
	static let sandboxToolNames: Set<String> = [editFile, readFile, fileSearch]

	static let editFileDescription = """
Apply edits to the delegated file. Provide exactly ONE of these three modes:

**Mode 1: Rewrite** - Replace entire file content
`{"path": "file.swift", "rewrite": "new content..."}`
...
struct DelegateEditToolSurface {
	let name: String
...
enum DelegateEditToolSurfaces {
	static func surfaces(for purpose: MCPRunPurpose) -> [DelegateEditToolSurface] {
		guard purpose == .delegateEditRun else { return [] }
		return [editFile]
	}

	private static let editFile = DelegateEditToolSurface(
		name: DelegateEditToolNames.editFile,
		description: DelegateEditToolNames.editFileDescription,
...
struct DelegateEditArgsNormalizer {
	...
	static func normalize(
		...
		var extractedRawJSON: Bool = false
	...
	static func repairMalformedReplacement(in args: [String: Value]) -> (repaired: [String: Value], didRepair: Bool) {
		...
		let replacementKeys: Set<String> = ["replace", "with", "content"]
	...
	private static func allowedSiblingKeys(for tool: String) -> Set<String> {
		let common: Set<String> = ["_tabID", "_windowID", "_rawJSON", "context_id", "path", "verbose", "args"]
		switch tool {
		case "bind_context":
			return common.union(["op", "window_id", "working_dirs", "create_if_missing", "tab_name"])
		case "apply_edits", DelegateEditToolNames.editFile:
