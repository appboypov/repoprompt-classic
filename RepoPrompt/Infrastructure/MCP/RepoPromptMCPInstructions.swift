...
enum RepoPromptMCPInstructions {
	...
	static func text(for purpose: MCPRunPurpose = .unknown, codeMapsDisabled: Bool = false) -> String {
		switch purpose {
		case .agentModeRun:
			return agentModeText(codeMapsDisabled: codeMapsDisabled)
		case .discoverRun:
			return discoverText(codeMapsDisabled: codeMapsDisabled)
		case .delegateEditRun:
			return delegateEditText
		case .unknown:
			return externalMCPText(codeMapsDisabled: codeMapsDisabled)
		}
	}
	...
	private static func agentModeText(codeMapsDisabled: Bool) -> String {
		...
		return """
		RepoPrompt is a context IDE with battle-tested agent tools optimized for reliability and token efficiency.
		...
		CONTEXT WORKFLOW: manage_selection curates the file context used by oracle and workspace_context tools — update it before oracle calls. Use op=add/remove for incremental context changes, op=set mode=full only for complete replacement, op=set mode=slices to replace slices for specified files while preserving unrelated selection, and op=add with both paths+slices for mixed additions. Continue chats by passing the returned chat_id to ask_oracle.
	...
	private static func externalMCPText(codeMapsDisabled: Bool) -> String {
		...
		CONTEXT WORKFLOW: manage_selection curates the file context used by oracle and workspace_context tools — update it before oracle calls. Use op=add/remove for incremental context changes, op=set mode=full only for complete replacement, op=set mode=slices to replace slices for specified files while preserving unrelated selection, and op=add with both paths+slices for mixed additions. Continue chats by passing the returned chat_id to oracle_send.
		...
		Workspace tabs isolate contexts for parallel tasks. Use bind_context for multi-window/tab routing.
		"""
	}
	...
	/// Focused file editing — minimal toolset.
	private static let delegateEditText: String = """
		RepoPrompt is a context IDE with battle-tested agent tools optimized for reliability and token efficiency.

		Available tools:
		- file_search — find relevant content
		- read_file — read the delegated file with optional line-range slicing (start_line + limit)
		- \(DelegateEditToolNames.editFile) — edit the delegated file; supports path + rewrite, path + search/replace, or path + edits array
		"""
}
