...
struct ToolOutputFormatter {
	...
	static func bracketedModeList(chat: Bool, plan: Bool, edit: Bool) -> String {
		var items: [String] = []
		if chat { items.append("Chat") }
		if plan { items.append("Plan") }
		if edit { items.append("Edit") }
	...
	private static func searchMatchesTree(dto: ToolResultDTOs.SearchResultDTO) -> [String] {
		...
		for entry in dto.perFileCounts { contentPaths.insert(entry.path) }
		for group in dto.contentMatchGroups { contentPaths.insert(group.path) }
	...
	// MARK: - File Tree
	static func fileTreeSummary(rootsCount: Int, usesLegend: Bool, tree: String, note: String?) -> String {
		var out: [String] = []
	...
	// MARK: - Read File
	static func readFile(path: String, first: Int, last: Int, total: Int, language: String, message: String?, content: String) -> String {
		var out: [String] = []
	...
	static func selectedFiles(count: Int, blocks: [String]) -> String {
		...
		if !blocks.isEmpty { out.append("") ; out.append(blocks.joined(separator: "\n\n")) }
		return out.joined(separator: "\n")
	}
	...
	static func genericSummary(title: String = "Result", value: Value) -> String {
		...
		case .object(let obj):
			...
			if !scalarKeys.isEmpty {
				out.append("")
				out.append("### Summary")
				for k in scalarKeys.prefix(12) { out.append("- **\(k)**: \(obj[k]!.stringValue ?? String(describing: obj[k]!))") }
			}
		default:
			break
		}
	...
	static func operationFailed(title: String? = nil, issue: String, troubleshooting: [String]? = nil) -> String {
		...
		if let steps = troubleshooting, !steps.isEmpty {
			...
			for (i, s) in steps.enumerated() { out.append("\(i+1). \(s)") }
		}
		return out.joined(separator: "\n")
	}
}

struct AppSettingValueFormatter {
	...
	private static func decodeLongStringPreview(_ value: Any) -> (preview: String, length: Int?)? {
		...
		let length: Int?
		if let int = object["value_length"] as? Int {
			length = int
	...
	private static func quotedPreview(_ preview: String, length: Int?, maxChars: Int, markdown: Bool) -> String {
		...
		let tail: String
		if let length, length > visibleCount {
			tail = " (…+\(length - visibleCount) chars)"
		} else {
			tail = ""
		}
...
extension ToolOutputFormatter {
	...
	private static func formatAppSettingsList(args: [String: Value], object: [String: Value]) -> String {
		let settings = object["settings"]?.arrayValue?.compactMap { $0.objectValue } ?? []
	...
	private static func formatAppSettingsOptions(args: [String: Value], object: [String: Value]) -> String {
		...
		let options = object["options"]?.arrayValue?.compactMap { $0.objectValue } ?? []
		...
		if !filters.isEmpty {
			...
			let summarized = sortedFilterKeys.compactMap { filterKey -> String? in
				...
				let summary: String
				if let string = filterValue.stringValue {
					summary = "`\(string)`"
				} else {
					summary = AppSettingValueFormatter.summaryForMarkdown(filterValue, maxChars: 40)
				}
	...
	private static func appSettingsOptionDetailBlock(_ candidate: [String: Value]) -> [String] {
		...
		let header: String
		switch (label, valueSummary) {
		case let (label?, valueSummary?):
			header = "#### \(label) — \(valueSummary)"
		case let (label?, nil):
			header = "#### \(label)"
	...
	private static func bindingSummaryLine(_ binding: BindContextBindingDTO) -> String {
		...
		case "context":
			...
			let flags: [String] = [binding.explicit == true ? "explicit" : nil, binding.runScoped == true ? "run-scoped" : nil].compactMap { $0 }
			let flagSuffix = flags.isEmpty ? "" : " • " + flags.joined(separator: ", ")
			return "Context \(tab) in \(window)\(workspace)\(flagSuffix)"
		case "window":
			let window = binding.windowID.map { "window \($0)" } ?? "unknown window"
			let workspace = binding.workspaceName.map { " • workspace \($0)" } ?? ""
	...
	static func formatBindContext(args: [String: Value], value: Value) -> [MCP.Tool.Content] {
		...
		var out: [String] = []
		out.append("## Context Binding \(statusIcon(success: dto.binding.bindingKind != "unbound" || op == "list" || op == "status"))")
		...
		if op == "list" {
			...
			out.append("- Use `bind_context` with `op=bind` and a `context_id` to pin a specific tab.")
	...
	static func formatReadFile(args: [String: Value], value: Value) -> [MCP.Tool.Content] {
		...
		static func formatChatLog(value: Value, emitResources: Bool) -> [MCP.Tool.Content] {
			guard case .object(let obj) = value, let arr = obj["messages"]?.arrayValue else {
				return formatGeneric(value: value)
			}
	...
	static func formatChatList(value: Value) -> [MCP.Tool.Content] {
		...
		guard case .object(let obj) = value, let chatsArr = obj["chats"]?.arrayValue else {
			return formatGeneric(value: value)
		}
		var out: [String] = []
		...
		if !chatsArr.isEmpty {
			...
			for v in chatsArr {
				...
				let markers = [cur ? "⭐ current" : nil, activeForTab ? "📌 active-for-tab" : nil].compactMap { $0 }.joined(separator: ", ")
				let files = o["selected_files"]?.arrayValue?.compactMap { $0.stringValue } ?? []
	...
	static func formatFileTree(value: Value) -> [MCP.Tool.Content] {
		if let dto = value.decode(ToolResultDTOs.FileTreeDTO.self) {
			let text = fileTreeSummary(rootsCount: dto.rootsCount, usesLegend: dto.usesLegend, tree: dto.tree, note: dto.note)
			return [.text(text)]
		}
		if let s = value.stringValue {
	...
	static func formatChatSend(args: [String: Value], value: Value, emitResources: Bool) -> [MCP.Tool.Content] {
		...
		case .object(let obj):
			...
			if let errArr = obj["errors"]?.arrayValue {
				errors = errArr.compactMap { $0.stringValue }
			}
	...
	static func formatPromptState(value: Value) -> [MCP.Tool.Content] {
		...
		if let ctx = value.decode(ToolResultDTOs.PromptContextDTO.self) {
			...
			if let sel = ctx.selection, !sel.files.isEmpty {
				...
				if let copyMode = sel.userCopyCodeMapUsage, copyMode != "auto" {
					...
					let copyFileCount = sel.files.filter { file in
		...
		obj["prompt"] != nil || obj["selected_paths"] != nil || obj["file_slices"] != nil {
            ...
            let paths = obj["selected_paths"]?.arrayValue?.compactMap { $0.stringValue } ?? []
	...
	private static func formatPresetsList(_ presets: [ToolResultDTOs.CopyPresetListItemDTO]) -> [MCP.Tool.Content] {
		...
		for preset in presets {
			...
			if let xmlFormat = preset.xmlFormat {
				config.append("xml: \(xmlFormat)")
			}
			if let includeMCP = preset.includeMCPMetadata, includeMCP {
				config.append("mcp-metadata: yes")
			}
			if !config.isEmpty {
	...
	private static func formatPromptReply(_ dto: ToolResultDTOs.PromptReply) -> [MCP.Tool.Content] {
			...
			if let includesFiles = dto.includesFiles {
				...
				if let xmlFormat = dto.xmlFormat {
					out.append("- **XML Format**: \(xmlFormat)")
				}
				if let systemPromptFlavor = dto.systemPromptFlavor {
					out.append("- **System Prompt Flavor**: \(systemPromptFlavor)")
				}
            }
...
static func formatCodeStructure(value: Value) -> [MCP.Tool.Content] {
		if let dto = value.decode(ToolResultDTOs.SelectedCodeStructureDTO.self) {
			...
			if let unmapped = dto.unmappedPaths, !unmapped.isEmpty {
				...
				for (root, paths) in grouped {
					out.append("- **\(root)**")
					for p in paths { out.append("  - `\(p)`") }
				}
			}
			if !dto.content.isEmpty { out.append(""); out.append("```text\n\(dto.content)\n```") }
			return [.text(out.joined(separator: "\n"))]
		}
		if let s = value.stringValue {
			var out: [String] = []
			...
			out.append("- **Files**: \(count)")
			if !s.isEmpty { out.append(""); out.append("```text\n\(s)\n```") }
			return [.text(out.joined(separator: "\n"))]
		}
		return formatGeneric(value: value)
	}
	...
	static func formatSelectionReplyToString(_ dto: ToolResultDTOs.SelectionReply) -> String {
		...
		if let copyMode = dto.userCopyCodeMapUsage, copyMode != "auto", let files = dto.files {
			// Count files that would be included in copy preset (not hidden)
			let copyFileCount = files.filter { file in
				if let cp = file.copyPreset, cp.renderMode == "hidden" {
					return false
				}
				return true
	...
	static func formatManageSelection(args: [String: Value], value: Value) -> [MCP.Tool.Content] {
		...
		if let dto = value.decode(ToolResultDTOs.SelectionReply.self) {
			...
			if let ts = dto.tokenStats {
				...
				let visibleFiles = dto.files?.filter { file in
			...
			if let invalid = dto.invalidPaths, !invalid.isEmpty {
				out.append("")
				for p in invalid { out.append("Invalid: \(p)") }
			}
		...
		if case .object(let obj) = value {
			...
			if let invalidArr = obj["invalid_paths"]?.arrayValue, !invalidArr.isEmpty {
				out.append("")
				for v in invalidArr { if let p = v.stringValue { out.append("Invalid: \(p)") } }
			}
			return [.text(out.joined(separator: "\n"))]
		}
		if let s = value.stringValue {
	...
	private static func formatTokenCount(_ count: Int) -> String {
		if #available(macOS 12.0, iOS 15.0, *) {
			...
			return legacyTokenFormatter.string(from: NSNumber(value: count)) ?? "\(count)"
		}
	}
	...
	private static func copyPresetExplanation(mode: String) -> String {
		...
		case "selected":
			return "Selected mode copies codemaps when available to reduce tokens."
		case "complete":
			return "Complete mode includes ALL workspace files with codemaps (not shown above)."
		case "none":
			return "None mode uses full file content (no codemaps)."
		default:
			return ""
		}
	}
	...
	private static func renderPathTree<Item>(
		...
		func compactedNode(_ node: PathTreeNode<Item>) -> (node: PathTreeNode<Item>, relativePath: String) {
			guard case .folder(let name, let children) = node else { return (node, relativePath) }
			...
			while currentChildren.count == 1 {
				...
				case .folder(let childName, let grandchildren):
				...
				case .file(let item, let fileName):
	...
	private static func selectionRootGroups(from files: [ToolResultDTOs.SelectedFileInfo]) -> [SelectionRootGroup]? {
		...
		for file in files {
			guard let rawRoot = file.rootPath, !rawRoot.isEmpty,
				let rawPathWithinRoot = file.pathWithinRoot, !rawPathWithinRoot.isEmpty else {
				return nil
			}
				...
				if let workspacesArr = obj["workspaces"]?.arrayValue {
					...
					if !workspacesArr.isEmpty {
						...
						for ws in workspacesArr {
							...
							let repoPaths = o["repo_paths"]?.arrayValue?.compactMap { $0.stringValue } ?? []
							let showingWindows = o["showing_window_ids"]?.arrayValue?.compactMap { $0.intValue } ?? []
		...
		case "create":
			...
			case "create_tab", "close_tab":
				guard let tabsArr = obj["tabs"]?.arrayValue,
					let first = tabsArr.first?.objectValue else {
					return formatGeneric(value: value)
				}
	...
	static func formatDiscoverContext(value: Value) -> [MCP.Tool.Content] {
		if case .object(let obj) = value {
			...
			if let planObj = obj["plan"], case .object(_) = planObj {
			...
			if let reviewObj = obj["review"], case .object(_) = reviewObj {
				let separator = blocks.isEmpty ? "" : "\n\n---\n\n"
	...
	private static func formatAgentControlRun(
		...
		if let waitMeta, waitMeta["mode"]?.stringValue == "any" {
			...
			let pendingIDs = waitMeta["pending_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
	...
	private static func formatMultiStart(object: [String: Value], startMeta: [String: Value], title: String) -> [MCP.Tool.Content] {
		...
		let runningIDs = startMeta["running_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
		let terminalIDs = startMeta["terminal_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
		let interestingIDs = startMeta["interesting_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
	...
	private static func formatMultiPoll(object: [String: Value], pollMeta: [String: Value], title: String) -> [MCP.Tool.Content] {
		...
		let interestingIDs = pollMeta["interesting_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
		let runningIDs = pollMeta["running_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
		let terminalIDs = pollMeta["terminal_session_ids"]?.arrayValue?.compactMap { $0.stringValue } ?? []
	...
	static func formatAgentManage(args: [String: Value], value: Value) -> [MCP.Tool.Content] {
		...
		if rawOp == "extract_handoff" || rawOp == "handoff" {
			...
			var handoffLines: [String] = ["**Agent manage · Extract Handoff**"]
		...
		var lines: [String] = ["**Agent manage · \(op)**"]
			...
			for entry in agents {
				...
				for model in models {
					...
					if let effort, seen.contains(familyKey) {
						...
						if let idx = families.firstIndex(where: { "\(agentPrefix)\($0.base)" == familyKey }) {
	...
	private static func humanizedAgentDelivery(_ raw: String?) -> String? {
		...
		case let raw? where !raw.isEmpty:
			return prettifiedAgentStatus(raw)
		default:
			return nil
		}
	}
	...
	private static func selectionTag(for info: ToolResultDTOs.SelectedFileInfo) -> String {
		...
		case "slice":
			return "[SLICE]"
		case "codemap":
			return info.isAuto ? "[CODEMAP auto]" : "[CODEMAP]"
		default:
			return "[\(info.renderMode.uppercased())]"
		}
	}
