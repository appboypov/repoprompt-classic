...
public final class WorkspaceApprovalManager: ObservableObject {
    ...
    private init() {
        ...
        if let data = UserDefaults.standard.data(forKey: Self.settingsKey),
           let decoded = try? JSONDecoder().decode(WorkspaceApprovalSettings.self, from: data) {
            ...
            self.settings = WorkspaceApprovalSettings()
        }
    }
    ...
    public func resolveApproval(allow: Bool, alwaysAllow: Bool = false) {
        guard let request = pendingRequest,
              let continuation = currentContinuation else {
            return
        }
...
extension WorkspaceApprovalManager {
    
    /// Create and request approval for a workspace creation operation.
    public func requestCreateWorkspaceApproval(
        clientID: String,
    ...
    public func requestDeleteWorkspaceApproval(
        clientID: String,
        workspaceName: String,
        workspaceID: UUID,
        windowID: Int?
    ...
    public func requestAddFolderApproval(
        clientID: String,
        folderPath: String,
        workspaceName: String,
        workspaceID: UUID,
    ...
    public func requestRemoveFolderApproval(
        clientID: String,
        folderPath: String,
        workspaceName: String,
        workspaceID: UUID,
