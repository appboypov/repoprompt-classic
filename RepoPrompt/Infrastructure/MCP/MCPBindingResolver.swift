...
struct MCPContextBindingMatch: Sendable {
	let windowID: Int
	let tabID: UUID
...
struct MCPLogicalContextResolution: Sendable {
	let tabID: UUID
	let workspaceID: UUID
	let workspaceName: String
	let repoPaths: [String]
...
struct MCPLogicalContextBindingResolution: Sendable {
	let logicalContext: MCPLogicalContextResolution
	let windowID: Int
}

struct MCPBindingResolver {
	private struct LogicalContextKey: Hashable, Sendable {
		let tabID: UUID
	...
	private func collapseLogicalContextMatches(
		...
		guard !matches.isEmpty else {
			throw MCPError.invalidParams("No RepoPrompt context matches \(sourceDescription). Use bind_context op=list to discover available values.")
		}
		...
		guard resolutions.count == 1, let resolution = resolutions.first else {
			...
			throw MCPError.invalidParams("Ambiguous binding for \(sourceDescription). Matched multiple contexts:\n- \(details)")
		}
		return resolution
	}
	...
	private func resolveWindowForLogicalContext(
		...
		let available = windowIDs.map(String.init).joined(separator: ", ")
		throw MCPError.invalidParams("Context_id '\(logicalContext.tabID.uuidString)' is open in multiple windows (\(available)). Pass context_id with _windowID, or bind the connection to a window first.")
	}
