...
struct RepoPromptMCPServerConfiguration: Sendable, Equatable, Hashable {
	struct EnvironmentEntry: Sendable, Equatable, Hashable {
		let name: String
		let value: String
	...
	static let defaultServerName = "RepoPrompt"
	...
	func validateACPLaunchCommand(
		...
		guard let path = Self.resolvedFilesystemPath(for: trimmedCommand, workingDirectory: workingDirectory) else {
			guard let resolvedBareCommandPath,
				let resolvedPath = Self.resolvedFilesystemPath(for: resolvedBareCommandPath, workingDirectory: workingDirectory) else {
				return
			}
	...
	enum ACPCommandValidationError: LocalizedError, Equatable {
		...
		var errorDescription: String? {
			switch self {
			case .emptyCommand(let serverName):
				return "\(serverName) MCP command is empty. Configure a valid RepoPrompt CLI command before starting the agent."
			case .missingExecutable(let serverName, let path):
				return "\(serverName) MCP command does not exist at \(path). Launch RepoPrompt to refresh its CLI helper, or update the MCP command before starting the agent."
			case .unresolvedCommand(let serverName, let command):
				return "\(serverName) MCP command \(command) was not found in the agent launch environment. Use an absolute RepoPrompt CLI path or update PATH before starting the agent."
			case .notExecutable(let serverName, let path):
				return "\(serverName) MCP command is not executable at \(path). Fix the CLI helper permissions or update the MCP command before starting the agent."
			}
		}
	}
}
