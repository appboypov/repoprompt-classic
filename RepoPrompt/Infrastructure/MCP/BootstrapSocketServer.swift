...
#if DEBUG
private var bootstrapSocketServerDebugLoggingEnabled = false
...
actor BootstrapSocketServer {
	...
	func start(onNewConnection: @escaping (Int32, String, Int, String?) async -> Admission) throws {
		...
		guard bindResult == 0 else {
			Darwin.close(fd)
			throw BootstrapSocketError.bindFailed(errno: errno)
		}
		...
		// Create legacy symlink for backwards compatibility with old CLIs
		MCPFilesystemConstants.createLegacySymlink()
	...
	func stop() {
		...
		// Clean up socket file and legacy symlink
		try? FileManager.default.removeItem(at: socketURL)
		MCPFilesystemConstants.removeLegacySymlink()
	...
	struct ListenerDiagnostics: Sendable {
		let isRunning: Bool
	...
	private nonisolated static func readHandshakeRequestBlocking(from fd: Int32) -> MCPBootstrapRequest? {
		...
		// Read until newline using the shared bootstrap response budget.
		...
		while Date() < deadline {
			...
			if let newlineIndex = buffer.firstIndex(of: UInt8(ascii: "\n")) {
				...
				return try? JSONDecoder().decode(MCPBootstrapRequest.self, from: Data(jsonData))
			}
