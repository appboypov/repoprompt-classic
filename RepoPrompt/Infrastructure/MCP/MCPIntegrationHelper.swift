...
struct MCPIntegrationHelper {
	...
	struct RepoPromptPermissionAutoApprovalMatch: Sendable, Equatable {
	...
	private static func collectPermissionRequestStrings(
		...
		for path in paths {
			guard let value = permissionRequestValue(at: path, in: input),
				let stringValue = trimmedPermissionRequestString(value) else {
				continue
			}
	...
	@discardableResult
	static func ensureGeminiServerForDiscovery() -> (success: Bool, wasAlreadyPresent: Bool) {
		GeminiIntegrationConfiguration.ensureServerForDiscovery()
	}

	static func geminiConfigContainsRepoPrompt() -> Bool {
		GeminiIntegrationConfiguration.configContainsRepoPrompt()
	}

	@discardableResult
	static func installInGemini() -> (success: Bool, wasAlreadyPresent: Bool) {
		let result = GeminiIntegrationConfiguration.installPersistentMCPConfig()
		if result.success {
			setMCPServerInstalled()
		}
		return result
	...
	// MARK: – Gemini System Settings (Environment Variable Approach) ---
	...
	static let geminiSystemSettingsEnvKey = GeminiIntegrationConfiguration.systemSettingsEnvKey

	/// Returns excluded tools for the given Gemini context
	static func geminiExcludedTools(for context: CLIToolContext) -> [String] {
		GeminiIntegrationConfiguration.excludedTools(for: context)
	}

	static func geminiPolicyFileURL(
		for context: CLIToolContext,
		includeRepoPromptMCPAllowance: Bool
	) throws -> URL {
		try GeminiIntegrationConfiguration.policyFileURL(
			for: context,
	...
	static func geminiSystemSettingsURL(
		for context: CLIToolContext = .agentRun,
		includeRepoPromptMCPServer: Bool = true,
		includeRepoPromptMCPPolicyAllowance: Bool? = nil
	) throws -> URL {
		try GeminiIntegrationConfiguration.systemSettingsURL(
			for: context,
			includeRepoPromptMCPServer: includeRepoPromptMCPServer,
			includeRepoPromptMCPPolicyAllowance: includeRepoPromptMCPPolicyAllowance
		)
	}
	...
	private static let skillNames = ["rp-investigate", "rp-build", "rp-reminder", "rp-oracle-export", "rp-review", "rp-refactor", "rp-orchestrate", "rp-optimize", "rp-deep-plan"]
	...
	/// Returns the URL for the legacy Claude commands directory in a workspace.
	...
	/// Returns the URL for the Claude skills directory in a workspace.
	...
	/// Checks if Claude Code skills are installed for a workspace.
	...
	static func claudeCommandsInstalled(workspacePath: String, useCLIVariant: Bool) -> Bool {
	...
	/// Installs or updates Claude Code skills in a workspace directory.
	///
	...
	/// - Parameters:
	///   - workspacePath: Path to the workspace/project folder.
	///   - useCLIVariant: If true, installs CLI variant skills (rp-cli syntax). Defaults to false (MCP syntax).
	...
	@discardableResult
	static func installClaudeCodeCommands(workspacePath: String, useCLIVariant: Bool = false, mode: CommandInstallMode = .fullInstall) -> Int {
		let suffix = useCLIVariant ? "-cli" : ""
	...
	/// Installs Claude Code slash commands in multiple workspace directories.
	///
	/// - Parameters:
	///   - workspacePaths: Array of workspace folder paths.
	///   - useCLIVariant: If true, installs CLI variant commands (rp-cli syntax). Defaults to false (MCP syntax).
	///   - mode: Installation mode. `.fullInstall` creates missing files (UI buttons), `.updateExistingOnly` only updates existing managed files (validation).
	/// - Returns: Total number of commands successfully installed across all workspaces.
	@discardableResult
	static func installClaudeCodeCommands(workspacePaths: [String], useCLIVariant: Bool = false, mode: CommandInstallMode = .fullInstall) -> Int {
		guard !workspacePaths.isEmpty else { return 0 }

		var totalSuccess = 0
		for path in workspacePaths {
			totalSuccess += installClaudeCodeCommands(workspacePath: path, useCLIVariant: useCLIVariant, mode: mode)
		}
		return totalSuccess
	}

	/// Uninstalls Claude Code skills from a workspace directory.
	...
	@discardableResult
	static func uninstallClaudeCodeCommands(workspacePath: String, useCLIVariant: Bool = false) -> Int {
		let suffix = useCLIVariant ? "-cli" : ""
	...
	/// Uninstalls Claude Code slash commands from multiple workspace directories.
	///
	/// - Parameters:
	///   - workspacePaths: Array of workspace folder paths.
	///   - useCLIVariant: If true, uninstalls CLI variant commands. Defaults to false (MCP syntax).
	/// - Returns: Total number of commands successfully removed across all workspaces.
	@discardableResult
	static func uninstallClaudeCodeCommands(workspacePaths: [String], useCLIVariant: Bool = false) -> Int {
		guard !workspacePaths.isEmpty else { return 0 }

		var totalRemoved = 0
		for path in workspacePaths {
			totalRemoved += uninstallClaudeCodeCommands(workspacePath: path, useCLIVariant: useCLIVariant)
		}
		return totalRemoved
	}
	...
	///
	/// - Parameters:
	///   - useCLIVariant: If true, installs CLI variant commands (rp-cli syntax). Defaults to true for Codex.
	///   - mode: Installation mode. `.fullInstall` creates missing files (UI buttons), `.updateExistingOnly` only updates existing managed files (validation).
	...
	static func installCodexCommands(useCLIVariant: Bool = true, mode: CommandInstallMode = .fullInstall) -> Int {
		...
		var successCount = 0
		let variant: ClaudeCodeCommands.ToolVariant = useCLIVariant ? .cli : .mcp
		let suffix = useCLIVariant ? "-cli" : ""
		let currentVersion = ClaudeCodeCommands.skillsVersion
		...
		let commands: [(name: String, content: () -> String)] = [
			("rp-investigate", { ClaudeCodeCommands.rpInvestigate(variant: variant) }),
			("rp-build", { ClaudeCodeCommands.rpBuild(variant: variant) }),
			("rp-reminder", { ClaudeCodeCommands.rpReminder(variant: variant) }),
			("rp-oracle-export", { ClaudeCodeCommands.rpOracleExport(variant: variant) }),
			("rp-review", { ClaudeCodeCommands.rpReview(variant: variant) }),
			("rp-refactor", { ClaudeCodeCommands.rpRefactor(variant: variant) }),
			("rp-orchestrate", { ClaudeCodeCommands.rpOrchestrate(variant: variant) }),
			("rp-optimize", { ClaudeCodeCommands.rpOptimize(variant: variant) }),
			("rp-deep-plan", { ClaudeCodeCommands.rpDeepPlan(variant: variant) })
		]
	...
	private static func agentsSkillsVersionKey(useCLIVariant: Bool, perProject: Bool) -> String {
		if perProject {
			return useCLIVariant ? agentsCLISkillsPerProjectVersionDefaultsKey : agentsSkillsPerProjectVersionDefaultsKey
		} else {
			return useCLIVariant ? agentsCLISkillsVersionDefaultsKey : agentsSkillsVersionDefaultsKey
		}
	}
	...
	private static func installSkillsToDirectory(
		...
		for (name, contentGenerator) in skills {
			...
			do {
				...
				try ClaudeCodeCommands.codexSkillAgentPolicy(forSkillNamed: name, variant: variant).write(to: skillPolicyFileURL, atomically: true, encoding: .utf8)
				successCount += 1
			} catch {
	...
	/// Installs the CLI to /usr/local/bin as rp-cli (release) or rp-cli-debug (debug).
	...
	/// The CLI command name that will be installed (rp-cli or rp-cli-debug).
	...
	/// The claude-rp command name that will be installed (claude-rp or claude-rp-debug).
