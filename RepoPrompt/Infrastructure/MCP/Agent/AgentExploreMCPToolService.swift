...
struct AgentExploreMCPToolService {
	...
	private func executeStart(args: [String: Value]) async throws -> Value {
		...
		let sessionIDs = started.map { $0.outcome.snapshot.sessionID }
	...
	private func executeControl(args: [String: Value], op: String) async throws -> Value {
		let allowed: Set<String> = {
			switch op {
			case "poll": return Self.pollKeys
			case "wait": return Self.waitKeys
			case "cancel": return Self.cancelKeys
			default: return ["op"]
			}
		}()
	...
	private func startExploreRuns(
		...
		for (index, target) in targets.enumerated() {
			...
			do {
				...
				let startedIDs = started.map { $0.outcome.snapshot.sessionID.uuidString }.joined(separator: ", ")
	...
	private func validateExploreChildSessions(
		...
		for sessionID in sessionIDs {
			...
			guard liveSession.parentSessionID == callerSessionID,
				liveSession.mcpControlContext?.taskLabelKind == .explore else {
	...
	private func batchStartValue(
		...
		let interestingIDs = snapshots.filter { $0.interaction != nil || $0.status.isTerminal }.map { $0.sessionID }
		return .object([
			"start": .object([
				"mode": .string("many"),
