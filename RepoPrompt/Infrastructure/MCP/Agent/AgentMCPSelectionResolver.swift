...
/// 3. **Legacy format**: `agent-selection:v1:...` — backward compatible
enum AgentMCPSelectionResolver {
	...
	struct ResolvedSelection: Sendable {
		let agentRaw: String?
		let modelRaw: String?
	...
	static func resolve(
		...
		guard let agent = DiscoverAgentKind(rawValue: parsed.agentRaw) else {
			throw MCPError.invalidParams(
