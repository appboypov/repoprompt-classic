...
struct AgentManageMCPToolService {
	...
	private func executeExtractHandoff(args: [String: Value]) async throws -> Value {
		...
		if includeFileContents {
			...
			let block = await agentModeVM.buildCurrentTabHandoffFileContentsBlock(tokenCap: 60_000)
	...
	private func resolveHandoffSession(
		...
		let agentKind = persisted.agentKind.flatMap { DiscoverAgentKind(rawValue: $0) }
	...
	private func clampedInt(
		...
		case .double(let doubleValue):
			guard doubleValue.isFinite else { parsed = nil; break }
