...
struct MCPOracleToolService {
	...
	typealias ExecContext = MCPServerViewModel.ExecContext
	...
	let resolveExecContext: (RequestMetadata) -> ExecContext
	...
	func executeOracleChatLog(args: [String: Value]) async throws -> Value {
		...
		let allowedArgs: Set<String> = ["chat_id", "limit", "include_user"]
		...
		let result = try await chatVM.tool_oracleChatLog(
			args: args,
			tabID: tabID,
	...
	func executeAskOracle(args: [String: Value]) async throws -> Value {
		let allowedArgs: Set<String> = ["message", "mode", "chat_id", "new_chat", "export_response"]
		let unsupported = args.keys
			.filter { !$0.hasPrefix("_") && !allowedArgs.contains($0) }
			.sorted()
		if !unsupported.isEmpty {
		...
		let exportResponse = try parseExportResponseFlag(args, mode: modeRaw)
		...
		if let normalizedChatID {
			guard let session = chatVM.resolveSession(id: normalizedChatID) else {
		...
		let tabContext: ChatViewModel.ChatSendTabContext
		if let virtualContext, virtualContext.tabID == tabID {
			tabContext = Self.chatSendTabContext(from: virtualContext, owner: owner)
		} else {
			...
			tabContext = ChatViewModel.ChatSendTabContext(
				tabID: tabID,
		...
		var result = try await withHeartbeat(
			...
			try await chatVM.tool_chatSend(
	...
	func executeOracleSend(args: [String: Value]) async throws -> Value {
		let allowedArgs: Set<String> = ["message", "mode", "chat_id", "new_chat", "model", "export_response"]
		let unsupported = args.keys
			.filter { !$0.hasPrefix("_") && !allowedArgs.contains($0) }
			.sorted()
		if !unsupported.isEmpty {
		...
		if exportResponse || runPurpose == .agentModeRun {
			targetWindow = try requireTargetWindow()
		} else {
			targetWindow = nil
		}
		let metadata = await captureRequestMetadata()
		let execContext = resolveExecContext(metadata)
		var tabContext: ChatViewModel.ChatSendTabContext? = nil
		...
		if case .virtual = execContext {
			...
			if runPurpose == .agentModeRun, let targetWindow {
				...
				tabContext = Self.chatSendTabContext(from: context, owner: owner)
			} else {
				tabContext = Self.chatSendTabContext(from: context)
			}
		}

		let exportDestination: OracleExportDestination?
		if exportResponse, let targetWindow {
			exportDestination = try MCPServerViewModel.makeOracleExportDestination(
				workspace: targetWindow.workspaceManager.activeWorkspace,
				windowID: targetWindow.windowID,
	...
	private func parseExportResponseFlag(_ args: [String: Value], mode: String) throws -> Bool {
		...
		if boolValue, mode == "edit" {
			throw MCPError.invalidParams("export_response is not supported for edit mode. Use chat, plan, or review mode.")
		}
		return boolValue
	}
	...
	private func validateCommonOracleArgs(_ args: [String: Value]) throws {
		...
		let modeRaw = args["mode"]?.stringValue?
			.trimmingCharacters(in: .whitespacesAndNewlines)
			.lowercased() ?? "chat"
		guard ["chat", "plan", "edit", "review"].contains(modeRaw) else {
			throw MCPError.invalidParams("Invalid mode: \(modeRaw). Valid modes: chat, plan, edit, review")
		}
	...
	private func executeOracleModelsUtility() async throws -> Value {
		let showModelPresets = UserDefaults.standard.object(forKey: "mcpShowModelPresets") as? Bool ?? false
		...
		if showModelPresets {
			let presets = temporarilyDisabled ? [] : await ModelPresetsManager.shared.getAllPresets()
			if !presets.isEmpty {
				for preset in presets {
					let supportedModes: ToolResultDTOs.SupportedModesInfo = {
						...
						return ToolResultDTOs.SupportedModesInfo(chat: true, plan: true, edit: true, review: true)
					}()
				...
				models.append(await defaultCurrentChatModelInfo())
			}
		} else {
			models.append(await defaultCurrentChatModelInfo())
		}
		...
		func bracketedModes(_ supportedModes: ToolResultDTOs.SupportedModesInfo?) -> String {
			let modes = supportedModes ?? ToolResultDTOs.SupportedModesInfo(chat: true, plan: true, edit: true, review: true)
			var items: [String] = []
			if modes.chat { items.append("Chat") }
			if modes.plan { items.append("Plan") }
			if modes.edit { items.append("Edit") }
			if modes.review { items.append("Review") }
			return "[\(items.joined(separator: ", "))]"
		}

		var lines: [String] = ["Available models:"]
		for model in models {
	...
	private func defaultCurrentChatModelInfo() async -> ToolResultDTOs.ModelInfo {
		let planningModel = await promptVM.planningModel
		let planningAvailable = await promptVM.isModelAvailable(planningModel)
		let effectiveModel = planningAvailable ? planningModel : await promptVM.preferredAIModel
		return ToolResultDTOs.ModelInfo(
			id: "current_chat_model",
			name: effectiveModel.displayName,
			description: "MCP Oracle Model",
			supportedModes: ToolResultDTOs.SupportedModesInfo(chat: true, plan: true, edit: true, review: true)
		)
	}

	private func executeLiveOracleSessions(args: [String: Value]) async throws -> Value {
		let allowedArgs: Set<String> = ["limit", "scope", "context_id"]
		...
		var result = try await chatVM.tool_chatList(args: listArgs)
