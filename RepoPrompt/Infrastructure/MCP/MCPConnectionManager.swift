...
#if DEBUG
private var mcpConnectionManagerDebugLoggingEnabled = false
private var mcpRoutingDebugLoggingEnabled = false
private var mcpPolicyDebugLoggingEnabled = false

private func mcpConnectionManagerLog(_ message: @autoclosure () -> String) {
...
public enum MCPRunPurpose: String, Sendable, Codable {
	case discoverRun      // Discovery agent exploring codebase
	case agentModeRun     // Agent mode interactive session
	case delegateEditRun  // Delegate edit agent making targeted changes
...
enum ConnectionTransport: String, Sendable, Codable {
	case network
	case filesystem
}

/// Summary state for dashboard display
enum ConnectionStateSummary: String, Sendable, Codable {
	case setup
	case waiting
	case ready
	case failed
...
struct ConnectionDashboardEntry: Sendable, Identifiable, Codable {
	let id: UUID
...
struct NetworkDashboardSnapshot: Sendable {
	let isRunning: Bool
...
struct IdentityContextSnapshot: Sendable {
...
actor ServerNetworkManager {
	...
	private static func validatedLiveRunID(
		...
		guard connectionIDToRunID[connectionID] == candidateRunID,
			connectionIDByRunID[candidateRunID] == connectionID else {
			return nil
		}
	...
	nonisolated static func shouldRehydrateContextID(for toolName: String) -> Bool {
		toolName == "bind_context" || toolName == "oracle_utils"
	}
	...
	nonisolated static func shouldSkipGenericTabBinding(for toolName: String) -> Bool {
		toolName == "manage_workspaces" || toolName == "bind_context" || isAppWideTool(toolName)
	}
	...
	nonisolated static func multiWindowSelectionGuidance() -> String {
		"Multiple RepoPrompt windows detected. Bind your connection to route tool calls:\n\n" +
		"**Recommended: Bind by exact workspace root paths (auto-resolves to the matching workspace/window)**\n" +
		...
		"- `bind_context` with `{\"op\":\"bind\",\"context_id\":\"<id>\"}` to pin a specific compose tab\n" +
...
#if DEBUG
	private struct DebugConnectionEvent: Sendable {
		let seq: UInt64
		let timestamp: Date
		let event: String
		let restartID: UUID?
	...
	private struct DebugRestartStatus: Sendable {
		let restartID: UUID
		var state: String
	...
	private let debugConnectionHistoryLimit = 1_000
	...
	private func emitDashboardUpdate() {
		mcpConnectionManagerLog("Emitting dashboard update notification")
		onDashboardUpdate?()
	}
	...
	static func toolErrorResult(rawJSON: Bool, message: String) -> CallTool.Result {
		...
		return CallTool.Result(content: [.text(ToolOutputFormatter.rawJSONString(value))], isError: true)
	}

	// Tool call observers for agent UI feedback (runID-based for stability across handovers)
	// Now supports multiple observers per runID (e.g., AgentToolTracker + DelegateEditAgentRunner)
	...
	// Delegate edit sandboxes (single-file virtual FS per runID)
	private var delegateSandboxesByRunID: [UUID: DelegateEditSandbox] = [:]
	...
	/// the policy expires. This keeps Discover Agent sandboxes from leaking into shared MCP sessions yet
	...
	private struct ClientConnectionPolicy {
		...
		// Tab context for discover agents
		let tabID: UUID?
		let runID: UUID?
    ...
    struct ToolCallHistoryEntry: Sendable {
        let timestamp: Date
        let toolName: String
        let clientName: String
        let connectionID: UUID
    ...
    // ─────────────  Bootstrap admission reservations (prevents burst oversubscription)  ─────────────
    ...
    private struct BootstrapReservation: Sendable {
        let connectionID: UUID
        let createdAt: Date
    }
    ...
    private func cleanupExpiredBootstrapReservations() {
        ...
        if !expired.isEmpty {
            let expiredIDs = expired.map { $0.key.uuidString }.joined(separator: ", ")
	...
	private var maxGlobalConnections: Int { defaultsInt("mcp.maxGlobalConnections", 128) }
	private var maxConnectionsPerClient: Int { defaultsInt("mcp.maxConnectionsPerClient", 64) }
	private var maxCallsPerConnection: Int { defaultsInt("mcp.maxCallsPerConnection", 8) }
	private var pressureEvictIdleSeconds: Int { defaultsInt("mcp.pressureEvictIdleSeconds", 7200) } // 2h
	private var connectingTimeoutSeconds: Int { defaultsInt("mcp.connectingTimeoutSeconds", 60) }
	private var preserveOnePerClient: Bool {
		(UserDefaults.standard.object(forKey: "mcp.preserveOnePerClient") as? Bool) ?? true
	}
	
	// 🆕 Task-local key to expose current connection inside tool calls
	@TaskLocal
	...
	private func markActiveToolOwner(windowID: Int, connectionID: UUID, toolName: String) {
		...
		mcpConnectionManagerLog("Tool '\(toolName)' ownership marked for connection \(connectionID) on window \(windowID)")
	...
	private func clearActiveToolOwner(windowID: Int, connectionID: UUID) {
		...
		if activeToolOwnerByWindow[windowID] == connectionID {
			if let toolName = activeToolNameByWindow[windowID] {
				mcpConnectionManagerLog("Tool '\(toolName)' ownership cleared for connection \(connectionID) on window \(windowID)")
			}
	...
	private func ensureWindowBindingIfUnambiguous(connectionID: UUID, reason: String) async -> Int? {
		...
		let (mcpEnabledWindows, multiWindowEffective) = await MainActor.run {
			let windows = WindowStatesManager.shared.allWindows.filter { $0.mcpServer.windowToolsEnabled }
		...
		if mcpEnabledWindows.count == 1, let window = mcpEnabledWindows.first {
			...
			mcpConnectionManagerLog("\(reason): auto-bound connection \(connectionID) to single MCP-enabled window \(windowID)")
			return windowID
		}
		...
		if let recordedWindowCount = windowCountAtConnectionTime[connectionID] {
			...
			if multiWindowEffective {
				mcpConnectionManagerLog("\(reason): missing connection-time window count for \(connectionID); treating as multi-window")
			}
		}
		if connectedDuringSingleWindow || !multiWindowEffective {
			if let firstWindow = await WindowStatesManager.shared.firstMCPEnabledWindow() {
				...
				mcpConnectionManagerLog("\(reason): auto-bound connection \(connectionID) to window \(windowID) (\(bindReason))")
				return windowID
			}
		}
		
		// Multi-window ambiguous - caller must handle
		mcpConnectionManagerLog("\(reason): connection \(connectionID) has no unambiguous window binding (multi-window active)")
		return nil
	}
	...
	func runIDForConnection(_ connectionID: UUID) async -> UUID? {
		...
		guard let resolved else {
			mcpConnectionManagerLog("runIDForConnection: no run mapping found for connection \(connectionID)")
			return nil
		}
	...
	func observerRunIDForCompletionForToolTracking(
		callTimeRunID: UUID?,
		connectionID: UUID,
		toolName: String,
		invocationID: UUID,
	...
	func mapConnectionToRunID(
		...
		if let explicitWindowID {
			resolvedWindowID = explicitWindowID
		} else if let mappedWindowID = connectionWindowMap[connectionID] {
			resolvedWindowID = mappedWindowID
		} else {
			resolvedWindowID = await windowIDForRunID(runID)
		}
		...
		let registrationSucceeded = await MainActor.run { () -> Bool in
			...
			if didRegister {
				mcpConnectionManagerLog("mapConnectionToRunID: mapped connection \(connectionID) to runID \(runID) in window \(windowID)")
			}
			return didRegister
		}
	...
	private nonisolated static func isKnownAgentClientName(_ clientName: String) -> Bool {
		...
		for agent in DiscoverAgentKind.allCases {
	...
	private func preferredLiveRunAffinity(
		...
		guard let matchedKey = matchingKeys.first,
			let affinity = liveRunAffinityByClientSession[matchedKey]?[sessionKey] else {
			return nil
		}
	...
	func installDelegateEditSandbox(windowID: Int, runID: UUID, allowedPath: String, originalContent: String) async {
		let sandbox = DelegateEditSandbox(allowedPath: allowedPath, original: originalContent)
		delegateSandboxesByRunID[runID] = sandbox
		windowIDByRunID[runID] = windowID
	}

	func removeDelegateEditSandbox(runID: UUID) async {
		delegateSandboxesByRunID.removeValue(forKey: runID)
	}

	func delegateEditFinalContent(for runID: UUID) async -> String? {
		await delegateSandboxesByRunID[runID]?.currentContent()
	}

	func delegateSandbox(for runID: UUID) -> DelegateEditSandbox? {
		delegateSandboxesByRunID[runID]
	}
	...
	func setConnectionApprovalHandler(_ handler: @escaping ConnectionApprovalHandler) {
		mcpConnectionManagerLog("Setting connection approval handler")
	...
	func registerToolCallObserver(for runID: UUID, observer: @escaping @Sendable (String) -> Void) -> UUID {
		...
		mcpConnectionManagerLog("Registered tool call observer for runID: \(runID) token: \(token)")
		return token
	}
	...
	func unregisterToolCallObserver(for runID: UUID, token: UUID) async {
		...
		mcpConnectionManagerLog("Unregistered tool call observer token \(token) for runID: \(runID)")
	}
	...
	func unregisterToolObservers(for runID: UUID) async {
		...
		mcpConnectionManagerLog("Unregistered all tool observers for runID: \(runID)")
	}
		...
		await MainActor.run {
			...
			for window in windows {
				if let connectionID = window.mcpServer.connectionID(forRunID: runID) {
					...
					mcpConnectionManagerLog("Cleaned up runID mapping for runID \(runID) in window \(window.windowID)")
				}
			}
		}
	}
	...
	func fireToolCallObservers(runID: UUID, toolName: String) async -> Int {
		...
		mcpConnectionManagerLog("Tool call observer fired for runID \(runID) tool \(toolName) count \(observers.count)")
	...
	func registerToolEventObserver(for runID: UUID, observer: ToolEventObserver) -> UUID {
		...
		mcpConnectionManagerLog("Registered tool event observer for runID: \(runID) token: \(token)")
		return token
	}
	...
	func unregisterToolEventObservers(for runID: UUID) async {
		toolEventObservers.removeValue(forKey: runID)
		mcpConnectionManagerLog("Unregistered all tool event observers for runID: \(runID)")
	}
	...
	func fireToolCalledObservers(runID: UUID, invocationID: UUID, toolName: String, args: [String: Value]?) async -> Int {
		...
		mcpConnectionManagerLog("Tool called observers fired for runID \(runID) tool \(toolName) count \(observers.count)")
		return observers.count
	}
	...
	func fireToolCompletedObservers(runID: UUID, invocationID: UUID, toolName: String, args: [String: Value]?, resultJSON: String, isError: Bool) async -> Int {
		...
		mcpConnectionManagerLog("Tool completed observers fired for runID \(runID) tool \(toolName) count \(observers.count)")
		return observers.count
	}
	...
	private func notifyConnectionWaiters(connectionID: UUID, clientName: String?) {
		...
		mcpConnectionManagerLog("Found new connection \(connectionID) for waiting client: \(waiter?.clientName ?? "any") (actual: \(actualName))")
		resumeWaiter(matchID, with: connectionID)
	}

	/// Wait for a new connection from a specific client (for discover agent).
	...
	func waitForNewConnection(clientName: String?, timeout: TimeInterval = 10.0) async -> UUID? {
		...
		if let match = findMatchingConnection(excluding: existing, clientName: clientName) {
			...
			mcpConnectionManagerLog("Found existing connection \(match) for client: \(label)")
			return match
		}
		...
		return await withTaskCancellationHandler {
			await withCheckedContinuation { (continuation: CheckedContinuation<UUID?, Never>) in
				...
				if let match = findMatchingConnection(excluding: existing, clientName: clientName) {
					let label = clientIDByConnection[match] ?? pendingConnections[match] ?? "unknown"
					mcpConnectionManagerLog("Found connection \(match) for client: \(label) (post-registration recheck)")
	...
	private func handleWaiterTimeout(_ waiterID: UUID, clientName: String?) {
		...
		if let clientName {
			mcpConnectionManagerLog("Timed out waiting for connection from client: \(clientName)")
		} else {
			mcpConnectionManagerLog("Timed out waiting for connection without client hint")
		}
    ...
    func start() async {
		mcpConnectionManagerLog("Starting network manager")
		isRunningState = true

		// Start periodic maintenance loop for cleanup tasks
		startMaintenanceLoop()

		// Start bootstrap socket server for UNIX socket connections
		bootstrapSocketTask?.cancel()
		bootstrapSocketTask = Task { [weak self] in
			guard let self = self else { return }
			await self.startBootstrapSocketServer()
		}
	}
	...
	private func cleanupExpiredClientConnectionPolicies() {
		...
		if removedCount > 0 {
			mcpConnectionManagerLog("Cleaned up \(removedCount) expired client connection policies")
		}
	}
	...
	func ensureBootstrapHealthy(force: Bool = false) async {
		...
		guard let server = bootstrapSocketServer else {
			mcpConnectionManagerLog("BootstrapSocketServer nil while running. Restarting.")
	...
	private func performBootstrapRestart(reason: String, token: UUID) async {
		...
		guard bootstrapRestartToken == token else {
			mcpConnectionManagerLog("Skipping bootstrap restart (reason=\(reason)) - superseded")
			return
		}
		bootstrapRestartToken = nil

		guard isRunningState else {
			mcpConnectionManagerLog("Skipping bootstrap restart (reason=\(reason)) - manager not running")
			return
		}

		let socketPath = MCPFilesystemConstants.bootstrapSocketURL().path
		mcpConnectionManagerLog("Restarting bootstrap socket server (reason=\(reason), socket=\(socketPath), failures=\(bootstrapStartFailures))")
	...
	private func startBootstrapSocketServer() async {
		...
		mcpConnectionManagerLog("Starting bootstrap socket server...")

		let server = BootstrapSocketServer(logger: log)
		self.bootstrapSocketServer = server
		...
		do {
			try await server.start { [weak self] (clientFD, sessionToken, clientPid, clientName) async -> BootstrapSocketServer.Admission in
				...
				return await self.handleBootstrapConnection(
					clientFD: clientFD,
	...
	private func handleBootstrapConnection(
		...
		let connectionID = UUID()
		mcpConnectionManagerLog("Bootstrap socket connection: \(connectionID) from '\(clientName ?? "unknown")' (pid=\(clientPid), session=\(sessionToken.prefix(8))...)")
		...
		mcpConnectionManagerLog("handleBootstrapConnection: returning Admission.accept for \(connectionID)")
		...
		// These closures are short-lived and not stored long-term.
			return .accept(
				postAccept: { [self] in
					...
					await self.releaseBootstrapReservation(connectionID: connectionID)
					if let replacedNow = await self.existingConnectionID(forSessionToken: sessionToken) {
						await self.softDisconnectConnection(replacedNow, reason: .connectionReplaced, message: "Replaced by new connection for same session")
					}
					await self.registerAndStartBootstrapConnection(
						connectionID: connectionID,
						sessionToken: sessionToken,
						clientPid: clientPid,
						clientName: clientName,
			...
			onAcceptWriteFailed: { [self] in
				...
				await self.releaseBootstrapReservation(connectionID: connectionID)
				mcpConnectionManagerLog("Bootstrap connection \(connectionID) rolled back (accept write failed)")
			}
		)
	}
	...
	private func registerAndStartBootstrapConnection(
		...
		Task { @MainActor [weak self] in
			...
			await self.setWindowCountAtConnectionTime(connectionID: connectionID, count: count)
		}
	}
	...
	private func startMCPServerForConnection(
		...
		connectionTasks[connectionID] = Task {
			...
			do {
				...
				mcpConnectionManagerLog("Starting MCP server for bootstrap connection \(connectionID)")
				mcpACPLog("[MCP-ACP] starting MCP server connection=\(connectionID)")
				try await manager.start { clientInfo in
					mcpConnectionManagerLog("MCP handshake callback for \(connectionID): clientName='\(clientInfo.name)'")
					...
					mcpConnectionManagerLog("Calling approval handler for \(connectionID) client='\(clientInfo.name)'")
					let approved = await approvalHandler(connectionID, clientInfo)
					mcpConnectionManagerLog("Approval handler returned \(approved) for \(connectionID)")
					if approved {
						...
						// Wait for tool catalog readiness before completing handshake.
						// This ensures the first tools/list response has all tools (including git).
						...
						// Warm the tool cache if we have a window
	...
	func stop() async {
		mcpConnectionManagerLog("Stopping network manager")
		...
		for (id, connectionManager) in connections {
			mcpConnectionManagerLog("Stopping connection: \(id)")
#if DEBUG
	...
	private struct TerminationSemantics: Sendable {
	...
	private func markClientAsUserKilled(_ clientID: String) {
		...
		mcpConnectionManagerLog("Marked clientID '\(clientID)' for \(Int(userKilledClientCooldown))s reconnection cooldown")
	}
	...
	private func terminateConnection(
		...
		mcpConnectionManagerLog("Terminating connection \(id) with reason: \(reason.rawValue), session: \(sessionToken?.prefix(8) ?? "nil")..., semantics: killSignal=\(sem.writeKillSignal), markSession=\(sem.markSessionKilled), markClient=\(sem.markClientUserKilled)")
#if DEBUG
		...
		if sem.writeKillSignal, let token = sessionToken {
			do {
				...
				mcpConnectionManagerLog("Wrote kill signal for session \(token.prefix(8))...")
			} catch {
		...
		mcpConnectionManagerLog("Successfully terminated and removed connection \(id)")
	...
	private func softDisconnectConnection(_ id: UUID, reason: TerminationReason, message: String?) async {
		guard let connection = connections[id] else {
			mcpConnectionManagerLog("softDisconnectConnection: connection \(id) not found")
			return
		}
		mcpConnectionManagerLog("Soft-disconnecting connection \(id) reason=\(reason.rawValue)")
#if DEBUG
	...
	func handleConnectionReplaced(
		...
		if let oldToken, let newToken, oldToken == newToken {
			mcpConnectionManagerLog("RunID \(runID) collision: same session (\(oldToken.prefix(8))…) - soft-disconnect old=\(oldID) for new=\(newID)")
		...
		mcpConnectionManagerLog("RunID \(runID) collision: different session - hard-terminate old=\(oldID) (token=\(oldToken?.prefix(8) ?? "nil")) for new=\(newID) (token=\(newToken?.prefix(8) ?? "nil"))")
    ...
    func removeConnection(_ id: UUID) async {
		...
		guard connections[id] != nil
			...
			mcpConnectionManagerLog("removeConnection: \(id) already removed; ignoring duplicate call")
			return
		}
		
		mcpConnectionManagerLog("Removing connection: \(id)")
		...
		if cancelledToolCount > 0 {
			mcpConnectionManagerLog("Cancelled \(cancelledToolCount) active tool execution(s) owned by disconnected connection \(id)")
		}
	...
	private var bindingResolver: MCPBindingResolver {
		MCPBindingResolver(
			...
			clientIdentifier: { [self] connectionID in
				await clientIdentifier(forConnection: connectionID)
			},
			...
			preferredLiveRunWindowID: { [self] clientName, sessionKey in
				await preferredLiveRunAffinity(for: clientName, sessionKey: sessionKey)?.windowID
			},
			preferredWindowID: { [self] clientName, sessionKey in
				await self.preferredWindowID(for: clientName, sessionKey: sessionKey)
			}
		)
	}
	...
	private func cachedSchema(for name: String, schema: JSONSchema, purpose: MCPRunPurpose) async throws -> Value {
		...
		if toolSchemaCache.count >= maxToolSchemaCacheEntries {
			mcpConnectionManagerLog("toolSchemaCache flushed (exceeded cap=\(maxToolSchemaCacheEntries))")
	...
	nonisolated static func augmentSchemaWithCanonicalBindingParams(
		...
		guard case .object(var dict) = schemaValue,
			dict["type"]?.stringValue == "object" else {
			return schemaValue
		}
	...
	private nonisolated func schemaDeclaresWindowID(schema: JSONSchema) -> Bool {
		...
		guard let schemaValue = try? Value(schema),
			  case .object(let dict) = schemaValue,
			  let props = dict["properties"]?.objectValue else {
			return false
		}
	...
	private nonisolated func injectWindowIDIfNeeded(
		...
		guard let windowID = routingWindowID,
			  schemaDeclaresWindowID(schema: schema) else {
			return args
		}
	...
	private func awaitAgentBootstrapPolicyBeforeAcceptIfNeeded(
		...
		guard let bootstrapClientName,
			Self.isKnownAgentClientName(bootstrapClientName) else {
			return .notRequired
		}
		...
		guard hasPIDGatedPendingPolicy(for: bootstrapClientName)
			|| preferredLiveRunAffinity(for: bootstrapClientName, sessionKey: sessionKey) != nil else {
			return .notRequired
		}
	...
	private nonisolated func debugBindingKindString(_ kind: MCPServerViewModel.ConnectionBindingSnapshot.BindingKind) -> String {
		...
		case .windowOnly: return "window_only"
	...
	private func debugBindingSnapshot(for connectionID: UUID, selectedWindowID: Int?) async -> MCPServerViewModel.ConnectionBindingSnapshot {
		await MainActor.run {
			...
			if let explicit = snapshots.first(where: { $0.bindingKind == .context && $0.explicitlyBound && $0.runID == nil }) {
				return explicit
			}
			if let runScoped = snapshots.first(where: { $0.bindingKind == .context && $0.runID != nil }) {
				return runScoped
			}
			if let context = snapshots.first(where: { $0.bindingKind == .context }) {
				return context
			}
	...
	private func debugWindowObjects() async -> [[String: Any]] {
		await MainActor.run {
			WindowStatesManager.shared.allWindows.map { window in
				...
				if workspace?.isSystemWorkspace == true {
					workspaceName = "Default (no workspace loaded)"
				} else {
					workspaceName = workspace?.name ?? NSNull()
				}
	...
	private func debugPersistedRecordObjects(clientNameFilter: String?, sessionFingerprint: String?) -> [[String: Any]] {
		let keys: [String]
		if let clientNameFilter, !clientNameFilter.isEmpty {
			keys = matchingClientKeys(for: clientNameFilter, in: Array(routingState.records.keys))
		} else {
			keys = Array(routingState.records.keys).sorted()
		}
	...
	private func debugLiveAffinityObject(clientName: String?, sessionKey: String?) -> [String: Any] {
		...
		guard let key = keys.first,
			let affinity = liveRunAffinityByClientSession[key]?[sessionKey] else {
			return ["present": false, "window_id": NSNull(), "run_id": NSNull(), "purpose": NSNull()]
		}
		return [
			"present": true,
	...
	func debugClearRoutingStatePayload(currentConnectionID: UUID, clientName: String?, allClients: Bool) -> [String: Any] {
		...
		if allClients {
			...
			for key in keys { routingState.records.removeValue(forKey: key) }
	...
	private func debugSetRestartStatus(_ restartID: UUID, state: String, error: String? = nil) {
		...
		if debugRestartStatesByID.count > debugRestartStatusLimit {
			...
			for id in oldest { debugRestartStatesByID.removeValue(forKey: id) }
		}
	}
	...
	func debugScheduleShutdownAndRestartPayload(restartID: UUID, delayMS: Int, downMS: Int, mode: String) -> [String: Any] {
		...
		Task { [weak self] in
			...
			await self.debugSetRestartStatus(restartID, state: "stopping")
			await self.debugRecordConnectionEvent("restart_stop_begin", restartID: restartID, reason: mode)
			await self.stop()
			await self.debugSetRestartStatus(restartID, state: "down")
			await self.debugRecordConnectionEvent("restart_stop_end", restartID: restartID, reason: mode)
			if downMS > 0 {
				try? await Task.sleep(for: .milliseconds(downMS))
			}
			await self.debugSetRestartStatus(restartID, state: "starting")
			await self.debugRecordConnectionEvent("restart_start_begin", restartID: restartID, reason: mode)
			await self.start()
			try? await Task.sleep(for: .milliseconds(100))
			await self.ensureBootstrapHealthy(force: true)
			await self.debugSetRestartStatus(restartID, state: "completed")
			await self.debugRecordConnectionEvent("restart_start_end", restartID: restartID, reason: mode)
		}
		return [
			"ok": true,
	...
	func debugRestartStatusPayload(restartID: UUID?) -> [String: Any] {
		let status: DebugRestartStatus?
		if let restartID {
			status = debugRestartStatesByID[restartID]
		} else {
			status = debugRestartStatesByID.values.sorted { $0.updatedAt > $1.updatedAt }.first
		}
		return [
			"ok": true,
			"op": "restart_status",
	...
	func debugListToolNames(
		...
		if isEnabledState {
			...
			for surface in DelegateEditToolSurfaces.surfaces(for: policy.purpose) {
				if seenNames.insert(surface.name).inserted {
	...
	func debugHandleDelegateSandboxToolCall(
		connectionID: UUID,
		toolName: String,
		args: [String: Value],
		rawJSON: Bool = false
	) async -> CallTool.Result? {
		await handleDelegateSandboxToolCallIfNeeded(
			connectionID: connectionID,
			toolName: toolName,
			args: args,
			rawJSON: rawJSON
	...
	func sendProgress(
		...
		mcpConnectionManagerLog("progress client=\(clientName) connection=\(connectionID) tool=\(tool) kind=\(kind.rawValue) stage=\(stage) message=\(message)")
#endif
		if let mgr = connections[connectionID] {
	...
	private func applyLiveRunAffinity(
		...
		mcpConnectionManagerLog("Applying live run affinity (\(reason)): client \(clientName) → window \(liveAffinity.windowID) runID=\(liveAffinity.runID)")
		setConnectionWindowMapping(connectionID, windowID: liveAffinity.windowID)
	...
	private func applyRoutingFallback(
		...
		if let preferredWindowID = await preferredWindowID(for: clientName, sessionKey: sessionKey) {
			mcpConnectionManagerLog("Applying persisted routing affinity: client \(clientName) → window \(preferredWindowID)")
			setConnectionWindowMapping(connectionID, windowID: preferredWindowID)
			await updateRoutingRecordForConnection(connectionID, clientID: clientName)
			await notifyToolListChanged(connectionID: connectionID)
		}
	...
	/// Install tab context from a connection policy (for discover agents)
	private func installTabContextFromPolicy(
		...
		mcpConnectionManagerLog("Installed tab context from policy: tab=\(tabID) run=\(runID) window=\(windowID) workspace=\(resolved.workspaceID) client=\(clientName)")
	}
	...
	private func ensureTabBoundForRunIfPossible(
		...
		do {
			...
			if bindingResult.didBind {
				mcpConnectionManagerLog("Tab binding: rebound connection \(connectionID) to cached run tab \(tabID) for run \(runID) in window \(windowID)")
			}
			return true
		} catch {
    ...
    func registerHandlers(for server: MCP.Server, connectionID: UUID) async {
		...
		await server.withMethodHandler(ListPrompts.self) { _ in
			mcpConnectionManagerLog("Handling ListPrompts request for \(connectionID)")
		...
		await server.withMethodHandler(GetPrompt.self) { params in
			mcpConnectionManagerLog("Handling GetPrompt request name=\(params.name) for \(connectionID)")
		...
		await server.withMethodHandler(ListResources.self) { _ in
			mcpConnectionManagerLog("Handling ListResources request for \(connectionID)")
		...
		await server.withMethodHandler(ListResourceTemplates.self) { _ in
			mcpConnectionManagerLog("Handling ListResourceTemplates request for \(connectionID)")
		...
		await server.withMethodHandler(ReadResource.self) { [weak self] params in
			...
			mcpConnectionManagerLog("Handling ReadResource request for \(connectionID), uri=\(params.uri)")
			
			switch params.uri {
			case "repoprompt://instructions":
				let hydratedPolicy = await self.hydratePersistedAgentModePolicyForConnectionIfNeeded(
					connectionID: connectionID,
					reason: "resources/read-instructions"
				)
				let purpose = await self.effectivePolicyState(for: connectionID).purpose
				if hydratedPolicy {
		...
		await server.withMethodHandler(ListTools.self) { [weak self] _ in
			...
			mcpConnectionManagerLog("Handling ListTools request for \(connectionID)")
			...
			let windowID = await self.ensureWindowBindingIfUnambiguous(connectionID: connectionID, reason: "tools/list")
			let isReady = await MCPToolCatalogReadiness.shared.awaitReady(
				windowID: windowID,
			...
			if !isReady {
				...
				if windowID != nil {
					...
					mcpConnectionManagerLog("Tool catalog readiness skipped for multi-window ambiguous connection \(connectionID)")
				}
			}
			...
			_ = await self.hydratePersistedAgentModePolicyForConnectionIfNeeded(
				connectionID: connectionID,
				reason: "tools/list"
			)
			...
			let policy = await self.effectivePolicyState(for: connectionID)
			let restricted = policy.restricted
			let additionalTools = policy.additional
#if DEBUG
			...
			if await self.isEnabledState {
				...
				for service in registeredServices {
					...
					for tool in await service.tools {
						...
						let schemaValue = try await self.cachedSchema(
							for: tool.name,
							schema: tool.inputSchema,
							purpose: policy.purpose
						)
						let description = self.advertisedToolDescription(
							for: tool.name,
				...
				for surface in DelegateEditToolSurfaces.surfaces(for: policy.purpose) {
					guard seenNames.insert(surface.name).inserted else { continue }
					let schemaValue = try await self.cachedSchema(
						for: surface.name,
...
#if DEBUG
			await self.debugPolicyDiagnostic("toolsList", connectionID: connectionID, policy: policy, extra: [
...
#endif
			mcpConnectionManagerLog("Returning \(tools.count) available tools for \(connectionID)")
        ...
        await server.withMethodHandler(CallTool.self) { [weak self] params in
...
#endif
			if let reason = await MainActor.run(body: { ToolAvailabilityStore.shared.globalSuppressionReason(for: toolName) }) {
#if DEBUG
				await debugPolicyDiagnostic("toolsCallRejected", connectionID: connectionID, extra: [
					"toolName": toolName,
					"reason": "global_suppression",
					"detail": reason
				])
#endif
				return CallTool.Result.err("\(toolName) is unavailable: \(reason)")
			}
			...
			for warning in normalized.warnings {
				mcpConnectionManagerLog("Argument normalization: \(warning)")
			}
			...
			let attemptedEarlyDelegateSandboxIntercept: Bool
			if toolName == DelegateEditToolNames.editFile {
				attemptedEarlyDelegateSandboxIntercept = true
			} else if DelegateEditToolNames.sandboxToolNames.contains(toolName) {
				attemptedEarlyDelegateSandboxIntercept = await self.isDelegateEditContext(connectionID: connectionID)
			} else {
				attemptedEarlyDelegateSandboxIntercept = false
			}
			if attemptedEarlyDelegateSandboxIntercept {
				let sandboxState = EditFlowPerf.begin(
					EditFlowPerf.Stage.MCPToolCall.delegateSandboxIntercept,
					EditFlowPerf.Dimensions(toolName: toolName, status: "early")
				)
				let sandboxResult = await self.handleDelegateSandboxToolCallIfNeeded(
					connectionID: connectionID,
					toolName: toolName,
				...
				EditFlowPerf.end(
					EditFlowPerf.Stage.MCPToolCall.delegateSandboxIntercept,
					sandboxState,
					EditFlowPerf.Dimensions(toolName: toolName, status: sandboxResult == nil ? "early_miss" : "early_hit")
				)
			...
			_ = await self.hydratePersistedAgentModePolicyForConnectionIfNeeded(
				connectionID: connectionID,
				reason: "tools/call-pre-routing"
			)

			var effectiveTabID = extractedTabID
			...
			do {
				...
				if !Self.shouldBypassLogicalContextPreResolution(for: toolName) {
					do {
						if let logicalBinding = try await self.bindingResolver.resolveLogicalContextBinding(
							connectionID: connectionID,
							...
							effectiveTabID = effectiveTabID ?? logicalBinding.logicalContext.tabID
							preResolvedWindowID = logicalBinding.windowID
							await self.setConnectionWindowMapping(connectionID, windowID: logicalBinding.windowID)
							mcpConnectionManagerLog(
						...
						let routePolicy = await self.effectivePolicyState(for: connectionID)
			...
			// Fallback sandbox interception for delegate edit runs (single-file virtual FS).
			// Sandbox-owned tool names are attempted earlier, before broader logical-context work.
			if !attemptedEarlyDelegateSandboxIntercept {
				let sandboxState = EditFlowPerf.begin(
					EditFlowPerf.Stage.MCPToolCall.delegateSandboxIntercept,
					EditFlowPerf.Dimensions(toolName: toolName)
				...
				EditFlowPerf.end(
					EditFlowPerf.Stage.MCPToolCall.delegateSandboxIntercept,
					sandboxState,
					EditFlowPerf.Dimensions(toolName: toolName, status: sandboxResult == nil ? "miss" : "hit")
				)
				if let result = sandboxResult {
					return result
				}
			...
			do {
				...
				// Opportunistic persisted hydration before gating checks.
				// Only cached run policy is allowed to restore policy-gated tools.
				_ = await self.hydratePersistedAgentModePolicyForConnectionIfNeeded(
					connectionID: connectionID,
					reason: "tools/call-pre-gating"
				)

				// Block policy-gated tools unless explicitly granted via additionalTools
				if MCPPolicyGatedTools.names.contains(toolName) {
					let effectivePolicy = await self.effectivePolicyState(for: connectionID)
					if !effectivePolicy.additional.contains(toolName) {
#if DEBUG
						await self.debugPolicyDiagnostic("toolsCallRejected", connectionID: connectionID, policy: effectivePolicy, extra: [
							"toolName": toolName,
			...
			// Rehydrate canonical binding fields into dispatch args when the tool
			// explicitly accepts them. context_id remains the hidden per-call routing
			// handle for non-bind_context tools; working_dirs is bind_context-only.
			...
			// Prepare args for formatter (adds hidden/derived binding context on top)
			var argsForFormatter = dispatchArguments
			if let tabID = effectiveTabID {
			...
			let capturedTabID = effectiveTabID
			...
			let limiter = await self.limiter(for: connectionID)
			return await limiter.withPermit {
				// Wrap entire call so inner services can query currentConnectionID
				...
				let (windowCount, allServices, multiWindowModeEffective) = await MainActor.run {
					let windows = WindowStatesManager.shared.allWindows
				...
				if !bypassWindowRouting, let requestedWindowID = capturedWindowID {
					...
					if existingMapping == nil {
						...
						mcpConnectionManagerLog("Tool call: bound unassigned connection \(connectionID) to window \(requestedWindowID) via _windowID")
					} else if let prev = existingMapping, prev != requestedWindowID {
						mcpConnectionManagerLog("Tool call: applying per-call _windowID override \(prev) → \(requestedWindowID) for connection \(connectionID) (default binding unchanged)")
					} else {
						mcpConnectionManagerLog("Tool call: routed connection \(connectionID) to window \(requestedWindowID) via _windowID")
					}
				}
				...
				if !bypassWindowRouting,
					...
					mcpConnectionManagerLog("Tool call: auto-routed connection \(connectionID) to window \(windowID) via clientName '\(clientName)' reuse")
				}
				...
				if !bypassWindowRouting,
					...
					if let liveAffinity = await self.preferredLiveRunAffinity(for: clientName, sessionKey: sessionKey) {
						...
						mcpConnectionManagerLog("Tool call: restored live run affinity for connection \(connectionID) → runID \(liveAffinity.runID)")
						...
						mcpConnectionManagerLog("Tool call: auto-routed connection \(connectionID) to window \(preferredWindowID) via persisted routing affinity for client '\(clientName)'")
					}
				}
            ...
            mcpConnectionManagerLog("Tool call: \(logName) [conn=\(connectionID) window=\(windowStr)]")

			// Notify tool call observers using the connection's resolved run mapping.
			let observerRunIDForCallbacksFinal = await self.runIDForConnection(connectionID)
			...
			await self.recordToolCall(for: connectionID, toolName: toolName)
				defer { mcpConnectionManagerLog("CALL end   conn=\(connectionID) tool=\(toolName) window=\(windowStr)") }
				...
				// ────────────────────────────────────────────────────────
				// Tab binding via hidden _tabID parameter
				// ────────────────────────────────────────────────────────
				if let tabID = capturedTabID, !Self.shouldSkipGenericTabBinding(for: toolName) {
					...
					do {
						try await MainActor.run {
							...
							guard let activeWorkspace = windowState.workspaceManager.activeWorkspace else {
								throw MCPError.invalidParams("No active workspace in window \(windowID). Use manage_workspaces action='list' to see available workspaces, then action='switch' to load one.")
							}
							try windowState.mcpServer.bindTabForConnection(
								connectionID: connectionID,
								clientName: clientName,
						...
						mcpConnectionManagerLog("Tab binding: bound connection \(connectionID) to tab \(tabID) in window \(windowID)")
					} catch {
				...
				for service in allServices {
					...
					if let wsSvc, shouldTrackToolOwnership {
						...
						do {
							return try await self.withWindowToolOwnership(
								...
								guard let value else {
									...
									if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "window-scoped nil-result") {
								...
								if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "window-scoped success") {
								...
								// The spawned agent connection is terminated by DiscoverAgentViewModel
							...
							if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "window-scoped error") {
						...
						do {
								...
								guard let value else {
									if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "global nil-result") {
										let errorJSON = ToolOutputFormatter.rawJSONString(.object(["error": .string("Tool not found: \(toolName)"), "tool": .string(toolName)]))
										let eventObserverCount = await self.fireToolCompletedObservers(runID: runID, invocationID: invocationID, toolName: toolName, args: capturedArguments, resultJSON: errorJSON, isError: true)
#if DEBUG
										if toolName == "agent_run" {
								...
								// Fire completion observer with result for detailed UI rendering
								if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "global success") {
									let resultJSON = ToolOutputFormatter.rawJSONString(value)
									let eventObserverCount = await self.fireToolCompletedObservers(runID: runID, invocationID: invocationID, toolName: toolName, args: capturedArguments, resultJSON: resultJSON, isError: false)
#if DEBUG
									if toolName == "agent_run" {
							...
							log.error("Error executing tool \(toolName): \(error.localizedDescription)")
							if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "global error") {
								let errorJSON = ToolOutputFormatter.rawJSONString(.object(["error": .string(error.localizedDescription), "tool": .string(toolName)]))
								let eventObserverCount = await self.fireToolCompletedObservers(runID: runID, invocationID: invocationID, toolName: toolName, args: capturedArguments, resultJSON: errorJSON, isError: true)
#if DEBUG
										if toolName == "agent_run" {
				...
				if let runID = await self.observerRunIDForCompletionForToolTracking(callTimeRunID: observerRunIDForCallbacksFinal, connectionID: connectionID, toolName: toolName, invocationID: invocationID, context: "final tool-not-found fallthrough") {
	...
	func broadcastToolListChanged() async {
		mcpConnectionManagerLog("Broadcasting ToolListChanged notification to all clients")
	...
	func dashboardSnapshot() async -> NetworkDashboardSnapshot {
		...
		for (id, manager) in connections {
			...
			let stateSummary: ConnectionStateSummary
			switch rawState {
			case .ready:     stateSummary = .ready
			case .connecting: stateSummary = .setup
			case .failed(_): stateSummary = .failed
			case .cancelled: stateSummary = .cancelled
			}
			...
			let activeToolName: String?
			if let winID = windowID, activeToolOwnerByWindow[winID] == id {
				activeToolName = activeToolNameByWindow[winID]
			} else if let ownedWindow = activeToolOwnerByWindow.first(where: { $0.value == id })?.key {
				activeToolName = activeToolNameByWindow[ownedWindow]
			} else {
				activeToolName = nil
			}
	...
	func identityContextSnapshots() -> [IdentityContextSnapshot] {
		identityContextByConnection.map { (connectionID, ctx) in
			let source: IdentityContextSnapshot.Source
			switch ctx.source {
			case .unknown: source = .unknown
			case .filesystemMeta: source = .filesystemMeta
			case .handshake: source = .handshake
			}
	...
	private static func languageTag(forPath path: String) -> String {
		...
		return ToolOutputFormatter.languageTag(forPath: path)
	}
	...
	func tryReserveConnectionSlot(connectionID: UUID, clientID: String) async -> Bool {
		...
		mcpConnectionManagerLog("Admitted connection \(connectionID) with clientID='\(clientID)'")
	...
	private func updateRoutingRecordForConnection(_ connectionID: UUID, clientID: String) async {
		...
		guard let sessionKey = capability else {
			mcpConnectionManagerLog("Skipping routing persistence for token-less client: \(clientID)")
			return
		}
    ...
    func recordToolCall(for connectionID: UUID, toolName: String) {
		...
		if canonicalToolName == "set_status" { return }
    ...
    private func evictLeastValuableGlobalForAdmission(preserveOnePerClient: Bool) async -> Bool {
        ...
        for (cid, set) in activeConnectionsByClient { countsByClient[cid] = set.count }
        ...
        if candidates.isEmpty && preserveOnePerClient {
            ...
            for (_, cnt) in countsByClient { if cnt > 1 { anyHasMoreThanOne = true; break } }
