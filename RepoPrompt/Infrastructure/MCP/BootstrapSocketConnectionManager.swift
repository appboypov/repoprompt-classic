...
actor BootstrapSocketConnectionManager: MCPServerConnection {
	...
	nonisolated var isFilesystemBacked: Bool { false }
	/// No connection folder
	nonisolated var connectionFolderURL: URL? { nil }
	/// Capability token is the session token
	nonisolated var capabilityToken: String? { sessionToken }
	
	/// Verified peer PID for this connection (from the bootstrap socket).
	func peerPID() -> Int { clientPid }
	...
	init(
		...
		self.transport = UnixSocketMCPTransport(connectedFD: connectedFD, logger: bootstrapLog)
	...
	func start(approvalHandler: @escaping (MCP.Client.Info) async -> Bool) async throws {
		...
		do {
			...
			try await server.start(transport: transport) { [weak self] clientInfo, _ in
				...
				await self.markHandshakeComplete()
			}
	...
	func connectionState() -> ConnectionStateSnapshot { state }
	...
	func sendProgress(tool: String, kind: RepoPromptProgressKind, stage: String, message: String) async {
		...
		let notification: RepoPromptControlNotification<RepoPromptProgressParams>
		switch kind {
		case .stage:
			notification = .stage(tool: tool, stage: stage, message: message)
		case .heartbeat:
			notification = .heartbeat(tool: tool, stage: stage, message: message)
		}
		
		guard let data = notification.jsonLineData() else {
