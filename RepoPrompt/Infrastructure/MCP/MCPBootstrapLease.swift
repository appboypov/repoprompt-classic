...
/// Used by both agent-mode and headless (discover/delegate-edit) paths.
struct MCPBootstrapLeaseSpec: Sendable {
	let runID: UUID
	let gateID: UUID
	let windowID: Int
	let tabID: UUID?
...
actor MCPBootstrapLease {
	...
	@discardableResult
	func releaseWhenRouted(timeoutMs: Int = 10_000) async -> Bool {
		if hasReleased {
