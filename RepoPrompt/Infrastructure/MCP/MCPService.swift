...
actor MCPService: Sendable {
    ...
    nonisolated var stateStream: AsyncStream<Snapshot> { updates.stream }
    ...
    func setLaunchAllowed(_ allowed: Bool) async {
        mcpServiceLog("Setting MCP launch allowed: \(allowed)")
        await controller.setLaunchAllowed(allowed)
    }
    ...
    func continuePendingApproval(allow: Bool, alwaysAllow: Bool = false) async {
        await controller.resolvePendingApproval(allow: allow,
	...
	struct DashboardConnection: Sendable, Identifiable {
		let id: UUID
	...
	struct ToolCallHistoryEntry: Sendable, Identifiable {
		let id = UUID()
		let timestamp: Date
		let toolName: String
		let clientName: String
