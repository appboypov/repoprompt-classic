...
private struct RegexScanTraits: Sendable {
	let anchored: Bool
...
private struct SearchLineIndex {
	...
	init(content: String) {
		...
		self.lineStartsUTF8 = startsUTF8
	}
...
private final class SearchLineIndexBox: NSObject {
	...
	init(lineIndex: SearchLineIndex) {
		...
		self.lineCount = lineIndex.lineRanges.count
	}
}
...
private enum SearchLineIndexCacheIdentity {
	...
	var utf16Length: Int {
		switch self {
		case .versioned(_, _, let length), .hashed(_, let length, _):
			return length
		}
	}
...
private struct SearchDocument {
	...
	init(filePath: String, text: String, lineIndex: SearchLineIndex, contextLines: Int) {
		self.filePath = filePath
		self.text = text
		self.lineIndex = lineIndex
	...
	var fullRange: NSRange { NSRange(location: 0, length: nsText.length) }
	var lineRanges: [NSRange] { lineIndex.lineRanges }
	...
	func lineSlice(at lineNumber: Int) -> Substring {
		guard lineNumber >= 0, lineNumber < lineIndex.lineRanges.count,
			let range = Range(lineIndex.lineRanges[lineNumber], in: text) else {
			return Substring()
		}
		return text[range]
	}
...
actor FileSearchActor {
	...
	private static func compileEngine(
		...
		if !wholeWord,
			...
			return .anchoredDeclaration(anchoredDeclaration, try compilePCRE2(pattern))
		}
	...
	private static func lineIndexCacheKey(identity: SearchLineIndexCacheIdentity) -> NSString {
		switch identity {
		case .versioned(let fileID, let contentRevision, let utf16Length):
			return "v|\(fileID.uuidString)|\(contentRevision)|\(utf16Length)" as NSString
		case .hashed(let filePath, let utf16Length, let hash):
			return "h|\(filePath)|\(utf16Length)|\(hash)" as NSString
		}
	}
	...
	private func searchWithErrors(
		...
		func hasMatches(_ result: SearchContentResult) -> Bool { result.totalCount > 0 }
	...
	private func filterFiles(_ files: [FileViewModel], options: SearchOptions) -> [FileViewModel] {
	...
	private func searchContentWithErrors(
		...
		return SearchContentResult(
			...
			matchedFileCount: Set(emittedMatches.map { $0.filePath }).count,
			perFileErrors: perFileErrors
		)
	}
	...
	private static func scanFileWithErrorHandling(
		...
		do {
			...
			let snapshot = await EditFlowPerf.measure(
				...
				await file.searchContentSnapshot(freshnessPolicy: plan.contentFreshnessPolicy)
			}
	...
	private static func scanRegexCountOnlyFastPath(
		...
		switch engine {
		case .asciiWholeWord(let literal):
		...
		case .anchoredDeclaration(let plan, _):
	...
	private static func scan(
		...
		if let engine = engine {
			...
			case .asciiMarker(let plan, let fallbackRequest):
			...
			case .anchoredDeclaration(let plan, let fallbackRegex):
				if let result = plan.scanMatchingLines(
					in: document.text,
					collectMatches: !countOnly,
					maxCollectedMatches: maxCollectedMatches,
	...
	/// absolute path resolution is handled via `RepoFileManagerViewModel`/`PathMatchWorker`.
	///
	...
	private static func scanPath(_ file: FileViewModel, plan: SearchPathScanPlan) throws -> Bool {
	...
	private static func candidatePaths(for file: FileViewModel, aliasByRootPath: [String: String]?) -> [String] {
		var seen = Set<String>()
		...
		// absolute-path matching should use RepoFileManagerViewModel + PathMatchWorker.
	...
	func searchUnified(
		...
		if effectiveMode == .path || effectiveMode == .both {
			do {
				pathHits = try await self.searchPaths(
					pattern        : pattern,
		...
		if effectiveMode == .content || effectiveMode == .both {
			do {
				if options.countOnly {
					let result = try await self.searchWithErrors(
						pattern: pattern,
						isRegex: effectiveIsRegex,
						wasAutoCorrected: &wasAutoCorrected,
					...
					contentFileCount = Set(contentHits.map { $0.filePath }).count
					perFileErrors.append(contentsOf: result.perFileErrors)
				}
			} catch let e as RegexPatternFailure {
				contentError = e
	...
	static func inferredAutoMode(_ raw: String) -> SearchMode {
		...
		if raw.contains("\\") {
			if Self.backslashesOnlyEscapeRegexMeta(raw) {
				return .content
			}
			if raw.contains(" ") && raw.count > 20 {
				return .content
	...
	static func containsRegexSyntax(_ pattern: String) -> Bool {
		...
		if pattern.contains("|") {
			...
			let nonEmptyCount = components.filter { !$0.trimmingCharacters(in: .whitespaces).isEmpty }.count
			if nonEmptyCount >= 2 {
				return true
			}
		}
