...
enum SearchPathClause: Sendable, Equatable {
...
struct SearchPathFilterSpec: Sendable, Equatable {
	let caseInsensitive: Bool
	let clauses: [SearchPathClause]
}

struct FileSearchPathSnapshot: Sendable {
...
struct FileSearchPathFilterResult: Sendable, Equatable {
...
struct FileSearchPathIndexFilterResult: Sendable, Equatable {
...
private func compileSearchPathClauses(_ clauses: [SearchPathClause]) -> [CompiledSearchPathClause] {
	clauses.map { clause in
		switch clause {
		case .exactFile(let absPath, let relPath, let restrictedRootPath):
			return .exactFile(absPath: absPath, relPath: relPath, restrictedRootPath: restrictedRootPath)
		case .exactFolder(let absLower, let relLower, let restrictedRootPath):
			return .exactFolder(
				absLower: absLower,
		...
		case .glob(let pattern, let restrictedRootPath):
			return .glob(pattern: pattern, restrictedRootPath: restrictedRootPath)
		case .legacyPrefix(let candidateLower):
			return .legacyPrefix(
...
func filterPathIndicesResult(
	...
	for index in snapshots.indices {
		...
		for clause in compiledClauses {
			...
			case .exactFolder(let absLower, let absPrefix, let relLowerClause, let relPrefix, let restrictedRootPath):
			...
			case .legacyPrefix(let candidateLower, let normalizedPrefix):
...
func resolveFoldersBySuffixFragment<T>(
	...
	guard let candidate = normalizedFolderSuffixFragment(fragment, caseInsensitive: caseInsensitive),
			let lastComponent = candidate.split(separator: "/").last.map(String.init) else {
		return []
	}
