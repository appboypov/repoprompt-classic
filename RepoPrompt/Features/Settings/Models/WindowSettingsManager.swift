...
protocol SettingsManaging {
	...
	func setGlobalDiscoverAgentSelection(agentRaw: String, modelRaw: String, markUserDefined: Bool)
	...
	func setAllowDiffModelsToRewrite(_ allowed: Bool, commit: Bool)
	...
	func updateProEditSettings(_ mutation: (inout ProEditSettingsSnapshot) -> Void, commit: Bool)
	func proEditAgentModeMigrated() -> Bool
	func setProEditAgentModeMigrated(_ migrated: Bool, commit: Bool)
...
final class WindowSettingsManager: ObservableObject, SettingsManaging {
	...
	func globalDiscoverAgentSelection() -> (agentRaw: String?, modelRaw: String?) {
		store.globalDiscoverAgentSelection()
	}

	func setGlobalDiscoverAgentSelection(agentRaw: String, modelRaw: String, markUserDefined: Bool = true) {
	...
	func allowDiffModelsToRewrite() -> Bool {
		store.allowDiffModelsToRewrite()
	}

	func setAllowDiffModelsToRewrite(_ allowed: Bool, commit: Bool = true) {
		store.setAllowDiffModelsToRewrite(allowed, commit: commit)
	}
	...
	func proEditSettings() -> ProEditSettingsSnapshot {
		store.proEditSettings()
	}

	func updateProEditSettings(_ mutation: (inout ProEditSettingsSnapshot) -> Void, commit: Bool = true) {
		store.updateProEditSettings(mutation, commit: commit)
	}

	func proEditAgentModeMigrated() -> Bool {
		store.proEditAgentModeMigrated()
	}

	func setProEditAgentModeMigrated(_ migrated: Bool, commit: Bool = true) {
		store.setProEditAgentModeMigrated(migrated, commit: commit)
	}
	...
	/// to the global store.
	func commitAllVisitedWorkspaces() {
		for (_, s) in copyOverlays { store.updateCopySettings(s) }
		for (_, s) in chatOverlays { store.updateChatSettings(s) }
	}
