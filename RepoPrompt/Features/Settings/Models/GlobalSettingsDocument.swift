...
///
/// Schema v1 contains the original GlobalSettingsStore data: copy settings,
/// chat settings, and cross-workspace global defaults. Schema v2 adds optional
/// scalar preference groups migrated from legacy UserDefaults/AppStorage keys.
/// Schema v3 forces `.gitignore` handling back on once for existing installs.
/// Scalar fields stay optional so older/missing JSON can fall back through the
/// typed GlobalSettingsStore accessors without losing legacy default behavior.
struct GlobalSettingsDocument: Codable {
	...
	init(
		...
		self.copySettingsByWorkspaceID = Self.encodeUUIDKeyedDictionary(copySettings)
		self.chatSettingsByWorkspaceID = Self.encodeUUIDKeyedDictionary(chatSettings)
...
/// Optional scalar preferences migrated into schema v2.
///
/// Keep groups and fields optional during the migration window so missing values
/// continue to use the same defaults as the legacy UserDefaults/AppStorage sites.
struct GlobalScalarPreferences: Codable, Equatable {
	...
	struct PromptPackagingSettings: Codable, Equatable {
		...
		init(
			...
			allowDiffModelsToRewrite: Bool? = nil,
			...
			self.allowDiffModelsToRewrite = allowDiffModelsToRewrite
