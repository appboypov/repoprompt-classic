...
struct AIModelDropdown: View {
	...
	init(
		...
		self.useBorderlessStyle = useBorderlessStyle
		self.isInGeneralSettings = isInGeneralSettings
		self.destination = .chatModel(promptVM: promptViewModel)
	}
	...
	private func aiModelMenuItems() -> [StableMenuItem] {
		...
		return sortedProviders.flatMap { provider -> [StableMenuItem] in
			...
			let providerItems: [StableMenuItem]
			if provider == .codex {
				providerItems = AIModel.codexMenuGroups(for: models).map { group in
				...
				providerItems = AIModel.openCodeMenu(for: models).providerGroups.flatMap { providerGroup -> [StableMenuItem] in
				...
				providerItems = models.map(aiModelMenuItem)
			}
	...
	private func compatibleClaudeBackendOptionDisplayName(
		...
		case .claudeSlotMapping(let mapping):
			...
			switch descriptor.requestedModelRaw {
			case .some(AgentModel.claudeHaiku.rawValue):
				backendModelID = normalized.haiku
			case .some(AgentModel.claudeOpus.rawValue):
				backendModelID = normalized.opus
			case .some(AgentModel.claudeSonnet.rawValue), nil:
				backendModelID = normalized.sonnet
			default:
				backendModelID = nil
			}
	...
	private var displayedModelName: String {
		let currentModel = destination.currentRawValue
		...
		if let customModel = promptViewModel.apiSettingsViewModel?.customOpenRouterModels.first(where: { "openrouter_custom_\($0)" == currentModel }) {
			return "oRouter/\(customModel)"
		}

		// Check available models
		if let selectedModel = promptViewModel.availableModels.first(where: { $0.rawValue == currentModel }) {
			if let descriptor = ClaudeCodeAIModelCatalog.compatibleBackendDescriptor(for: selectedModel) {
		...
		return promptViewModel.availableModels.first?.displayName ?? "Select a model"
	}
