...
struct AgentModelsSettingsView: View {
	...
	init(
		...
		discoverVM: DiscoverAgentViewModel,
		...
		self._viewModel = StateObject(wrappedValue: viewModel ?? AgentModelsSettingsViewModel(
			promptVM: promptVM,
		...
		settingsCard {
			...
			if !viewModel.hasConnectedCLIProvider {
				HStack(spacing: 8) {
					Image(systemName: "info.circle")
						.foregroundColor(.secondary)
					Text("Connect Claude Code, Codex, Gemini, OpenCode, or Cursor to configure role defaults.")
						.font(.caption)
						.foregroundColor(.secondary)
					Spacer(minLength: 0)
					Button {
	...
	/// "Advanced" disclosure, which made useful cross-links (Oracle presets,
	/// benchmarks, legacy Pro Edit) easy to miss. "Advanced" now owns only real
	/// tunables (Sync toggle, Built-in Chat override) so related-settings
	/// navigation no longer requires a click to uncover.
	///
	/// SEARCH-HELPER: Agent Models related settings, Oracle Model Presets link,
	/// Benchmark Model link, Pro Edit link, related-settings footer
	private var relatedSettingsSection: some View {
		VStack(alignment: .leading, spacing: 10) {
			...
			linkRow(
				...
				title: "Pro Edit",
				detail: "Pro Edit configuration. Superseded by Agent Mode.",
				tab: .proEdit
			)
			linkRow(
				icon: "gauge",
...
// DiscoverAgentViewModel / APISettingsViewModel instances that are only
