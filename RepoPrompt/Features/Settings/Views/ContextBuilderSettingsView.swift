...
/// question timeout, UI-run-only toggles (clarifying questions, post-discovery
...
/// timeout, clarifying questions, post-discovery analysis, custom prompts
///
/// Related:
...
/// - Discover VM:                        /RepoPrompt/ViewModels/Chat/DiscoverAgentViewModel.swift
...
struct ContextBuilderSettingsView: View {
			...
			VStack(alignment: .leading, spacing: 16) {
				...
				VStack(alignment: .leading, spacing: 6) {
					HStack {
						Text("Context Budget")
							.font(fontPreset.font)
						Spacer()
						Text("\(discoverVM.tokenBudget / 1000)k")
							.font(fontPreset.font)
							.foregroundColor(.secondary)
							.monospacedDigit()
					}
					Slider(
						value: Binding(
							get: { Double(discoverVM.tokenBudget) },
							set: { discoverVM.tokenBudget = Int($0) }
						),
						in: 10000...200000,
						step: 5000
					)
				...
				VStack(alignment: .leading, spacing: 6) {
					Text("Prompt Enhancement")
						.font(fontPreset.font)
					Picker("", selection: $discoverVM.enhancementMode) {
				...
				VStack(alignment: .leading, spacing: 6) {
					Text("Question Timeout")
						.font(fontPreset.font)
					Picker("", selection: $discoverVM.questionTimeoutSeconds) {
	...
	private var enhancementDescription: String {
		switch discoverVM.enhancementMode {
		case .fullRewrite:
			return "Agent rewrites the prompt based on discoveries."
		case .augment:
			return "Keeps your instructions, appends discovered context."
		case .preserve:
	...
	private var uiRunsSection: some View {
		Section {
			VStack(alignment: .leading, spacing: 16) {
				...
				Toggle(isOn: $discoverVM.allowClarifyingQuestions) {
					VStack(alignment: .leading, spacing: 2) {
				...
				Toggle(isOn: $discoverVM.autoGeneratePlan) {
					VStack(alignment: .leading, spacing: 2) {
						Text("Post-Discovery Analysis")
							.font(fontPreset.font)
						Text("Auto-run plan/review/question after discovery")
							.font(fontPreset.captionFont)
							.foregroundColor(.secondary)
					}
				}
				...
				if discoverVM.autoGeneratePlan {
					VStack(alignment: .leading, spacing: 8) {
						...
						HStack {
							Text("Analysis Budget")
								.font(fontPreset.captionFont)
							Spacer()
							Text("\(discoverVM.planTokenBudget / 1000)k")
								.font(fontPreset.captionFont)
								.monospacedDigit()
						}
						Slider(
							value: Binding(
								get: { Double(discoverVM.planTokenBudget) },
								set: { discoverVM.planTokenBudget = Int($0) }
							),
							in: 40000...200000,
							step: 5000
						)
	...
	private var mcpRunsSection: some View {
		Section {
			VStack(alignment: .leading, spacing: 16) {
				...
				// Clarifying Questions
				Toggle(isOn: $discoverVM.allowClarifyingQuestionsForMCP) {
					VStack(alignment: .leading, spacing: 2) {
						Text("Allow Clarifying Questions")
							.font(fontPreset.font)
				...
				if discoverVM.allowClarifyingQuestionsForMCP {
					HStack(spacing: 6) {
	...
	private var formattedQuestionTimeout: String {
		let seconds = discoverVM.questionTimeoutSeconds
		if seconds < 60 {
