        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 8) {
                    ...
                    // Show pro editing override if not default
                    if preset.proEditingOverride != .useDefault {
                        HStack(spacing: 4) {
                            Image(systemName: "wand.and.stars")
                                .font(FontScalePreset.current.captionFont)
                            Text(preset.proEditingOverride == .forceOn ? "Pro Edit: On" : "Pro Edit: Off")
                                .font(FontScalePreset.current.captionFont)
                        }
                        .foregroundColor(preset.proEditingOverride == .forceOn ? .green : .orange)
                    }
                }
            }
...
struct ModelPresetEditView: View {
    ...
    @State private var proEditingOverride: ProEditingOverride = .useDefault
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Form {
				...
				// Mode section (Chat, Plan, Edit) with mapping selectors – toggles enabled, default ON for new presets
                Section {
					VStack(alignment: .leading, spacing: 12) {
						...
						// Edit mode
						VStack(alignment: .leading, spacing: 4) {
							Toggle("Available for Edit", isOn: $editEnabled)
								.font(FontScalePreset.current.font)
        ...
        .onAppear {
            if let preset = preset {
				...
				if let mappings = preset.chatPresetMappings {
					...
					editPresetID = mappings.editPresetID
					reviewPresetID = mappings.reviewPresetID
				} else {
					// Migration: if no mappings exist, set defaults based on pro editing override
					chatPresetID = ChatPreset.BuiltIn.chat.id
					planPresetID = ChatPreset.BuiltIn.plan.id
					
					// For edit mode, respect the old pro editing override setting
					if preset.proEditingOverride == .forceOff {
						// User explicitly opted out of pro mode, use Edit preset
						editPresetID = ChatPreset.BuiltIn.edit.id
					} else if preset.proEditingOverride == .forceOn {
						// User explicitly wanted pro mode, use Pro Edit preset
						editPresetID = ChatPreset.BuiltIn.proEdit.id
					} else {
						editPresetID = ChatPreset.BuiltIn.proEdit.id
					}
					// Default review preset
...
struct ChatPresetSelectorButton: View {
	...
	private var currentPreset: ChatPreset? {
		...
		case .edit:
			return ChatPreset.BuiltIn.proEdit
		case .proEdit:
			return ChatPreset.BuiltIn.proEdit
		case .review:
	...
	private func chatPresetPickerPopover() -> some View {
		// Get all presets for the specific mode
		// For edit mode, show both edit and proEdit presets
		// Exclude Manual preset from the list
		let allPresets = chatPresetManager.allPresets.filter { preset in
			...
			if mode == .edit {
				return (preset.mode == .edit || preset.mode == .proEdit) && chatPresetManager.isPresetVisible(preset)
			} else {
		...
		let defaultPresetId: UUID = {
			switch mode {
			case .chat:
				return ChatPreset.BuiltIn.chat.id
			case .plan:
				return ChatPreset.BuiltIn.plan.id
			case .edit:
				return ChatPreset.BuiltIn.proEdit.id
			case .proEdit:
				return ChatPreset.BuiltIn.proEdit.id
			case .review:
				return ChatPreset.BuiltIn.review.id
			}
		}()
		...
		PresetTwoPanePopover_Chat(
			...
			previewBuilder: { preset -> AnyView in
				return AnyView(ChatPresetPreviewView(preset: preset, fontPreset: fontPreset, promptViewModel: promptViewModel))
			},
			onSelect: { preset in
				selection = preset.id
