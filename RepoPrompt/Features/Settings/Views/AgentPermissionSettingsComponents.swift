...
/// (read/write/integrity/decode failure or unsupported future schema). The copy never
...
struct AgentPermissionSecureStorageDegradedBanner: View {
	...
	var body: some View {
		if diagnostics.isSecurePermissionStorageDegraded {
			VStack(alignment: .leading, spacing: 10) {
				HStack(alignment: .top, spacing: 10) {
					...
					VStack(alignment: .leading, spacing: 4) {
						Text("Agent permission storage needs recovery")
							.font(.body).bold()
							.foregroundColor(.orange)
						Text("RepoPrompt is using safe default permissions because stored Agent Permission settings could not be loaded or saved. This can happen after moving to a new Mac with Migration Assistant.")
							.font(.callout)
						...
						if let resetFailureMessage = diagnostics.resetFailureMessage {
				...
				HStack(spacing: 8) {
					Button {
						...
						if diagnostics.isResettingAgentPermissions {
							ProgressView()
								.scaleEffect(0.7)
								.frame(height: 16)
						} else {
							Label("Reset Agent Permissions…", systemImage: "arrow.counterclockwise")
						}
					}
					.buttonStyle(.borderedProminent)
					.controlSize(.small)
					.disabled(diagnostics.isResettingAgentPermissions)

					Text("Resets only Agent Permission settings to safe defaults.")
						.font(.footnote)
						.foregroundColor(.secondary)
					Spacer(minLength: 0)
				}
			...
			.accessibilityLabel("Agent permission storage needs recovery. RepoPrompt is using safe default permissions. Reset Agent Permissions is available.")
			.alert(isPresented: $showingResetConfirmation) {
				Alert(
					title: Text("Reset Agent Permissions?"),
					message: Text("This resets only Agent Permission settings to safe defaults. It does not affect API keys, provider logins, license state, chats, or model settings."),
					primaryButton: .destructive(Text("Reset")) {
						diagnostics.resetAgentPermissionsToSafeDefaults()
					},
	...
	private static func userFacingReason(for kind: AgentPermissionStorageDiagnostic.Kind) -> String? {
		...
		case .keychainReadFailed:
			return "secure storage could not be read"
		case .keychainWriteFailed:
			return "a recent permission change could not be saved securely"
		case .decodeFailed:
			return "stored permissions could not be decoded"
		case .integrityCheckFailed:
			return "stored permissions could not be verified after a device or Keychain change"
		case .unsupportedFutureSchema:
			return "stored permissions were written by a newer version"
