...
struct CopyPresetsSettingsView: View {
    ...
    private var filteredPresets: [CopyPreset] {
        ...
        let list = presetManager.allPresets
            .filter { p in
                // Exclude Manual and Manual (XML) presets from settings - they're not configurable
                guard p.name != "Manual" && p.name != "Manual (XML)" else { return false }
            ...
            .filter { p in
                ...
                case .shown:  return presetManager.isPresetVisible(p)
                case .hidden: return !presetManager.isPresetVisible(p)
                }
            }
    ...
    private func presetCategory(_ preset: CopyPreset) -> Int {
        if !preset.isBuiltIn { return 4 } // Custom
        if preset.name.contains("XML") || preset.name == "Pro Edit (XML)" { return 2 } // XML
        if preset.name.hasPrefix("MCP") { return 3 } // MCP
        return 1 // Standard
    }
    ...
    private func presetOrder(_ preset: CopyPreset) -> Int {
        ...
        // XML modes order
        case "Standard (XML)": return 10
        case "Plan (XML)": return 11
        case "Pro Edit (XML)": return 12
        // MCP modes order
        case "MCP Pair Program": return 20
        case "MCP Agent": return 21
        default: return 100
        }
    }
    ...
    var body: some View {
        ...
        .sheet(item: $duplicatingPreset) { preset in
            CopyPresetEditView(
                promptViewModel: promptViewModel,
                preset: duplicatePresetData(preset),
	...
	private var headerSection: some View {
		VStack(alignment: .leading, spacing: 4) {
			...
			Text("Reusable prompt templates for the copy-to-paste workflow. Each preset bundles a system prompt, code-map mode, file-tree mode, and git-diff inclusion. These don't affect Agent Mode.")
				.font(.subheadline)
    ...
    private func duplicatePresetData(_ preset: CopyPreset) -> CopyPreset {
...
private struct CopyPresetSummaryChips: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            // System prompt (green)
            if let flavor = preset.systemPromptFlavor {
                ReadableChip(text: readableSystemPrompt(flavor), tint: .green, fontPreset: fontPreset)
            }
            
            // XML format (shown when present)
            if let xml = preset.xmlFormat {
                ReadableChip(text: readableXML(xml), tint: .blue, fontPreset: fontPreset)
            }
            // File tree (shown when not .auto and not .none)
    ...
    private func readableXML(_ xml: ApplyPromptFormat) -> String {
        switch xml {
        case .diff:      return "XML Diff"
        case .whole:     return "XML Whole"
        case .architect: return "XML Architect"
        }
    }
    
    private func readableTree(_ mode: FileTreeOption) -> String? {
        switch mode {
        case .files:    return "Tree Files"
        case .selected: return "Tree Selected"
        case .auto:     return nil // default, not shown
        case .none:     return nil // not shown
        }
    }
    
    private func readableMap(_ use: CodeMapUsage) -> String? {
        switch use {
        case .auto:     return "Map Auto"
        case .complete: return "Map Complete"
        case .selected: return "Map Selected"
        case .none:     return nil // not shown
        }
    }
    
    private func readableSystemPrompt(_ flavor: SystemPromptFlavor) -> String {
        switch flavor {
        case .mcpAgent: return "Sys: Agent"
        case .mcpPairProgram: return "Sys: Pair"
        case .mcpDiscover: return "Sys: Discover"
        case .mcpBuilder: return "Sys: Builder"
        case .architectPlan: return "Sys: Architect"
        case .codeEditDiff: return "Sys: Edit"
        case .codeEditWhole: return "Sys: Rewrite"
        case .review, .mcpPairPlan: return "System"
        }
    }
...
private struct CopyPresetEditView: View {
    ...
    @State private var systemPromptFlavor: SystemPromptFlavor? = nil
	@State private var includeMCPMetadata: Bool = false
    @FocusState private var iconFieldFocused: Bool
    
    private var titleText: String {
        switch editMode {
        case .createCustom:
            return "Create Copy Preset"
        case .editCustom:
            return "Edit Copy Preset"
        case .builtInOverrides:
            return "Customize Built-in Preset"
        }
    }
        ...
        VStack(spacing: 12) {
                    ...
                    section(title: "File Tree") {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text(fileTreeMode.caption)
                                .font(.caption)
                                .foregroundColor(.secondary)
                            Text("ASCII project structure helps the AI understand organization and file locations.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    }
            ...
            // System Prompt
                    section(title: "System Prompt") {
                        Picker("", selection: Binding<SystemPromptFlavor?>(
                            get: { systemPromptFlavor },
                            set: { systemPromptFlavor = $0 }
                        )) {
                            Text("None").tag(nil as SystemPromptFlavor?)
                            Text("MCP Agent").tag(SystemPromptFlavor.mcpAgent as SystemPromptFlavor?)
                            Text("MCP Pair Program").tag(SystemPromptFlavor.mcpPairProgram as SystemPromptFlavor?)
                            Text("MCP Discover").tag(SystemPromptFlavor.mcpDiscover as SystemPromptFlavor?)
                        }
                        .pickerStyle(.segmented)
                        .labelsHidden()
                        .disabled(isReadOnly)
                        Text("Choose a built-in system prompt. These are immutable and maintained by RepoPrompt.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
					...
					section(title: "MCP Metadata") {
						VStack(alignment: .leading, spacing: 4) {
							Toggle("Include MCP metadata", isOn: $includeMCPMetadata)
								.disabled(isReadOnly)
							Text("Include window and tab binding information for MCP agents. Enabled by default for MCP system prompts.")
								.font(.caption)
								.foregroundColor(.secondary)
								.padding(.leading, 20)
						}
    ...
    private func preload() {
        ...
        if let p = preset {
            ...
            systemPromptFlavor = effective.systemPromptFlavor
			includeMCPMetadata = effective.includeMCPMetadata ?? false
        } else {
    ...
    private func save() {
        ...
        case .editCustom, .createCustom:
            let updated = CopyPreset(
				...
				notes: preset?.notes,
				includeMCPMetadata: includeMCPMetadata
            )
            onSave(updated)
        }
    }
