		...
		ScrollView {
			VStack(alignment: .leading, spacing: 24) {
				...
				VStack(alignment: .leading, spacing: 6) {
					...
					Text("API keys for direct model access — primarily used by the Oracle Model, built-in chat, and legacy integrations. To add CLI-backed agents (Claude Code, Codex, Gemini, OpenCode, Cursor), use CLI Providers under Agent Mode.")
	...
	private func apiKeySection(
		...
		VStack(alignment: .leading, spacing: 8) {
			...
			HStack(spacing: 10) {
				...
				.disabled((isURL && key.wrappedValue == "http://localhost:11434") ||
			...
			if let serviceTier = serviceTier, title == "OpenAI API Key" {
				VStack(alignment: .leading, spacing: 8) {
			...
			else if let customModel = customModel {
				VStack(alignment: .leading, spacing: 4) {
					HStack {
						Text("Custom Model")
							.font(.subheadline)
	...
	private var azureSettingsView: some View {
		VStack(alignment: .leading, spacing: 8) {
			...
			HStack(spacing: 10) {
				...
				.disabled(viewModel.azureBaseURL.isEmpty
	...
	private var ollamaURLView: some View {
		VStack(alignment: .leading, spacing: 8) {
			...
			VStack(alignment: .leading, spacing: 4) {
				...
				if viewModel.availableLocalModels.isEmpty {
					...
					HStack {
						...
						.pickerStyle(MenuPickerStyle())
						.labelsHidden()
						.onChange(of: viewModel.ollamaModel) { _, newValue in
							Task {
			...
			HStack(spacing: 10) {
				...
				.disabled(viewModel.ollamaURL.isEmpty ||
							isLoadingOllama)
				.buttonStyle(CustomButtonStyle())
