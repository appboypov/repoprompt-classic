...
struct ModelPresetsSettingsView: View {
    ...
    // "Presets hidden" flag is owned by GlobalSettingsStore; the store still
    // mirrors the legacy "mcpTemporarilyDisablePresets" UserDefaults key so
    // non-settings surfaces keep observing the same value.
