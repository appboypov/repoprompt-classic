...
struct CLIProvidersSettingsView: View {
	...
	init(
		...
		self._providerPermissionsVM = StateObject(wrappedValue: providerPermissionsViewModel())
	}
	...
	@State private var showGeminiTraceDump = false
	...
	@State private var isGeminiExpanded: Bool = false
	...
	private var shouldShowStatusBanner: Bool {
		return viewModel.isClaudeCodeConnected
			|| viewModel.isCodexConnected
			|| viewModel.isGeminiConnected
	...
	private var codexStatusText: String? {
		...
		case .resolvingExecutable:
			return "Looking for Codex CLI in your login-shell PATH…"
		case .refreshingAuth:
			return "Checking Codex authentication…"
		case .testingAppServer:
			return "Testing Codex app-server…"
		case .loggingIn:
			return "Waiting for ChatGPT login to complete…"
		case .authRequired:
			return "Codex authentication needs attention. Use Login with ChatGPT, then retry."
	...
	var body: some View {
		ScrollView {
			VStack(alignment: .leading, spacing: 16) {
				...
				VStack(alignment: .leading, spacing: 6) {
					...
					Text("Primary way to add Agent Mode model support. Connect Claude Code, Codex, Gemini, OpenCode, or Cursor to leverage your existing subscriptions — OpenCode can also proxy any API key.")
						.font(.subheadline)
			...
			Task {
				...
				await MainActor.run {
					if !hasSetInitialExpansion {
						...
						isGeminiExpanded = !viewModel.isGeminiConnected
		...
		.alert(isPresented: $showAlert) {
			...
			} else if showGeminiTraceDump && viewModel.hasGeminiTrace() {
				return Alert(
					title: Text("CLI Provider Management"),
					message: Text(alertMessage),
					primaryButton: .default(Text("Save Trace to Downloads"), action: dumpGeminiTrace),
					secondaryButton: .cancel(Text("OK"), action: { showGeminiTraceDump = false })
				)
	...
	private var claudeCodeCLIUnavailableDetail: String {
		if case .binaryMissing(let message) = viewModel.claudeCodeCLIStatus {
	...
	private func permissionSummaryText(for providerID: AgentProviderBindingID) -> String {
		...
		case .gemini:
			let level = GeminiAgentToolPreferences.permissionLevel(defaults: defaults, secureStore: secureStore)
			return "Session mode: \(level.displayName)"
		case .openCode:
				...
				if viewModel.isClaudeCodeConnected {
					...
					HStack(spacing: 10) {
						...
						if let error = viewModel.claudeCodeError, !error.isEmpty {
							...
							VStack(alignment: .leading, spacing: 3) {
								Text(viewModel.claudeCodeCLIStatus.isKnownMissing
	...
	private var claudeCompatibleBackendsSection: some View {
		...
		let activeCount = backends.filter { viewModel.compatibleBackendIsActive($0) }.count
	...
	private func claudeCompatibleBackendTestSection(
		...
		let canAttempt = viewModel.compatibleBackendCanAttemptTest(backendID)
		VStack(alignment: .leading, spacing: 8) {
			HStack(spacing: 8) {
			...
			set: { newValue in
				...
				case .claudeSlotMapping:
					if case .claudeSlotMapping = updated.modelBehavior {
						...
						if case .claudeSlotMapping(let preset) = ClaudeCodeCompatibleBackendID.glmZAI.defaultPreset.modelBehavior {
	...
	private func claudeCompatibleSlotMappingEditor(
		...
		let mapping: ClaudeCodeCompatibleBackendConfig.ClaudeSlotMapping = {
			if case .claudeSlotMapping(let existing) = config.modelBehavior {
				return existing
			}
			if case .claudeSlotMapping(let preset) = ClaudeCodeCompatibleBackendID.glmZAI.defaultPreset.modelBehavior {
				return preset
		...
		func updateSlot(_ slot: ClaudeSlot, value: String) {
			...
			var editableMapping: ClaudeCodeCompatibleBackendConfig.ClaudeSlotMapping = {
				if case .claudeSlotMapping(let current) = updated.modelBehavior { return current }
				return mapping
			}()
			switch slot {
		...
		providerCard(
			...
			VStack(alignment: .leading, spacing: 12) {
				...
				if viewModel.isCodexConnected {
					...
					if case .connected(let resolvedExecutable) = viewModel.codexConnectionPhase,
	...
	private var geminiCard: some View {
		providerCard(
			title: "Gemini CLI",
			subtitle: "Uses your existing Gemini CLI installation with your Google account authentication.",
			infoURL: "https://geminicli.com/",
			isConnected: viewModel.isGeminiConnected,
			isExpanded: $isGeminiExpanded
		) {
			VStack(alignment: .leading, spacing: 12) {
				if viewModel.isGeminiConnected {
					HStack(spacing: 8) {
						...
						Button(action: { signOutFromGemini() }) {
							Text("Sign Out")
								.foregroundColor(.secondary)
						}
						.buttonStyle(CustomButtonStyle())
					...
					permissionSummaryLinkRow(for: .gemini)
				} else {
					HStack(spacing: 10) {
						Button(action: { testGeminiConnection() }) {
							if isLoadingGemini {
						...
						if let error = viewModel.geminiError, !error.isEmpty {
							Text(error)
								.font(.caption)
								.foregroundColor(.red)
								.fixedSize(horizontal: false, vertical: true)
						} else {
							Text("Run `gemini auth login` in your terminal to authenticate.")
								.font(.caption)
								.foregroundColor(.secondary)
						}
					}
	...
	private func backendSavedAlertMessage(for backendID: ClaudeCodeCompatibleBackendID) -> String {
		switch backendID {
		case .kimi: return "Kimi API key saved. CC Moonshot is now available in Agent Mode."
		case .custom: return "Custom backend API key saved."
		case .glmZAI: return ""
		}
	}
	...
	private func backendDeletedAlertMessage(for backendID: ClaudeCodeCompatibleBackendID) -> String {
		switch backendID {
		case .kimi: return "Kimi API key deleted."
		case .custom: return "Custom backend API key deleted."
		case .glmZAI: return ""
		}
	}
	...
	private func testGeminiConnection() {
		...
		Task {
			do {
				let ok = try await viewModel.testGeminiConnection()
				await MainActor.run {
					isLoadingGemini = false
					if ok {
						alertMessage = "Gemini CLI connected."
						showGeminiTraceDump = false
					}
					showAlert = true
					onAPIKeyUpdated?()
				...
				await MainActor.run {
					...
					showGeminiTraceDump = viewModel.hasGeminiTrace()
					showAlert = true
				}
			}
		}
	...
	private func dumpGeminiTrace() {
		do {
			let url = try viewModel.dumpGeminiTrace()
			alertMessage = "Trace saved to Downloads/\(url.lastPathComponent)."
		} catch let error as CLIProcessLogCollectorError {
			switch error {
			case .noEntries:
	...
	private func signOutFromGemini() {
		viewModel.isGeminiConnected = false
		viewModel.geminiError = nil
		UserDefaults.standard.set(false, forKey: "GeminiCLIConnected")
		NotificationCenter.default.post(name: .geminiConnectionChanged, object: nil, userInfo: ["windowID": windowID])
		Task {
			await viewModel.updateAvailableModels()
		}
		alertMessage = "Signed out from Gemini CLI"
		showAlert = true
		onAPIKeyUpdated?()
	}
