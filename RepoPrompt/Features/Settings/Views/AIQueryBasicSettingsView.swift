...
struct AIQueryBasicSettingsView: View {
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 10) {
			...
			Text("Chat Edit Mode Prompt")
				.font(.body)
				.foregroundColor(promptViewModel.proFileEdits ? .secondary : .primary)
			...
			.labelsHidden()
			.disabled(promptViewModel.proFileEdits || !promptViewModel.preferredAIModel.isModelCapableOfDiff)
			//.opacity(promptViewModel.proFileEdits ? 0.7 : 1.0)
	...
	private var explanationText: String {
		if promptViewModel.proFileEdits {
			return "These settings are not applicable when Pro Edit mode is active. Pro Edit uses its own configuration below."
		} else if !promptViewModel.preferredAIModel.isModelCapableOfDiff && promptViewModel.fileEditFormat == .diff {
