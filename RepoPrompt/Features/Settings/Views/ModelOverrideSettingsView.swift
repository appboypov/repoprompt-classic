...
struct ModelOverridesSettingsView: View {
	...
	private static func computeSections(from models: [AIModel]) -> [(provider: AIProviderType, models: [AIModel])] {
	...
	private func scheduleRecomputeSections(updateExpanded: Bool, immediate: Bool = false) {
		...
		recomputeTask = Task.detached(priority: .utility) { [models] in
			...
			await MainActor.run {
				...
				let withOverrides = models.filter { hasAnyOverride($0) }.map { $0.rawValue }
