...
struct MCPSettingsView: View {
	...
	// Scalar MCP preferences now flow through GlobalSettingsStore so the JSON
	// document stays authoritative; the store still mirrors to legacy
	// UserDefaults keys ("mcpAutoStart", SettingKeys.mcpShowModelPresets,
	// "mcpTemporarilyDisablePresets") during the rollback window, so older
	// builds continue to observe the same values.
	...
	private var showModelPresets: Bool { globalSettings.mcpShowModelPresets() }
	private var mcpTemporarilyDisablePresets: Bool { globalSettings.mcpTemporarilyDisablePresets() }
				...
				Menu {
					Section("MCP Server") {
						...
						Button("Gemini CLI") {
							...
							showFeedback(result.success ? (result.wasAlreadyPresent ? "Gemini already configured" : "Gemini configured") : "Gemini config failed", isError: !result.success)
						}
						Button("OpenCode") {
	...
	private var cliSection: some View {
		VStack(alignment: .leading, spacing: 12) {
			...
			#if DEBUG
			Text("Debug builds install as rp-cli-debug and claude-rp-debug.")
				.font(fontPreset.captionFont)
				.foregroundColor(.orange)
			#endif
		}
	...
	private func cliStatusText(for status: CLIPathInstaller.InstallationStatus) -> String {
		...
		case .installedButStale: return "Needs update"
		case .notInstalled: return "Not installed"
		case .installedByOther: return "Exists but not managed"
		case .directoryMissing: return "Directory missing"
		}
	}
	...
	private func claudeRPStatusText(for status: CLIPathInstaller.ClaudeRPInstallationStatus) -> String {
		switch status {
		case .installed: return "Installed"
		case .installedButOutdated: return "Needs update"
		case .notInstalled: return "Not installed"
		case .installedByOther: return "Exists but not managed"
		case .directoryMissing: return "Directory missing"
		}
