...
/// browsing. IDE-era URL scheme docs were removed — the docs site is the
/// canonical reference.
///
/// SEARCH-HELPER: Advanced Settings, File System, AI Behavior, Code Maps,
/// gitignore, symlinks, saved prompts, diff models, datetime instructions
///
/// Related:
...
struct AdvancedSettingsView: View {
	// MARK: - AI Behavior
	//
	// `allowDiffModelsToRewrite` is routed through PromptViewModel so the same
	// packaging-invalidation side effect fires whether the toggle is flipped
	// here or inside the view model. `enableKeyboardShortcuts` persists to the
	// JSON-backed global store directly; the legacy UserDefaults mirror remains
	// live for consumers that still observe the raw key.
		...
		VStack(alignment: .leading, spacing: 16) {
			...
			SettingSection(
				...
				SettingToggle(
					...
					description: "Display folders in the file tree even if they contain no files.",
	...
	private var aiBehaviorSection: some View {
		VStack(alignment: .leading, spacing: 16) {
			...
			SettingSection(
				...
				description: "Controls how diff-capable models may rewrite files."
			) {
				SettingToggle(
					title: "Allow diff models to rewrite entire files",
					description: "When disabled, diff edit models can only make partial changes and cannot rewrite entire files.",
					isOn: $promptViewModel.allowDiffModelsToRewrite
				)
			}
		}
	}
