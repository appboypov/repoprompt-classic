...
/// in the JSON-backed global settings store. Legacy UserDefaults mirrors are
/// maintained by GlobalSettingsStore for rollback compatibility.
class ModelOverridesSettings: ObservableObject {
	...
	func streamOverride(for modelRaw: String) -> Bool? {
		return streamOverrides[modelRaw]
	}
	...
	func temperatureOverride(for modelRaw: String) -> Double? {
		return temperatureOverrides[modelRaw]
	}
	...
	func responsesOverride(for modelRaw: String) -> Bool? {
		return responsesOverrides[modelRaw]
	}
