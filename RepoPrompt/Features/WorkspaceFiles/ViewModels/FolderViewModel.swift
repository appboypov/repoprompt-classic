...
class FolderViewModel: ObservableObject, Identifiable, FileSystemItemViewModel, Equatable, Hashable {
	...
	init(
		...
		self.nameSortKey = folder.name.lowercased()
		self.fileExtension = nil
		self.fullPath = folder.path
		...
		self.storageSortedBy = sortMethodOverride ?? sortMethod
	}
	...
	func addChildrenBatch(_ newChildren: [FileSystemItemType], options: AddChildrenBatchOptions) {
		...
		if options.ensureSorted {
			let effective = effectiveSortMethod(requested: currentSortMethod)
			let canMergeIntoSortedStorage = !sortDirty && storageSortedBy == effective && (!subfolders.isEmpty || !files.isEmpty)
			if canMergeIntoSortedStorage {
				mergeSortedBatch(subfolders: newSubfolders, files: newFiles, by: effective)
				rebuildChildrenFromStorage()
			} else {
				appendUnsortedBatch(subfolders: newSubfolders, files: newFiles)
				applySort(requested: currentSortMethod)
			}
		} else {
			appendUnsortedBatch(subfolders: newSubfolders, files: newFiles)
	...
	@MainActor
	private func appendUnsortedBatch(subfolders newSubfolders: [FolderViewModel], files newFiles: [FileViewModel]) {
	...
	@MainActor
	private func mergeSortedBatch(subfolders newSubfolders: [FolderViewModel], files newFiles: [FileViewModel], by method: SortMethod) {
		if !newSubfolders.isEmpty {
			subfolders = mergedSorted(existing: subfolders, additions: newSubfolders, by: method)
		}
		if !newFiles.isEmpty {
			files = mergedSorted(existing: files, additions: newFiles, by: method)
		}
	}

	private func mergedSorted<T: FileSystemItemViewModel>(existing: [T], additions: [T], by method: SortMethod) -> [T] {
		let sortedAdditions = additions.sorted { compare($0, $1, by: method) }
		guard !existing.isEmpty else { return sortedAdditions }

		var merged: [T] = []
		merged.reserveCapacity(existing.count + sortedAdditions.count)
		var existingIndex = existing.startIndex
		var additionsIndex = sortedAdditions.startIndex
		while existingIndex < existing.endIndex && additionsIndex < sortedAdditions.endIndex {
			let existingItem = existing[existingIndex]
			let addition = sortedAdditions[additionsIndex]
			if compare(addition, existingItem, by: method) {
				merged.append(addition)
				additionsIndex += 1
			} else {
				merged.append(existingItem)
				existingIndex += 1
			}
		}
		...
		if additionsIndex < sortedAdditions.endIndex {
			merged.append(contentsOf: sortedAdditions[additionsIndex...])
		}
		return merged
	}
	...
	@MainActor
	public func forceCheckRecursive() {
