...
struct WorkspaceFileLoadResult {
	...
	let cacheHit: Bool
	let workingStateMigrated: Bool
	let discoverContextMigrated: Bool
	var migrationRequiresSave: Bool {
		workingStateMigrated || discoverContextMigrated
	}
	let migrationSaveTask: Task<Void, Never>?
}

private struct WorkspaceFileDecodeCacheKey: Hashable, Sendable {
...
final class WorkspaceFileDecodeCache: @unchecked Sendable {
	...
	private var scheduledMigrationSaveKeys: Set<WorkspaceFileDecodeCacheKey> = []
	...
	fileprivate func loadWorkspace(at fileURL: URL) throws -> WorkspaceFileCachedLoadResult {
		...
		if let cached = cachedWorkspacesByKey[keyBeforeRead] {
			...
			return WorkspaceFileCachedLoadResult(
				workspace: cached,
				cacheHit: true,
				workingStateMigrated: cached.migrationRequiresSave,
				discoverContextMigrated: cached.migrationRequiresSave,
		...
		let decodedWorkingStateMigrated = workspace.migrationRequiresSave
		let workingMigrated = workspace.migrateWorkingStateToTabsIfNeeded()
		let discoverMigrated = workspace.migrateDiscoverAndCBStateToTabsIfNeeded()
		let migrationRequiresSave = decodedWorkingStateMigrated || workingMigrated || discoverMigrated || workspace.migrationRequiresSave
		workspace.migrationRequiresSave = migrationRequiresSave
		...
		return WorkspaceFileCachedLoadResult(
			workspace: workspace,
			cacheHit: false,
			workingStateMigrated: decodedWorkingStateMigrated || workingMigrated,
			discoverContextMigrated: discoverMigrated,
			cacheKey: keyBeforeRead
		)
	}
	...
	fileprivate func claimMigrationSave(for key: WorkspaceFileDecodeCacheKey) -> Bool {
		lock.lock()
		defer { lock.unlock() }
		guard !scheduledMigrationSaveKeys.contains(key) else { return false }
		scheduledMigrationSaveKeys.insert(key)
		return true
	}

	fileprivate func isMigrationSaveClaimed(for key: WorkspaceFileDecodeCacheKey) -> Bool {
		lock.lock()
		defer { lock.unlock() }
		return scheduledMigrationSaveKeys.contains(key)
	}

	fileprivate func finishMigrationSave(for key: WorkspaceFileDecodeCacheKey) {
		lock.lock()
		defer { lock.unlock() }
		scheduledMigrationSaveKeys.remove(key)
	...
	func invalidate(url: URL) {
		...
		scheduledMigrationSaveKeys = scheduledMigrationSaveKeys.filter { $0.standardizedPath != standardizedPath }
	}
	...
	func removeAllForTesting() {
		...
		scheduledMigrationSaveKeys.removeAll()
	}
}
...
enum WorkspaceOpenError: LocalizedError {
	...
	var errorDescription: String? {
		switch self {
		case .noActiveWorkspace:
			return "No active workspace. Open or create a workspace first."
		}
	}
}
...
class WorkspaceManagerViewModel: ObservableObject {
	...
	nonisolated private static func normalizedRepoPathsForComparison(_ paths: [String]) -> [String] {
	...
	func getLastSearchQuery() -> String? {
		return activeWorkspace?.lastSearchQuery
	}
	...
	func awaitInitialized() async {
		...
		await withCheckedContinuation { continuation in
			onceInitialized { [weak self] in
				...
				if !self.isSwitchingWorkspace {
					continuation.resume()
					return
				}
				self.onceSwitchingComplete {
					continuation.resume()
				}
			}
		}
	...
	private var legacyAggregatorFileURL: URL {
	...
	nonisolated private func ensureBaseRootExists(at baseRoot: URL) throws {
	...
	init(fileManager: RepoFileManagerViewModel, promptViewModel: PromptViewModel) {
		...
		do {
			...
			print("Error migrating from legacy aggregator: \(error)")
		}
		...
		fileManager.$selectedFiles
			...
			.sink { [weak self] newSelection in
				...
				self.checkIfActivePresetIsDirty(with: newSelection)
				self.bumpStateVersion(for: self.activeWorkspaceID)
				// No disk save here - publish in-memory snapshot for mirroring
				self.publishActiveComposeTabSnapshot(commitToMemory: true)
			}
		...
		var workingStateMigrationCount = 0
		var discoverContextMigrationCount = 0
		var migrationSaveBackCount = 0
		#endif
		...
		for entry in indexEntries {
			do {
				...
				if FileManager.default.fileExists(atPath: wURL.path) {
					...
					if loadResult.workingStateMigrated { workingStateMigrationCount += 1 }
					if loadResult.discoverContextMigrated { discoverContextMigrationCount += 1 }
					if loadResult.migrationRequiresSave { migrationSaveBackCount += 1 }
					#endif
					loaded.append(ws)
				} else {
					#if DEBUG
		...
		if let initStartMS {
			...
			WorkspaceRestorePerfLog.log(
				"workspaceManager.init managerID=\(instanceID.uuidString.prefix(8)) indexEntries=\(indexEntries.count) decoded=\(decodedWorkspaceCount) decodeCacheHits=\(workspaceDecodeCacheHitCount) missingFiles=\(missingWorkspaceFileCount) decodeFailures=\(failedWorkspaceDecodeCount) workingMigrations=\(workingStateMigrationCount) discoverCBMigrations=\(discoverContextMigrationCount) migrationSaveBacks=\(migrationSaveBackCount) indexLoad=\(indexDuration) decodeAndMigration=\(decodeDuration) totalBeforeDefaultSwitch=\(WorkspaceRestorePerfLog.formatElapsedMS(since: initStartMS))"
			)
		}
		#endif
		...
		fileManager.onRootFolderUnloadRequested
			.sink { [weak self] folderPath in
				Task {
					await self?.handleRootFolderUnloadRequest(folderPath)
				}
			}
			.store(in: &cancellables)
		
		fileManager.onRootFoldersReordered
			.sink { [weak self] newOrder in
				Task {
					await self?.handleRootFoldersReordered(newOrder)
				}
			}
			.store(in: &cancellables)
		...
		NotificationCenter.default.publisher(for: .workspaceListDidChange)
			.sink { [weak self] notification in
				guard
					...
					senderID != self.instanceID
				else { return }
				self.reloadWorkspacesFromDisk()
			}
			.store(in: &cancellables)
		...
		NotificationCenter.default.publisher(for: .workspacePresetsDidChange)
			.sink { [weak self] notification in
				...
				self.reloadPresetsFromDisk()
			}
			.store(in: &cancellables)
		
		self.fileManager.onRequestRefresh = { [weak self] in
			Task {
				guard let self = self, let activeWS = self.activeWorkspace else { return }
	...
	private func purgeStaleCodeMapCachesForKnownRoots(coalesceAcrossInitialManagers: Bool = false) {
		let roots = workspaces.flatMap { $0.repoPaths }
		#if DEBUG
	...
	private static func performStaleCodeMapCachePurge(fileManager: RepoFileManagerViewModel, roots: [String]) async {
		#if DEBUG
	...
	private func startPollTimer() {
		...
		pollTimer = Timer.scheduledTimer(withTimeInterval: pollInterval, repeats: true) { [weak self] _ in
			...
			Task { @MainActor [weak self] in
				...
				await self.pollAndSaveStateAsync()
			}
		}
	}
	...
	private func migrateLegacyAggregatorIfNeeded() throws {
		let legacyURL = legacyAggregatorFileURL
		guard FileManager.default.fileExists(atPath: legacyURL.path) else {
			return
		}
		
		print("⚠️ Detected legacy aggregator at \(legacyURL.path). Attempting migration.")
		let data = try Data(contentsOf: legacyURL)
		let oldWorkspaces = try JSONDecoder().decode([WorkspaceModel].self, from: data)
		
		for ws in oldWorkspaces {
			_ = try ensureWorkspaceDirectoryExists(for: ws)
			_ = try saveWorkspaceToFile(ws)
		}
		
		let indexEntries: [WorkspaceIndexEntry] = oldWorkspaces.map {
			WorkspaceIndexEntry(
				id: $0.id,
				name: $0.name,
		...
		try saveWorkspaceIndex(indexEntries)
		if let legacyCustom = indexEntries.first(where: { $0.customStoragePath != nil })?.customStoragePath {
			self.globalCustomStorageURL = legacyCustom
		}
		
		let legacyBackup = legacyURL.deletingLastPathComponent().appendingPathComponent("workspaces_legacy.json")
		try FileManager.default.moveItem(at: legacyURL, to: legacyBackup)
		print("✅ Migration complete. Legacy aggregator moved to \(legacyBackup.path)")
	}
	...
	func reloadWorkspacesFromDisk() {
		...
		reloadWorkspacesTask = Task.detached(priority: .utility) { [weak self, indexURL, base, currentActiveID, token] in
			...
			for entry in indexEntries {
				...
				if FileManager.default.fileExists(atPath: wURL.path) {
					do {
						loadedMutable.append(try Self.loadWorkspaceFromFile(at: wURL))
					} catch {
						print("Error loading workspace from new scheme: \(error)")
					}
				} else {
			...
			await MainActor.run { [weak self, loaded, token, currentActiveID] in
				...
				guard self.reloadWorkspacesToken == token else { return }
				
				self.workspaces = loaded
				self.recordRepoPathBaselines(for: loaded)
				if let currentActiveID,
					loaded.contains(where: { $0.id == currentActiveID }) {
					self.activeWorkspaceID = currentActiveID
				}
				self.purgeStaleCodeMapCachesForKnownRoots()
	...
	func reloadPresetsFromDisk() {
		...
		reloadPresetsTask = Task.detached(priority: .utility) { [weak self, indexURL, base, token] in
			...
			await MainActor.run { [weak self, updates, token] in
				...
				guard self.reloadPresetsToken == token else { return }
				...
				let indexMap = Dictionary(
					self.workspaces.enumerated().map { ($1.id, $0) },
					uniquingKeysWith: { _, last in last }
				)
				
				for update in updates {
					guard let idx = indexMap[update.id],
						self.workspaces.indices.contains(idx) else { continue }
					self.workspaces[idx].presets = update.presets
					self.workspaces[idx].activePresetID = update.activePresetID
				}
				
				if self.activeWorkspace != nil {
					self.checkIfActivePresetIsDirty(with: self.fileManager.selectedFiles)
				}
				
				// Clear running task reference
				self.reloadPresetsTask = nil
			}
		}
	}
	...
	func createWorkspaceFromDraft() -> WorkspaceModel? {
		...
		let newWorkspace = createWorkspace(name: workspaceName,
	...
	func switchWorkspace(to newWorkspace: WorkspaceModel, saveState: Bool = true, reason: String = "internal") async {
		...
		let totalStart = Date()
		var initialRootLoadScanDeferralStarted = false
		var shouldFlushDeferredInitialRootLoadScans = false
		...
		#if DEBUG
		WorkspaceRestorePerfLog.log(
			"workspaceSwitch.begin managerID=\(instanceID.uuidString.prefix(8)) reason=\(reason) restored=\(reason == "restore") saveState=\(saveState) effectiveSaveState=\(effectiveSaveState) targetWorkspaceID=\(WorkspaceRestorePerfLog.shortID(newWorkspace.id)) targetName=\(newWorkspace.name) previousWorkspaceID=\(WorkspaceRestorePerfLog.shortID(previousWorkspaceID)) previousName=\(previousWorkspaceName) targetRoots=\(Self.loadableRepoPaths(for: newWorkspace).count) loadedRoots=\(fileManager.rootFolders.count)"
		)
		#endif
		...
		defer {
			...
			if let restorePerfStartMS {
					...
					WorkspaceRestorePerfLog.log(
						"workspaceSwitch.end managerID=\(instanceID.uuidString.prefix(8)) reason=\(reason) restored=\(reason == "restore") saveState=\(saveState) effectiveSaveState=\(effectiveSaveState) targetWorkspaceID=\(WorkspaceRestorePerfLog.shortID(newWorkspace.id)) activeWorkspaceID=\(WorkspaceRestorePerfLog.shortID(activeWorkspaceID)) loadedRoots=\(counts.rootCount) loadedFolders=\(counts.folderCount) loadedFiles=\(counts.fileCount) shouldReturnToSystem=\(shouldReturnToSystem) total=\(WorkspaceRestorePerfLog.formatElapsedMS(since: restorePerfStartMS))"
					)
				}
			#endif
			...
			if initialRootLoadScanDeferralStarted {
				if shouldFlushDeferredInitialRootLoadScans {
					fileManager.flushDeferredInitialRootLoadScans()
				} else {
					fileManager.discardDeferredInitialRootLoadScans()
				}
			}
		...
		#if DEBUG
		WorkspaceRestorePerfLog.log(
			"workspaceSwitch.diskLoad managerID=\(instanceID.uuidString.prefix(8)) reason=\(reason) restored=\(reason == "restore") workspaceID=\(WorkspaceRestorePerfLog.shortID(newWorkspace.id)) duration=\(WorkspaceRestorePerfLog.formatMS(diskLoadDuration * 1_000))"
		)
		#endif
		...
		// Load all folders in the newly-active workspace
		let folderLoadStart = Date()
		logWorkspaceSwitch("folder load BEGIN workspace=\"\(activeWS.name)\" roots=\(activeWS.repoPaths.count)")
		fileManager.beginDeferringInitialRootLoadScans()
		initialRootLoadScanDeferralStarted = true
		...
		logWorkspaceSwitch("folder load END workspace=\"\(activeWS.name)\" duration=\(String(format: "%.3f", folderLoadDuration))s")
		#if DEBUG
		let folderCounts = fileManager.restorePerfLoadedTreeCounts()
		WorkspaceRestorePerfLog.log(
			"workspaceSwitch.folderLoad managerID=\(instanceID.uuidString.prefix(8)) reason=\(reason) restored=\(reason == "restore") workspaceID=\(WorkspaceRestorePerfLog.shortID(activeWS.id)) rootCount=\(activeWS.repoPaths.count) loadedRoots=\(folderCounts.rootCount) loadedFolders=\(folderCounts.folderCount) loadedFiles=\(folderCounts.fileCount) duration=\(WorkspaceRestorePerfLog.formatMS(folderLoadDuration * 1_000))"
		)
		#endif
		if shouldReturnToSystemAfterSwitchCancellation { return }
		
		// Keep the overlay up through the initial restore so the first visible frame
		// doesn't race tab restoration and post-switch listener fan-out.
		await Task.yield()
		...
		#if DEBUG
		WorkspaceRestorePerfLog.log(
			"workspaceSwitch.restoreState managerID=\(instanceID.uuidString.prefix(8)) reason=\(reason) restored=\(reason == "restore") workspaceID=\(WorkspaceRestorePerfLog.shortID(activeWS.id)) duration=\(WorkspaceRestorePerfLog.formatMS(restoreDuration * 1_000))"
		)
		#endif
		if shouldReturnToSystemAfterSwitchCancellation { return }
		...
		// Notify that workspace switched
		#if DEBUG
		let listenerStartMS = WorkspaceRestorePerfLog.timestampMSIfEnabled()
		#endif
		...
		if let listenerStartMS {
			WorkspaceRestorePerfLog.log(
				"workspaceSwitch.listeners managerID=\(instanceID.uuidString.prefix(8)) reason=\(reason) restored=\(reason == "restore") workspaceID=\(WorkspaceRestorePerfLog.shortID(activeWS.id)) listenerCount=\(workspaceDidSwitchListeners.count) duration=\(WorkspaceRestorePerfLog.formatElapsedMS(since: listenerStartMS))"
			)
		}
		#endif
		...
		completeInitialization()
		shouldFlushDeferredInitialRootLoadScans = true
	...
	private func schedulePostSwitchGitDataLoad(for workspace: WorkspaceModel, reason: String) {
		...
		postSwitchGitDataLoadTask = Task { @MainActor [weak self] in
			...
			await self.ensureGitDataRootLoadedForActiveWorkspace(
				reason: reason,
		...
		defer {
			...
			WorkspaceRestorePerfLog.event(
				...
				fields: [
					...
					"reason": reason,
					"outcome": outcome,
					"duration": startMS.map { WorkspaceRestorePerfLog.formatElapsedMS(since: $0) } ?? WorkspaceRestorePerfLog.formatMS(duration * 1_000)
				]
			)
			#endif
		}
	...
	func collectComposeTabSnapshot(name: String, base: ComposeTabState? = nil) -> ComposeTabState {
		...
		if isActiveTab {
			// Active tab: capture live state from view models
	...
	func applyComposeTabStateAsync(tab: ComposeTabState, windowID: Int? = nil) async {
		...
		let applyTask = Task { @MainActor [weak self] in
			...
			await self.applyComposeTabStateInternal(tabID: tabID, markWorkspaceDirtyAfterApply: true)
		}
	...
	private func applyComposeTabFastUIState(_ tab: ComposeTabState) async {
		...
		if let explicit = tab.activeSubView {
			...
			promptViewModel.setFilesTabSelection(.followLicenseDefault, source: .workspaceApply)
		}
	...
	func resolveComposeTabRoutingSnapshot(for tabID: UUID) -> (workspaceID: UUID, snapshot: ComposeTabState, usesLiveUIState: Bool)? {
		Self.resolveComposeTabRoutingSnapshot(
			...
			return self.collectComposeTabSnapshot(name: base.name, base: base)
		}
	}
	...
	struct ComposeTabBindingCandidate: Equatable, Sendable {
		let tabID: UUID
	...
	nonisolated private static func sortedWorkspaceMatches(_ workspaces: [WorkspaceModel]) -> [WorkspaceModel] {
	...
	nonisolated private static func bindingCandidates(
		matchingWorkingDirs dirs: [String],
		workspaces: [WorkspaceModel],
		activeWorkspaceID: UUID?,
		includeHidden: Bool = false
		...
		guard !normalizedDirs.isEmpty,
			...
			includeHidden || !workspace.isHiddenInMenus,
			workspaceMatchesWorkingDirs(workspace, normalizedDirs: normalizedDirs) else {
			return []
		}
	...
	nonisolated private static func hasAnyWorkspaceMatch(
		matchingWorkingDirs dirs: [String],
		workspaces: [WorkspaceModel]
	) -> Bool {
		let normalizedDirs = normalizedBindingDirs(dirs)
	...
	nonisolated private static func normalizedBindingDirs(_ dirs: [String]) -> [String] {
		dirs.map(Self.normalizeBindingPath).filter { !$0.isEmpty }
	}

	nonisolated private static func workspaceMatchesWorkingDirs(
	...
	nonisolated private static func bindingPath(_ child: String, isWithinOrEqualTo parent: String) -> Bool {
	...
	struct ActiveAgentSessionReferenceCleanupResult: Sendable, Equatable {
	...
	struct ActiveAgentSessionBinding: Sendable, Equatable {
		let tabID: UUID
		let workspaceID: UUID
		let isStashed: Bool
		let sessionID: UUID?
	...
	@MainActor
	func activeAgentSessionBinding(forTabID tabID: UUID, inWorkspaceID workspaceID: UUID? = nil) -> ActiveAgentSessionBinding? {
		for workspace in workspaces where workspaceID == nil || workspace.id == workspaceID {
			if let tab = workspace.composeTabs.first(where: { $0.id == tabID }) {
				return ActiveAgentSessionBinding(
					tabID: tabID,
					workspaceID: workspace.id,
					isStashed: false,
					sessionID: tab.activeAgentSessionID
				)
			}
	...
	func activeAgentSessionID(forTabID tabID: UUID, inWorkspaceID workspaceID: UUID? = nil) -> UUID? {
		activeAgentSessionBinding(forTabID: tabID, inWorkspaceID: workspaceID)?.sessionID
	}
		...
		if !changedWorkspaceIDs.isEmpty {
			...
			Task { @MainActor [weak self] in
				...
				for workspace in workspacesToSave {
					_ = try? await self.saveWorkspaceToFileAsync(workspace)
				}
			}
		}
	...
	nonisolated static func isPresetSelectionDirty(
		...
		let selAbs = Set(selectionPaths.map { $0.absolute })
		let selRel = Set(selectionPaths.map { $0.relative })
	...
	func rebaseSlicesForFileAcrossTabs(
		...
		for var tab in tabs {
			...
			let nextRanges = SliceRangeMath.normalize(await asyncTransform(existingRanges))
			var nextSlices = normalizedSlices
			if nextRanges.isEmpty {
				nextSlices.removeValue(forKey: standardizedFullPath)
			} else {
	...
	private func updateWorkspaceState(
		at index: Int,
		with state: (expandedFolders: [String],
						selection: StoredSelection,
						promptText: String,
						promptIDs: [UUID])
	) -> Bool {
		workspaces[index].workingExpandedFolders = state.expandedFolders
		workspaces[index].workingStoredSelection = state.selection
		workspaces[index].workingFilePaths = state.selection.selectedPaths
	...
	@discardableResult
	private func captureActiveTabSnapshotForWorkspaceIndex(_ index: Int) -> ComposeTabState? {
		...
		if let activeTabID = workspaces[index].activeComposeTabID,
			...
			let snapshot = collectComposeTabSnapshot(name: name, base: workspaces[index].composeTabs[tabIndex])
	...
	public func pollAndSaveState() {
		...
		let snapshot = captureActiveTabSnapshotForWorkspaceIndex(index)
	...
	public func pollAndSaveStateAsync() async {
		guard let active = activeWorkspace,
				let index  = workspaces.firstIndex(where: { $0.id == active.id }) else { return }
		...
		if let active = activeWorkspace {
			for listener in beforeSaveListeners { listener(active) }
		}
		...
		await saveWorkspaceAsync() // see change below to avoid big reassign
	...
	func restoreWorkspaceState(_ workspace: WorkspaceModel) async {
		...
		let migrationFlags = WorkspaceRestorePerfLog.workspaceMigrationFlags(for: workspace)
		#endif
		...
		#if DEBUG
		let migrationStartMS = WorkspaceRestorePerfLog.timestampMSIfEnabled()
		let composeTabsBeforeMigration = upgraded.composeTabs.count
		#endif
		upgraded.migrateWorkingStateToTabsIfNeeded()
		upgraded.migrateDiscoverAndCBStateToTabsIfNeeded()
		if upgraded.activeComposeTabID == nil {
			upgraded.activeComposeTabID = upgraded.composeTabs.first?.id
		}
		workspaces[index] = upgraded
		#if DEBUG
		WorkspaceRestorePerfLog.event(
			"workspaceSwitch.restoreState.migration",
			fields: [
				"workspaceID": WorkspaceRestorePerfLog.shortID(wsID),
				"composeTabsBefore": "\(composeTabsBeforeMigration)",
				"composeTabsAfter": "\(upgraded.composeTabs.count)",
				"workingStateMigrated": "\(migrationFlags.workingState)",
				"discoverOrCBMigrated": "\(migrationFlags.discoverOrContextBuilder)",
				"activeComposeTabID": WorkspaceRestorePerfLog.shortID(upgraded.activeComposeTabID),
				"duration": migrationStartMS.map { WorkspaceRestorePerfLog.formatElapsedMS(since: $0) } ?? "notMeasured"
			]
		)
		#endif
		...
		guard let activeID = upgraded.activeComposeTabID,
				let activeTab = upgraded.composeTabs.first(where: { $0.id == activeID }) ?? upgraded.composeTabs.first else {
			#if DEBUG
			WorkspaceRestorePerfLog.event(
				"workspaceSwitch.restoreState.abort",
				fields: [
		...
		workspaces[idx2].workingStoredSelection = latestAppliedTab.selection
		workspaces[idx2].workingExpandedFolders = latestAppliedTab.expandedFolders
		workspaces[idx2].workingFilePaths = latestAppliedTab.selection.selectedPaths
		#if DEBUG
		WorkspaceRestorePerfLog.event(
		...
		WorkspaceRestorePerfLog.event(
			...
			fields: [
				"workspaceID": WorkspaceRestorePerfLog.shortID(wsID),
				"duration": tokenRecountStartMS.map { WorkspaceRestorePerfLog.formatElapsedMS(since: $0) } ?? "notMeasured"
			]
		)
		#endif
	}
	...
	private struct WorkspaceDuplicateWindowSnapshot: Sendable, Equatable {
		let windowID: Int
		let workspaceID: UUID
	...
	@MainActor
	func duplicateWorkspaceGroups(windowStates: WindowStatesManager = .shared) -> [WorkspaceDuplicateGroupSummary] {
	...
	@MainActor
	func consolidateDuplicateWorkspaces(windowStates: WindowStatesManager = .shared) async -> WorkspaceDuplicateCleanupResult {
		...
		for plan in postSwitchPlans {
			...
			do {
				...
				_ = try saveWorkspaceToFile(merged)
			} catch {
				for duplicate in commitDuplicates {
					skipped.append(
						WorkspaceDuplicateCleanupSkippedItem(
	...
	nonisolated private static func duplicateWorkspaceGroupPlans(
		workspaces: [WorkspaceModel],
	...
	nonisolated private static func cleanupCanonicalWorkspace(
	...
	nonisolated private static func cleanupWorkspaceSort(_ lhs: WorkspaceModel, _ rhs: WorkspaceModel) -> Bool {
	...
	nonisolated private static func closeImpactHasActiveWork(_ impact: WindowCloseImpactSnapshot) -> Bool {
		impact.activeItems.contains { $0.count > 0 } || impact.mcp.activeExecutionCount > 0
	}

	nonisolated private static func duplicateDeletionDecision(
	...
	nonisolated static func mergedCanonicalWorkspace(
		...
		if merged.activePresetID == nil {
			merged.activePresetID = duplicates.lazy.compactMap { $0.activePresetID }.first
		}
		...
		if merged.activeComposeTabID == nil {
			merged.activeComposeTabID = duplicates.lazy.compactMap { $0.activeComposeTabID }.first
		}
		...
		merged.normalizeComposeAndStashedTabs()

		merged.selectedMetaPromptIDs = Array(Set(allWorkspaces.flatMap(\.selectedMetaPromptIDs)))
			.sorted { $0.uuidString < $1.uuidString }

		if merged.copyPresetId == nil {
			merged.copyPresetId = duplicates.lazy.compactMap { $0.copyPresetId }.first
		}
		if merged.copyCustomizations == nil {
			merged.copyCustomizations = duplicates.lazy.compactMap { $0.copyCustomizations }.first
		}
		if merged.chatPresetId == nil {
			merged.chatPresetId = duplicates.lazy.compactMap { $0.chatPresetId }.first
		}

		return merged
	}

	nonisolated private static func writeDuplicateCleanupBackup(for plans: [WorkspaceDuplicateGroupPlan]) throws -> URL {
	...
	nonisolated private static func duplicateCleanupBackupDirectoryURL() throws -> URL {
	...
	nonisolated private static func deduplicatedSkippedItems(
	...
	func setWorkspaceHiddenFromSnapshot(_ workspace: WorkspaceModel, hidden: Bool) async throws -> WorkspaceModel {
		...
		let finalURL = try await saveWorkspaceToFileAsync(updated)
		await WorkspaceDiskWriter.shared.flush(url: finalURL)
	...
	private func loadWorkspaceFolders(
		...
		for (rootIndex, path) in pathsToLoad.enumerated() {
			...
			#endif
			logWorkspaceSwitch("loadFolder BEGIN root=\"\(stdURL.path)\"")
			
			do {
				try await fileManager.loadFolder(
					at: stdURL,
					for: workspace,
					freshStart: false,
					refreshRootFolderStateAfterLoad: false
				...
				logWorkspaceSwitch("loadFolder END root=\"\(stdURL.path)\" duration=\(String(format: "%.3f", perRootDuration))s")
				#if DEBUG
				WorkspaceRestorePerfLog.event(
					"workspaceSwitch.loadWorkspaceFolders.userRoot",
					fields: [
						"workspaceID": WorkspaceRestorePerfLog.shortID(workspace.id),
						"rootIndex": "\(rootIndex)",
						"rootCount": "\(pathsToLoad.count)",
					...
					let perRootDuration = Date().timeIntervalSince(perRootStart)
				Self.logger.error("loadFolder ERROR root=\"\(stdURL.path)\" duration=\(String(format: "%.3f", perRootDuration))s error=\(error)")
				#if DEBUG
				WorkspaceRestorePerfLog.event(
					"workspaceSwitch.loadWorkspaceFolders.userRoot",
					fields: [
	...
	actor WorkspaceDiskWriter {
		...
		private struct Pending {
			var newestData: Data           // last payload received
			var task: Task<Void, Never>?
		}
		private var pendingByURL: [URL: Pending] = [:]
		
		// NEW: Waiters per URL for flush support
		private var waitersByURL: [URL: [CheckedContinuation<Void, Never>]] = [:]
		
		// MARK: public API
		static let shared = WorkspaceDiskWriter()          // ← one per process
		
		/// Enqueue a write; if another write for the same URL is already queued or
		/// running we coalesce, keeping only the newest JSON payload.
		func enqueue(data: Data, url: URL) {
			if pendingByURL[url] != nil {
				pendingByURL[url]!.newestData = data      // overwrite – coalesce
				return
			}
			
			// first writer for this URL → start a detached writer chain
			pendingByURL[url] = Pending(newestData: data, task: nil)
			runNext(for: url)
		}
		
		/// Enqueue and wait until all writes for this URL have flushed to disk.
		func enqueueAndWait(data: Data, url: URL) async {
			enqueue(data: data, url: url)
			await flush(url: url)
		}
		
		/// Writes a migration-only payload only when doing so cannot overtake or
		/// replace an explicit workspace save and the source file still has the
		/// metadata that produced the migrated snapshot.
		func writeMigrationIfUnchanged(
			data: Data,
			url: URL,
			...
			do {
				...
				print("💾 Migration write skipped \(url.lastPathComponent): \(error)")
				return false
			}
		}

		/// Wait until all coalesced writes for this URL have finished.
		func flush(url: URL) async {
			// Nothing pending – already flushed
		...
		private func runNext(for url: URL) {
			...
			slot.newestData = Data()          // reset, keeps slot alive
			pendingByURL[url] = slot
			
			slot.task = Task.detached(priority: .utility) { [weak self] in
				do {
					try payload.write(to: url, options: .atomic)
				} catch {
					print("💾 Write failed \(url.lastPathComponent): \(error)")
				}
				await self?.writerFinished(for: url)
			}
		}
		
		private func writerFinished(for url: URL) {
			guard var slot = pendingByURL[url] else { return }
			if slot.newestData.isEmpty {
				// all caught up – drain waiters
				...
				slot.task = nil
				pendingByURL[url] = slot
				runNext(for: url)                         // process next coalesced write
			}
		}
	}
	...
	private func saveWorkspaceAsync() async {
		guard let active = activeWorkspace,
		...
		do {
			let (merged, data, indexFieldsChanged, preservedDiskRepoPaths, url) = try await Task.detached(priority: .utility) {
				...
				if let customStoragePath {
					workspaceDir = customStoragePath
				} else {
					workspaceDir = baseRoot.appendingPathComponent(workspaceDirName)
				}
				...
				let diskWorkspace: WorkspaceModel?
				if fm.fileExists(atPath: url.path) {
					diskWorkspace = try? Self.loadWorkspaceFromFile(at: url)
				} else {
					diskWorkspace = nil
				}
			...
			await WorkspaceDiskWriter.shared.enqueue(data: data, url: url)
	...
	func saveWorkspaceToFileAsync(
		...
		let finalURL = try await saveWorkspaceToFileAsync(workspaceToSave, baseRoot: currentBaseRoot)
		recordRepoPathBaseline(for: workspaceToSave)
		return finalURL
	}
	
	nonisolated func saveWorkspaceToFileAsync(_ workspace: WorkspaceModel, baseRoot: URL) async throws -> URL {
		// Encode JSON
	...
	/// Legacy synchronous version - kept for compatibility during migration
	func saveWorkspaceToFile(_ workspace: WorkspaceModel) throws -> URL {
		// Encode JSON and prepare file path
		let encoded  = try JSONEncoder().encode(workspace)
		let folder   = try ensureWorkspaceDirectoryExists(for: workspace)
		let finalURL = folder.appendingPathComponent("workspace.json")
		
		// Write synchronously for migration compatibility
		WorkspaceFileDecodeCache.shared.invalidate(url: finalURL)
	...
	nonisolated static func loadWorkspaceFromFileResult(at fileURL: URL) throws -> WorkspaceFileLoadResult {
		...
		let migrationSaveTask: Task<Void, Never>?
		if cachedResult.migrationRequiresSave,
		   WorkspaceFileDecodeCache.shared.claimMigrationSave(for: cachedResult.cacheKey) {
			...
			migrationSaveTask = Task.detached(priority: .utility) {
				do {
					guard WorkspaceFileDecodeCache.shared.isMigrationSaveClaimed(for: cacheKey),
						  let currentKey = try? WorkspaceFileDecodeCache.shared.metadataKey(for: saveURL),
						  currentKey == cacheKey else {
						WorkspaceFileDecodeCache.shared.finishMigrationSave(for: cacheKey)
						return
					}
					let encoded = try JSONEncoder().encode(workspaceToSave)
					_ = await WorkspaceDiskWriter.shared.writeMigrationIfUnchanged(
						data: encoded,
						url: saveURL,
					...
					print("💾 Failed to persist migrated workspace \(saveURL.lastPathComponent): \(error)")
				}
				WorkspaceFileDecodeCache.shared.finishMigrationSave(for: cacheKey)
			}
		} else {
			migrationSaveTask = nil
		}

		return WorkspaceFileLoadResult(
			workspace: cachedResult.workspace,
			cacheHit: cachedResult.cacheHit,
			workingStateMigrated: cachedResult.workingStateMigrated,
			discoverContextMigrated: cachedResult.discoverContextMigrated,
			migrationSaveTask: migrationSaveTask
		)
	}
	...
	func resetGlobalStorageToDefault(migrate: Bool = true) throws {
		...
		for ws in workspaces {
			...
			if !FileManager.default.fileExists(atPath: newFolder.path) {
				try FileManager.default.createDirectory(at: newFolder,
		...
		self.globalCustomStorageURL = nil
	...
	public func unloadAllFolders(from workspace: WorkspaceModel) async {
	...
	func updateGlobalStoragePath(_ newURL: URL) throws {
		...
		self.globalCustomStorageURL = newURL
	...
	@MainActor
	public func refreshWorkspace(soft: Bool, for workspace: WorkspaceModel) async {
		let wsID = workspace.id
		...
		// Refresh watchers/filters for roots that are already loaded. Soft
		// refreshes must stay in-place so deferred replay overflow does not
		// hard-unload/reload every root and churn root VM identities.
	...
	private func handleRootFolderUnloadRequest(_ folderPath: String) async {
		if isRefreshing { return }
		await removeFolder(folderPath)
	}
	
	private func handleRootFoldersReordered(_ newOrder: [String]) async {
		guard let activeWS = self.activeWorkspace,
				let index = workspaces.firstIndex(where: { $0.id == activeWS.id }) else { return }
		
		var seen = Set<String>()
		let normalizedUnique = newOrder
		...
		guard !Self.repoPathsEquivalent(workspaces[index].repoPaths, normalizedUnique) else { return }
		
		// Update the workspace with the new order
		workspaces[index].repoPaths = normalizedUnique
		workspaces[index].dateModified = Date()
		
		// Save to disk
		do {
			let workspaceToSave = workspaces[index]
			let finalURL = try await saveWorkspaceToFileAsync(workspaceToSave, preserveDiskRepoPathsIfUnchangedSinceBaseline: false)
			await WorkspaceDiskWriter.shared.flush(url: finalURL)
			recordRepoPathBaseline(for: workspaceToSave)
	...
	public func removeFolder(_ folderPath: String) async {
		guard let activeWS = self.activeWorkspace else { return }
		await removeFolder(folderPath, from: activeWS)
	}
	
	public func removeFolder(_ folderPath: String, from workspace: WorkspaceModel) async {
	...
	@MainActor
	public func addFolder(_ folderURL: URL, to workspace: WorkspaceModel) async throws {
		guard let index = workspaces.firstIndex(where: { $0.id == workspace.id }) else { return }
	...
	enum AddFolderError: LocalizedError {
		...
		var agentMessage: String {
			switch self {
			case .nestedWithinExisting(let candidate, let existing):
				return "Cannot add folder. Candidate '\(candidate)' is inside existing workspace root '\(existing)'. Remove the existing root or choose a different folder."
			case .containsExisting(let candidate, let existing):
				return "Cannot add folder. Candidate '\(candidate)' contains existing workspace root '\(existing)'. Remove the existing root or choose a different folder."
			}
		}
		...
		var errorDescription: String? {
			switch self {
			case .nestedWithinExisting(let candidate, let existing):
				return "Folder \"\(candidate)\" is inside existing workspace root \"\(existing)\". Choose a different folder or remove the existing root first."
			case .containsExisting(let candidate, let existing):
				return "Folder \"\(candidate)\" contains existing workspace root \"\(existing)\". Remove the existing root first or choose a different folder."
			}
		}
	}
	...
	@MainActor
	public func addFolder(_ folderURL: URL) async throws {
		if activeWorkspace == nil ||
	...
	@MainActor
	public func pickFolderAndOpenWorkspace(
		title: String,
		message: String,
	...
	@MainActor
	public func getOrCreateSystemWorkspace() -> WorkspaceModel {
	...
	func createPreset(for workspace: WorkspaceModel, name: String) async {
		...
		let selectedPaths = fileManager.selectedFiles.map { $0.fullPath }
	...
	func applyPreset(_ presetID: UUID) async {
		guard let active = activeWorkspace,
				...
				let preset = workspaces[wsIndex].presets.first(where: { $0.id == presetID }) else {
	...
	func saveCurrentPreset() async {
		guard let active = activeWorkspace,
				...
				let presetIdx = workspaces[wsIndex].presets.firstIndex(where: { $0.id == pid }) else {
			return
		}
		...
		do {
			let finalURL = try await saveWorkspaceToFileAsync(workspaces[wsIndex])
			await WorkspaceDiskWriter.shared.flush(url: finalURL)
			
			// The selection now matches the preset; not dirty
			self.activePresetIsDirty = false
			NotificationCenter.default.post(
				name: .workspacePresetsDidChange,
				object: nil,
				userInfo: ["managerID": instanceID]
	...
	func updatePromptText(_ newText: String) {
		guard let active = activeWorkspace,
				let index = workspaces.firstIndex(where: { $0.id == active.id }) else {
			return
		}
	...
	func saveToPreset(_ index: Int, isWindowFocused: Bool) {
		...
		if index <= presets.count && index > 0 {
			...
			let selection = fileManager.selectedFiles.map { $0.fullPath }
	...
	func checkIfActivePresetIsDirty(with newSelection: [FileViewModel]) {
		guard let ws = activeWorkspace,
			...
			let preset = ws.presets.first(where: { $0.id == pid }) else {
			activePresetIsDirty = false
			return
		}
	...
	func deletePreset(_ preset: WorkspacePreset, from workspace: WorkspaceModel) {
		...
		Task {
			do {
				let finalURL = try await saveWorkspaceToFileAsync(workspaces[widx])
				await WorkspaceDiskWriter.shared.flush(url: finalURL)
				await MainActor.run {
					NotificationCenter.default.post(
						name: .workspacePresetsDidChange,
	...
	func exportUserDataToDownloadsFolderIfNeeded() async throws {
		...
		guard let promptData = UserDefaults.standard.data(forKey: "StoredPrompts"),
				!promptData.isEmpty else {
			print("No stored prompts found; skipping export.")
			return
		}
		
		let localWorkspaceIndexFileURL = self.workspaceIndexFileURL
	...
	func restoreUserDataFromDownloadsFolder() async throws {
		let fm = FileManager.default
		guard let downloads = fm.urls(for: .downloadsDirectory, in: .userDomainMask).first else {
			throw NSError(domain: "WorkspaceManager", code: 2,
		...
		for backupWsDir in workspaceDirectories {
			...
			// Build the local folder path, e.g. ~/Library/Application Support/RepoPrompt/Workspaces/Workspace-Name-UUID
			let localWsDir: URL
			if let customURL = workspace.customStoragePath {
				localWsDir = customURL
			} else {
				localWsDir = defaultWorkspaceRoot()
		...
		self.workspaces = newlyLoadedWorkspaces
		...
		if let oldID = oldActiveID,
			let newlyActive = self.workspaces.first(where: { $0.id == oldID }) {
			// If the old ID is still around, re-activate it
			await switchWorkspace(to: newlyActive, saveState: false)
		} else if !self.workspaces.isEmpty {
			// Otherwise pick the first or a "default"
			await switchWorkspace(to: self.workspaces[0], saveState: false)
		}
