...
struct ManageWorkspacesView: View {
	...
	private func windowStatusText(for windowIDs: [Int]) -> String {
		if windowIDs.isEmpty {
			...
			return "open in window \(windowIDs[0])"
		} else {
			return "open in windows \(windowIDs.map(String.init).joined(separator: ", "))"
		}
	}
	...
	private var createNewWorkspaceSection: some View {
		VStack(alignment: .leading, spacing: 12) {
			...
			HStack(spacing: 8) {
				...
				.buttonStyle(CustomButtonStyle(verticalPadding: 4,
											   horizontalPadding: 8,
			...
			.buttonStyle(CustomButtonStyle(verticalPadding: 6,
