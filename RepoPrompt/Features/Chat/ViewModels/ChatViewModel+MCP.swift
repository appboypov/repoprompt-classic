...
extension ChatViewModel {
	...
	// 1) Presets OFF: prefer chat preset override, else current chat model.
	// 2) Presets ON & no presets exist: use planning model (only case that uses planning).
	// 3) Presets ON & presets exist: use a compatible available preset; if none available, fall back to current chat model.
	@MainActor
	private func selectModel(
		...
		// Locals
		let currentModel  = promptVM.preferredAIModel
		let planningModel = promptVM.planningModel
		let chatPresetOverride = promptVM.modelFromCurrentChatPreset() // Model override defined by the current chat preset (if any)
		
		// Helpers for consistent info labels
		func infoLine(reason: String, model: AIModel) -> String {
			return "\(modeLabel) mode • \(reason) (\(model.displayName))"
		}
		...
		// ─────────────────────────────────────────────────────────────────
		if !useModelPresets {
			// MCP always uses planningModel when presets are off
			if !promptVM.isModelAvailable(planningModel) {
				throw ChatToolError.invalidParams(
					"MCP oracle model '\(planningModel.displayName)' is not available. " +
					oracleModelAvailabilityGuidance(for: planningModel)
				)
			}
		...
		// 1) No presets defined at all → use planning model (ONLY scenario using planning).
		// 2) Presets exist → pick an available preset for the mode; if none available, fall back to current chat model.
		// ─────────────────────────────────────────────────────────────────
		
		// B.1: No model presets exist at all → use default MCP model (error if unavailable)
		if !hasAnyModelPresets {
			// Default MCP model must be available when presets are enabled but none defined
			if !promptVM.isModelAvailable(planningModel) {
				throw ChatToolError.invalidParams(
					"MCP oracle model '\(planningModel.displayName)' is not available. " +
					oracleModelAvailabilityGuidance(for: planningModel) +
					" or create model presets for MCP chat."
				)
			}
			let resolvedPreset = resolveChatPreset(for: mode, from: nil)
			let info = resolvedPreset.name ?? infoLine(reason: "MCP Oracle Model", model: planningModel)
			// Respect explicit model only for the sentinel/current display name
			if let mp = modelParam {
				let mpn = norm(mp)
				if mpn == "current_chat_model" ||
					mpn == norm(planningModel.displayName) ||
					mpn == norm(currentModel.displayName) {
					return .init(
						model: planningModel,
						mcpControlInfo: info,
						isAutoSelected: false,
		...
		let presetNames = supporting.map { $0.name }.joined(separator: ", ")
		throw ChatToolError.invalidParams(
	...
	func resolveMCPFollowUpModel(
		...
		let allPresets = presetsManager.getAllPresets()
	...
	private func findBuiltInPreset(for mode: String) -> ChatPreset? {
		...
		case "edit":
			return manager.defaultPreset(for: .proEdit)
				?? manager.defaultPreset(for: .edit)
				?? manager.builtInPresets.first { $0.mode == .proEdit }
				?? manager.builtInPresets.first { $0.mode == .edit }
		case "review":
	...
	private func validateModeCompatibility(
		...
		guard isSupported else {
			...
			if supportedModes.edit { supportedModesList.append("edit") }
			...
			// Find alternatives
			let alternatives = allPresets.filteredForMode(mode).map { $0.name }
			let alternativesNote: String
			if alternatives.isEmpty {
				alternativesNote = " No defined presets support '\(mode)' mode. Use `oracle_utils op=models` to view each preset's supported modes."
			} else {
				alternativesNote = " Alternative presets for \(mode) mode: \(alternatives.joined(separator: ", "))"
			}
	...
	private func buildMCPMessageLog(from parsedMessages: [AIChatMessage], includeDiffs: Bool = false) -> [Value] {
		var log: [Value] = []
		...
		for msg in parsedMessages {
			...
			// Prefer the extracted "core content" for assistant turns so we
			// avoid sending giant diff blocks back to the caller.
			let baseText: String
			if msg.isUser {
				baseText = msg.content
			} else {
				baseText = msg.extractedCoreContent ?? msg.content
			}
			...
			// Include diff summaries when requested
			if includeDiffs && !msg.diffSummaries.isEmpty {
				let diffsArray = msg.diffSummaries.map { diff -> Value in
					var diffDict: [String: Value] = [
						"path": .string(diff.path)
					]
					
					// Build edit summary object once
					var editSummaryDict: [String: Value] = [
						"status": .string(diff.editSummary.status),
						"edits_requested": .int(diff.editSummary.editsRequested),
						"edits_applied": .int(diff.editSummary.editsApplied)
					]
					
					if let totalLinesChanged = diff.editSummary.totalLinesChanged {
						editSummaryDict["total_lines_changed"] = .int(totalLinesChanged)
					}
					
					if let totalChunks = diff.editSummary.totalChunks {
						editSummaryDict["total_chunks"] = .int(totalChunks)
					}
					
					diffDict["edit_summary"] = .object(editSummaryDict)
					
					if let diffText = diff.diffText {
						diffDict["diff_text"] = .string(diffText)
					}
					
					return .object(diffDict)
				}
				dict["diffs"] = .array(diffsArray)
			}
	...
	private func buildMCPMessageLog(includeDiffs: Bool = false, limit: Int? = nil) -> [Value] {
		let messagesToProcess = if let limit = limit, limit > 0 {
			Array(messages.suffix(limit))
		} else {
			messages
		}
		return buildMCPMessageLog(from: messagesToProcess, includeDiffs: includeDiffs)
	}
	...
	private func buildMCPMessageLog(
		...
		let storedToProcess = if let limit = limit, limit > 0 {
		...
		for stored in storedToProcess {
			parsedMessages.append(await Self.parseSingleRawMessage(stored))
		}
		return buildMCPMessageLog(from: parsedMessages, includeDiffs: includeDiffs)
	}
	...
	private static func strongestOracleOwnerBucket(
		...
		guard let strongestRank = ranked.map({ $0.rank }).min() else { return [] }
		return ranked
	...
	private func applyOracleOwnerIfNeeded(
		...
		Task { [weak self] in
			...
			_ = try? await self.autosaveSession(sessionToSave)
		}
	}
	...
	@discardableResult
	@MainActor
	func locateOrCreateChat(_ idString: String?,
							desiredName: String? = nil,
							forceNew: Bool = false,
							tabID: UUID? = nil,
							activateInUI: Bool = true,
							agentModeSessionID: UUID? = nil,
							agentModeRunID: UUID? = nil) async throws -> UUID {
		...
		func findCandidate(allowUnownedLegacy: Bool) -> ChatSession? {
			...
			if hasOwner {
				candidates = Self.strongestOracleOwnerBucket(
				...
				candidates = scopedSessions.filter { eligible($0, allowUnownedLegacy: allowUnownedLegacy) }
			}
	...
	@MainActor
	func tool_chatSend(args      : [String: Value],
					promptVM  : PromptViewModel,
					fileMgr   : RepoFileManagerViewModel,
					tabContext: ChatSendTabContext? = nil) async throws
					-> [String : Value] {
		...
		let modeRaw  = args["mode"]?.stringValue?.lowercased() ?? "chat"
		let mode     = ["plan", "edit", "chat", "review"].contains(modeRaw) ? modeRaw : "chat"
		...
		let includeDiffsFlag = args["include_diffs"]?.boolValue ?? true
		...
		if let mcpControlledModel = mcpControlledModel {
		...
		// ────────── 4. Determine mode ──────────
		let effectiveMode = PromptViewModel.PlanActMode(rawValue: mode.capitalized) ?? .edit
		...
		// ────────── 6. Auto-apply edits (edit mode) ──────────
		var errors:[String] = []
		if mode == "edit", let q = queryId {
			await acceptAllAndSave(forMessageId: q, includeDiffs: includeDiffsFlag)
			for t in delegateEditTasks[q] ?? [] where t.status != .completed {
				errors.append("\(t.filePath): \(t.status)")
			}
		}
		...
		let diffArr: [MessageDiff]? = {
			guard includeDiffsFlag, let q = queryId else { return nil }
			return getDiffSummaries(for: q)
		}()
		...
		let replyObj = ChatSendReply(
			...
			mode     : mode,
			response : aiMsg?.extractedCoreContent ?? aiMsg?.content,
	...
	private func runHeadless(
		...
		// 4) Stream via AIQueriesService WITHOUT touching ChatViewModel.messages
