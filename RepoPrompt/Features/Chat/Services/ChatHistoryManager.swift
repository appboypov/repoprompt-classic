...
struct ChatSessionStubLoadBatchResult {
	...
	var loadedCount: Int { sessions.count }
...
public enum ChatHistoryLimit: Int, CaseIterable {
	...
	public static func from(rawValue: Int) -> ChatHistoryLimit {
		return ChatHistoryLimit(rawValue: rawValue) ?? .unlimited
	}
}
...
actor ChatDataService {
	...
	/// The JSON encoder/decoder we'll use
	...
	/// Load a ChatSession from disk, automatically attempting any old->new migrations.
	func loadChatSession(
		...
		do {
			...
			// Attempt fallback decode into OldChatSession
			do {
				let data = try Data(contentsOf: fileURL)
				var oldSession = try decoder.decode(OldChatSession.self, from: data)
				oldSession.fileURL = fileURL
				
				let migrated = migrateOldSessionToNew(oldSession)
				// Attempt to re-save as new
				_ = try saveMigratedSession(migrated, at: fileURL)
				return migrated
			} catch {
				// Delete if both new and old fail
				try FileManager.default.removeItem(at: fileURL)
				throw ChatDataError.decodingFailed(decodeError)
			}
			*/
		} catch {
	...
	nonisolated func loadChatSessionStubs(
		...
		var outcomes = Array<ChatSessionStubLoadOutcome?>(repeating: nil, count: files.count)
		...
		await withTaskGroup(of: ChatSessionStubLoadOutcome.self) { group in
			...
			while let outcome = await group.next() {
				switch outcome {
				case .success(let index, _), .failure(let index, _, _):
		...
		for outcome in outcomes {
			...
			switch outcome {
			case .success(_, let session):
				sessions.append(session)
			case .failure(let index, let fileURL, let message):
	...
	/// Find a specific chat session by UUID or short ID without mutating ChatViewModel state.
	func findSession(
		for workspace: WorkspaceModel,
		id rawID: String,
		composeTabID: UUID? = nil
	...
	// MARK: - Internal Migration Helpers
	
	private func migrateOldSessionToNew(_ old: OldChatSession) -> ChatSession {
		let newStoredMessages = old.messages.map { oldMsg in
			StoredMessage(
				id: oldMsg.id,
				isUser: oldMsg.isUser,
				rawText: oldMsg.rawText,
	...
	private func saveMigratedSession(_ session: ChatSession, at fileURL: URL) throws -> URL {
		var sessionCopy = session
		sessionCopy.fileURL = fileURL
		sessionCopy.savedAt = Date()
		
		let data = try encoder.encode(sessionCopy)
		try data.write(to: fileURL, options: .atomic)
	...
	// MARK: - Old Format Stubs
	private struct OldChatSession: Codable {
		let id: UUID
		var name: String
		var savedAt: Date
		var fileURL: URL?
		var messages: [OldStoredMessage]
	}
	
	private struct OldStoredMessage: Codable {
		let id: UUID
		let isUser: Bool
		let rawText: String
		let timestamp: Date
