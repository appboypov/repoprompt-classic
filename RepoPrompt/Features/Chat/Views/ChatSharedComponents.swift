...
struct CompactPrefPromptTag: View {
	let title: String
	...
	var body: some View {
		HStack(spacing: 4) {
			Text(title)
				...
				.font(envFont?.scale(0.9) ?? .system(size: 11.7)) // Unwrap optional and provide fallback
				...
				HStack(spacing: 2) {
					Button(action: {
						...
						Image(systemName: "magnifyingglass")
							.foregroundColor(.primary)
							.font(envFont?.scale(0.75) ?? .system(size: 9.75)) // Unwrap optional and provide fallback
...
struct EmptyChatOverlay: View {
	...
	var body: some View {
		VStack(spacing: 20) {
			...
			Text("Select your files from the sidebar then type a message and send.\nIf using pro edit mode, be sure to configure you ai model settings.")
				.font(envFont ?? .body)
				.foregroundColor(.secondary)
