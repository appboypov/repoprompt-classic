...
struct DiscoverAgentView: View {
	@ObservedObject var viewModel: DiscoverAgentViewModel
	...
	private var introHeader: some View {
		VStack(alignment: .leading, spacing: 4) {
			...
			Text("Use Codex or Claude Code to explore your codebase, enhance your prompt, and assemble relevant context for your task.")
				.font(.subheadline)
		...
		VStack(alignment: .leading, spacing: 12) {
			...
			if let pendingAskUser = viewModel.pendingAskUser(for: subjectTabID) {
				AgentAskUserWizardCard(
					...
					onSubmit: {
						guard let tabID = subjectTabID else { return }
						viewModel.submitAskUserResponse(tabID: tabID)
					},
					onSkipAll: {
						guard let tabID = subjectTabID else { return }
						viewModel.skipAskUser(tabID: tabID)
					},
					onUserActivity: {
						guard let tabID = subjectTabID else { return }
	...
	/// Whether this specific tab has an active discovery run
	private var isDiscoveryRunningForTab: Bool {
		guard let tabID = subjectTabID else { return false }
		return viewModel.tabsWithActiveDiscoverRun.contains(tabID)
	}
	...
	private var canGeneratePlan: Bool {
		...
		case .generating:
			return false
		default:
			// Also block if discovery is running for this tab
			return !isDiscoveryRunningForTab
		}
	}
	...
	private var planModelName: String {
		chatViewModel.promptViewModel.preferredAIModel.displayName
	}

	/// Text describing what MCP will do after discovery completes
	private var mcpWaitingText: String {
		...
		switch responseType {
		case "plan":
			return "MCP: Will generate plan after Context Builder completes"
		case "question":
			return "MCP: Will answer question after Context Builder completes"
		case "clarify":
	...
	private var selectedFollowUpType: AnalysisFollowUpType {
	...
	private var backgroundPlanSection: some View {
		...
		VStack(alignment: .leading, spacing: 8) {
			...
			if !viewModel.isMCPControlledRun {
				...
				HStack(spacing: 4) {
					...
					Toggle("", isOn: $viewModel.autoGeneratePlan)
						...
						.controlSize(.mini)
						.labelsHidden()
						.disabled(isDiscoveryRunningForTab)
				}
			...
			if case .ready(let chatID, let previewText) = status {
	...
	private var planStatusIndicator: some View {
		...
		case .idle:
			...
			let isWaitingForDiscovery = isDiscoveryRunningForTab &&
	...
	private var planModelPicker: some View {
		OptimizedModelPicker(
			selection: $chatViewModel.promptViewModel.preferredModel,
			availableModels: chatViewModel.promptViewModel.availableModels,
			font: .callout,
			widthStyle: .flexible()
		)
		.disabled(isDiscoveryRunningForTab)
	...
	private var followUpTypePicker: some View {
		Menu {
			ForEach(AnalysisFollowUpType.allCases, id: \.self) { type in
	...
	/// Generate a plan/review/answer from discovery results based on selected follow-up type.
	...
	/// Open the generated plan in chat view
	private func viewGeneratedPlan(chatID: String) {
		NotificationCenter.default.post(name: .switchToChatView, object: nil)
		chatViewModel.selectSession(byShortID: chatID)
	}
	...
	private var headerSection: some View {
		HStack(spacing: 8) {
				...
				StableMenuButton(
					items: discoveryAgentModelMenuItems,
	...
	private var instructionsSection: some View {
		VStack(alignment: .leading, spacing: 8) {
			...
			ContextBuilderHeaderBar(
				discoveryInstructions: $viewModel.discoveryInstructions,
				...
				isRunning: isDiscoveryRunningForTab,
				isDisabled: !isDiscoveryRunningForTab && viewModel.isAgentBusy,
			...
			// Text editor
			DiscoveryInstructionsEditor(
				text: $viewModel.discoveryInstructions,
				windowID: windowID,
				enhancementMode: viewModel.enhancementMode,
	...
	private func runOrCancelAction() {
		if isDiscoveryRunningForTab {
			...
			viewModel.runDiscoverAgent()
		}
	}

	private func discoveryAgentModelMenuItems() -> [StableMenuItem] {
...
private struct DiscoveryInstructionsEditor: View {
	...
	private var placeholderText: String {
		...
		case .augment:
			return "Add extra details to help the agent find relevant files and enhance your prompt"
		case .preserve:
			return "Describe what files to look for (your instructions won't be modified)"
		}
	}
...
private struct ContextBuilderHeaderBar: View {
	@Binding var discoveryInstructions: String
		...
		HStack(spacing: 8) {
			...
			HStack(spacing: 6) {
				...
				Button(action: {
					...
					Image(systemName: "xmark.circle.fill")
						.font(.caption)
						.foregroundColor(discoveryInstructions.isEmpty ? .gray.opacity(0.5) : .secondary)
				}
				.buttonStyle(PlainButtonStyle())
				.disabled(discoveryInstructions.isEmpty)
				.hoverTooltip("Clear")
			}
...
private struct CompactRunButton: View {
	...
	private var tooltipText: String {
		if isCancelling {
			...
			return "Cancel Context Builder run"
		} else if isBusy {
			return "Context Builder is cleaning up"
		} else {
			return "Run Context Builder (Cmd + Return)"
		}
	}
				...
				VStack(alignment: .leading, spacing: 16) {
					...
					VStack(alignment: .leading, spacing: 10) {
						...
						if showAutoPlanBudget {
							VStack(alignment: .leading, spacing: 8) {
								Text("Post-discovery analysis uses CLI/API calls which support larger context windows.")
									.font(.caption)
									.foregroundColor(.secondary)
									.fixedSize(horizontal: false, vertical: true)
					...
					VStack(alignment: .leading, spacing: 10) {
						...
						Text("How the agent modifies your instructions after discovery.")
							.font(.caption)
							.foregroundColor(.secondary)
					...
					VStack(alignment: .leading, spacing: 10) {
						...
						Text("Allow the agent to ask you questions during discovery to better understand your intent.")
							.font(.caption)
							.foregroundColor(.secondary)
							.fixedSize(horizontal: false, vertical: true)
						...
						VStack(spacing: 8) {
							SettingsClarifyingToggleRow(
								...
								description: "When you click Run",
...
private extension View {
	@ViewBuilder
	func discoveryKeyboardShortcut(enabled: Bool) -> some View {
		if enabled {
