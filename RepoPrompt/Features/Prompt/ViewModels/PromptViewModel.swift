...
enum GitArtifactPublishError: LocalizedError {
	...
	var errorDescription: String? {
		switch self {
		case .noActiveWorkspace: return "No active workspace"
		case .noGitRepository: return "No git repository found"
		case .noDiffModeSelected: return "No diff mode selected"
		case .noFilesSelected: return "No changed files selected"
		}
	}
}

// MARK: - Complex-file edit strategy

/// How a single, complex file edit should be broken down.
enum ComplexEditStrategy: String, CaseIterable, Identifiable {
	case single = "Single query"
	case parallelSplit = "Parallel split"
	case sequential = "Sequential split"
	
	var id: String { rawValue }
	
	/// Brief description shown in the user interface.
	var caption: String {
		switch self {
		case .single:
			return "[Cheapest] One request handles all edits at once. May overwhelm the model for large edits."
		case .parallelSplit:
			return "[Fastest] Edits split into groups and processed simultaneously. May cause merge conflicts."
		case .sequential:
			return "[Most accurate] Edits split into groups and applied one by one, updating the file each time."
		}
	}
}
...
class PromptViewModel: ObservableObject {
	...
	@Published private(set) var fileManager: RepoFileManagerViewModel
	...
	let windowID: Int
	private weak var aiResponseViewModel: AIResponseViewModel?
	...
	private static let backgroundAgentComposeTabHardLimitUserDefaultsKey = "agentMode.maxBackgroundAgentComposeTabs"
	private let maxComposeTabs = PromptViewModel.defaultComposeTabSoftLimit
	private var isDirtyStateUpdateScheduled = false
	
	enum ComposeTabCapacityPolicy: Sendable, Equatable {
		case uiInteractive
	...
	private var backgroundAgentComposeTabHardLimit: Int {
		let configuredLimit = UserDefaults.standard.integer(forKey: Self.backgroundAgentComposeTabHardLimitUserDefaultsKey)
		let rawLimit = configuredLimit > 0 ? configuredLimit : Self.defaultBackgroundAgentComposeTabHardLimit
		return max(maxComposeTabs, rawLimit)
	}
	
	// MARK: - Tab Close Listeners
	/// Async listeners that are called before tabs are closed, allowing cleanup of running tasks.
	enum ComposeTabRemovalReason: Sendable, Equatable {
		case close
		case stash
		case deleteStashed
	}

	struct AgentSessionCascadePlan: Sendable, Equatable {
	...
	var composeTabLimit: Int { maxComposeTabs }
	...
	private var filesTabSelection: FilesTabSelection = .followLicenseDefault
	@Published var selectedFileForPreview: FileViewModel?

	// MARK: - Files Tab Selection

	var storedActiveSubView: FilesTab? {
		if case .explicit(let tab) = filesTabSelection {
			return tab
		}
		return nil
	}
	...
	var disabledPromptSections: Set<PromptSection> { Set<PromptSection>() }
	...
	@AppStorage("fileDisplayMode") var fileDisplayMode: String = FileDisplayMode.folders.rawValue
	...
	var planningModel: AIModel {
		let model = AIModel.fromModelName(planningModelName) ?? preferredAIModel
		return model
	}
	...
	/// Mirrors the per-workspace Pro Edit toggle and persists through the global settings manager.
	@Published var proFileEdits: Bool = false {
		didSet {
			guard oldValue != proFileEdits else { return }
			...
			if settings.proFileEdits != proFileEdits {
				settings.proFileEdits = proFileEdits
	...
	/// Agent Edit Mode: use a single MCP agent to apply delegate edits.
	/// Cursor is intentionally excluded from Pro Edit because ACP cannot sandbox Cursor's built-in tools.
	@Published var proEditAgentMode: Bool = false {
		didSet {
			guard oldValue != proEditAgentMode else { return }
			guard !isRestoringProEditState else { return }
			settingsManager.updateProEditSettings({ snapshot in
				snapshot.agentMode = proEditAgentMode
			}, commit: true)
		}
	}
	@Published private(set) var availableAgentKinds: [DiscoverAgentKind] = AgentModelCatalog.selectableAgents(availability: .none)
	@Published private(set) var availableProEditAgentKinds: [DiscoverAgentKind] = AgentModelCatalog.selectableAgents(availability: .none, surface: .sandboxedDelegateEdit)
	
	/// Preferred context-builder agent (global, persisted via GlobalSettingsStore)
	@Published var contextBuilderAgent: DiscoverAgentKind = .claudeCode {
		didSet {
	...
	func contextBuilderModelOptions(for agentKind: DiscoverAgentKind) -> [AgentModelOption] {
		AgentModelCatalog.options(for: agentKind, availability: agentAvailabilityContext)
	}

	func proEditModelOptions(for agentKind: DiscoverAgentKind) -> [AgentModelOption] {
		guard AgentModelCatalog.AgentSelectionSurface.sandboxedDelegateEdit.allows(agentKind),
			AgentModelCatalog.isAgentAvailable(agentKind, availability: agentAvailabilityContext) else { return [] }
		let knownModels = Set(AgentModel.modelsForAgent(agentKind).map(\.rawValue))
		return AgentModelCatalog.options(for: agentKind, availability: agentAvailabilityContext).filter { option in
			if knownModels.contains(option.rawValue) { return true }
			guard agentKind == .codexExec,
				CodexServiceTierVariantCatalog.isFastVariant(rawModel: option.rawValue),
				let resolved = AgentModel.resolvedModel(forRaw: option.rawValue, agentKind: agentKind) else { return false }
			return knownModels.contains(resolved.rawValue)
		}
	}

	var availableProEditModels: [AIModel] {
		availableModels.filter { isProEditSafeModel($0) }
	}

	private func isProEditSafeModel(_ model: AIModel) -> Bool {
	...
	private func defaultModelRaw(for agentKind: DiscoverAgentKind) -> String {
		AgentModelCatalog.defaultModelRaw(for: agentKind, availability: agentAvailabilityContext)
	}

	private func defaultProEditModelRaw(for agentKind: DiscoverAgentKind) -> String {
		AgentModelCatalog.defaultModelRaw(for: agentKind, availability: agentAvailabilityContext)
	}
	...
	private func refreshAvailableAgentKinds() {
		...
		availableProEditAgentKinds = AgentModelCatalog.selectableAgents(availability: agentAvailabilityContext, surface: .sandboxedDelegateEdit)
	}
	...
	private func normalizedProEditAgentSelection(agentRaw: String?, modelRaw: String?) -> AgentModelCatalog.NormalizedAgentSelection {
		AgentModelCatalog.normalizeSelection(agentRaw: agentRaw, modelRaw: modelRaw, availability: agentAvailabilityContext, surface: .sandboxedDelegateEdit)
	}

	private func normalizedPersistedProEditAgentSelection(agentRaw: String?, modelRaw: String?) -> AgentModelCatalog.NormalizedAgentSelection {
		AgentModelCatalog.normalizePersistedSelection(agentRaw: agentRaw, modelRaw: modelRaw, availability: agentAvailabilityContext, surface: .sandboxedDelegateEdit)
	}
	...
	private func isContextBuilderModelRawValidForAgent(_ rawModel: String, agent: DiscoverAgentKind) -> Bool {
		AgentModelCatalog.isValid(rawModel: rawModel, for: agent, availability: agentAvailabilityContext)
	}

	private func isProEditAgentModelValid(_ model: AgentModel) -> Bool {
		isProEditAgentModelRawValid(model.rawValue)
	}

	private func isProEditAgentModelRawValid(_ rawModel: String) -> Bool {
		AgentModelCatalog.AgentSelectionSurface.sandboxedDelegateEdit.allows(proEditAgentKind)
			&& AgentModelCatalog.isValid(rawModel: rawModel, for: proEditAgentKind, availability: agentAvailabilityContext)
	}
	...
	private func handleAgentProviderAvailabilityChanged(reason: String) {
		...
		let normalizedProEdit = normalizedPersistedProEditAgentSelection(
			agentRaw: proEditAgentKind.rawValue,
			modelRaw: proEditAgentModelRaw
		)
		let resolvedProEditModel = AgentModel.resolvedModel(
			forRaw: normalizedProEdit.modelRaw,
			agentKind: normalizedProEdit.agent
		) ?? .defaultModel
		if normalizedProEdit.agent != proEditAgentKind || resolvedProEditModel != proEditAgentModel {
			isRestoringProEditState = true
			proEditAgentKind = normalizedProEdit.agent
			proEditAgentModel = resolvedProEditModel
			isRestoringProEditState = false
			postRecommendationsShouldRefresh(reason: reason)
		}
	}
	...
	/// Selected agent kind to run delegate edits (Claude Code or Codex)
	@Published var proEditAgentKind: DiscoverAgentKind = .claudeCode {
		didSet {
			guard proEditAgentKind != oldValue else { return }
			guard !isRestoringProEditState else { return }
			if !AgentModelCatalog.AgentSelectionSurface.sandboxedDelegateEdit.allows(proEditAgentKind) {
				let normalized = normalizedProEditAgentSelection(
					agentRaw: proEditAgentKind.rawValue,
					modelRaw: proEditAgentModelRaw
				)
				isRestoringProEditState = true
				proEditAgentKind = normalized.agent
				proEditAgentModel = AgentModel.resolvedModel(
					forRaw: normalized.modelRaw,
					agentKind: normalized.agent
				) ?? .defaultModel
				isRestoringProEditState = false
				settingsManager.updateProEditSettings({ snapshot in
					snapshot.agentKindRaw = normalized.agent.rawValue
					snapshot.agentModelRaw = normalized.modelRaw
				}, commit: true)
				return
			}
			// Ensure model remains valid for the agent
			let currentRaw = proEditAgentModelRaw
			let modelRawToPersist: String
			if !isProEditAgentModelRawValid(currentRaw) {
				let defaultRaw = defaultProEditModelRaw(for: proEditAgentKind)
				isRestoringProEditState = true
				proEditAgentModel = AgentModel.resolvedModel(
					forRaw: defaultRaw,
					agentKind: proEditAgentKind
				) ?? .defaultModel
				isRestoringProEditState = false
				modelRawToPersist = defaultRaw
			} else {
				modelRawToPersist = currentRaw
				if let resolved = AgentModel.resolvedModel(forRaw: currentRaw, agentKind: proEditAgentKind), resolved != proEditAgentModel {
					isRestoringProEditState = true
					proEditAgentModel = resolved
					isRestoringProEditState = false
				}
			}
			settingsManager.updateProEditSettings({ snapshot in
				snapshot.agentKindRaw = proEditAgentKind.rawValue
				snapshot.agentModelRaw = modelRawToPersist
			}, commit: true)
			postRecommendationsShouldRefresh(reason: "proEditAgentKindChanged")
		}
	}
	var proEditAgentModelRaw: String {
		let storedRaw = settingsManager.proEditSettings().agentModelRaw
		let normalized = normalizedProEditAgentSelection(
			agentRaw: proEditAgentKind.rawValue,
			modelRaw: storedRaw ?? proEditAgentModel.rawValue
		)
		guard normalized.agent == proEditAgentKind else { return proEditAgentModel.rawValue }
		return normalized.modelRaw
	}

	var proEditAgentModelDisplayName: String {
		AgentModelCatalog.displayName(
			for: proEditAgentModelRaw,
			agentKind: proEditAgentKind,
			availability: agentAvailabilityContext
		)
	}

	func selectProEditAgentModel(rawModel: String) {
		let normalized = normalizedProEditAgentSelection(
			agentRaw: proEditAgentKind.rawValue,
			modelRaw: rawModel
		)
		let resolvedModel = AgentModel.resolvedModel(
			forRaw: normalized.modelRaw,
			agentKind: normalized.agent
		) ?? .defaultModel
		isRestoringProEditState = true
		proEditAgentKind = normalized.agent
		proEditAgentModel = resolvedModel
		isRestoringProEditState = false
		settingsManager.updateProEditSettings({ snapshot in
			snapshot.agentKindRaw = normalized.agent.rawValue
			snapshot.agentModelRaw = normalized.modelRaw
		...
		postRecommendationsShouldRefresh(reason: "proEditAgentModelChanged")
	}

	/// Selected model for the chosen agent (per agent's supported list)
	@Published var proEditAgentModel: AgentModel = .defaultModel {
		didSet {
			guard proEditAgentModel != oldValue else { return }
			guard !isRestoringProEditState else { return }
			// Validate model is compatible with current agent
			if !isProEditAgentModelValid(proEditAgentModel) {
				isRestoringProEditState = true
				proEditAgentModel = AgentModel.resolvedModel(
					forRaw: defaultProEditModelRaw(for: proEditAgentKind),
					agentKind: proEditAgentKind
				) ?? .defaultModel
				isRestoringProEditState = false
				return
			...
			settingsManager.updateProEditSettings({ snapshot in
				snapshot.agentKindRaw = proEditAgentKind.rawValue
				snapshot.agentModelRaw = proEditAgentModel.rawValue
			}, commit: true)
			postRecommendationsShouldRefresh(reason: "proEditAgentModelChanged")
		}
	}

	/// Flag to prevent recursive updates when validating Pro Edit agent/model pairing
	private var isRestoringProEditState = false
	...
	private var preservedCustomizations: CopyCustomizations? = nil
	...
	var preferredAIModel: AIModel {
		...
		let model = AIModel.fromModelName(preferredModel) ?? .claude4Sonnet
		//print("model=\(model)")
		return model
	}
	...
	@AppStorage("editSettingsMediumSmall") var editSettingsMediumSmall: String = AIModel.gemini3FlashPreview.rawValue
	@AppStorage("editSettingsMediumLarge") var editSettingsMediumLarge: String = AIModel.claude4Sonnet.rawValue
	@AppStorage("editSettingsHighSmall") var editSettingsHighSmall: String = AIModel.gpt41.rawValue
	@AppStorage("editSettingsHighLarge") var editSettingsHighLarge: String = AIModel.claude4Sonnet.rawValue
	
	/// User-defined maximum number of changes per subgroup when splitting a complex edit
	@AppStorage("delegateEditGroupSizeV2") var delegateEditGroupSize: Double = 8 // slider range 3-20
	var delegateEditGroupSizeInt: Int { max(3, Int(delegateEditGroupSize)) }
	
	@AppStorage("largeFileThresholdV2") var largeFileThreshold: Double = 500
	...
	var allowDiffModelsToRewrite: Bool {
		get { settingsManager.allowDiffModelsToRewrite() }
		set {
			guard newValue != allowDiffModelsToRewrite else { return }
			settingsManager.setAllowDiffModelsToRewrite(newValue, commit: true)
			handlePromptPackagingSettingChanged()
		}
	}
	...
	var complexEditStrategy: ComplexEditStrategy {
		get { ComplexEditStrategy(rawValue: settingsManager.complexEditStrategyRaw()) ?? .sequential }
		set {
			guard newValue != complexEditStrategy else { return }
			settingsManager.setComplexEditStrategyRaw(newValue.rawValue, commit: true)
			handlePromptPackagingSettingChanged()
		}
	}
	...
	// MARK: - Copy Context Settings (used in PromptView/Copy operations)
	...
	private func syncModelSelectionFromSettingsManager() {
		if let planningModelRaw = settingsManager.planningModelRaw(), !planningModelRaw.isEmpty {
			_planningModel = planningModelRaw
		}
		if sessionPreferredModelOverrideRaw == nil,
			let preferredComposeRaw = settingsManager.preferredComposeModelRaw(), !preferredComposeRaw.isEmpty {
			_preferredModel = preferredComposeRaw
		}
	}
	...
	func syncSettingsFromSettingsManager() {
		...
		proFileEdits = isChatManual
			? (chatSettings.manualProFileEdits ?? chatSettings.proFileEdits)
			: chatSettings.proFileEdits
		...
		if isChatManual {
			...
			// Apply the preset's git inclusion value
			let currentChatPreset = resolvedChatPreset(for: selectedChatPresetID ?? ChatPreset.BuiltIn.edit.id)
		...
		// Sync context-builder agent/model from global discover settings (single source of truth)
		refreshAvailableAgentKinds()
		...
		// Load Pro Edit agent settings from the global settings store.
		isRestoringProEditState = true
		let proEditSettings = settingsManager.proEditSettings()
		proEditAgentMode = proEditSettings.agentMode
		let normalizedProEdit = normalizedPersistedProEditAgentSelection(
			agentRaw: proEditSettings.agentKindRaw,
			modelRaw: proEditSettings.agentModelRaw
		)
		proEditAgentKind = normalizedProEdit.agent
		proEditAgentModel = AgentModel.resolvedModel(
			forRaw: normalizedProEdit.modelRaw,
		...
		settingsManager.updateProEditSettings({ snapshot in
			snapshot.agentMode = proEditAgentMode
			snapshot.agentKindRaw = normalizedProEdit.agent.rawValue
			snapshot.agentModelRaw = normalizedProEdit.modelRaw
		}, commit: true)
	...
	@AppStorage("includeDiffFormattingInClipboard") var _includeDiffFormattingInClipboard: Bool = false
	...
	var includeDiffFormattingInClipboard: Bool {
		get { _includeDiffFormattingInClipboard }
		set {
			if _includeDiffFormattingInClipboard != newValue {
				_includeDiffFormattingInClipboard = newValue
				isDirty = true
			}
		}
	}
	
	// MARK: - XML Clipboard Format Properties
	
	@AppStorage("xmlClipboardFormatPreference") var xmlClipboardFormatPreference: DiffViewModel.PromptFormat = .whole
	@AppStorage("xmlCopyPromptFormat") private var _xmlCopyPromptFormat: String = ApplyPromptFormat.diff.rawValue
	
	var xmlCopyPromptFormat: ApplyPromptFormat {
		get {
			ApplyPromptFormat(rawValue: _xmlCopyPromptFormat) ?? .diff
		}
		set {
			_xmlCopyPromptFormat = newValue.rawValue
		}
	}
	...
	@Published var selectedChatPresetID: UUID? = ChatPreset.BuiltIn.edit.id
	...
	private struct PromptContextTokenBaselineKey: Equatable {
		...
		let codeMapUsage: CodeMapUsage
		let gitInclusion: GitInclusion
		let systemPromptFlavor: SystemPromptFlavor?
		let storedPromptIds: [UUID]
		let includeMCPMetadata: Bool
	}
	...
	private struct ChatContextTokenBaselineCacheKey: Equatable {
		...
		let autoCodemapVersion: UInt64
		let fileAPIsVersion: UInt64
		let fileAPIsContentVersion: UInt64
	...
	private var chatFileAPIsContentVersion: UInt64 = 0
	private var chatFileSystemDeltaVersion: UInt64 = 0
	private var chatContextTokenBaselineCache: ChatContextTokenBaselineCache?
	private(set) var lastChatContextTokenBaselineDiagnostic: String = ""
	
	// MARK: - Computed Properties for Token Counting (Legacy Support)
	
	var tokenCount: String { tokenCountingViewModel.tokenCount }
	var tokenCountFilesOnly: String { tokenCountingViewModel.tokenCountFilesOnly }
	var charCount: Int { tokenCountingViewModel.charCount }
	var totalTokenCount: Int { tokenCountingViewModel.totalTokenCount }
	var totalTokenCountFilesOnly: Int { tokenCountingViewModel.totalTokenCountFilesOnly }
	var totalTokenCountWithDiff: Int { tokenCountingViewModel.totalTokenCountWithDiff }
	var tokenCountWithDiff: String { tokenCountingViewModel.tokenCountWithDiff }
	var folderTokenInfo: [String: TokenInfo] { tokenCountingViewModel.folderTokenInfo }
	var fileTokenInfo: [UUID: TokenInfo] { tokenCountingViewModel.fileTokenInfo }
	var codeMapFileCount: Int { tokenCountingViewModel.codeMapFileCount }
	var codeMapTokenCount: Int { tokenCountingViewModel.codeMapTokenCount }
	var cachedFileAPIs: [FileAPI] { tokenCountingViewModel.cachedFileAPIs }
	var fileTreeContent: String { tokenCountingViewModel.fileTreeContent }
	var codeMapContent: String { tokenCountingViewModel.codeMapContent }
	var scannedLanguages: Set<LanguageType> { tokenCountingViewModel.scannedLanguages }
	var combinedTreeAndCodeMapContent: String { tokenCountingViewModel.combinedTreeAndCodeMapContent }
	var tokenCalculationCompletedPublisher: PassthroughSubject<Void, Never> { tokenCountingViewModel.tokenCalculationCompletedPublisher }
	
	var fileTreeTokenCount: Double { tokenCountingViewModel.fileTreeTokenCount }
	var tooManyFileTreeTokens: Bool { tokenCountingViewModel.tooManyFileTreeTokens }
	var gitDiffTokenCount: Int { tokenCountingViewModel.gitDiffTokenCount }
	var gitDiffTokenCountString: String { tokenCountingViewModel.gitDiffTokenCountString }
	var tokenBreakdownDescription: String { tokenCountingViewModel.tokenBreakdownDescription }
	...
	var architectPromptID: UUID { architectPrompt.id }
	var engineerPromptID: UUID { engineerPrompt.id }
	var mcpAgentPromptID: UUID { mcpAgentPrompt.id }
	var mcpPairProgramPromptID: UUID { mcpPairProgramPrompt.id }
	var reviewPromptID: UUID { reviewPrompt.id }
...
2. **Context Preparation**:
	...
	- Use `manage_selection` with `op=get` and `view=files` to check current context and tokens
	- Search for additional files if the current selection is insufficient
	- Use `manage_selection` with `op=add` / `op=remove` for incremental context changes; use `op=set` with `mode=full` only when switching focus via complete replacement
...
5. **Error Handling & Recovery**:
	...
	- If approaching token limits: Use `op=remove` for completed files, `op=add` for new ones, or `op=set` with `mode=slices` to replace slices for specified files while preserving unrelated selection
...
6. **Token Management During Long Sessions**:
	- Monitor token count every 3-5 messages with `manage_selection` `op=get`
	- When above 80k tokens: Remove files that are complete or no longer needed
	- Use `op=set` with `mode=full` only when shifting to a new component/feature by completely replacing context
...
4. **Chat Tool Strategy (for complex tasks)**:
	...
	- Use `manage_selection` with `op=set` and `mode=full` to set focused context as a complete replacement; use `op=add` for mixed paths+slices additions
...
5. **Multi-file Refactoring Workflow**:
	...
	- Use `manage_selection` with `op=get` to verify current context
	- For large refactorings: Break into phases, using `op=add`/`op=remove` for incremental changes or `op=set` `mode=full` for complete phase replacement
	- Apply changes systematically: interfaces first, then implementations
	- Verify each phase before moving to the next

6. **Token Management**:
	- Check token count before adding files: `manage_selection` with `op=get`
	- If approaching limits: Focus on files currently being modified
	- Use `op=remove`/`op=add` to swap completed files for new ones
...
"""
	...
	init(fileManager: RepoFileManagerViewModel,
		...
		self.gitViewModel = GitViewModel(fileManager: fileManager)
		...
		self.codeMapsGloballyDisabled = GlobalSettingsStore.shared.globalCodeMapsDisabled()
		
		self.queryType = .compose
		...
		self.setupObservers()
        self.setupAPISettingsObserver()
        self.configureTokenCountingViewModel()
		self.startTokenCountUpdateTimer()
		self.loadStoredPrompts()
		self.updateFileTree()
		...
		self.fileManager.initCodeScanState(self.shouldEnableCodeScanning())
		self.syncSettingsFromSettingsManager()
        ...
        self.loadPromptSectionOrder()
		suppressPromptPackagingSettingInvalidation = false

        // Initialize chat preset with Pro Edit default.
		self.selectChatPreset(ChatPreset.BuiltIn.proEdit.id)

		// Perform one-time migration to agent mode if applicable
		self.migrateToAgentModeIfNeeded()

        if self.selectedCopyPresetID == nil {
			// Gather current UI/AppStorage-backed values
			let includeFiles       = self.includeFilesInClipboard
			let includeUserPrompt  = self.includeUserPromptInClipboard
			let includeMetaPrompts = self.includeSavedPromptsInClipboard
			// Approximate "include file tree" using prior related toggles
			let includeFileTree    = self._includeFolderDirectory || self._includeFileTreeInAIQuery
			let fileTreeMode       = self.fileTreeOption
			// Use raw preference for migration so we preserve the user's intent.
			let migratedCodeMapUsage = self.codeMapUsage
			
			// Create a custom preset representing the user's previous configuration
			let migratedPreset = CopyPresetManager.shared.createPresetFromSettings(
				name: "Your Previous Settings",
				...
				xmlFormat: nil,                 // legacy clipboard did not enforce XML format
				fileTreeMode: fileTreeMode,
				codeMapUsage: migratedCodeMapUsage,
				gitInclusion: .none             // legacy clipboard had no dedicated git inclusion toggle
			)
			
			// Select the migrated preset for the user
			self.selectCopyPreset(migratedPreset.id)
		}
	}
	...
	private func setupObservers() {
		...
		$promptText
			...
			.sink { [weak self] _ in
				...
				self.tokenCountingViewModel.markPromptDirty()
		...
		fileManager.codeMapUpdatePublisher
			...
			.sink { [weak self] in
				...
				self?.workspaceManager?.markWorkspaceDirty()
				self?.updateActiveTabDirtyState()
				self?.bumpChatPromptEntriesFileAPIsVersion(contentChanged: true)
			}
			.store(in: &cancellables)
		...
		fileManager.fileSystemDeltasAppliedPublisher
			...
			.sink { [weak self] _ in
				...
				// Track them in the cache key so exact callers rebuild, but retain the previous baseline so
				// diagnostics can report the miss reason and preview can decide whether a stable baseline applies.
				guard let self else { return }
				self.chatFileSystemDeltaVersion &+= 1
			}
			.store(in: &cancellables)
		...
		// Observe Claude Code (and future agent provider) connection changes
		NotificationCenter.default.publisher(for: .claudeCodeConnectionChanged)
			.receive(on: DispatchQueue.main)
			.sink { [weak self] notification in
				guard let self = self else { return }
				if let targetWindowID = notification.userInfo?["windowID"] as? Int,
		...
		NotificationCenter.default.publisher(for: .codexConnectionChanged)
			.receive(on: DispatchQueue.main)
			.sink { [weak self] notification in
				guard let self = self else { return }
				if let targetWindowID = notification.userInfo?["windowID"] as? Int,
		...
		NotificationCenter.default.publisher(for: .recommendationsDidApply)
			...
			.sink { [weak self] notification in
				...
				if let targetWorkspaceID = notification.userInfo?["workspaceID"] as? UUID {
					guard targetWorkspaceID == self.currentWorkspaceID else {
						if Self.debugLoggingEnabled {
							print("[PromptVM] Skipping sync - notification for workspace \(targetWorkspaceID), but this VM is for \(self.currentWorkspaceID?.uuidString ?? "nil")")
						}
						return
					}
					...
					self.settingsManager.discardWindowOverrides(for: targetWorkspaceID)
					if Self.debugLoggingEnabled {
	...
	fileprivate func invalidateChatPromptEntriesCache() {
		...
		// Keep the last token baseline around. Its versioned key still forces exact callers to rebuild,
		// while chat preview can reuse it only when the mismatch is explicitly classified as stable.
	}
	...
	private func bumpChatPromptEntriesFileAPIsVersion(contentChanged: Bool = false) {
		chatFileAPIsVersion &+= 1
		if contentChanged {
			chatFileAPIsContentVersion &+= 1
		}
		invalidateChatPromptEntriesCache()
	}
	...
	private func flushAndSnapshotSourceTabIfNeeded(
		...
		case .duplicateCurrent:
			...
			return true
		case .forkDuplicate(let sourceTabID):
	...
	func switchComposeTab(_ id: UUID) async {
		...
		let task = Task { [weak self, weak manager] in
			...
			await self.withComposeTabSwitching(targetTabID: id) {
	...
	private func deleteStashedTabs(withIDs stashedTabIDs: Set<UUID>, expandCascade: Bool) async {
			...
			if !composeTabIDsBeingDeleted.isEmpty {
				...
				let fallbackActiveID: UUID? = {
					guard let previousActiveID,
						composeTabIDsBeingDeleted.contains(previousActiveID) else {
						return nil
					}
		...
		let tabIDs = Set(stashedTabsToDelete.map { $0.tab.id })
	...
	private func updateActiveTabDirtyState() {
		...
		DispatchQueue.main.async { [weak self] in
			guard let self = self else { return }
			self.isDirtyStateUpdateScheduled = false
			self.performActiveTabDirtyStateUpdate()
		}
	}
	...
	private func makeComposeTab(
		...
		case .preset(let preset):
			baseName = explicitName ?? preset.name
		default:
			baseName = explicitName ?? autoNameForNewTab(existing)
		}
		
		switch strategy {
		case .duplicateCurrent:
			guard let activeID = manager.workspaces[workspaceIndex].activeComposeTabID,
					let activeIdx = manager.workspaces[workspaceIndex].composeTabs.firstIndex(where: { $0.id == activeID }) else {
		...
		case .blank:
			...
			// Seed with pinned context builder prompts for discovery agent
			blank.discover.selectedContextBuilderPromptIDs = ContextBuilderPromptStorage.shared.pinnedPromptIDs
			return blank
		case .preset(let preset):
			...
			// Seed with pinned context builder prompts for discovery agent
			tab.discover.selectedContextBuilderPromptIDs = ContextBuilderPromptStorage.shared.pinnedPromptIDs
			return tab
		}
	}
	...
	func setAPISettingsViewModel(_ viewModel: APISettingsViewModel) {
		self.apiSettingsViewModel = viewModel
		self.setupAPISettingsObserver()
	}
			...
			getSettings: { [weak self] in
				guard let self = self else {
					return TokenCountingViewModel.TokenCalculationSettings(
						...
						xmlClipboardFormatPreference: .whole,
						allowDiffModelsToRewrite: true,
				...
				return TokenCountingViewModel.TokenCalculationSettings(
					fileTreeOption: self.fileTreeOption,
					codeMapUsage: self.effectiveCopyCodeMapUsage(),
					filePathDisplayOption: self.filePathDisplayOption,
					xmlClipboardFormatPreference: self.xmlClipboardFormatPreference,
					allowDiffModelsToRewrite: self.allowDiffModelsToRewrite,
					includeFilesInClipboard: self.includeFilesInClipboard,
					duplicateUserInstructionsAtTop: self.duplicateUserInstructionsAtTop,
					onlyIncludeRootsWithSelectedFiles: self.onlyIncludeRootsWithSelectedFiles,
					codeMapsGloballyDisabled: self.codeMapsGloballyDisabled
				)
			},
			getCopyContext: { [weak self] in
				guard let self = self else { return .default }
				return self.currentCopyContextSnapshot()
			}
		)
	}
	...
	private func currentCopyContextSnapshot() -> TokenCountingViewModel.CopyContextSnapshot {
		...
		let metadata = mcpMetadataBlockIfNeeded(for: resolved)
		return TokenCountingViewModel.CopyContextSnapshot(
			...
			xmlFormat: resolved.xmlFormat,
			fileTreeMode: resolved.effectiveFileTreeMode,
			...
			systemPromptFlavor: resolved.systemPromptFlavor
		)
	}
	...
	func loadPromptSectionOrder() {
		if let d = promptSectionsOrderRaw.data(using: .utf8),
			let decoded = try? JSONDecoder().decode([PromptSection].self, from: d) {
			
			// Check if we have all sections
			let decodedSet = Set(decoded)
			let allSectionsSet = Set(PromptSection.allCases)
			
			if decodedSet == allSectionsSet {
				// All sections present. If the user still has the previous
				// default order, migrate to the new default (Diff Formatting
				// before Meta Prompts). Otherwise, respect their custom order.
				let oldDefault: [PromptSection] = [ .fileMap, .fileContents, .gitDiff, .metaPrompts, .diffFormatting, .userInstructions ]
				if decoded == oldDefault {
					promptSectionsOrder = PromptAssemblyBuilder.defaultSectionOrder
					savePromptSectionOrder()
					print("PromptViewModel: Migrated prompt section order to new default (Diff Formatting before Meta Prompts).")
				} else {
					// Custom user order – use as-is
					promptSectionsOrder = decoded
				}
			} else if decodedSet.isSubset(of: allSectionsSet) && !decoded.isEmpty {
				// Missing some sections (like the new gitDiff), merge non-destructively
				let missingSections = allSectionsSet.subtracting(decodedSet)
				
				// Find the position after fileContents if it exists, otherwise append at end
				var mergedOrder = decoded
				if let fileContentsIndex = decoded.firstIndex(of: .fileContents),
					missingSections.contains(.gitDiff) {
					// Insert gitDiff right after fileContents
					mergedOrder.insert(.gitDiff, at: fileContentsIndex + 1)
				} else {
					// Append any missing sections at the end
					mergedOrder.append(contentsOf: missingSections)
				}
				
				promptSectionsOrder = mergedOrder
				savePromptSectionOrder() // Save the merged order
				
				// Log that we performed a merge
				print("PromptViewModel: Merged missing sections into custom order. Added: \(missingSections)")
			} else {
				// Invalid data or empty, use default
				promptSectionsOrder = PromptAssemblyBuilder.defaultSectionOrder
				savePromptSectionOrder() // Save the default order
			}
		} else {
			// No saved data or decoding failed, use default
			promptSectionsOrder = PromptAssemblyBuilder.defaultSectionOrder
			savePromptSectionOrder() // Save the default order
		}
	}
	...
	@MainActor
	func copyXMLInstructions(_ instructions: String) {
		let effectiveFormat = xmlCopyPromptFormat

		// Capture all needed properties before Task to minimize actor hopping
		let filePathDisplay = self.filePathDisplayOption
		let allowDiffRewrite = self.allowDiffModelsToRewrite
		let includeDatetimeInUserInstructions = self.includeDatetimeInUserInstructions
		let promptSectionsOrder = self.promptSectionsOrder
		let disabledPromptSections = self.disabledPromptSections
		let duplicateUserInstructionsAtTop = self.duplicateUserInstructionsAtTop
		// Always include meta prompts for XML copy
		let includeMetaPrompts = true
		let metaInstructions = self.metaInstructions
		let promptContext = resolvePromptContext()
		let codeMapUsage = promptContext.codeMapUsage
		...
		// Tree markers must reflect explicit selection membership, not prompt inclusion.
		let selectedIDs = explicitSelectedFileIDs()
		let fileTreeContent = Self.fileTreeContentIfNeeded(
			for: promptContext,
		...
		let metadataBlock = mcpMetadataBlockIfNeeded(for: promptContext)
		Task {
			...
			let gitDiff: String? = await {
				...
				case .none:
					return nil
				case .selected:
					return await self.gitViewModel.getDiffUsing(inclusionMode: .selectedFiles, forceRefreshStatus: true)
				case .complete:
					return await self.gitViewModel.getDiffUsing(inclusionMode: .all, forceRefreshStatus: true)
				}
			}()
			
			let clipboard = await PromptPackagingService.generateDiffClipboardContent(
				instructions: instructions,
				files: fileEntries,
				format: effectiveFormat,
				includeFiles: true,
				filePathDisplay: filePathDisplay,
				allowDiffRewrite: allowDiffRewrite,
	...
	func copyToClipboard() {
		...
		let metadataBlock = mcpMetadataBlockIfNeeded(for: promptContext)
		let metaInstructions = self.metaInstructions
		let promptText = self.promptText
		let codeMapUsage = promptContext.codeMapUsage
		let fileEntries = fileManager.buildPromptEntries(
			codeMapUsage: codeMapUsage,
			allFileAPIs: tokenCountingViewModel.cachedFileAPIs
		...
		let (diffEntries, _) = PromptPackagingService.partitionPromptEntriesForGitDiff(fileEntries)
		let filePathDisplayOption = self.filePathDisplayOption
		// Tree markers must reflect explicit selection membership, not prompt inclusion.
		let selectedIDs = explicitSelectedFileIDs()
		let fileTreeContent = Self.fileTreeContentIfNeeded(
			for: promptContext,
		...
		let includeDiffFormatting = false //self.includeDiffFormattingInClipboard
		let includeSavedPrompts = self.includeSavedPromptsInClipboard
		let includeUserPrompt = self.includeUserPromptInClipboard
		let xmlClipboardFormatPreference = self.xmlClipboardFormatPreference
		let includeDatetime = self.includeDatetimeInUserInstructions
		let promptSectionsOrder = self.promptSectionsOrder
		let disabledPromptSections = self.disabledPromptSections
		let duplicateUserInstructions = self.duplicateUserInstructionsAtTop
		let includeFiles = self.includeFilesInClipboard && !fileEntries.isEmpty
		...
		Task {
			...
			let clipboardContent = await PromptPackagingService.generateClipboardContent(
				...
				includeUserPrompt: includeUserPrompt,
				filePathDisplay: filePathDisplayOption,
				selectedXMLFormat: xmlClipboardFormatPreference,
	...
	func promptSelection(for context: PromptSelectionContext) -> Set<UUID> {
		...
		case .chat:
			return selectedPromptIDsForChat
		}
	}
	...
	func shouldShowXMLPicker(for context: PromptSelectionContext) -> Bool {
		switch context {
		case .copy:
			let preset = currentCopyPreset()
			let resolved = resolvePromptContext(preset, custom: workingCopyCustomizations)
			return (resolved.xmlFormat != nil) || (preset.xmlFormat != nil)
		case .chat:
			let preset = currentChatPreset()
			return preset.mode == .proEdit
		}
	}
	...
	func updateMetaInstructions() {
		...
		// For token counting, include both stored prompts AND MCP system prompts
		// Note: XML-related prompts (codeEditDiff, architectPlan) are counted separately via xmlFormattingPrompt
		var allInstructionsForCounting = metaInstructions.map { $0.content }
		
		// Add ONLY MCP system prompt content for token counting (if present)
		// XML prompts are already counted via xmlFormattingPrompt in TokenCountingViewModel
		let resolved = resolvePromptContext()
		if let flavor = resolved.systemPromptFlavor {
			// Only add MCP-specific prompts, not XML-related ones
			let isMCPPrompt = [SystemPromptFlavor.mcpAgent, .mcpPairProgram, .mcpDiscover].contains(flavor)
			if isMCPPrompt {
				let language = "Swift"  // Default language
				let systemContent = SystemPromptService.systemPrompt(for: flavor, language: language)
				if !systemContent.isEmpty {
					// Prepend MCP system prompt content for token counting
					allInstructionsForCounting.insert(systemContent, at: 0)
				}
			}
		}
	...
	/// Migrate prompts from UserDefaults to file storage if needed
	private func migratePromptsFromDefaultsIfNeeded() {
		let oldDefaultsKey = "StoredPrompts"
		let didMigrateKey = "DidMigratePrompts"
		...
		if let data = defaults.data(forKey: oldDefaultsKey),
		let oldPrompts = try? JSONDecoder().decode([StoredPrompt].self, from: data) {
			// Write them to the new JSON-based storage
			print("📦 Migrating \(oldPrompts.count) prompt(s) from UserDefaults to file storage...")
			PromptStorage.shared.savePrompts(oldPrompts)
		...
		// Mark migration as done, so we don't do it again
	...
	func loadStoredPrompts() {
		// Attempt the migration first (no-op if already done)
		migratePromptsFromDefaultsIfNeeded()

		// Now load from new JSON storage
		let loadResult = PromptStorage.shared.loadPrompts()

		// Track the count before we make changes to determine if we need to save
		let countBeforeLoad = storedPrompts.count
	...
	func updateAIQueriesService(_ newService: AIQueriesService?) {
		self.aiQueriesService = newService
	}
	...
	func sendPrompt() async throws {
		...
		do {
			guard let aiResponseViewModel = aiResponseViewModel else {
				throw PromptError.aiResponseViewModelNotAvailable
			}
			
			switch queryType {
			case .compose:
				/*
				selectedModel = AIModel(rawValue: preferredModel) ?? .claude4Sonnet
				let systemPrompt = SystemPromptService.getDiffGenerationPrompt()
				let aiMessage = await PromptPackagingService.generatePrompt(
					systemPrompt: systemPrompt,
					metaInstructions: selectedInstructionsText,
					userInstructions: promptText,
					files: fileManager.selectedFiles,
					includeFileTree: includeFolderDirectory,
					rootFolders: fileManager.rootFolders
				)
				try await aiQueriesService.updateProvider(model: selectedModel)
				let stream = try await aiQueriesService.sendPrompt(aiMessage)
				try await processStream(stream)
				*/
				break
			case .diff:
				await MainActor.run {
					//aiResponseViewModel.updateQueryIdentifier(self.queryIdentifier)
					aiResponseViewModel.updateRawOutput("")
					aiResponseViewModel.clearResponses()
				}
				// Process the parsed changes from DiffViewModel
				let changes = await diffViewModel.createFileChanges()
				await aiResponseViewModel.updateResponses(changes)
				await aiResponseViewModel.finalizeResponses()
			}
	...
	private func processStream(_ stream: AsyncThrowingStream<(String, [FileChanges]?, String?, Bool), Error>) async throws {
		do {
			for try await (rawOutput, responses, summary, isFinalized) in stream {
				if Task.isCancelled {
					throw CancellationError()
				}
				
				await MainActor.run { [weak self] in
					self?.aiResponseViewModel?.updateRawOutput(rawOutput)
					
					if let summary = summary {
						self?.aiResponseViewModel?.updateOverallSummary(summary)
					}
				}
				
				if let responses = responses {
					await self.aiResponseViewModel?.updateResponses(responses)
				}
				
				if isFinalized {
					await self.aiResponseViewModel?.finalizeResponses()
					await MainActor.run { [weak self] in
			...
			if error is CancellationError {
				await MainActor.run {
					self.aiResponseViewModel?.updateOverallSummary("Query cancelled by user.")
				}
			} else {
				throw error
			}
	...
	private func processDiffStream(_ stream: AsyncThrowingStream<FileChanges, Error>) async throws {
		let accumulator = FileChangesAccumulator()
		
		do {
			for try await fileChange in stream {
				if Task.isCancelled {
					throw CancellationError()
				}
				
				await accumulator.add(fileChange)
				
				let allChanges = await accumulator.getAll()
				await self.aiResponseViewModel?.updateResponses(allChanges)
			}
	...
	func generateNewQueryIdentifier(queryType: QueryType) {
		queryIdentifier = UUID()
		self.queryType = queryType
		aiResponseViewModel?.updateQueryIdentifier(queryIdentifier)
	}
	...
	func currentChatPreset() -> ChatPreset {
		...
		// Default to Edit to match the UI's initial selection
		return ChatPreset.BuiltIn.edit
	}
	...
	private func resolvedChatPreset(for id: UUID) -> ChatPreset {
		...
		if id == ChatPreset.BuiltIn.edit.id   { return ChatPreset.BuiltIn.edit }
		if id == ChatPreset.BuiltIn.proEdit.id { return ChatPreset.BuiltIn.proEdit }
	...
	func selectCopyPreset(_ id: UUID, applySettings: Bool = true, restoreManualSnapshot: Bool = true) {
		...
		if restoreManualSnapshot && !wasManual && preservedCustomizations == nil {
			let currentPreset = currentCopyPreset()
			let resolved = resolvePromptContext(currentPreset, custom: workingCopyCustomizations)
			if resolved.xmlFormat != nil || resolved.systemPromptFlavor != nil {
				var customizations = CopyCustomizations()
				customizations.xmlFormat = resolved.xmlFormat
				customizations.systemPromptFlavor = resolved.systemPromptFlavor
				preservedCustomizations = customizations
			}
		}
		...
		if applySettings && !willBeManual {
			...
			if workingCopyCustomizations.hasCustomizations {
				...
				overrides.systemPromptFlavor = nil
				overrides.xmlFormat = nil
			...
			let gitMode: GitDiffInclusionMode
			switch cfg.gitInclusion {
			case .none:     gitMode = .none
			case .selected: gitMode = .selectedFiles
			case .complete: gitMode = .all
			}
		...
		let presetForScanning = CopyPresetManager.shared.preset(with: id) ?? BuiltInCopyPresets.standard
		let cfgForScanning = resolvePromptContext(presetForScanning, custom: workingCopyCustomizations)
		if cfgForScanning.codeMapUsage != .none {
	...
	private func snapshotManualChatSettings(commit: Bool) {
		...
		s.manualProFileEdits = proFileEdits
	...
	private func restoreManualChatSettingsIfAvailable() {
		...
		if let v = s.manualPlanActMode { planActMode = v }
		if let v = s.manualProFileEdits { proFileEdits = v }
	...
	@MainActor
	func applyXMLFormatOverride(_ desiredFormat: ApplyPromptFormat?, for context: PromptSelectionContext) {
		switch context {
		case .copy:
			var overrides = workingCopyCustomizations
			if overrides.xmlFormat != desiredFormat {
				overrides.xmlFormat = desiredFormat
				workingCopyCustomizations = overrides
				markSettingsDirty()
			...
			if let desiredFormat {
				xmlCopyPromptFormat = desiredFormat
			}
		case .chat:
			// Chat presets currently rely on copy configuration when composing.
			// We intentionally avoid mutating chat state here until a dedicated
			// chat XML workflow exists.
			break
		}
	}
	
	@MainActor
	func applySystemPromptOverride(_ flavor: SystemPromptFlavor?, for context: PromptSelectionContext) {
		switch context {
		case .copy:
			var overrides = workingCopyCustomizations
			if overrides.systemPromptFlavor != flavor {
				overrides.systemPromptFlavor = flavor
				workingCopyCustomizations = overrides
				markSettingsDirty()
				// Update meta instructions to include the new system prompt content
				updateMetaInstructions()
				tokenCountingViewModel.markPromptDirty()
			}
		case .chat:
			// System prompt overrides for chat presets are managed through chat presets themselves.
			break
		}
	}
	...
	func packagePrompt(
		...
		let useProFileEdit: Bool = {
			// Pro Edit preset → use Pro Edit behavior.
			if preset.mode == .proEdit {
				return true
			}
			// Manual preset → respect user's toggle
			if preset.id == ChatPreset.BuiltIn.manual.id {
				return proFileEdits
			}
			// Other presets → standard editing
			return false
		}()
		
		// Determine effective mode (preserving previous behavior)
		let effectiveMode: PlanActMode = {
			if let override = overrideMode { return override }
			// For Manual preset, use the UI's planActMode (so that manual respects .plan/.edit etc)
			if preset.id == ChatPreset.BuiltIn.manual.id {
				return self.planActMode
			}
            switch preset.mode {
			case .chat:    return .chat
			case .plan:    return .plan
			case .edit:    return .edit
			case .proEdit: return .edit
		...
		case .edit:
			if useProFileEdit {
				let lang = SystemPromptService.predominantLanguage(from: selectedFilesForPrompt(selectionOverride))
				systemPrompt = SystemPromptService.architectPrompt(language: lang)
				// Keep default addWarning = true (matches previous semantics)
			} else {
				systemPrompt = getSystemPrompt(for: activeModel)
				addWarning = targetFileEditFormat(for: activeModel) != .none
			}
		}
		
		// Capture all needed properties before async operations
		let filePathDisplay = self.filePathDisplayOption
		let temperature = setModelTemperature ? modelTemperature : nil
		
		// Use override prompt config if provided, else default to current state
		let effectiveCodeMapUsage = activeConfig.codeMapUsage
		let includeFileTree = activeConfig.includeFileTree
		
		let allFileAPIs = tokenCountingViewModel.cachedFileAPIs
		let selectedFiles = selectedFilesForPrompt(selectionOverride)
		let fileEntries: [PromptFileEntry] = {
			if let selectionOverride {
				return fileManager.buildPromptEntries(
					for: selectionOverride,
		...
		let selectedCodeFiles = codeEntries.map(\.file)
		...
		let fileTreeString: String = {
			guard includeFileTree else { return "" }
			let selectedIDs = explicitSelectedFileIDs(for: selectionOverride)
			let tree = Self.fileTreeContentIfNeeded(
				for: activeConfig,
				rootFolders: fileManager.visibleRootFolders,
				selectedFileIDs: selectedIDs,
				filePathDisplay: filePathDisplay,
			...
			let hasCodemapEntries = codeEntries.contains(where: { $0.isCodemap && $0.file.fileAPI != nil })
			let defBlock: String = {
			...
			return [tree, defBlock].filter { !$0.isEmpty }.joined(separator: "\n\n")
		}()
		...
		let gitDiff: String? = await PromptPackagingService.resolveGitDiff(
			...
			case .selected:
				if selectionOverride != nil {
					let selectedPaths = selectedFiles.map(\.fullPath)
					return await gitViewModel.getDiffForAbsolutePaths(selectedPaths, vs: effectiveBase, forceRefreshStatus: true)
				}
				return await gitViewModel.getDiffUsing(inclusionMode: .selectedFiles, vs: effectiveBase, forceRefreshStatus: true)
			case .complete:
		...
		let aiMessage = PromptPackagingService.buildAIMessage(
			systemPrompt: systemPrompt,
	...
	@MainActor
	private func explicitSelectedFileIDs(for selectionOverride: StoredSelection? = nil) -> Set<UUID> {
		if let selectionOverride {
			return fileManager.computeSelectedIDs(from: selectionOverride)
		}
		return Set(fileManager.selectedFiles.map(\.id))
	}

	@MainActor
	private func selectedFilesForPrompt(_ selectionOverride: StoredSelection?) -> [FileViewModel] {
		guard let selectionOverride else {
			return fileManager.selectedFiles
		}
		return resolvedSelectedFiles(from: selectionOverride)
	}

	@MainActor
	private func resolvedSelectedFiles(from selection: StoredSelection) -> [FileViewModel] {
		var seen = Set<String>()
		var files: [FileViewModel] = []
		for rawPath in selection.selectedPaths {
	...
	func getSystemPrompt() -> String {
		if fileManager.visibleRootFolders.isEmpty {
			return "You are a helpful assistant operating in an app called Repo Prompt designed by the talented programmer Eric Provencher. His twitter handle is (@pvncher). The app works best if the user selects a folder. Chat with the user and answer any questions they may have the best of your knowledge. Always include a chatname in your response that describes the current chat. Put it on it's own line for easy parsing, and format it like so <chatName=\"Unique name describing user request\"/>"
		}

		let lang = SystemPromptService.predominantLanguage(from: fileManager.selectedFiles)

		switch targetFileEditFormat {
		case .none:
			return "You are a helpful assistant operating in an app called Repo Prompt designed by the talented programmer Eric Provencher. His twitter handle is (@pvncher). Chat with the user and answer any questions they may have the best of your knowledge. If you produce a file edit, at the end of your response, you can remind the user that changing the file edit format will be better suited to this task. Always include a chatname in your response that describes the current chat. Put it on it's own line for easy parsing, and format it like so <chatName=\"Unique name describing user request\"/>"
		case .diff:
			return SystemPromptService.codeEditPrompt(format: .diff,
														allowRewrite: allowDiffModelsToRewrite,
														language: lang)
		case .whole:
			return SystemPromptService.codeEditPrompt(format: .whole,
														language: lang)
		}
	}
	...
	func getChatPrompt() -> String {
		...
		End every response with a descriptive chatname tag:
	...
	func getSystemPrompt(for model: AIModel) -> String {
		...
		let lang = SystemPromptService.predominantLanguage(from: fileManager.selectedFiles)
		let format = targetFileEditFormat(for: model)
	...
	func performMergeAction() {
		generateNewQueryIdentifier(queryType: .diff)
		DispatchQueue.main.async { [weak self] in
			guard let self = self else { return }
			NotificationCenter.default.post(
				name: .switchToAIQueryView,
				object: nil,
				userInfo: ["windowID": self.windowID])
		}
	}
	
	func performChatMergeAction() {
		generateNewQueryIdentifier(queryType: .compose)
		DispatchQueue.main.async { [weak self] in
			guard let self = self else { return }
			NotificationCenter.default.post(
				name: .switchToAIQueryView,
	...
	func refreshAvailableModels() async {
		await apiSettingsViewModel?.loadStoredDataWithCompletion {
			self.doModelUpdate()
		}
	}
	...
	func doModelUpdate() {
		...
		self.availableModels = self.apiSettingsViewModel?.availableModels ?? []
		syncModelSelectionFromSettingsManager()
		self.validatePreferredModel()
	}
	...
	private func isModelPreferencePreservable(_ rawValue: String) -> Bool {
		...
		if let model = AIModel.fromModelName(rawValue) {
			...
			switch model.providerType {
			case .claudeCode, .codex, .geminiCli, .openCode, .cursor:
				return true
			default:
	...
	private func validatePreferredModel() {
		...
		if sessionPreferredModelOverrideRaw == nil && !isModelPreferencePreservable(self.preferredModel) {
			pickDiffCapableFallback()
		}

		// Validate dedicated planning model
		if !isModelPreferencePreservable(self.planningModelName) {
			pickPlanningModelFallback()
		}

		// Verify edit settings models - only fallback if provider is not configured
		verifyEditSettingsAvailability()
	}
	...
	@MainActor
	private func pickPlanningModelFallback() {
		// For planning model, we use the same priority as preferred model
		// 1) High diff
		if let model = AIModel.findBestAvailableModel(
			in: availableModels,
			desiredFormat: .diff,
			...
			setPlanningModelRaw(model.rawValue, markDirty: false, reason: "prompt.validate_planning_model.fallback.diff.high")
			return
		}
		// 2) Medium diff
		if let model = AIModel.findBestAvailableModel(
			...
			setPlanningModelRaw(model.rawValue, markDirty: false, reason: "prompt.validate_planning_model.fallback.diff.medium")
			return
		}
		// 3) Simple diff
		if let model = AIModel.findBestAvailableModel(
			...
			setPlanningModelRaw(model.rawValue, markDirty: false, reason: "prompt.validate_planning_model.fallback.diff.simple")
			return
		}
		
		// Fallback to preferred model or first available
		if !preferredModel.isEmpty {
			setPlanningModelRaw(preferredModel, markDirty: false, reason: "prompt.validate_planning_model.fallback.preferred")
		} else if !availableModels.isEmpty {
			setPlanningModelRaw(availableModels[0].rawValue, markDirty: false, reason: "prompt.validate_planning_model.fallback.first_available")
		} else {
			setPlanningModelRaw("", markDirty: false, reason: "prompt.validate_planning_model.fallback.empty")
		}
	}

	@MainActor
	private func verifyEditSettingsAvailability() {
		// Medium/Small => Whole
		if !isSettingAvailable(editSettingsMediumSmall, desiredFormat: .whole, fallbackList: AIModel.mediumWholePriority) {
			editSettingsMediumSmall = fallbackModelRaw(desiredFormat: .whole, priorities: AIModel.mediumWholePriority)
				?? fallbackProEditSettingRaw()
		}
		// Medium/Large => Diff
		if !isSettingAvailable(editSettingsMediumLarge, desiredFormat: .diff, fallbackList: AIModel.mediumDiffPriority) {
			editSettingsMediumLarge = fallbackModelRaw(desiredFormat: .diff, priorities: AIModel.mediumDiffPriority)
				?? fallbackProEditSettingRaw()
		}
		// High/Small => Whole
		if !isSettingAvailable(editSettingsHighSmall, desiredFormat: .whole, fallbackList: AIModel.highWholePriority) {
			editSettingsHighSmall = fallbackModelRaw(desiredFormat: .whole, priorities: AIModel.highWholePriority)
				?? fallbackProEditSettingRaw()
		}
		// High/Large => Diff
		if !isSettingAvailable(editSettingsHighLarge, desiredFormat: .diff, fallbackList: AIModel.highDiffPriority) {
			editSettingsHighLarge = fallbackModelRaw(desiredFormat: .diff, priorities: AIModel.highDiffPriority)
				?? fallbackProEditSettingRaw()
		}
	}
	
	/// Checks if an edit setting model is available.
	/// Uses provider-based validation instead of array membership for stability.
	@MainActor
	private func isSettingAvailable(
		_ rawValue: String,
		desiredFormat: FileEditFormat,
		fallbackList: [AIModel]
	) -> Bool {
		if let model = AIModel.fromModelName(rawValue), !isProEditSafeModel(model) {
			return false
		}
		return isModelCurrentlyUsable(rawValue)
	}
	
	private func fallbackModelRaw(
		desiredFormat: FileEditFormat,
		priorities: [AIModel]
	) -> String? {
		if let found = AIModel.findBestAvailableModel(in: availableProEditModels, desiredFormat: desiredFormat, priorities: priorities) {
			return found.rawValue
		}
		return nil
	}

	private func fallbackProEditSettingRaw() -> String {
		if let preferred = AIModel.fromModelName(preferredModel),
			isProEditSafeModel(preferred),
			isModelCurrentlyUsable(preferredModel) {
			return preferredModel
		}
		if let first = availableProEditModels.first {
			return first.rawValue
		}
		return AIModel.claude4Sonnet.rawValue
	}
	...
	@MainActor
	func getAppropriateEditSettings(
		fileSize: Int,
		...
		// --------- model selection ---------
		let isLargeFile = fileSize > Int(largeFileThreshold)
		
		let modelName: String
		switch difficulty {
		case .simple, .medium:
			modelName = isLargeFile ? editSettingsMediumLarge : editSettingsMediumSmall
		case .high:
			modelName = isLargeFile ? editSettingsHighLarge   : editSettingsHighSmall
		}
		
		let selectedModel = AIModel.fromModelName(modelName)
		let desiredFormat: FileEditFormat = isLargeFile ? .diff : .whole
		let fallbackPriorities: [AIModel] = {
			switch difficulty {
			case .simple, .medium:
				return isLargeFile ? AIModel.mediumDiffPriority : AIModel.mediumWholePriority
			case .high:
				return isLargeFile ? AIModel.highDiffPriority : AIModel.highWholePriority
			}
		}()
		let fallbackRaw = fallbackModelRaw(desiredFormat: desiredFormat, priorities: fallbackPriorities)
		let fallbackModel = fallbackRaw.flatMap(AIModel.fromModelName)
		let model = [selectedModel, fallbackModel, preferredAIModel, .claude4Sonnet]
			.compactMap { $0 }
			.first { isProEditSafeModel($0) && isModelAvailable($0) }
			?? fallbackModel
			?? .claude4Sonnet
		
		// Get language if not provided
		let lang = language ?? SystemPromptService.predominantLanguage(from: fileManager.selectedFiles)
		
		// --------- prompt / diff-vs-rewrite logic ---------
		// 1. Parallel jobs → *always* diff, never rewrite
		if isParallel, model.isModelCapableOfDiff {
			let prompt = SystemPromptService.getFileEditPromptFor(format: .diff, allowRewrite: false, language: lang)
			return (model, prompt)
		}
		
		// 2. If the model CAN produce diffs, decide whether rewrite is permitted
		if model.isModelCapableOfDiff {
			// Rewriting is never allowed on large files
			let allowRewriteForSize = !isLargeFile
			
			// Global flag + forceDiffOnly gate
			let allowRewrite =
			allowRewriteForSize &&
			!forceDiffOnly &&
			allowDiffModelsToRewrite            // global toggle from settings
			
			let prompt = SystemPromptService.getFileEditPromptFor(format: .diff,
																	allowRewrite: allowRewrite,
																	language: lang)
			return (model, prompt)
		}
		
		// 3. Fallback: model incapable of diffs → whole-file prompt
		let prompt = SystemPromptService.getFileEditPromptFor(format: .whole, language: lang)
		return (model, prompt)
	}
	
	@MainActor
	func autoConfigureProEditSettings() {
		// 3) Medium / small
		if let modelMedSmall = AIModel.findBestAvailableModel(
			in: availableProEditModels,
			desiredFormat: .whole,
			priorities: AIModel.mediumWholePriority) {
			editSettingsMediumSmall = modelMedSmall.rawValue
		}

		// 4) Medium / large
		if let modelMedLarge = AIModel.findBestAvailableModel(
			in: availableProEditModels,
			desiredFormat: .diff,
			priorities: AIModel.mediumDiffPriority) {
			editSettingsMediumLarge = modelMedLarge.rawValue
		}

		// 5) High / small
		if let modelHighSmall = AIModel.findBestAvailableModel(
			in: availableProEditModels,
			desiredFormat: .whole,
			priorities: AIModel.highWholePriority) {
			editSettingsHighSmall = modelHighSmall.rawValue
		}

		// 6) High / large
		if let modelHighLarge = AIModel.findBestAvailableModel(
			in: availableProEditModels,
			desiredFormat: .diff,
			priorities: AIModel.highDiffPriority) {
			editSettingsHighLarge = modelHighLarge.rawValue
		}
	}

	// MARK: - Agent Provider Migration

	/// Migrates to agent mode when validated agent providers are available.
	/// This only runs once globally using a single migration flag.
	/// Designed to be scalable: add new agent providers to the mapping and they'll auto-migrate.
	@MainActor
	func migrateToAgentModeIfNeeded() {
		// Check if already migrated - only migrate once ever
		guard !settingsManager.proEditAgentModeMigrated() else { return }

		// Skip if already using agent mode
		guard !proEditAgentMode else { return }

		// Define mapping of agent providers that can be auto-enabled
		// Key: UserDefaults connection key, Value: (agentKind, preferredModel)
		let agentProviders: [(connectionKey: String, agentKind: DiscoverAgentKind, preferredModel: AgentModel)] = [
			("ClaudeCodeConnected", .claudeCode, .claudeSonnet)
			// Add future agent providers here, e.g.:
			// ("CursorConnected", .cursor, .defaultModel)
		]

		// Check each provider - use the first connected one
		for provider in agentProviders {
			let isConnected = UserDefaults.standard.bool(forKey: provider.connectionKey)

			// If connected, enable agent mode with this provider
			if isConnected {
				proEditAgentMode = true
				proEditAgentKind = provider.agentKind
				proEditAgentModel = provider.preferredModel

				// Mark as migrated (globally - won't run again for other agents)
				settingsManager.setProEditAgentModeMigrated(true, commit: true)

				print("✓ Auto-enabled agent mode with \(provider.agentKind.commandName) (\(provider.preferredModel.displayName))")
				break
			}
		}
	}
	...
	func refreshCodeMapScanningState() {
		Task {
			await refreshCodeScanEnabledForEffectiveState()
		}
	}
...
extension PromptViewModel {
	...
	/// Returns the effective codeMapUsage after resolving presets, customizations, and global code-map state.
	...
	func effectiveChatCodeMapUsage() -> CodeMapUsage {
		codeMapsGloballyDisabled ? .none : codeMapUsageForChat
	}
	...
	func resolvePromptContext(_ preset: CopyPreset, custom: CopyCustomizations?) -> PromptContextResolved {
		...
		if xmlFormat == nil, let flavor = preset.systemPromptFlavor {
			...
			case .codeEditDiff, .codeEditWhole:
				...
				xmlFormat = self.xmlCopyPromptFormat
			default:
				break
			}
		}
	...
	private func shouldIncludeMCPMetadata(for config: PromptContextResolved) -> Bool {
		return config.includeMCPMetadata
	}
	
	func mcpMetadataBlockIfNeeded(
		for config: PromptContextResolved,
		tabOverride: (id: UUID, name: String)? = nil
	) -> String? {
		guard shouldIncludeMCPMetadata(for: config) else { return nil }
		return generateMCPMetadataBlock(tabOverride: tabOverride)
	}
	...
	func generateMCPMetadataBlock(
		...
		if hasMultipleWindows {
			...
			if let state = windows.first(where: { $0.windowID == self.windowID }) {
				if let instance = state.workspaceInstanceNumber {
					lines.append("workspace_instance: \(instance)")
				}
		...
		// Always include active tab info for easy binding
		let tabInfo: (id: UUID, name: String)? = {
			if let tabOverride {
				return (tabOverride.id, tabOverride.name.isEmpty ? "Tab" : tabOverride.name)
			}
			if let activeTabID = activeComposeTabID,
			let activeTab = currentComposeTabs.first(where: { $0.id == activeTabID }) {
				let name = activeTab.name.isEmpty ? "Tab" : activeTab.name
		...
		if let tabInfo {
			...
			lines.append("tab_id: \(tabInfo.id.uuidString)")
			lines.append("tab_name: \(tabInfo.name)")
		}

		// Include routing guide
		if includeWindowRouting {
			lines.append("")
			lines.append("context_binding_guide:")
			lines.append("  Before making tool calls to RepoPrompt, bind to this context:")
			
			var stepNum = 1
			// Only include window selection step if multiple windows
			if hasMultipleWindows {
				lines.append("  \(stepNum). Bind to the correct window:")
				lines.append("     tool: bind_context")
				lines.append("     args: {\"op\": \"bind\", \"window_id\": \(windowID)}")
				stepNum += 1
			}
			
			if let tabInfo {
				lines.append("  \(stepNum). Bind to this tab (recommended for stable context):")
				lines.append("     tool: bind_context")
				lines.append("     args: {\"op\": \"bind\", \"context_id\": \"\(tabInfo.id.uuidString)\"}")
			}
		}
...
extension PromptViewModel {
	internal static func fileTreeContentIfNeeded(
	...
	internal static func mapGitInclusion(
		custom: GitInclusion?,
		preset: GitInclusion?,
	...
	func metaInstructions(
		...
		// Compose meta prompts: system flavor (if any) + user-selected meta prompts (when requested)
		var combinedMeta: [MetaInstruction] = []
		if let flavor = cfg.systemPromptFlavor {
			// For XML formats, we already include the dedicated
			// <xml_formatting_instructions> section via PromptPackagingService.
			// Do NOT duplicate the system flavor as a meta prompt; it breaks
			// prompt section ordering by lumping edits into meta.
			if cfg.xmlFormat == nil {
				let systemTitle: String
				switch flavor {
				case .architectPlan:   systemTitle = "System: Architect Plan"
				case .codeEditDiff:    systemTitle = "System: Code Edit (Diff)"
				case .codeEditWhole:   systemTitle = "System: Code Edit (Whole)"
				case .review:          systemTitle = "System: Code Review"
				case .mcpAgent:        systemTitle = "System: MCP Agent"
				case .mcpPairProgram:  systemTitle = "System: MCP Pair Program"
				case .mcpPairPlan:     systemTitle = "System: MCP Pair Plan"
				case .mcpDiscover:     systemTitle = "System: MCP Discover"
				case .mcpBuilder:      systemTitle = "System: MCP Builder"
				}
				let systemContent = SystemPromptService.systemPrompt(for: flavor, language: language)
				combinedMeta.append(MetaInstruction(title: systemTitle, content: systemContent))
			}
		}
		...
		if let promptIds = cfg.storedPromptIds, !promptIds.isEmpty {
			...
			if let overrideIds = selectedPromptIDsOverride {
				...
				combinedMeta.append(contentsOf: self.metaInstructions)
			}
		}
	...
	func buildClipboard(for inputConfig: PromptContextResolved, promptTextOverride: String? = nil) async -> String {
		let cfg = applyingGlobalCodeMapOverride(inputConfig)
		let promptText = promptTextOverride ?? self.promptText
		let fileEntries = fileManager.buildPromptEntries(
		...
		// 1) File tree only (no codemap here - packaging will add it)
		let combinedTreeAndMap = Self.fileTreeContentIfNeeded(
			for: cfg,
			rootFolders: fileManager.visibleRootFolders,
			selectedFileIDs: explicitSelectedFileIDs(),
			filePathDisplay: filePathDisplayOption,
			onlyIncludeRootsWithSelectedFiles: onlyIncludeRootsWithSelectedFiles,
			showCodeMapMarkers: !codeMapsGloballyDisabled
		)
	...
	case .selected:
		gitDiff = diffEntries.isEmpty
			? await gitViewModel.getDiffUsing(inclusionMode: .selectedFiles, forceRefreshStatus: true)
			: nil
	case .complete:
		gitDiff = diffEntries.isEmpty
		...
		// 2.5) System/meta prompts assembly (include system flavor when appropriate)
		// Determine predominant language for system prompt generation
		let language = SystemPromptService.predominantLanguage(from: selectedCodeFiles)
		let combinedMeta = metaInstructions(for: cfg, language: language)
		let includeMetaBlock = !combinedMeta.isEmpty
		
		// 3) Generate clipboard string via existing packaging service
		let metadataBlock = mcpMetadataBlockIfNeeded(for: cfg)
		if let fmt = cfg.xmlFormat {
			// XML-edit clipboard (diff/whole/architect)
			return await PromptPackagingService.generateDiffClipboardContent(
				...
				filePathDisplay: filePathDisplayOption,
				allowDiffRewrite: allowDiffModelsToRewrite,
			...
			// Regular clipboard (no XML block)
	...
	private func promptContextTokenBaselineKey(_ cfg: PromptContextResolved) -> PromptContextTokenBaselineKey {
		PromptContextTokenBaselineKey(
			...
			includeFileTree: cfg.rendersFileTree,
			xmlFormat: cfg.xmlFormat,
			fileTreeMode: cfg.effectiveFileTreeMode,
			codeMapUsage: cfg.codeMapUsage,
			gitInclusion: cfg.gitInclusion,
			systemPromptFlavor: cfg.systemPromptFlavor,
			storedPromptIds: cfg.storedPromptIds ?? [],
			includeMCPMetadata: cfg.includeMCPMetadata
		)
	}
	...
	private func chatContextTokenBaselineCacheKey(
		...
		return ChatContextTokenBaselineCacheKey(
			...
			onlyIncludeRootsWithSelectedFiles: onlyIncludeRootsWithSelectedFiles,
			xmlClipboardFormatPreference: xmlClipboardFormatPreference,
			allowDiffModelsToRewrite: allowDiffModelsToRewrite,
			includeDatetimeInUserInstructions: includeDatetimeInUserInstructions,
			promptSectionsOrder: promptSectionsOrder,
			...
			fileAPIsContentVersion: chatFileAPIsContentVersion,
	...
	private func promptTextDuplicateFactor(for cfg: PromptContextResolved) -> Int {
		let canIncludePrompt = cfg.xmlFormat != nil || cfg.includeUserPrompt
		guard canIncludePrompt else { return 0 }
	...
	private func chatContextTokenBaselineMismatchReasons(
		cached: ChatContextTokenBaselineCacheKey,
		current: ChatContextTokenBaselineCacheKey
	) -> [String] {
		var reasons: [String] = []
		func appendIfChanged<T: Equatable>(_ name: String, _ lhs: T, _ rhs: T) {
			if lhs != rhs { reasons.append(name) }
		}

		appendIfChanged("workspaceID", cached.workspaceID, current.workspaceID)
		appendIfChanged("selectedChatPresetID", cached.selectedChatPresetID, current.selectedChatPresetID)
		appendIfChanged("chatPreset", cached.chatPreset, current.chatPreset)
		appendIfChanged("resolvedContext", cached.resolvedContext, current.resolvedContext)
		appendIfChanged("fileTreeOptionForChat", cached.fileTreeOptionForChat, current.fileTreeOptionForChat)
		appendIfChanged("codeMapUsageForChat", cached.codeMapUsageForChat, current.codeMapUsageForChat)
		appendIfChanged("gitDiffInclusionModeForChat", cached.gitDiffInclusionModeForChat, current.gitDiffInclusionModeForChat)
		appendIfChanged("codeMapsGloballyDisabled", cached.codeMapsGloballyDisabled, current.codeMapsGloballyDisabled)
		appendIfChanged("filePathDisplayOption", cached.filePathDisplayOption, current.filePathDisplayOption)
		appendIfChanged("selectedFilesSortMethod", cached.selectedFilesSortMethod, current.selectedFilesSortMethod)
		appendIfChanged("fileTreeSortMethod", cached.fileTreeSortMethod, current.fileTreeSortMethod)
		appendIfChanged("onlyIncludeRootsWithSelectedFiles", cached.onlyIncludeRootsWithSelectedFiles, current.onlyIncludeRootsWithSelectedFiles)
		appendIfChanged("xmlClipboardFormatPreference", cached.xmlClipboardFormatPreference, current.xmlClipboardFormatPreference)
		appendIfChanged("allowDiffModelsToRewrite", cached.allowDiffModelsToRewrite, current.allowDiffModelsToRewrite)
		appendIfChanged("includeDatetimeInUserInstructions", cached.includeDatetimeInUserInstructions, current.includeDatetimeInUserInstructions)
		appendIfChanged("promptSectionsOrder", cached.promptSectionsOrder, current.promptSectionsOrder)
		appendIfChanged("disabledPromptSections", cached.disabledPromptSections, current.disabledPromptSections)
		appendIfChanged("duplicateUserInstructionsAtTop", cached.duplicateUserInstructionsAtTop, current.duplicateUserInstructionsAtTop)
		appendIfChanged("selectedPromptIDsForChat", cached.selectedPromptIDsForChat, current.selectedPromptIDsForChat)
		appendIfChanged("hasManualChatPromptSelection", cached.hasManualChatPromptSelection, current.hasManualChatPromptSelection)
		appendIfChanged("storedPrompts", cached.storedPrompts, current.storedPrompts)
		appendIfChanged("hierarchyGenerationSignature", cached.hierarchyGenerationSignature, current.hierarchyGenerationSignature)
		appendIfChanged("rootOrder", cached.rootOrder, current.rootOrder)
		appendIfChanged("selectionVersion", cached.selectionVersion, current.selectionVersion)
		appendIfChanged("slicesVersion", cached.slicesVersion, current.slicesVersion)
		appendIfChanged("autoCodemapVersion", cached.autoCodemapVersion, current.autoCodemapVersion)
		appendIfChanged("fileAPIsVersion", cached.fileAPIsVersion, current.fileAPIsVersion)
		appendIfChanged("fileAPIsContentVersion", cached.fileAPIsContentVersion, current.fileAPIsContentVersion)
		appendIfChanged("fileSystemDeltaVersion", cached.fileSystemDeltaVersion, current.fileSystemDeltaVersion)
		return reasons
	}

	private func canUseStablePreviewBaseline(mismatchReasons: [String]) -> Bool {
		// TokenCountingViewModel.cachedFileAPIs can republish while preserving the same user-visible
		// chat context shape. Real code-map/FileAPI content changes also bump fileAPIsContentVersion,
		// so they are not eligible for stale preview reuse.
		!mismatchReasons.isEmpty && mismatchReasons.allSatisfy { $0 == "fileAPIsVersion" }
	}

	private func estimatedChatTokenCount(
		from cache: ChatContextTokenBaselineCache,
		promptTextTokens: Int,
		hasPromptText: Bool,
		duplicateFactor: Int,
		allowApproximatePromptTextDelta: Bool
	) -> Int? {
		if duplicateFactor == 0 {
			return cache.baseTokensWithoutPromptText
		}
		if hasPromptText {
			if cache.supportsPromptTextDeltas || allowApproximatePromptTextDelta {
				return cache.baseTokensWithoutPromptText + (promptTextTokens * duplicateFactor)
			}
			return nil
		}
		if let emptyPromptTokenCount = cache.emptyPromptTokenCount {
			return emptyPromptTokenCount
		}
		return allowApproximatePromptTextDelta ? cache.baseTokensWithoutPromptText : nil
	}

	private func recordChatContextTokenBaselineDiagnostic(_ diagnostic: String) {
		lastChatContextTokenBaselineDiagnostic = diagnostic
		#if DEBUG
		if Self.debugLoggingEnabled {
			print("[PromptVM] Chat token baseline \(diagnostic)")
		}
		#endif
	}
	...
	/// Exact callers keep building current clipboard context on misses. Typing/upcoming-token preview callers can opt into
	/// a stable baseline so high-churn, equivalent FileAPI republishes do not rebuild exact clipboard context on the MainActor.
	func calculateTokensForChatContext(preferStablePreviewBaseline: Bool = false) async -> Int {
		...
		let purpose = preferStablePreviewBaseline ? "preview" : "exact"
		var missReasonForStore: String?

		if !preferStablePreviewBaseline, cfg.gitInclusion != .none {
			recordChatContextTokenBaselineDiagnostic("bypass purpose=\(purpose) reason=git exactContext=true")
			let text = await buildClipboard(for: cfg)
			return estimateTokens(for: text)
		}

		if let cache = chatContextTokenBaselineCache {
			let mismatchReasons = chatContextTokenBaselineMismatchReasons(cached: cache.key, current: cacheKey)
			if mismatchReasons.isEmpty,
				let tokenCount = estimatedChatTokenCount(
					from: cache,
					promptTextTokens: promptTextTokens,
					hasPromptText: hasPromptText,
					duplicateFactor: duplicateFactor,
					allowApproximatePromptTextDelta: preferStablePreviewBaseline
				) {
				recordChatContextTokenBaselineDiagnostic("hit purpose=\(purpose) reason=exactKey git=\(cfg.gitInclusion.rawValue)")
				return tokenCount
			}

			if preferStablePreviewBaseline,
				canUseStablePreviewBaseline(mismatchReasons: mismatchReasons),
				let tokenCount = estimatedChatTokenCount(
					from: cache,
					promptTextTokens: promptTextTokens,
					hasPromptText: hasPromptText,
					duplicateFactor: duplicateFactor,
					allowApproximatePromptTextDelta: true
				) {
				recordChatContextTokenBaselineDiagnostic("hit purpose=\(purpose) reason=stablePreview mismatches=\(mismatchReasons.joined(separator: ",")) git=\(cfg.gitInclusion.rawValue)")
				return tokenCount
			}

			let reason = mismatchReasons.isEmpty ? "promptShape" : mismatchReasons.joined(separator: ",")
			missReasonForStore = reason
			recordChatContextTokenBaselineDiagnostic("miss purpose=\(purpose) reason=\(reason) git=\(cfg.gitInclusion.rawValue)")
		} else {
			missReasonForStore = "cold"
			recordChatContextTokenBaselineDiagnostic("miss purpose=\(purpose) reason=cold git=\(cfg.gitInclusion.rawValue)")
		}

		let text = await buildClipboard(for: cfg, promptTextOverride: promptTextSnapshot)
		...
		if currentCacheKey == cacheKey {
			...
			let afterMiss = missReasonForStore.map { " afterMiss=\($0)" } ?? ""
			recordChatContextTokenBaselineDiagnostic("store purpose=\(purpose) reason=exactBuild\(afterMiss) git=\(cfg.gitInclusion.rawValue)")
		} else {
			recordChatContextTokenBaselineDiagnostic("bypass purpose=\(purpose) reason=changedDuringBuild exactContext=true git=\(cfg.gitInclusion.rawValue)")
		}
	...
	/// By default this opens the Apply XML tab when an XML format is active.
	func performCopy(using preset: CopyPreset, promptTextOverride: String? = nil, openApplyXMLTab: Bool = true) {
		...
		Task {
			...
			NSPasteboard.general.clearContents()
			NSPasteboard.general.setString(clipboard, forType: .string)
			// Open Apply XML tab when copying XML-formatted content
			if openApplyXMLTab && cfg.xmlFormat != nil {
				NotificationCenter.default.post(
	...
	func applyChatPresetMode(using presetOverride: ChatPreset? = nil) {
		...
		let desiredMode: PlanActMode
		switch chatPreset.mode {
		case .chat:
			desiredMode = .chat
		case .plan:
			desiredMode = .plan
	...
	func applyChatPreset(_ preset: ChatPreset, using identifier: UUID? = nil) {
		...
		if let gitSetting = preset.gitInclusion {
			let newMode: GitDiffInclusionMode
			switch gitSetting {
			case .none:
				newMode = .none
			case .selected:
				newMode = .selectedFiles
			case .complete:
				newMode = .all
			}
	...
	// Do not toggle global Pro Edit state here; ChatViewModel decides per-send overrides.
	}
	...
	func resolvedPromptContext(from chatPreset: ChatPreset) -> PromptContextResolved? {
		...
		let resolvedGit: GitInclusion
		if let gi = chatPreset.gitInclusion {
			resolvedGit = gi
			...
			case .none:
				resolvedGit = .none
			case .selectedFiles:
				resolvedGit = .selected
			case .all:
				resolvedGit = .complete
			}
		} else {
			switch gitDiffInclusionModeForChat {
			case .none:
		...
		return PromptContextResolved(
			...
			xmlFormat: nil,
			fileTreeMode: chatPreset.fileTreeMode ?? self.fileTreeOptionForChat,
			codeMapUsage: codeMapsGloballyDisabled ? .none : (chatPreset.codeMapUsage ?? self.codeMapUsageForChat),
...
enum PromptError: Error {
	...
	case aiResponseViewModelNotAvailable
}
...
extension Notification.Name {
	static let switchToAIQueryView = Notification.Name("switchToAIQueryView")
