...
class CopyPresetManager: ObservableObject {
    ...
    func load() {
        ...
        overrides = Dictionary(document.copyOverrides.map { ($0.presetID, $0) }, uniquingKeysWith: { _, new in new })
    }
    ...
    /// Create a "Your Settings" preset from legacy settings
    func createLegacySettingsPreset(
        includeFiles: Bool,
        includeUserPrompt: Bool,
        includeMetaPrompts: Bool,
        includeFileTree: Bool,
        ...
        // Check if we already have a "Your Settings" preset
        if let existing = userPresets.first(where: { $0.name == "Your Settings" }) {
            return existing
        }
    ...
    private func apply(overrides: CopyPresetOverrides, to base: CopyPreset) -> CopyPreset {
        ...
        return CopyPreset(
            ...
            xmlFormat: overrides.xmlFormat ?? base.xmlFormat,
            ...
            systemPromptFlavor: overrides.systemPromptFlavor ?? base.systemPromptFlavor,
            storedPromptIds: overrides.storedPromptIds ?? base.storedPromptIds,
            notes: base.notes,
            includeMCPMetadata: overrides.includeMCPMetadata ?? base.includeMCPMetadata
        )
    }
