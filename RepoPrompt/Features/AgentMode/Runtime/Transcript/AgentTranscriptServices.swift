...
#if DEBUG
struct AgentTranscriptProtectedTailScanMetrics: Sendable, Equatable {
...
struct AgentTranscriptCompactionMetrics: Sendable, Equatable {
...
struct AgentTranscriptWorkingSourceItemsMetrics: Sendable, Equatable {
	let transcriptTurnCount: Int
...
struct AgentTranscriptRebuildMetrics: Sendable, Equatable {
...
struct AgentTranscriptProjectionBuildMetrics: Sendable, Equatable {
	let turnCount: Int
...
struct AgentTranscriptRefreshAttemptMetrics: Sendable, Equatable {
	let reason: String
...
struct AgentTranscriptPresentationPublishMetrics: Sendable, Equatable {
...
struct AgentTranscriptSessionItemsReplacementMetrics: Sendable, Equatable {
	let reason: String
...
struct AgentTranscriptProjectionIdentityMetrics: Sendable, Equatable {
...
enum AgentTranscriptDebugInstrumentation {
	...
	nonisolated(unsafe) private static let refreshSignatureLock = NSLock()
	nonisolated(unsafe) private static var lastRefreshInputSignatureByTabID: [UUID: String] = [:]
...
enum BashToolResultParser {
	...
	private static let livenessScanCharacterLimit = 32_768
...
final class AgentToolResultProcessingContext: @unchecked Sendable {
	...
	static let maxCachedJSONBytes = 16_384
...
fileprivate struct ClusterSummaryCacheKey: Hashable {
	let rowIDs: [UUID]
}

fileprivate final class AgentTranscriptProjectionBuildContext {
...
fileprivate struct CollapsibleLeafDescriptor {
...
fileprivate func mergedToolExecutionMetadata(
...
struct AgentTranscriptImportPolicy: Sendable, Equatable {
...
enum AgentTranscriptIO {
	...
	static func workingSourceItems(from transcript: AgentTranscript) -> [AgentChatItem] {
		...
		if AgentTranscriptDebugInstrumentation.isEnabled {
			AgentTranscriptDebugInstrumentation.workingSourceItemsHandler?(.init(
				...
				fullTurnCount: transcript.turns.filter { $0.retentionTier == .full }.count,
	...
	fileprivate static func buildCollapsedSummary(
		...
		let hiddenAssistantCount = visibleActivities.filter {
			($0.itemKind == .assistant || $0.itemKind == .assistantInline)
				&& AgentDisplayableText.hasDisplayableBody($0.text)
		}.count
		let hiddenProgressCount = visibleActivities.filter { $0.itemKind == .thinking }.count
		let hiddenNoteCount = visibleActivities.filter {
	...
	private static func middleSummaryText(
		...
		let assistantMessageCount = activities.filter {
			($0.itemKind == .assistant || $0.itemKind == .assistantInline)
				&& AgentDisplayableText.hasDisplayableBody($0.text)
		}.count
		var parts: [String] = []
...
enum AgentTranscriptCompactionMode: Sendable, Equatable {
	case recomputeAll
...
enum AgentTranscriptCompactor {
	...
	private struct CompactionPressure {
		...
		init(transcript: AgentTranscript) {
			self.workingUnitCount = AgentTranscriptCompactor.estimatedWorkingUnitCount(for: transcript)
		}
...
struct AgentTranscriptTurnProjectionCache: Sendable, Equatable {
	struct ValidationToken: Sendable, Equatable {
		let turnID: UUID
...
struct AgentTranscriptProjectionBuildResult: Sendable, Equatable {
...
enum AgentTranscriptProjectionBuilder {
	...
	static func projectionCounts(for transcript: AgentTranscript) -> AgentTranscriptProjectionCounts {
		...
		for turn in transcript.turns {
			...
			switch turn.retentionTier {
			case .full:
				visibleRowCount = fullTurnVisibleRowCount(
					for: turn,
					detailedToolTailLimit: detailedToolTailLimits[turn.id] ?? 0
				)
			case .condensed, .summary:
				visibleRowCount = compactTierVisibleRowCount(for: turn)
			case .archived:
				visibleRowCount = compactedVisibleRowCount(for: turn)
			}
	...
	private static func turnID(
		...
		switch targetID {
		case .row(let rowID):
			return transcript.turns.first { turn in
	...
	private static func turnID(for anchor: AgentTranscriptAnchor) -> UUID {
		switch anchor {
		case .request(let turnID), .summary(let turnID), .groupedHistory(let turnID, _):
			return turnID
		case .activity(let turnID, _, _), .conclusion(let turnID, _):
			return turnID
		}
	}
	...
	fileprivate static func standaloneToolBlockCount(for turn: AgentTranscriptTurn) -> Int {
		...
		return turn.responseSpans.reduce(into: 0) { partial, span in
			...
			partial += kinds.filter { $0 == .standaloneTool }.count
		}
	}
	...
	private static func blocksForTurn(
		...
		let unsortedBlocks: [AgentTranscriptRenderBlock]
		if protectDetachedFocus, turn.retentionTier != .archived {
			unsortedBlocks = fullBlocks(
				for: turn,
				archived: archived,
			...
			case .condensed, .summary:
				unsortedBlocks = compactTierBlocks(
					for: turn,
					archived: archived,
					context: context
				)
			case .archived:
				unsortedBlocks = compactedBlocks(for: turn, archived: archived)
			}
		}
	...
	private static func estimatedBlockCount(for turn: AgentTranscriptTurn, detailedToolTailLimit: Int) -> Int {
		switch turn.retentionTier {
		case .full:
			return estimatedFullBlockCount(for: turn, detailedToolTailLimit: detailedToolTailLimit)
		case .condensed, .summary:
			return compactTierBlockCount(for: turn)
		case .archived:
			return 0
	...
	private static func compactTierBlockCount(for turn: AgentTranscriptTurn) -> Int {
		...
		for span in turn.responseSpans {
			...
			let contentCount = leafDescriptors.filter { $0.kind != .conclusion }.count
			let conclusionCount = leafDescriptors.filter { $0.kind == .conclusion }.count
	...
	private static func collapseOlderHistory(
		...
		if !plan.groupedPrefix.isEmpty {
			let frozenSummary: AgentTranscriptGroupedHistorySummary?
			if let cache = span.fullRenderGroupedHistoryCache,
				cache.detailedToolTailLimit == detailedToolTailLimit,
				cache.collapseDigest == plan.collapseDigest {
				frozenSummary = cache.summary
			} else {
				frozenSummary = nil
			}
	...
	private static func groupedHistorySummary(
		...
		let hiddenProgressCount = childBlocks.reduce(0) { partial, block in
			partial + block.rows.filter { $0.kind == .thinking }.count
		}
		let hiddenNoteCount = childBlocks.reduce(0) { partial, block in
			partial + block.rows.filter { $0.kind == .system || $0.kind == .error }.count
		}
		let summaryRows = (summarySourceBlocks ?? childBlocks).flatMap(\.rows)
		let summary = AgentTranscriptGroupedHistorySummary(
			hiddenToolCardCount: childBlocks.filter { $0.kind == .standaloneTool }.count,
			hiddenAssistantCount: hiddenAssistantCount,
			hiddenProgressCount: hiddenProgressCount,
			hiddenNoteCount: hiddenNoteCount,
	...
	private static func groupedSectionAffinity(for block: AgentTranscriptRenderBlock) -> GroupedSectionAffinity {
		...
		case .standaloneTool, .standaloneAssistant:
			return .activity
		case .standaloneNote:
			return block.rows.contains(where: { $0.kind == .thinking }) ? .activity : .notes
		case .request, .activityCluster, .groupedHistory, .middleSummary, .conclusion:
			return .mixed
		}
	}
	...
	private static func clusterSummary(
		...
		if let narration, !narration.isEmpty {
			shortNarration = narration.count > 120 ? String(narration.prefix(120)) + "…" : narration
		} else {
			shortNarration = nil
		}
	...
	private static func conclusionRow(for turn: AgentTranscriptTurn) -> AgentChatItem? {
		if let conclusionActivityID = turn.conclusionActivityID,
			...
			switch turn.retentionTier {
			case .full, .condensed:
				text = displayableAssistantText(activity.text)
			case .summary, .archived:
				text = displayableAssistantText(turn.summary?.compactConclusionText)
	...
	private static func displayableAssistantText(_ text: String?) -> String? {
		guard let text,
			AgentDisplayableText.hasDisplayableBody(text) else {
			return nil
		}
		return text
	}
...
enum AgentTranscriptAnalyticsBuilder {
	static func build(from transcript: AgentTranscript, selectedAgent: DiscoverAgentKind?) -> AgentTranscriptAnalyticsSnapshot {
