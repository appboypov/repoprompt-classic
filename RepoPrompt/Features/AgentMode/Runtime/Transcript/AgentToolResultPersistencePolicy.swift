...
enum AgentToolResultPersistencePolicy {
	static let maxPersistedToolSummaryBytes = 2_048
	private static let cursorACPPersistedDiffBytesLimit = 1_200
	private static let cursorACPFallbackPersistedDiffBytesLimit = 600
	private static let cursorACPGeneratedDiffInputBytesLimit = 16_384
	...
	static func persistedToolResultSummary(
		...
		let allowedStructuredSummary: String? = {
			...
			guard let normalizedToolName,
				...
				!exceedsPersistedToolSummaryBudget(resultJSON) else {
				return nil
			}
			...
			if normalizedToolName == "agent_manage",
				...
				intValue(object, keys: ["deleted_count", "deletedCount"]) != nil
					|| intValue(object, keys: ["skipped_count", "skippedCount"]) != nil {
				return resultJSON
			}
			return nil
		}()
	...
	static func sanitizeTranscriptWithMetrics(
		...
		let reusedTurnCount: Int = {
			switch purpose {
			case .runtimePresentation:
				return validatedReusablePrefixTurnCount(
					in: transcript,
		...
		let containsSanitizableActivities: Bool = {
			switch purpose {
			case .runtimePresentation:
				return containsToolResultActivities(in: transcript)
			case .persistentStorage:
				return containsToolExecutionActivities(in: transcript)
			}
		}()
		...
		let preservedRowIDs: Set<UUID> = {
			switch purpose {
			case .runtimePresentation:
				// Runtime projections re-apply visible raw payloads from the ephemeral
				// payload map after sanitization. Keep the canonical transcript/source
				// suffix sanitized so persistence and derived-state reuse stay aligned.
				return preservedVisibleToolResultRowIDs ?? []
			case .persistentStorage:
				return []
			}
		}()
	...
	private static func sanitizeTurn(
		...
		for spanIndex in turn.responseSpans.indices {
			...
			for activityIndex in turn.responseSpans[spanIndex].activities.indices {
				...
				switch purpose {
				case .runtimePresentation:
					didSanitize = sanitizeRuntimeActivity(
						&activity,
						preservedVisibleToolResultRowIDs: preservedVisibleToolResultRowIDs,
						context: context
					)
				case .persistentStorage:
					didSanitize = sanitizePersistentStorageActivity(&activity, context: context)
				}
	...
	private static func sanitizedSummaryJSON(
		...
		case "ask_oracle", "oracle_send", "chat_send":
	...
	private static func oracleChatSummaryJSON(
		...
		let errors = compactErrorStrings(from: rawErrors)
	...
	private static func compactErrorStrings(from rawErrors: Any?) -> [String] {
		guard let rawErrors = rawErrors as? [Any] else { return [] }
		return rawErrors.prefix(3).compactMap { error in
			let text: String?
			if let string = error as? String {
				text = string
			} else if let object = error as? [String: Any] {
				text = stringValue(object, keys: ["message", "error", "description"])
			} else {
				text = nil
			}
			return clippedCursorACPString(text ?? "", limit: 240)?.value
		}
	}
