...
actor AgentSessionDataService {
	...
	enum FastMetadataRecordsSource: String, Sendable {
		case memoryIndex
		case diskIndex
	}

	struct FastMetadataRecordsResult: Sendable {
	...
	private struct AgentSessionHeader: Decodable {
		...
		let geminiACPResumeMetadata: GeminiACPResumeMetadata?
	...
	private func normalizeLoadedSession(
		...
		let repairedWorkingCount: Int
		if let repairTerminalState {
			repairedWorkingCount = AgentTranscriptQualityRepair.finalizePendingTerminalTools(
				in: &workingItems,
	...
	private func metadataIndexNeedsFilenameReconciliation(_ index: AgentSessionMetadataIndex, folder: URL) throws -> Bool {
		...
		#endif
		let fileNames = Set(try agentSessionFiles(in: folder).map(\.lastPathComponent))
	...
	private func scheduleMetadataIndexReconciliationIfNeeded(
		...
		WorkspaceRestorePerfLog.event(
			...
			fields: [
				...
				"delayMS": "\(Int((effectiveDelaySeconds * 1_000).rounded()))",
				"reason": reason,
		...
		let task = Task(priority: .utility) { [weak self] in
			...
			await self.reconcileMetadataIndexInBackground(
				folder: folder,
	...
	func loadAgentSession(from fileURL: URL) async throws -> AgentSession {
		...
		do {
			...
			if let persistedSession = normalized.persistedSessionToRewrite {
				...
				await upsertMetadataRecordIfIndexPresent(normalized.runtimeSession, fileURL: fileURL)
			}
			return normalized.runtimeSession
		} catch {
	...
	func loadAgentSessionStub(
		...
		do {
			...
			return AgentSession(
				...
				geminiACPResumeMetadata: header.geminiACPResumeMetadata,
				...
				codexServiceTier: header.codexServiceTier,
	...
	func listAgentSessionsMeta(
		...
		do {
			...
			let files = try await listAgentSessions(for: workspace)
			let boundedFiles: ArraySlice<URL>
			if let limit {
				boundedFiles = files.prefix(max(limit, 0))
			} else {
				boundedFiles = files[...]
			}
