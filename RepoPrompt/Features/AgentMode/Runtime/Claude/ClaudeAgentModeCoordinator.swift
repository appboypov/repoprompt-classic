...
final class ClaudeAgentModeCoordinator {
	...
	private func updateProviderSessionIDIfNeeded(
		...
		guard let candidate = candidate?.trimmingCharacters(in: .whitespacesAndNewlines),
			!candidate.isEmpty,
			session.providerSessionID != candidate else {
			return false
		}
	...
	private func finalizeSession(
		...
		await viewModel?.prepareForTerminalPersistence(
			session: session,
			terminalState: state,
			reason: "claude-coordinator-finalize"
		)
	...
	func events(for session: AgentModeViewModel.TabSession) async -> AsyncStream<ClaudeNativeProcessSessionController.Event>? {
	...
	func scheduleApplyCurrentClaudeModelAndEffortIfPossible(
		...
		guard session.selectedAgent.usesClaudeNativeRuntime,
			session.claudeController != nil else {
			return
		}
		Task { @MainActor [weak self, weak session] in
			guard let self, let session else { return }
			await self.applyCurrentClaudeModelAndEffortIfPossible(for: session, reason: reason)
		}
	}
	...
	func applyCurrentClaudeModelAndEffortIfPossible(
		...
		guard session.selectedAgent.usesClaudeNativeRuntime,
			let controller = session.claudeController else {
			return
		}
	...
	func ensureClaudeNativeSession(
		...
		do {
			...
			await finalizeSession(session, state: .failed, save: true)
		}
	}

	private func startOrResumeWithFallback(
		controller: any ClaudeSessionControlling,
		session: AgentModeViewModel.TabSession,
		runID: UUID,
		model: String?,
		...
		do {
			...
			viewModel?.stageClaudeResumeRecoveryHandoffIfNeeded(for: session)
	...
	private func shouldRetryFreshStartWithoutResume(
		...
		guard
			...
			let controllerError = error as? ClaudeNativeProcessSessionController.ControllerError
		else {
			return false
		}
	...
	private func awaitSteeringInterruptSafePoint(
		...
		let deadline = ContinuousClock.now.advanced(by: .milliseconds(Int64(effectiveTimeoutSeconds * 1_000)))
		while true {
	...
	func sendClaudeNativeMessage(
		...
		guard let controller = session.claudeController else {
			await finalizeSession(session, state: .failed)
			return false
		}
		...
		if hadTurnInFlight {
			if let runID = session.runID {
				...
				case .ready:
					break
				case .timedOut(_, _, let stillActive) where !stillActive:
		...
		do {
			...
			if let viewModel {
				outboundText = viewModel.prependPendingHandoffIfNeeded(text, session: session)
			} else {
				outboundText = text
			}
	...
	func submitApprovalDecision(
		...
		guard let request = session.pendingApproval,
			...
			case .claudeControl(let requestID) = request.requestID
		else {
			return
		}
	...
	func prepareClaudeCancelSync(_ session: AgentModeViewModel.TabSession) -> (any ClaudeSessionControlling)? {
	...
	func beginClaudeResumeTransferIfNeeded(
		for session: AgentModeViewModel.TabSession,
		oldController: (any ClaudeSessionControlling)?
	) {
		...
		session.pendingClaudeResumeTransferTask = Task { @MainActor [self, session] in
			return await self.cancelClaudeRunAndCaptureSessionRef(session, oldController: oldController)
		}
	}
	...
	func cancelClaudeRun(_ session: AgentModeViewModel.TabSession, oldController: (any ClaudeSessionControlling)?) async {
	...
	private func cancelClaudeRunAndCaptureSessionRef(
		_ session: AgentModeViewModel.TabSession,
		oldController: any ClaudeSessionControlling
	) async -> ClaudeNativeProcessSessionController.SessionRef {
		let controller = oldController
		let interruptOutcome = await controller.interruptTurn(reason: "interrupt")
		// Clean up tool tracking immediately — no need to wait for the grace period.
		await clearClaudeToolTracking(for: session)
	...
	private func clearClaudeToolTracking(for session: AgentModeViewModel.TabSession) async {
	...
	private func agentModeSystemPromptOverride(for session: AgentModeViewModel.TabSession) -> String? {
		switch ClaudeAgentToolPreferences.agentModePromptDelivery() {
		case .userMessageXML:
			return nil
		case .userMessageXMLWithEmptySystemPrompt:
			return ""
		case .nativeSystemPrompt:
			return agentModeInstructionInjection(for: session)
		}
	}
	...
	private func providerBoundUserMessage(_ outboundText: String, instructions: String) -> String {
		...
		case .userMessageXML, .userMessageXMLWithEmptySystemPrompt:
			return ClaudeCodePromptDelivery.decoratedUserMessage(outboundText, instructions: instructions)
		case .nativeSystemPrompt:
			return outboundText
		}
	}
