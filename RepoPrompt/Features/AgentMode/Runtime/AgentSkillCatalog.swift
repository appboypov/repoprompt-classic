...
struct AgentSkillDefinition: Equatable, Hashable, Identifiable, Sendable {
	...
	enum Source: Int, Sendable {
		...
		// Codex / Gemini roots (generic .agents namespace)
...
final class AgentSkillCatalog {
	...
	private enum SkillRootFormat: Sendable {
	...
	private struct SkillRoot: Sendable {
		let url: URL
	...
	private struct ScannedSkillDefinition: Sendable {
		let definition: AgentSkillDefinition
		let definitionFileKey: String
	}

	private struct DiscoveredSkill: Sendable {
		let definition: AgentSkillDefinition
		let precedenceRank: Int
		let rootOrdinal: Int
	}
	...
	private var loadedAgentKind: DiscoverAgentKind?
	...
	func scheduleRefresh(workspacePaths: [String], agentKind: DiscoverAgentKind) {
		refreshTask?.cancel()
		refreshGeneration &+= 1
		let generation = refreshGeneration
		refreshTask = Task { @MainActor [weak self] in
			guard let self else { return }
			await self.refresh(workspacePaths: workspacePaths, agentKind: agentKind, generation: generation)
	...
	func refresh(workspacePaths: [String], agentKind: DiscoverAgentKind, generation: UInt64? = nil) async {
	...
	func refreshIfNeeded(workspacePaths: [String], agentKind: DiscoverAgentKind) async {
		let canonicalPaths = workspacePaths.compactMap { Self.canonicalWorkspacePath($0) }
		if hasLoaded,
			loadedWorkspacePaths == canonicalPaths,
			loadedAgentKind == agentKind,
	...
	nonisolated private static func canonicalWorkspacePath(_ path: String) -> String? {
		let trimmed = path.trimmingCharacters(in: .whitespacesAndNewlines)
		guard !trimmed.isEmpty else { return nil }
		return URL(fileURLWithPath: trimmed).standardizedFileURL.path
	}

	private func discoverSkills(workspacePaths: [String], agentKind: DiscoverAgentKind) async -> [AgentSkillDefinition] {
	...
	private func skillSearchRoots(workspacePaths: [String], agentKind: DiscoverAgentKind) -> [SkillRoot] {
		var roots: [SkillRoot] = []
		...
		case .codexExec, .gemini, .openCode, .cursor:
			// Codex, Gemini, OpenCode, and Cursor continue to share only the generic `.agents` namespace.
			appendWorkspaceRoots(
				relativeRoot: ".agents/skills",
				source: .workspaceAgentsSkills,
				format: .skillFolderSKILLMd,
	...
	nonisolated private static func scanSkillDefinitions(
		in directory: URL,
		source: AgentSkillDefinition.Source,
		format: SkillRootFormat,
		...
		switch format {
		case .skillFolderSKILLMd:
			return scanSkillFolderDefinitions(
				in: directory,
				source: source,
				rootDirectoryKey: rootDirectoryKey,
				fileManager: fileManager
		...
		case .legacyFlatMd:
			return scanLegacyFlatDefinitions(in: directory, source: source, fileManager: fileManager)
		}
	}

	/// Scan for `SKILL.md` files inside per-skill subdirectories (e.g. `.claude/skills/deploy/SKILL.md`).
	nonisolated private static func scanSkillFolderDefinitions(
		in directory: URL,
		source: AgentSkillDefinition.Source,
		rootDirectoryKey: String,
		fileManager: FileManager
	...
	nonisolated private static func scanLegacyFlatDefinitions(
		in directory: URL,
		source: AgentSkillDefinition.Source,
		fileManager: FileManager
	) -> [ScannedSkillDefinition] {
	...
	nonisolated private static func canonicalExistingDirectoryKey(
		for url: URL,
		fileManager: FileManager
	) -> String? {
	...
	nonisolated private static func canonicalExistingFileKey(
		for url: URL,
		fileManager: FileManager
	) -> String? {
	...
	nonisolated private static func existingPathKey(
		for url: URL,
	...
	nonisolated private static func parseSkillDefinition(from fileURL: URL, source: AgentSkillDefinition.Source) -> AgentSkillDefinition? {
	...
	nonisolated private static func parseLegacyFlatDefinition(from fileURL: URL, source: AgentSkillDefinition.Source) -> AgentSkillDefinition? {
		guard let content = try? String(contentsOf: fileURL, encoding: .utf8) else {
			return nil
		}
		let frontmatter = parseFrontmatter(from: content)
	...
	nonisolated private static func buildDefinition(
		content: String,
	...
	nonisolated private static func parseFrontmatter(from content: String) -> [String: String] {
	...
	nonisolated private static func renderPromptDirectoryTree(
	...
	nonisolated private static func renderFolderTree(
		rootURL: URL,
		fileManager: FileManager,
		homeDirectoryURL: URL,
	...
	@discardableResult
	nonisolated private static func appendDirectoryTree(
		at directoryURL: URL,
		prefix: String,
		depth: Int,
		lines: inout [String],
	...
	nonisolated private static func directoryEntries(
		at directoryURL: URL,
		fileManager: FileManager
	) -> [DirectoryTreeEntry]? {
	...
	nonisolated private static func displayPath(_ url: URL, homeDirectoryURL: URL) -> String {
