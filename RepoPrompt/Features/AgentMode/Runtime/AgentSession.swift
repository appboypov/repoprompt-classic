...
enum AgentSessionError: Error, LocalizedError {
	...
	var localizedDescription: String {
		switch self {
		case .emptySession:
			return "Cannot save an empty agent session"
		case .invalidFilename(let name):
			return "Invalid agent session filename: \(name)"
		case .decodingFailed(let error):
			return "Failed to decode agent session: \(error.localizedDescription)"
		case .loadFailed(let error):
			return "Failed to load agent session: \(error.localizedDescription)"
		case .noActiveWorkspace:
			return "No active workspace for agent session"
		case .invalidHandoffCutoff:
			return "The selected handoff point is no longer available in this session transcript."
		}
	}
...
struct AgentTokenUsagePersist: Codable, Equatable, Sendable {
	...
	init(from decoder: Decoder) throws {
		...
		promptTokens = max(0, try container.decode(Int.self, forKey: .promptTokens))
		completionTokens = max(0, try container.decode(Int.self, forKey: .completionTokens))
		...
		estimatedUserInputTokens = max(0, try container.decodeIfPresent(Int.self, forKey: .estimatedUserInputTokens) ?? 0)
		estimatedToolInputTokens = max(0, try container.decodeIfPresent(Int.self, forKey: .estimatedToolInputTokens) ?? 0)
		estimatedToolOutputTokens = max(0, try container.decodeIfPresent(Int.self, forKey: .estimatedToolOutputTokens) ?? 0)
...
struct GeminiACPResumeMetadata: Codable, Equatable, Sendable {
	var runtimeSessionID: String?
	var loadSessionID: String?
	var loadSessionIDVerifiedAt: Date?

	init(
		runtimeSessionID: String? = nil,
		loadSessionID: String? = nil,
		loadSessionIDVerifiedAt: Date? = nil
	) {
		self.runtimeSessionID = runtimeSessionID
		self.loadSessionID = loadSessionID
		self.loadSessionIDVerifiedAt = loadSessionIDVerifiedAt
	}
}

struct AgentSession: Codable, Identifiable, Sendable {
	static let currentSerializationVersion = 4
	...
	/// Agent kind used for this session (e.g., "claudeCode", "codexExec", "gemini")
	...
	var geminiACPResumeMetadata: GeminiACPResumeMetadata?
	var autoEditEnabled: Bool

	/// Persisted per-turn token usage for non-Codex providers (Claude/Gemini).
	...
	init(
		...
		geminiACPResumeMetadata: GeminiACPResumeMetadata? = nil,
		...
		self.geminiACPResumeMetadata = geminiACPResumeMetadata
	...
	init(from decoder: Decoder) throws {
		...
		geminiACPResumeMetadata = try container.decodeIfPresent(GeminiACPResumeMetadata.self, forKey: .geminiACPResumeMetadata)
		...
		codexServiceTier = try container.decodeIfPresent(String.self, forKey: .codexServiceTier)
		...
		self.pendingHandoffDefersProviderLockUntilSend = try container.decodeIfPresent(Bool.self, forKey: .pendingHandoffDefersProviderLockUntilSend) ?? false
		self.isMCPOriginated = try container.decodeIfPresent(Bool.self, forKey: .isMCPOriginated) ?? false
	}
