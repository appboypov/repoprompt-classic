...
final class AgentModeProviderBindingService {
	...
	func providerSubagentPolicy(for providerID: AgentProviderBindingID) -> ProviderSubagentPermissionPolicy {
		AgentModePermissionPreferences.providerSubagentPolicy(for: providerID, defaults: preferences.defaults, secureStore: preferences.securePermissions)
	}

	func setProviderSubagentPolicy(
		_ policy: ProviderSubagentPermissionPolicy,
		for providerID: AgentProviderBindingID
	) {
		AgentModePermissionPreferences.setProviderSubagentPolicy(policy, for: providerID, defaults: preferences.defaults, secureStore: preferences.securePermissions)
	}
	...
	func applyAutoEditGuidanceAction(
		...
		case .setCodexReadOnly:
			return setPermissionLevel(.codex(.readOnly))
		case .setClaudeRequireApproval:
			return setPermissionLevel(.claude(.requireApproval))
		}
	}
		...
		for session in sessions where session.selectedAgent.providerBindingID == providerID {
			...
			case .gemini, .openCode:
				...
				Task { @MainActor in
					...
					if DiscoverAgentService.enableDebugLogging { print("[ACP-Runner] tab=\(session.tabID) applying \(providerName) session mode=\(sessionModeID)") }
					do {
						...
						if DiscoverAgentService.enableDebugLogging { print("[ACP-Runner] tab=\(session.tabID) failed to apply \(providerName) session mode=\(sessionModeID) error=\(error.localizedDescription)") }
					}
					if session.tabID == currentTabID {
						updateActiveBindings(session)
					}
