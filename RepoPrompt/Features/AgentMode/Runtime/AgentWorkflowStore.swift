...
final class AgentWorkflowStore: ObservableObject {
	...
	private init() {
		if let saved = UserDefaults.standard.stringArray(forKey: Self.hiddenBuiltInKey) {
			self.hiddenBuiltInIDs = Set(saved)
		} else {
			self.hiddenBuiltInIDs = Self.defaultHiddenBuiltInIDs
		}
		self.featuredWorkflowIDs = UserDefaults.standard.stringArray(forKey: Self.featuredWorkflowIDsKey)
