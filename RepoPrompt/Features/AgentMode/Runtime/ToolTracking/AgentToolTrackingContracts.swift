...
struct AgentToolTrackingHooks: Sendable {
	...
	let flushPendingAssistantDelta: (_ session: AgentModeViewModel.TabSession) -> Void

	/// End the active streaming assistant segment.
	let endActiveAssistantSegment: (_ session: AgentModeViewModel.TabSession) -> Void

	/// End the active reasoning segment.
	let endActiveReasoningSegment: (_ session: AgentModeViewModel.TabSession) -> Void

	/// Seal the assistant boundary so subsequent content starts a new bubble.
	let sealAssistantBoundary: (_ session: AgentModeViewModel.TabSession) -> Void

	/// Request a UI refresh for the given tab.
	let requestUIRefresh: (_ tabID: UUID, _ urgent: Bool) -> Void

	/// Schedule a persistence save for the given tab.
	let scheduleSave: (_ tabID: UUID) -> Void

	/// Account for tool input tokens from args payload.
	let addToolInputTokens: (_ payload: String?, _ session: AgentModeViewModel.TabSession) -> Void

	/// Account for tool output tokens from result payload.
	let addToolOutputTokens: (_ payload: String?, _ session: AgentModeViewModel.TabSession) -> Void
}
