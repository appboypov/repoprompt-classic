...
enum AgentModelCatalog {
	struct AvailabilityContext: Sendable {
		...
		func filteredForRecommendationProviders(_ providers: Set<RecommendationProviderKind>) -> AvailabilityContext {
			AvailabilityContext(
				...
				geminiAvailable: geminiAvailable && providers.contains(.geminiCLI),
				openCodeAvailable: false,
		...
		func isCompatibleBackendConfigured(_ id: ClaudeCodeCompatibleBackendID) -> Bool {
			switch id {
			case .glmZAI:
				return zaiConfigured
			case .kimi:
				return kimiConfigured
			case .custom:
				return customClaudeCompatibleConfigured
			}
		}

		func assumingAvailable(_ agentKind: DiscoverAgentKind) -> AvailabilityContext {
			AvailabilityContext(
				claudeCodeAvailable: claudeCodeAvailable || agentKind == .claudeCode,
				codexAvailable: codexAvailable || agentKind == .codexExec,
				geminiAvailable: geminiAvailable || agentKind == .gemini,
	...
	enum AgentSelectionSurface: Sendable, Equatable {
		case general
		case sandboxedDelegateEdit

		func allows(_ agentKind: DiscoverAgentKind) -> Bool {
			switch self {
			case .general:
				return true
			case .sandboxedDelegateEdit:
				// Pro Edit delegate runs require a sandbox/permission model RepoPrompt can control.
				// Cursor ACP only exposes canned modes and approval responses; it cannot disable
				// Cursor's built-in file/shell tools, so it is not safe for sandboxed delegate edits.
				return agentKind != .cursor
			}
		}
	}

	struct NormalizedAgentSelection: Sendable, Equatable {
	...
	static let supportedCLIProviderAgents: [DiscoverAgentKind] = [
		.codexExec,
		.claudeCode,
		.gemini,
		.openCode,
	...
	static func selectableAgents(
		...
		[.codexExec, .claudeCode, .gemini, .openCode, .cursor, .claudeCodeGLM, .kimiCode, .customClaudeCompatible]
	...
	static func isAgentAvailable(
		_ agentKind: DiscoverAgentKind,
		availability: AvailabilityContext = .current
	) -> Bool {
		switch agentKind {
		case .claudeCodeGLM:
			return availability.zaiConfigured
		case .kimiCode:
			return availability.kimiConfigured
		case .customClaudeCompatible:
			return availability.customClaudeCompatibleConfigured
		case .claudeCode:
			return availability.claudeCodeAvailable
		case .codexExec:
			return availability.codexAvailable
		case .gemini:
			return availability.geminiAvailable
		case .openCode:
			return availability.openCodeAvailable
		case .cursor:
			return availability.cursorAvailable
		}
	}
	...
	static func defaultModelRaw(
		...
		case .kimiCode:
			return AgentModel.kimiCode.rawValue
		case .customClaudeCompatible:
			return defaultCompatibleBackendModelRaw(for: .customClaudeCompatible)
		case .claudeCode:
			// Resolve to a concrete static alias so users see a clear picker option
			// instead of the vague "Default" placeholder.
			return AgentModel.claudeOpus.rawValue
		case .codexExec, .gemini, .openCode:
	...
	static func normalizeSelection(
		...
		if agent == .claudeCodeGLM {
			candidateModelRaw = canonicalClaudeGLMModelRaw(candidateModelRaw)
		} else if isCompatibleBackendAgent(agent) {
			candidateModelRaw = canonicalCompatibleBackendModelRaw(candidateModelRaw, for: agent)
		}
	...
	static func options(
		...
		case .claudeCode:
			let staticOptions = AgentModel.modelsForAgent(agentKind)
				.filter { isAvailable($0, for: agentKind, availability: availability) }
				.map { staticOption($0, for: agentKind) }
			return includeClaudeEffortVariants
				? expandedClaudeOptions(from: staticOptions, agentKind: agentKind)
				: staticOptions
		case .claudeCodeGLM, .kimiCode, .customClaudeCompatible:
			let staticOptions = compatibleBackendOptions(for: agentKind)
			return includeClaudeEffortVariants
				? expandedClaudeOptions(from: staticOptions, agentKind: agentKind)
				: staticOptions
		case .gemini, .openCode, .cursor:
	...
	static func isValid(
		...
		if agentKind.usesClaudeTooling {
			if isCompatibleBackendAgent(agentKind) {
			...
			guard let baseModel = specifier.baseModel else {
				if let effort = specifier.effortLevel {
					return agentKind == .claudeCode && claudeEffort(effort, isSupportedForBaseModelRaw: nil, agentKind: agentKind)
				}
	...
	static func displayName(
		...
		func baseDisplayName(for raw: String) -> String {
			if let compatibleDisplayName = compatibleBackendDisplayName(forRequestedModelRaw: raw, agentKind: agentKind) {
		...
		if agentKind.usesClaudeTooling {
			...
			if compatibleBackendModelBehavior(for: agentKind) == .noModel {
				return baseName
			}
	...
	static func openCodeMenu(for options: [AgentModelOption]) -> OpenCodeMenu {
		...
		let groups = groupedEntries.values.compactMap { groupEntries -> OpenCodeMenuGroup? in
			...
			let menuOptions = sortedEntries.map { entry in
				...
				let displayName: String
				if rendersAsSubmenu {
					displayName = variantDisplayName ?? "Default"
				} else {
					displayName = entry.providerID == nil ? entry.option.displayName : entry.modelDisplayName
				}
	...
	static func codexMenu(for options: [AgentModelOption]) -> CodexMenu {
		...
		func stripEffortSuffix(_ label: String) -> String {
			...
			guard let last = tokens.last,
				CodexReasoningEffort.parse(last) != nil else {
				return trimmed
			}
			tokens.removeLast()
		...
		func displayNameForBaseModel(
			...
			guard let serviceTier = specifier.serviceTier,
				let baseModel = specifier.baseModel else {
	...
	static func claudeMenu(for options: [AgentModelOption], agentKind: DiscoverAgentKind? = nil) -> ClaudeMenu {
		let defaultOption = options.first { $0.isPlaceholderDefault }
		let modelOptions = options.filter { !$0.isPlaceholderDefault }
		guard !modelOptions.isEmpty else {
	...
	static func modelOptionIsSelected(
		...
		guard selectedSpecifier.effortLevel == nil,
			let optionEffort = optionSpecifier.effortLevel else {
			return false
		}
	...
	static func updateLastUsedEffortIfEncoded(
		...
		if agentKind.usesClaudeTooling {
			...
			guard let effort = specifier.effortLevel,
				claudeEffort(effort, isSupportedForBaseModelRaw: specifier.baseModel, agentKind: agentKind) else {
				return false
			}
	...
	static func updateClaudeLastUsedEffortIfEncoded(
		...
		guard let effort = specifier.effortLevel,
			...
			updateLastUsedEffortIfEncoded(agentKind: agentKind, rawModel: rawModel, defaults: defaults) else {
			return nil
		}
		return effort
	}

	private static func expandedClaudeOptions(from baseOptions: [AgentModelOption], agentKind: DiscoverAgentKind) -> [AgentModelOption] {
	...
	private static func claudeEffort(
		...
		if let agentKind, isCompatibleBackendAgent(agentKind) {
			return false
		}
	...
	private static func strippedClaudeEffortSuffix(from label: String) -> String {
		...
		var tokens = trimmed.split(separator: " ").map(String.init)
		guard let last = tokens.last,
			ClaudeCodeEffortLevel.parse(last) != nil else {
			return trimmed
		}
		tokens.removeLast()
		return tokens.joined(separator: " ").trimmingCharacters(in: .whitespacesAndNewlines)
	...
	private static func claudeBaseModelKey(_ rawModel: String, agentKind: DiscoverAgentKind) -> String {
		if let normalized = canonicalCompatibleBackendModelRaw(rawModel, for: agentKind) {
	...
	private static func resolvedACPDiscoveredModels(
		...
		guard let providerID = agentKind.acpProviderID,
			...
			!snapshot.options.isEmpty else {
			return nil
		}
		return snapshot
	}

	private static func normalizedAgentKind(_ rawValue: String?) -> DiscoverAgentKind? {
		guard let rawValue else { return nil }
		let trimmed = rawValue.trimmingCharacters(in: .whitespacesAndNewlines)
		guard !trimmed.isEmpty else { return nil }
		return DiscoverAgentKind(rawValue: trimmed)
	}
	...
	private static func compatibleBackendID(for agentKind: DiscoverAgentKind) -> ClaudeCodeCompatibleBackendID? {
		switch agentKind {
		case .claudeCodeGLM:
			return .glmZAI
		case .kimiCode:
		...
		case .customClaudeCompatible:
			return .custom
		case .claudeCode, .codexExec, .gemini, .openCode, .cursor:
			return nil
		}
	}

	private static func isCompatibleBackendAgent(_ agentKind: DiscoverAgentKind) -> Bool {
		compatibleBackendID(for: agentKind) != nil
	}

	private static func compatibleBackendConfig(for agentKind: DiscoverAgentKind) -> ClaudeCodeCompatibleBackendConfig? {
		compatibleBackendID(for: agentKind).map { ClaudeCodeCompatibleBackendStore.shared.config(for: $0).normalized }
	}

	private static func compatibleBackendModelBehavior(for agentKind: DiscoverAgentKind) -> ClaudeCodeCompatibleBackendConfig.ModelBehavior? {
	...
	private static func noModelRawValue(for id: ClaudeCodeCompatibleBackendID) -> String {
		...
		case .kimi:
			return AgentModel.kimiCode.rawValue
		case .custom:
			return AgentModel.customClaudeCompatible.rawValue
		}
	}

	private static func defaultCompatibleBackendModelRaw(for agentKind: DiscoverAgentKind) -> String {
		guard let id = compatibleBackendID(for: agentKind),
			let config = compatibleBackendConfig(for: agentKind) else {
			return AgentModel.defaultModel.rawValue
		}
		switch config.modelBehavior {
		case .noModel:
			return noModelRawValue(for: id)
		case .claudeSlotMapping:
			return AgentModel.claudeSonnet.rawValue
		}
	}

	private static func compatibleBackendOptions(for agentKind: DiscoverAgentKind) -> [AgentModelOption] {
		guard let id = compatibleBackendID(for: agentKind),
	...
	private static func noModelDisplayName(
		...
		case .kimi:
			return "Kimi Code"
		case .glmZAI, .custom:
			return config.normalizedDisplayName
		}
	}
	...
	private static func canonicalCompatibleBackendBaseRaw(_ rawModel: String?, for agentKind: DiscoverAgentKind) -> String? {
		guard let id = compatibleBackendID(for: agentKind),
			let config = compatibleBackendConfig(for: agentKind) else { return nil }
		switch config.modelBehavior {
		case .noModel:
		...
		case .claudeSlotMapping:
			...
			return ClaudeCodeGLMIntegration.normalizedSlotModel(
				base,
				config: config,
				acceptGLMLegacyAliases: id == .glmZAI
			)
		}
	}

	private static func canonicalCompatibleBackendModelRaw(_ rawModel: String?, for agentKind: DiscoverAgentKind) -> String? {
		let specifier = ClaudeModelSpecifier(raw: rawModel)
	...
	private static func isValidCompatibleBackendModel(
		...
		case .claudeSlotMapping:
			...
			guard let base = canonicalSpecifier.baseModel,
				[AgentModel.claudeHaiku.rawValue, AgentModel.claudeSonnet.rawValue, AgentModel.claudeOpus.rawValue].contains(base) else {
				return false
			}
	...
	private static func canonicalModelRaw(_ rawModel: String, for agentKind: DiscoverAgentKind) -> String {
		...
		guard let discoveredOption = resolvedACPDiscoveredModels(for: .cursor)?.option(matching: rawModel),
			isCursorAutoOption(discoveredOption) else {
			return rawModel
		}
		return AgentModel.cursorAuto.rawValue
	}
	...
	private static func staticOption(_ model: AgentModel, for agentKind: DiscoverAgentKind) -> AgentModelOption {
		let displayName: String
		let description: String?
		if let compatibleDisplayName = compatibleBackendDisplayName(forRequestedModelRaw: model.rawValue, agentKind: agentKind) {
			displayName = compatibleDisplayName
			description = compatibleBackendDescription(forRequestedModelRaw: model.rawValue, agentKind: agentKind)
		} else {
	...
	static func selectableAgent(
		...
		guard !trimmed.isEmpty else { return nil }
		guard let match = DiscoverAgentKind.allCases.first(where: {
	...
	struct DiscoveryStartTarget: Sendable {
	...
	struct DiscoveryModel: Sendable {
		let id: String
		let name: String
		let description: String?
		let available: Bool
		...
		var hasMultipleTargets: Bool { startTargets.count > 1 }
	}

	/// Default selection metadata for an agent.
	struct DiscoveryDefaults: Sendable {
		let modelRaw: String?
		let reasoningEffort: CodexReasoningEffort?
		let selectionID: AgentModelSelectionID?
	}
	...
	struct DiscoveryAgent: Sendable {
		let agent: DiscoverAgentKind
		let description: String
		let available: Bool
		let runtime: String
	...
	enum TaskLabelKind: String, CaseIterable, Sendable {
		case explore
		case engineer
		case pair
		case design
	...
	struct DiscoveryTaskLabel: Sendable {
		let label: String
		let description: String
	...
	private static func candidateChain(for kind: TaskLabelKind) -> [SelectionCandidate] {
		...
		case .explore:
			return [
				...
				SelectionCandidate(agent: .gemini, modelRaw: AgentModel.gemini3FlashPreview.rawValue),
	...
	private static func isCandidateAvailable(
		...
		let hasDynamicModels = allOptions.contains(where: { $0.isProviderDefault })
		if !hasDynamicModels {
	...
	static func discoveryAgents(
		...
		DiscoverAgentKind.allCases.map { agent in
	...
	static func resolveSelectionID(
		...
		guard let agent = DiscoverAgentKind(rawValue: parsed.agentRaw) else { return nil }
	...
	private static func discoveryAgent(
		...
		let models: [DiscoveryModel]
		if agent == .codexExec {
			models = codexDiscoveryModels(
				from: agentOptions,
				agent: agent,
				defaultModelRaw: defaultRaw ?? ""
			)
		} else if agent.usesClaudeTooling {
			models = claudeDiscoveryModels(
				from: agentOptions,
				agent: agent,
				defaultModelRaw: defaultRaw ?? ""
			)
		} else {
			models = agentOptions.map { option in
	...
	private static func discoveryCapabilities(for agent: DiscoverAgentKind) -> [String] {
