...
struct ClaudeModelSpecifier: Sendable, Equatable {
	...
	var runtimeModelParam: String? {
		guard let baseModel = baseModel?.trimmingCharacters(in: .whitespacesAndNewlines),
			!baseModel.isEmpty,
			baseModel.caseInsensitiveCompare(AgentModel.defaultModel.rawValue) != .orderedSame else {
			return nil
		}
		return baseModel
	}

	var explicitEffortLevel: ClaudeCodeEffortLevel? { effortLevel }
}
