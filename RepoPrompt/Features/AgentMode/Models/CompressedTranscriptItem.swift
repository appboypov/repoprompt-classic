...
extension CompressedTranscriptItem {
	...
	static func compress<C: Collection>(_ items: C) -> [CompressedTranscriptItem] where C.Element == AgentChatItem {
	...
	static func hasRenderableDifference(_ lhs: [CompressedTranscriptItem], _ rhs: [CompressedTranscriptItem]) -> Bool {
		...
		for (left, right) in zip(lhs, rhs) {
			switch (left, right) {
			case (.single(let leftItem), .single(let rightItem)):
				if leftItem.id != rightItem.id {
					return true
				}
			case (.toolGroup(let leftGroup), .toolGroup(let rightGroup)):
