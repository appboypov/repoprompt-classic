...
public struct AgentChatItemPersist: Codable, Identifiable, Sendable, Equatable {
	...
	public init(from item: AgentChatItem, sanitizeToolResults: Bool = true) {
		...
		self.toolInvocationID = item.toolInvocationID
		self.toolArgsJSON = sanitizeToolResults && (item.kind == .toolCall || item.kind == .toolResult) ? nil : item.toolArgsJSON
		...
		self.isLocalControlPlaneEcho = item.isLocalControlPlaneEcho
		self.toolResultStatus = nil
		...
		if sanitizeToolResults, item.kind == .toolResult {
			...
			self.toolResultStatus = statusWord
			self.text = minimalJSON
			self.toolResultJSON = minimalJSON
			self.toolIsError = persistedSummary?.toolIsError ?? item.toolIsError
		} else if sanitizeToolResults, item.kind == .toolCall {
			self.text = AgentToolResultPersistencePolicy.sanitizedToolCallText(toolName: item.toolName)
			self.toolResultJSON = nil
			self.toolIsError = item.toolIsError
		} else {
			self.text = item.text
			self.toolResultJSON = item.toolResultJSON
			self.toolIsError = item.toolIsError
		}
	}
...
public enum AgentSessionRunState: String, Codable, Sendable, Equatable {
	...
	var touchesSidebarVisibleActivityOnTransition: Bool {
		switch self {
