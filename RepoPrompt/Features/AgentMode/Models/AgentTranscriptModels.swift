...
struct AgentTranscriptFullRenderGroupedHistoryCache: Sendable, Equatable {
...
enum AgentDetachedAuthorityFamily: Equatable, Sendable {
	case request(UUID)
...
func agentDetachedAuthorityFamily(for anchor: AgentTranscriptAnchor?) -> AgentDetachedAuthorityFamily? {
	...
	switch anchor {
	case .request(let turnID):
		return .request(turnID)
	case .activity(let turnID, let spanID, _), .groupedHistory(let turnID, let spanID):
		return .responseSpan(turnID: turnID, spanID: spanID)
	case .conclusion(let turnID, _):
		return .conclusion(turnID)
	case .summary(let turnID):
		return .summary(turnID)
	}
}

struct DetachedViewportAuthority: Equatable, Sendable {
...
enum AgentDetachedRebaseAction: Equatable, Sendable {
	case none
	case acceptDrift
	case restoreIntent
}
...
struct AgentTranscriptViewportState: Sendable, Equatable {
...
enum AgentTranscriptProjectionProtection: Sendable, Equatable {
	case none
	case protectedTurn(UUID)

	var protectedTurnID: UUID? {
		guard case .protectedTurn(let turnID) = self else { return nil }
		return turnID
	}
}
...
struct AgentTranscriptPresentationMetadata: Sendable, Equatable {
	...
	func hasScrollAffectingPresentationDelta(comparedTo other: Self) -> Bool {
		dynamicSummaryLockTargetTurnID != other.dynamicSummaryLockTargetTurnID
			|| recentAssistantItemIDs != other.recentAssistantItemIDs
			|| activeContextBuilderCallItemID != other.activeContextBuilderCallItemID
			|| activeContextBuilderResultItemID != other.activeContextBuilderResultItemID
			|| mostRecentEditItemID != other.mostRecentEditItemID
	}
...
struct AgentTranscriptPresentationSnapshot: Sendable, Equatable {
	let tabID: UUID?
	let revision: Int
	...
	func hasScrollAffectingPresentationDelta(comparedTo other: Self) -> Bool {
		hasVisiblePresentationDelta(comparedTo: other)
			|| rawToolResultPayloadRenderRevision != other.rawToolResultPayloadRenderRevision
			|| metadata.hasScrollAffectingPresentationDelta(comparedTo: other.metadata)
	}

	var scrollAffectingPresentationKey: AgentTranscriptScrollAffectingPresentationKey {
		AgentTranscriptScrollAffectingPresentationKey(snapshot: self)
	}

	static let empty = AgentTranscriptPresentationSnapshot()
}

struct AgentTranscriptScrollAffectingPresentationKey: Sendable, Equatable {
	let presentationRevision: Int
	let rawToolResultPayloadRenderRevision: Int
	let metadata: AgentTranscriptPresentationMetadata

	init(snapshot: AgentTranscriptPresentationSnapshot) {
		self.presentationRevision = snapshot.revision
		self.rawToolResultPayloadRenderRevision = snapshot.rawToolResultPayloadRenderRevision
		self.metadata = snapshot.metadata
	}
}

struct AgentArchivedHistoryState: Sendable, Equatable {
...
struct AgentArchivedTranscriptSnapshot: Sendable, Equatable {
...
struct AgentToolResultProcessingMetrics: Sendable, Equatable {
...
struct AgentTranscriptPerformanceSnapshot: Sendable, Equatable {
...
struct AgentTranscriptAnalyticsSnapshot: Sendable, Equatable {
