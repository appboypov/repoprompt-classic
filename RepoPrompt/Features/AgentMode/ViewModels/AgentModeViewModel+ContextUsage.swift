...
extension AgentModeViewModel {
	func nonCodexContextUsageEstimator(for agent: DiscoverAgentKind) -> (any ContextUsageEstimating)? {
		switch agent {
		case .claudeCode, .claudeCodeGLM, .kimiCode, .customClaudeCompatible:
			return claudeContextUsageEstimator
		case .gemini:
			return geminiContextUsageEstimator
	...
	nonisolated static func contextUsageFromGeminiProviderTokens(
		_ usage: [AgentTokenUsagePersist],
		modelContextWindow: Int?
	) -> AgentContextUsage? {
		guard !usage.isEmpty else { return nil }
		let turnTotals = usage.compactMap { turn -> Int? in
			if turn.providerTotalTokens > 0 {
				return turn.providerTotalTokens
			}
			if turn.estimatedInputTokens > 0 {
				return turn.estimatedInputTokens
			}
			return nil
		}
		...
		return AgentContextUsage(
			modelContextWindow: modelContextWindow,
			lastTotalTokens: turnTotals.last,
			totalTotalTokens: turnTotals.reduce(0, +)
		)
	}
}
