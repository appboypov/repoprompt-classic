...
/// The flow is a single unified path shown to every user.
enum AgentOnboardingStep: String, CaseIterable, Identifiable {
	...
	var title: String {
		switch self {
		case .welcome:          return "Welcome to RepoPrompt"
		case .agentModeIntro:   return "Agent Mode"
		case .contextBuilder:   return "Context Builder"
		case .mcpSetup:         return "MCP Server & Tools"
		case .providers:        return "CLI Providers"
	...
	var subtitle: String {
		switch self {
		case .welcome:          return "Your AI-powered context IDE"
		case .agentModeIntro:   return "AI agents with full context"
		case .contextBuilder:   return "Smart context, automatically"
		case .mcpSetup:         return "Optional — only needed for external MCP clients"
		case .providers:        return "Connect your tools"
		case .completion:       return "Let's go!"
		}
	}
	...
	var systemImage: String {
		...
		case .agentModeIntro:   return "terminal"
		case .contextBuilder:   return "doc.text.magnifyingglass"
		case .mcpSetup:         return "server.rack"
		case .providers:        return "link.circle"
...
final class AgentOnboardingWizardViewModel: ObservableObject {
	...
	@Published var isLoadingGemini = false
	...
	private func setupSubscriptions() {
		...
		if let api = apiSettingsViewModel {
			Publishers.MergeMany(
				...
				api.$isGeminiConnected.map { _ in () },
	...
	/// Build the unified onboarding step list shown to every user.
	private func rebuildSteps() {
		let previousStep = currentStep

		// Unified flow: welcome → Agent Mode → Context Builder → providers → MCP setup → completion.
		// CLI providers come before MCP setup because Agent Mode is the primary path,
		// and MCP server configuration is fully optional (only useful for external clients).
		steps = [
			.welcome,
			.agentModeIntro,
			.contextBuilder,
	...
	var geminiConnected: Bool {
		apiSettingsViewModel?.isGeminiConnected ?? false
	}
	...
	var geminiError: String? {
		apiSettingsViewModel?.geminiError
	}
	...
	func testGemini() {
		...
		Task {
			do {
				_ = try await apiSettingsViewModel?.testGeminiConnection()
			} catch {
				// Error state is set on apiSettingsViewModel
			}
	...
	func installMCPServer(in client: String) {
		...
		case "Gemini CLI":
			...
			showInstallFeedback(result.success ? (result.wasAlreadyPresent ? "Gemini already configured" : "Gemini configured") : "Gemini config failed", isError: !result.success)
		case "OpenCode":
