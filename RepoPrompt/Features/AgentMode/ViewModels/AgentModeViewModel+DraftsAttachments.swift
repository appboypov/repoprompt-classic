...
extension AgentModeViewModel {
	...
	func stagedSlashCommandProps(
		...
		let selectedWorkflow = session?.selectedWorkflow ?? (tabID == currentTabID ? self.selectedWorkflow : nil)
		let goalAction: AgentStagedSlashCommandProps.GoalAction = {
			switch action {
			case .show:
				return .show
			case .clear:
	...
	func commitPendingTaggedFile(tabID: UUID, relativePath: String, displayName: String? = nil) {
		...
		promoteSelectionPathsToFullContext(tabID: tabID, rawPaths: [normalizedPath])
	}
	...
	func markAttachmentsConsumed(for session: TabSession, reservationID: UUID?) {
		...
		case .idle:
			return
		case .reserved(let storedID, let attachments):
	...
	func finalizeAttachmentsForTurn(
		...
		case .idle:
			...
			return
		case .reserved(let storedID, let storedAttachments),
				.consumed(let storedID, let storedAttachments):
			if let reservationID, reservationID != storedID { return }
