...
enum AgentSidebarThreadKey: Hashable, Equatable, Sendable {
	case session(UUID)
	case tab(UUID)
...
extension AgentModeViewModel {
	...
	enum RunInteractionStateChangeReason: String, Sendable, Equatable {
	...
	enum AgentInteractionTransport: Sendable, Equatable {
		case ui
		case mcp(sessionID: UUID, originatingConnectionID: UUID?)
	}

	enum UIRefreshScope: String, Sendable, Equatable, Hashable {
		case full
		case transcriptPresentation
	...
	enum SidebarRefreshReason: String, Sendable, Equatable, Hashable {
		case sessionList
		case sessionIndex
		case sortDates
		case runState
	...
	struct AgentSessionSidebarTabMetadataSignature: Equatable, Sendable {
		let tabID: UUID
		let order: Int
	...
	struct AgentSessionSidebarTabSignature: Equatable, Sendable {
		let tabID: UUID
		let activeAgentSessionID: UUID?
	...
	struct AgentSessionSidebarContentFingerprint: Equatable, Sendable {
	...
	struct ActiveUIInvalidation: OptionSet, Sendable {
		let rawValue: Int
	...
	struct AgentMCPControlContext: Sendable, Equatable {
		let sessionID: UUID
	...
	enum MCPActiveInstructionDeliverySignalTiming: Sendable, Equatable {
	...
	enum ActiveProviderSteeringRoute: Sendable, Equatable {
		case acpPrompt
		case claudeNativeInterrupt
	}

	enum MCPInstructionDispatch: String, Sendable, Equatable {
	...
	struct MCPInteractionResponsePayload: Sendable, Equatable {
		let text: String?
		let skip: Bool
	...
	struct MCPSessionTarget: Sendable, Equatable {
	...
	struct AutoEditPermissionGuidance: Equatable, Sendable {
	...
	enum AttachmentTurnDisposition: Sendable, Equatable {
		case restoreToPending
		case deleteFiles
		case keepFiles
	}

	enum AttachmentTurnState: Sendable, Equatable {
		case idle
	...
	struct CodexComputerUseActivation: Sendable, Equatable {
		let id: UUID
		let createdAt: Date
	}
	...
	struct PendingHandoffState: Sendable, Equatable {
		var payload: String?
		var createdAt: Date?
	...
	struct CodexWatchdogState: Sendable, Equatable {
	...
	struct CodexResumeTimeoutState: Sendable, Equatable {
		var conversationID: String? = nil
		var rolloutPath: String? = nil
		var consecutiveTimeouts: Int = 0
	}

	struct CodexNativeToolLivenessState: Sendable, Equatable {
		struct Key: Hashable, Sendable {
			let invocationID: UUID?
			let fallbackSignature: String
		}

		struct Execution: Sendable, Equatable {
			let toolName: String
			let startedAt: Date
	...
	struct BashLiveExecutionState: Sendable, Equatable {
	...
	enum SourceItemsMutation: Sendable {
		...
		var touchesUserItem: Bool {
			switch self {
			case .append(_, let itemKind),
					.mutate(_, let itemKind),
					.remove(_, let itemKind):
				return itemKind == .user
			case .replace(_, let previousKind, let currentKind):
				return previousKind == .user || currentKind == .user
			case .replaceAll, .structural:
				return true
			}
		}
	...
	enum AgentTranscriptAutoFollowArmingState: String, Codable, Sendable, Equatable {
		case armed
		case disarmedAfterManualDetach
	}

	struct PendingSourceItemsMutationSummary: Sendable, Equatable {
		...
		init(_ mutation: SourceItemsMutation) {
			...
			self.earliestChangedIndex = changedRange?.lowerBound ?? 0
			self.latestChangedIndex = changedRange?.upperBound ?? Int.max
			self.containsRemoval = {
				if case .remove = mutation { return true }
				return false
			}()
			self.containsReplaceAll = {
				if case .replaceAll = mutation { return true }
				return false
			}()
			self.containsUserMutation = mutation.touchesUserItem
			self.containsStructuralMutation = mutation.isStructural
		}
	...
	struct PersistableTranscriptSnapshot: Sendable {
	...
	struct SidebarSession: Identifiable, Equatable, Sendable {
		let id: UUID
		let tabID: UUID
		let title: String
		let lastUserMessageAt: Date?
	...
	struct BuiltTranscriptPresentation: Sendable {
		let transcript: AgentTranscript
	...
	struct PendingUserTurnState {
		...
		var isEmpty: Bool {
			workflow == nil && imageAttachments.isEmpty && taggedFileAttachments.isEmpty
		}
	}
