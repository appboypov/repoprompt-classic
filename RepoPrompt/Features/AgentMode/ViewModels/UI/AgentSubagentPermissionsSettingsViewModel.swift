...
final class AgentSubagentPermissionsSettingsViewModel: ObservableObject {
	...
	init(
		...
		let resolvedSecure: AgentPermissionSecureStore?
		if let securePermissions {
			resolvedSecure = securePermissions
		} else if defaults === UserDefaults.standard {
			resolvedSecure = AgentPermissionSecureStore.shared
		} else {
			resolvedSecure = nil
		}
		...
		self.globalPolicy = AgentModePermissionPreferences.subagentPermissionPolicy(
			defaults: defaults,
			secureStore: resolvedSecure
		)
		self.providerPermissionLevelsByID = Self.loadProviderPermissionLevels(
			defaults: defaults,
			securePermissions: resolvedSecure
		)
