...
final class AgentModelsSettingsViewModel: ObservableObject {
	...
	let discoverVM: DiscoverAgentViewModel
	...
	init(
		...
		self.engine = AutoRecommendationEngine(
			settingsStore: settingsStore,
			apiSettingsViewModel: apiSettingsVM
		)
		self.syncChatWithOracle = settingsStore.syncChatModelWithOracle()
		self.restrictMCPAgentDiscoveryToRoleLabels = settingsStore.restrictMCPAgentDiscoveryToRoleLabels()
	...
	var roleDefaultsHasOverrides: Bool {
		roleDefaultsResolutions.contains(where: { $0.hasCustomOverride })
	}
	...
	func contextBuilderAgentModelMenuItems(windowID: Int) -> [StableMenuItem] {
		var items = promptVM.availableAgentKinds.map { agent in
			AgentModelStableMenuItems.agentSubmenu(
				...
				self.promptVM.contextBuilderAgent = selectedAgent
				self.promptVM.selectContextBuilderAgentModel(rawModel: selectedOption.rawValue)
				self.promptVM.commitContextBuilderSettings()
				self.refresh()
			}
		}
	...
	func roleDefaultMenuItems(
		...
		AgentModelCatalog.selectableAgents(availability: availability).map { agent in
			AgentModelStableMenuItems.agentSubmenu(
				...
				self.setRoleDefaultSelection(selection, for: resolution.role)
			}
		}
	}
