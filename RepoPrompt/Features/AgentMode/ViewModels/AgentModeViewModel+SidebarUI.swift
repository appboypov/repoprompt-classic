...
struct AgentSidebarFingerprintDeltaDiagnostics: Equatable {
	...
	var eventFields: [String: String] {
		[
			...
			"fingerprintDeltaDriver": deltaDriver,
	...
	private var deltaDriver: String {
		if categories == ["none"] { return "none" }
		if categories == ["initial"] { return "initial" }
		if changedSessionRunStateCount > 0 { return "runState" }
		if changedSessionLastActivityCount > 0 || changedSessionLastUserMessageCount > 0 || changedTabLastModifiedCount > 0 {
...
extension AgentModeViewModel.AgentSessionSidebarContentFingerprint {
	...
	private func debugTabMetadataChanges(
		...
		for tabID in Set(previousByID.keys).union(currentByID.keys) {
			...
			var changed = false
			if previousTab.order != currentTab.order { categories.insert("tabMetadata.order"); changed = true }
			if previousTab.normalizedName != currentTab.normalizedName {
				categories.insert("tabMetadata.name")
				changes.changedNameCount += 1
				changed = true
			}
			if previousTab.activeAgentSessionID != currentTab.activeAgentSessionID { categories.insert("tabMetadata.activeAgentSessionID"); changed = true }
			if previousTab.isPinned != currentTab.isPinned { categories.insert("tabMetadata.isPinned"); changed = true }
	...
	private func debugSessionSignatureChanges(
		...
		for tabID in Set(previousByID.keys).union(currentByID.keys) {
			...
			var changed = false
			if previousSession.activeAgentSessionID != currentSession.activeAgentSessionID { categories.insert("session.activeAgentSessionID"); changed = true }
			if previousSession.parentSessionID != currentSession.parentSessionID { categories.insert("session.parentSessionID"); changed = true }
			if previousSession.hasLoadedPersistedState != currentSession.hasLoadedPersistedState { categories.insert("session.hasLoadedPersistedState"); changed = true }
			if previousSession.itemsIsEmpty != currentSession.itemsIsEmpty { categories.insert("session.itemsIsEmpty"); changed = true }
