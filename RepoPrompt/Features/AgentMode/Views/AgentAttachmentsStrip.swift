...
struct AgentAttachmentStripSnapshot: Equatable {
	...
	var hasImages: Bool { !imageAttachments.isEmpty }
	var hasTaggedFiles: Bool { !taggedFileAttachments.isEmpty }
	var hasAny: Bool { hasImages || hasTaggedFiles }
...
enum AgentAttachmentStripLayout {
	static var imageStripHeight: CGFloat { FontScalePreset.current.scaledMetric(88) }
	static var fileOnlyStripHeight: CGFloat { FontScalePreset.current.scaledMetric(24) }
...
struct AgentAttachmentsStrip: View, Equatable {
	...
	var onRemoveTaggedFile: ((UUID) -> Void)? = nil
...
private struct AgentImageAttachmentThumbnailView: View, Equatable {
	...
	private var thumbnailKey: AgentAttachmentThumbnailKey? {
		guard case .localFile(let path) = attachment.source else { return nil }
	...
	func loadImage(
		...
		loadQueue.async { [weak self, weak token] in
			...
			DispatchQueue.main.async { [weak self, weak token] in
				guard let self, token?.isCancelled == false else { return }
				if let decodedImage {
					self.cache.setObject(decodedImage, forKey: key.cacheKey)
				}
