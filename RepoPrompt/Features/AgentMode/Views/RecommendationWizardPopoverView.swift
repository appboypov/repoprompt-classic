...
struct RecommendationWizardPopoverView: View {
    ...
    private var dismissedRecommendationsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if let proRec = viewModel.recommendations.proEdit, proRec.isMuted, !proRec.alreadySatisfied {
                    dismissedRow(
                        icon: "bolt.fill",
                        title: "Pro Edit",
                        detail: proRec.shouldEnableProEdit ? "Recommended: Enable" : "Recommended: Disable",
                        kind: .proEditMode
                    )
                }
    ...
    private func providerStatusGrid(_ status: ProviderStatusSnapshot) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ...
                providerRow("Gemini CLI", status: status.geminiCLI)
            }
        }
    }
        ...
        ScrollViewReader { proxy in
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    case .proEdit:
                        ProEditStepView(viewModel: viewModel)
                    case .presets:
    ...
    private var footerActions: some View {
        ...
        case .chatModel, .contextBuilder, .proEdit, .presets:
            Button("Skip") {
...
private struct IntroStepView: View {
    ...
    private var recommendationsPreviewSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let proRec = viewModel.recommendations.proEdit, !proRec.alreadySatisfied {
                    recommendationPreviewRow(
                        icon: "bolt.fill",
                        title: "Pro Edit",
                        detail: proRec.shouldEnableProEdit ? "Enable with \(proRec.recommendedAgent?.displayName ?? "agent")" : "Keep disabled",
                        kind: .proEditMode,
                        isMuted: proRec.isMuted
                    )
                }
    ...
    private func providerStatusSection(_ status: ProviderStatusSnapshot) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ...
                providerStatusRow("Gemini CLI", status: status.geminiCLI)
            }
        }
    }
    ...
    private func statusColor(_ status: ProviderStatusSnapshot.Availability) -> Color {
        ...
        case .notConfigured: return .gray.opacity(0.5)
        }
    }
    ...
    private func upgradeHintSection(_ status: ProviderStatusSnapshot) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("For optimal results, connect Codex CLI or add an OpenAI API key. Codex CLI enables GPT-5.5 agent defaults; OpenAI API enables GPT-5.5/Pro for stronger planning and review.")
                .font(.caption)
                .foregroundColor(.secondary)
...
/// Produces a compact summary string for role defaults, e.g. "Explore → Codex GPT-5.5 Low · ..."
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if let rec = viewModel.recommendations.chatModel {
                ...
                VStack(spacing: 12) {
                    ...
                    if rec.geminiOption != nil {
                        backendCard(.gemini, option: rec.geminiOption!)
                    }
                }
                .id("recommendationContent")
        ...
        Button {
            ...
            .background(
                RoundedRectangle(cornerRadius: 10)
                    .fill(viewModel.selectedChatBackend == backend
                          ? Color.accentColor.opacity(0.08)
                          : Color.primary.opacity(0.03))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .stroke(viewModel.selectedChatBackend == backend
                            ? Color.accentColor
...
// MARK: - Pro Edit Step View

private struct ProEditStepView: View {
    @ObservedObject var viewModel: RecommendationWizardViewModel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Pro Edit enables advanced code modification capabilities with agent-powered file editing.")
                .font(.subheadline)
                .foregroundColor(.secondary)
            
            if let rec = viewModel.recommendations.proEdit {
                // Main recommendation card
                proEditCard(rec)
                    .id("recommendationContent")
                
                // Show CLI setup guidance when no agents are available
                if !rec.shouldEnableProEdit {
                    missingAgentsGuidance
                }
            }
        }
    }
    
    private func proEditCard(_ rec: ProEditRecommendation) -> some View {
        HStack(spacing: 12) {
            Image(systemName: proEditIconName(rec))
                .font(.title)
                .foregroundColor(proEditIconColor(rec))
            
            VStack(alignment: .leading, spacing: 4) {
                Text(proEditHeadline(rec))
                    .font(.headline)
                ...
                // Show upgrade hint when available
                if let hint = rec.upgradeHint {
                    Text(hint)
                        .font(.caption2)
                        .foregroundColor(.orange)
    ...
    private func proEditIconName(_ rec: ProEditRecommendation) -> String {
    ...
    private func proEditIconColor(_ rec: ProEditRecommendation) -> Color {
        switch rec.recommendedMode {
    ...
    private func proEditHeadline(_ rec: ProEditRecommendation) -> String {
        switch rec.recommendedMode {
        case .agent:
            if let agent = rec.recommendedAgent, let model = rec.recommendedAgentModel {
                return "Enable Pro Edit with \(agent.displayName) + \(model.displayName)"
            } else if let agent = rec.recommendedAgent {
                return "Enable Pro Edit with \(agent.displayName)"
            }
            return "Enable Pro Edit (Agent Mode)"
        case .model:
            return "Enable Pro Edit (Model Mode)"
        case .disabled:
            return "Configure Providers for Pro Edit"
        }
    }
    
    @ViewBuilder
    private var missingAgentsGuidance: some View {
        if let status = viewModel.providerStatus, !status.hasAnyProEditCLIAgentReady {
            VStack(alignment: .leading, spacing: 10) {
                Text("To enable Pro Edit, connect at least one compatible CLI agent:")
                    .font(.caption.bold())
                
                VStack(alignment: .leading, spacing: 6) {
                    cliStatusRow("Claude Code", status: status.claudeCodeCLI, recommended: true)
                    cliStatusRow("Codex CLI", status: status.codexCLI, recommended: false)
                    cliStatusRow("Gemini CLI", status: status.geminiCLI, recommended: false)
                }
                
                Text("Claude Code is recommended for best Pro Edit experience. Install via: npm install -g @anthropic-ai/claude-code")
                    .font(.caption)
                    .foregroundColor(.secondary)
    ...
    private func cliStatusRow(_ name: String, status: ProviderStatusSnapshot.Availability, recommended: Bool) -> some View {
        HStack(spacing: 6) {
            Circle()
                .fill(statusColor(status))
                .frame(width: 8, height: 8)
    ...
    private func statusText(_ status: ProviderStatusSnapshot.Availability) -> String {
        switch status {
        case .ready: return "Connected"
        case .configured: return "Not Verified"
...
private struct SummaryStepView: View {
    ...
    private var alreadySatisfied: RecommendationSet {
        ...
        if let pro = recs.proEdit, pro.alreadySatisfied {
            result.proEdit = pro
        }
    ...
    private var mutedDifferences: RecommendationSet {
        ...
        if let pro = recs.proEdit, pro.isMuted, !pro.alreadySatisfied {
            result.proEdit = pro
        }
    ...
    private var appliedRecommendationsList: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let proRec = applied.proEdit, proRec.shouldEnableProEdit {
                appliedRow(
                    icon: "bolt.fill",
                    title: "Pro Edit",
                    detail: "Enabled with \(proRec.recommendedAgent?.displayName ?? "agent")"
                )
            }
    ...
    private var alreadySatisfiedList: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let proRec = alreadySatisfied.proEdit, proRec.shouldEnableProEdit {
                satisfiedRow(
                    icon: "bolt.fill",
                    title: "Pro Edit",
                    detail: "Enabled with \(proRec.recommendedAgent?.displayName ?? "agent")"
    ...
    private var mutedDifferencesList: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let proRec = mutedDifferences.proEdit {
                mutedRow(
                    icon: "bolt.fill",
                    title: "Pro Edit",
                    detail: proRec.shouldEnableProEdit ? "Recommended: Enable" : "Recommended: Disable",
