		...
		GeometryReader { proxy in
			...
			NavigationSplitView(
				...
				AgentModeDetailWithSidebarView(
					...
					discoverAgentVM: windowState.discoverAgentViewModel,
...
struct AgentModeChatDetailView: View {
	...
	private var lastTranscriptChangePinnedMaintenanceKey: AgentTranscriptScrollAffectingPresentationKey? {
		get { pinnedMaintenance.lastTranscriptChangeScrollAffectingKey }
		nonmutating set { pinnedMaintenance.lastTranscriptChangeScrollAffectingKey = newValue }
	}
	...
	private var liveBottomRestoreLastLayoutMutationAt: Date? {
		get { scrollEngine.rehydrate.liveBottomLastLayoutMutationAt }
		nonmutating set { scrollEngine.rehydrate.liveBottomLastLayoutMutationAt = newValue }
	}
	private var liveBottomRestoreDriveStartedAt: Date? {
		get { scrollEngine.rehydrate.liveBottomDriveStartedAt }
		nonmutating set { scrollEngine.rehydrate.liveBottomDriveStartedAt = newValue }
	}
	private var liveBottomRestoreCorrectionCount: Int {
		get { scrollEngine.rehydrate.liveBottomCorrectionCount }
		nonmutating set { scrollEngine.rehydrate.liveBottomCorrectionCount = newValue }
	}
	private var deferredLiveBottomRestoreSettleTask: Task<Void, Never>? {
		get { scrollEngine.rehydrate.liveBottomDeferredSettleTask }
		nonmutating set { scrollEngine.rehydrate.liveBottomDeferredSettleTask = newValue }
	}
	...
	private var runningIndicatorReservedHeight: CGFloat { fontPreset.scaledMetric(24) }
	...
	private static let rehydrateRestoreMaxCorrectiveScrollCount = 3
	private static let rehydrateRestoreMaxDriveDuration: TimeInterval = 1.25
	private static let rehydrateRestoreWatchdogSettleDelay: TimeInterval = 0.08
	...
	private func debugDescription(for phase: AgentTranscriptRehydrateRestorePhase) -> String {
		...
		case .idle:
			return "idle"
		case .awaitingHydration(let tabID, let target):
			return "awaiting(\(debugShortID(tabID)),target:\(target))"
		case .awaitingLayout(let tabID, let presentationRevision, let target):
			return "awaitingLayout(\(debugShortID(tabID)),rev:\(presentationRevision),target:\(target))"
		case .driving(let tabID, let presentationRevision, let target):
			return "driving(\(debugShortID(tabID)),rev:\(presentationRevision),target:\(target))"
		}
	}

	private func debugDescription(for intent: AgentTranscriptScrollIntent) -> String {
		switch intent {
		case .bottom(let animated, let reason):
			return "bottom(animated:\(animated),reason:\(reason))"
		case .anchor(let anchor, let placement, let animated, let reason):
			return "anchor(anchor:\(anchor),placement:\(placement),animated:\(animated),reason:\(reason))"
		case .viewportTarget(let targetID, let placement, let animated, let reason):
			return "viewportTarget(target:\(targetID),placement:\(placement),animated:\(animated),reason:\(reason))"
		}
	}
	...
	private func clearPendingTranscriptChangePinnedMaintenance() {
		let hasClearablePendingRequest: Bool
		switch pendingPinnedBottomSource {
		case .transcriptChangeWhilePinned?, .waitingStateChange?, .busyStateChange?, .bottomClearanceChange?:
			hasClearablePendingRequest = true
		default:
			hasClearablePendingRequest = false
		}
		let hasClearableDeferredRequest: Bool
		switch deferredPinnedCorrectionAfterSmoothSend {
		case .transcriptChangeWhilePinned?, .waitingStateChange?, .busyStateChange?, .bottomClearanceChange?:
			hasClearableDeferredRequest = true
		default:
			hasClearableDeferredRequest = false
		}
	...
	private func requestPinnedLiveBottomForTranscriptChange(proxy: ScrollViewProxy) {
		let scrollAffectingKey = transcriptSnapshot.presentation.scrollAffectingPresentationKey
		guard lastTranscriptChangePinnedMaintenanceKey != scrollAffectingKey else { return }
		lastTranscriptChangePinnedMaintenanceKey = scrollAffectingKey
	...
	private func requestPinnedLiveBottom(
		...
		pinnedBottomMaintenanceGate.schedule(after: mergedSource.gateDelay) {
			let requestToFlush = self.pendingPinnedMaintenanceRequest ?? self.pinnedMaintenance.makeRequest(source: mergedSource)
			let sourceToFlush = requestToFlush.source
			guard self.isPinnedToLiveBottom,
				!self.isInteractionBlockerVisible,
				!self.isUserInteractingWithScroll,
				!self.isRehydrateRestoreActive,
				self.pinnedMaintenance.isCurrent(requestToFlush)
			else {
				self.pendingPinnedMaintenanceRequest = nil
				return
			}
			let isAnimatingSmoothPinnedSend = self.smoothPinnedSendState?.phase == .animatingToBottom
				&& self.isSmoothPinnedSendActive(for: self.currentTabID)
			if isAnimatingSmoothPinnedSend, self.programmaticScrollGate.isInFlight {
				self.queueDeferredPinnedCorrection(sourceToFlush)
				self.pendingPinnedMaintenanceRequest = nil
				return
			}
			self.pendingPinnedMaintenanceRequest = nil
			self.bottomClearanceSuppressionUntil = Date().addingTimeInterval(0.20)
			self.notePinnedBottomRequest()
			self.requestScroll(
				proxy,
	...
	private func noteRehydrateRestoreTelemetry(_ event: String, extra: String = "") {
		#if DEBUG
		let now = Date()
		let mutationAgeMS = liveBottomRestoreLastLayoutMutationAt.map { Int(max(0, now.timeIntervalSince($0) * 1000)) }
		var details = [
			"distance=\(Int(scrollMetrics.distanceToBottom))",
			"content=\(Int(scrollMetrics.contentHeight))",
			"viewport=\(Int(scrollMetrics.viewportHeight))",
			"strict=\(scrollMetrics.distanceToBottom <= Self.actualBottomRepinDistanceThreshold)",
			"layoutToken=\(rehydrateLayoutPassToken)",
			"corrections=\(liveBottomRestoreCorrectionCount)",
			"mutationAgeMS=\(mutationAgeMS.map(String.init) ?? "nil")",
			"rawPayloadRev=\(transcriptSnapshot.presentation.rawToolResultPayloadRenderRevision)"
		]
		if !extra.isEmpty {
			details.append(extra)
		}
		let detailString = details.joined(separator: " ")
		debugConsoleLog("rehydrateRestore \(event)", details: detailString)
		if isStressHarnessEnabled {
			noteStressHarness("Restore \(event): \(detailString)")
		}
		#else
		_ = event
		_ = extra
	...
	private func transcriptBlockSupportsExpansion(_ block: AgentTranscriptRenderBlock) -> Bool {
		...
		case .activityCluster:
			return !block.rows.isEmpty
		case .groupedHistory:
			return !(block.groupedHistory?.sections.isEmpty ?? true)
	...
	private func shouldUpdateViewportMinY(_ current: CGFloat?, to next: CGFloat?) -> Bool {
		...
		case let (current?, next?):
			return abs(current - next) >= Self.viewportSnapshotMinYEpsilon
		default:
			return true
		}
	}
	...
	private var chatTranscript: some View {
		GeometryReader { geometry in
			...
			.onChange(of: currentTabID) { oldTabID, newTabID in
				...
				repinGraceState = nil
				lastTranscriptChangePinnedMaintenanceKey = nil
	...
	private func handleScrollAffectingPresentationChange(
		from oldSnapshot: AgentTranscriptPresentationSnapshot,
		to newSnapshot: AgentTranscriptPresentationSnapshot,
		proxy: ScrollViewProxy
	) {
		if isLiveBottomRehydrateRestoreActive() {
			liveBottomRestoreLastLayoutMutationAt = Date()
			noteRehydrateRestoreTelemetry(
				"presentationMutation",
				extra: "oldRawPayloadRev=\(oldSnapshot.rawToolResultPayloadRenderRevision) newRawPayloadRev=\(newSnapshot.rawToolResultPayloadRenderRevision)"
			)
			_ = advanceRehydrateRestoreIfNeeded(proxy: proxy)
			return
		}
		...
		guard isPinnedToLiveBottom,
			...
			!isUserInteractingWithScroll,
			!isRehydrateRestoreActive,
			isScrollResponsivenessSuppressed
		else {
			return
		}
	...
	private func handleTranscriptPresentationChange(
		...
		let scrollAffectingPresentationChanged = newSnapshot.hasScrollAffectingPresentationDelta(comparedTo: oldSnapshot)
		...
		#if DEBUG
		AgentModeLayoutHotspotDiagnostics.increment("view.transcript.presentationChange", tabID: currentTabID)
		if visiblePresentationChanged {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.presentationChange.visible", tabID: currentTabID)
		}
		if scrollAffectingPresentationChanged {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.presentationChange.scrollAffecting", tabID: currentTabID)
		}
		if visibleBlocksChanged {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.presentationChange.visibleBlocks", tabID: currentTabID)
		}
		if visibleRowsChanged {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.presentationChange.visibleRows", tabID: currentTabID)
		}
		#endif

		if scrollAffectingPresentationChanged {
			handleScrollAffectingPresentationChange(from: oldSnapshot, to: newSnapshot, proxy: proxy)
		}
	...
	private func modernScrollableContentBase(proxy: ScrollViewProxy, viewportHeight: CGFloat) -> some View {
		...
		.onScrollGeometryChange(for: AgentTranscriptScrollMetrics.self, of: { geometry in
			...
			#if DEBUG
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.geometry", tabID: currentTabID)
			#endif
			let oldMetrics = applyScrollMetrics(newMetrics)
			recordRehydrateLayoutSampleIfNeeded(metrics: newMetrics)
			recordRehydrateRestoreLayoutMutationIfNeeded(oldMetrics: oldMetrics, newMetrics: newMetrics)
			...
			if isRehydrateRestoreActive {
				if continueRehydrateRestoreIfNeeded(proxy: proxy, source: .geometry) {
					return
				}
				finishRehydrateRestoreIfSettled(proxy: proxy)
				return
			}
		...
		if let review = runInteractionSnapshot.pendingApplyEditsReview {
			...
			AgentRequestUserInputCard(
				...
				onSubmit: { response in
					guard let tabID = runInteractionSnapshot.currentTabID else { return }
			...
			AgentAskUserWizardCard(
				...
				onSubmit: {
					guard let tabID = currentTabID else { return }
					agentModeVM.submitAskUserResponse(tabID: tabID)
				},
				onSkipAll: {
					guard let tabID = currentTabID else { return }
					agentModeVM.skipAskUser(tabID: tabID)
				},
				onUserActivity: {
					guard let tabID = currentTabID else { return }
			...
			transcriptBlockRow {
				VStack(alignment: .leading, spacing: 8) {
					if !supportsExpansion || summaryLockTarget {
						...
						Button {
							#if DEBUG
							AgentModeLayoutHotspotDiagnostics.increment("view.transcript.blockExpansion.toggle", tabID: currentTabID)
							#endif
	...
	private func transcriptRowView(
		...
		.environment(\.agentRawToolResultPayloadResolver, { itemID in
			agentModeVM.rawToolResultPayloadForRendering(tabID: ownerTabID, itemID: itemID)
		})
		.environment(\.agentRawToolResultPayloadRenderRevision, transcriptSnapshot.presentation.rawToolResultPayloadRenderRevision)
	}
	...
	private func transcriptBlockRow<Content: View>(@ViewBuilder content: () -> Content) -> some View {
	...
	private func disambiguatedFileNames(_ paths: [String]) -> [String] {
		...
		for name in baseNames { counts[name, default: 0] += 1 }
	...
	private func collapsedSummaryCard<Content: View>(@ViewBuilder content: () -> Content) -> some View {
	...
	private func syncTranscriptBlockExpansion(for blocks: [AgentTranscriptRenderBlock]) {
		...
		let expansionChanged = transcriptBlockExpansion != nextExpansion
		let defaultsChanged = transcriptBlockDefaultExpansion != nextDefaults
		#if DEBUG
		AgentModeLayoutHotspotDiagnostics.increment("view.transcript.blockExpansionSync.called", tabID: currentTabID)
		if expansionChanged || defaultsChanged {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.blockExpansionSync.write", tabID: currentTabID)
		} else {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.blockExpansionSync.noop", tabID: currentTabID)
		}
		#endif
		if expansionChanged {
	...
	private func shouldAbortPinnedMaintenanceScrollExecution(_ intent: AgentTranscriptScrollIntent) -> Bool {
		guard case .bottom(_, let reason) = intent else {
			return false
		}
		switch reason {
		case .transcriptChangeWhilePinned, .waitingStateChange, .busyStateChange, .bottomClearanceChange:
	...
	private func requestScroll(
		...
		if case .bottom(_, let reason) = intent {
		...
		case .bottom:
			...
			topVisibleViewportTargetID = nil
		case .anchor(let semanticAnchor, _, _, _):
			pendingProgrammaticRestoreTargetID = nil
			pendingProgrammaticRestoreAnchor = semanticAnchor
			topVisibleViewportTargetID = nil
		case .viewportTarget(let targetID, _, _, _):
		...
		#if DEBUG
		AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.request", tabID: currentTabID)
		AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.request.reason.\(String(describing: intent.reason))", tabID: currentTabID)
		if immediate {
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.request.immediate", tabID: currentTabID)
		}
		#endif
		programmaticScrollGate.schedule(after: delay, settleAfter: intent.settleDelay, onSettled: {
			self.debugLog("requestScroll settled", details: "intent=\(self.debugDescription(for: intent))")
			#if DEBUG
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.settled", tabID: self.currentTabID)
			#endif
			if case .bottom(_, let settleReason) = intent {
				let bottomSettleAt = Date()
				self.lastBottomSettleAt = bottomSettleAt
				if AgentTranscriptPinnedBottomProtectionPolicy.shouldArmOnBottomSettle(
					runtime: self.scrollRuntimeState,
					nearBottomThreshold: Self.pinnedBottomProtectionNearBottomThreshold
				) {
					self.armPinnedBottomProtection(now: bottomSettleAt)
				} else {
					self.clearPinnedBottomProtection()
				}
				if settleReason == .sessionSwitchRestore {
					self.noteRehydrateRestoreTelemetry("settle")
				}
				#if DEBUG
				self.stressTelemetryState.lastSettledBottomReason = String(describing: settleReason)
				if self.isStressHarnessEnabled {
					self.noteStressHarness("Bottom settle: reason=\(settleReason) distance=\(Int(self.scrollMetrics.distanceToBottom)) nearBottom=\(self.isNearBottom) pinned=\(self.isPinnedToLiveBottom) smoothActive=\(self.smoothPinnedSendState != nil)")
				}
				#endif
				if let smoothPinnedSendState = self.smoothPinnedSendState {
					switch smoothPinnedSendState.phase {
					case .preservingBottomBeforeAnimation:
						if self.isNearBottom {
							_ = self.finishSmoothPinnedSendWithoutAnimation(reason: "bottomPreservedAfterStabilization")
						} else {
							self.flushDeferredPinnedCorrectionAfterSmoothSendIfNeeded(proxy: proxy)
							self.scheduleStagedSmoothPinnedSendLaunchIfNeeded(proxy: proxy)
							#if DEBUG
							if self.isStressHarnessEnabled, self.smoothPinnedSendState != nil {
								self.noteStressHarness("Smooth send still stabilizing after bottom settle: reason=\(settleReason) distance=\(Int(self.scrollMetrics.distanceToBottom)) nearBottom=\(self.isNearBottom)")
							}
							#endif
						}
					case .animatingToBottom:
						let didCompleteSmoothSend = self.completeSmoothPinnedSendIfNeeded(settleReason: settleReason)
						#if DEBUG
						if self.isStressHarnessEnabled, !didCompleteSmoothSend, self.smoothPinnedSendState != nil {
							self.noteStressHarness("Smooth send still active after bottom settle: reason=\(settleReason) distance=\(Int(self.scrollMetrics.distanceToBottom)) nearBottom=\(self.isNearBottom)")
						}
						#endif
				...
				if self.isPinnedToLiveBottom,
					!self.userDetachedAutoFollow,
					!self.isInteractionBlockerVisible,
					!self.isUserInteractingWithScroll,
					!self.isRehydrateRestoreActive,
					self.isPinnedMaintenanceReason(settleReason),
					self.scrollMetrics.distanceToBottom > Self.scrollToBottomSuccessDistanceThreshold {
					if settleReason != .bottomClearanceChange {
						self.requestPinnedLiveBottom(proxy: proxy, source: .bottomClearanceChange)
					}
				}
				self.resolvePendingBottomScrollOutcomeOnSettled(for: settleReason)
			}
			self.recordRehydrateLayoutSampleAfterProgrammaticSettle()
			if self.continueRehydrateRestoreIfNeeded(proxy: proxy, source: .settled) {
				return
			}
			self.finishRehydrateRestoreIfSettled(proxy: proxy)
		}, action: {
			self.debugLog("requestScroll executing", details: "intent=\(self.debugDescription(for: intent)) capturedTab=\(self.debugShortID(capturedTabID)) capturedRev=\(capturedPresentationRevision)")
			guard capturedTabID == currentTabID else {
				#if DEBUG
				AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.abort.tabMismatch", tabID: self.currentTabID)
				#endif
				self.cancelPendingBottomScrollOutcomeIfUnexecuted(reason: "tabMismatch")
				self.debugLog("requestScroll aborted tab mismatch", details: "intent=\(self.debugDescription(for: intent)) capturedTab=\(self.debugShortID(capturedTabID)) currentTab=\(self.debugCurrentTabLabel)")
				return
			}
			guard capturedPresentationRevision == self.currentTranscriptPresentationRevision
				|| intent.allowsRevisionMismatch
			else {
				#if DEBUG
				AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.abort.revisionGate", tabID: self.currentTabID)
				#endif
				self.cancelPendingBottomScrollOutcomeIfUnexecuted(reason: "revisionGate")
				self.debugLog("requestScroll aborted revision gate", details: "intent=\(self.debugDescription(for: intent)) capturedRev=\(capturedPresentationRevision) currentRev=\(self.currentTranscriptPresentationRevision)")
				return
			}
			if let pinnedMaintenanceGeneration,
				pinnedMaintenanceGeneration != self.pinnedMaintenanceGeneration {
				#if DEBUG
				AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.abort.pinnedMaintenanceGeneration", tabID: self.currentTabID)
				#endif
				self.cancelPendingBottomScrollOutcomeIfUnexecuted(reason: "pinnedMaintenanceGeneration")
				self.debugLog("requestScroll aborted pinned maintenance generation", details: "intent=\(self.debugDescription(for: intent)) requested=\(pinnedMaintenanceGeneration) current=\(self.pinnedMaintenanceGeneration)")
				return
			}
			if self.shouldAbortPinnedMaintenanceScrollExecution(intent) {
				#if DEBUG
				AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.abort.stalePinnedMaintenance", tabID: self.currentTabID)
				#endif
				self.cancelPendingBottomScrollOutcomeIfUnexecuted(reason: "stalePinnedMaintenance")
				self.debugLog("requestScroll aborted stale pinned maintenance", details: "intent=\(self.debugDescription(for: intent)) distance=\(Int(self.scrollMetrics.distanceToBottom)) interacting=\(self.isUserInteractingWithScroll) detached=\(self.userDetachedAutoFollow) suppression=\(self.isPinnedTranscriptChangeSuppressionActive())")
				return
			}
			if self.shouldAbortCompetingPinnedMaintenanceDuringExplicitBottomOutcome(intent.reason) {
				#if DEBUG
				AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.abort.competingPinnedMaintenance", tabID: self.currentTabID)
				#endif
				self.debugLog(
					"requestScroll aborted competing pinned maintenance during explicit bottom outcome",
					details: "intent=\(self.debugDescription(for: intent)) distance=\(Int(self.scrollMetrics.distanceToBottom))"
				)
				return
			}
			_ = self.markPendingBottomScrollOutcomeExecutedIfNeeded(for: intent.reason)

			#if DEBUG
			AgentModeLayoutHotspotDiagnostics.increment("view.transcript.scroll.execute", tabID: self.currentTabID)
			#endif
			let performScroll = {
				switch intent {
				case .bottom:
					proxy.scrollTo("bottomTarget", anchor: .bottom)
				case .anchor(let semanticAnchor, let placement, _, _):
				...
				case .viewportTarget(let targetID, let placement, _, _):
	...
	private func resolveVisibleViewportTargetID(_ targetID: AgentTranscriptViewportTargetID) -> AgentTranscriptViewportTargetID? {
		switch targetID {
		case .row(let rowID):
			return renderedTranscriptRowIDs.contains(rowID) ? targetID : nil
		case .block(let blockID):
	...
	private func groupedHistoryDescendantViewportCandidate(
		...
		guard case .groupedHistory(let turnID, let spanID)? = anchor else { return nil }
		let descendantCandidates = viewportRegistry.viewportCandidates.filter { candidate in
			...
			case .activity(let candidateTurnID, let candidateSpanID, _):
	...
	private func recordCurrentRehydrateLayoutSample() {
		...
		case .awaitingLayout(let phaseTabID, let phaseRevision, _),
			.driving(let phaseTabID, let phaseRevision, _):
			tabID = phaseTabID
	...
	private func recordRehydrateLayoutSampleAfterProgrammaticSettle() {
		guard isRehydrateRestoreActive else { return }
		guard AgentTranscriptRehydrateRestoreLayoutPolicy.hasValidLayoutSample(scrollMetrics) else { return }
		recordCurrentRehydrateLayoutSample()
	}

	private func isLiveBottomRehydrateRestoreActive() -> Bool {
		guard case .driving(_, _, let target) = rehydrateRestorePhase else { return false }
		guard case .liveBottom = target else { return false }
		return true
	}

	private func recordRehydrateRestoreLayoutMutationIfNeeded(
		oldMetrics: AgentTranscriptScrollMetrics,
		newMetrics: AgentTranscriptScrollMetrics
	) {
		guard isLiveBottomRehydrateRestoreActive() else { return }
		...
		liveBottomRestoreLastLayoutMutationAt = Date()
		noteRehydrateRestoreTelemetry("layoutMutation")
	}

	private func liveBottomRestoreRemainingQuietDelay(now: Date = Date()) -> TimeInterval? {
		AgentTranscriptBottomScrollOutcomeLayoutPolicy.remainingQuietDelay(
			lastLayoutMutationAt: liveBottomRestoreLastLayoutMutationAt,
			now: now,
	...
	private func canCompleteLiveBottomRehydrateRestore(
		...
		AgentTranscriptRehydrateRestoreLayoutPolicy.canCompleteLiveBottomRestore(
			...
			strictBottomDistanceThreshold: Self.actualBottomRepinDistanceThreshold,
			lastLayoutMutationAt: liveBottomRestoreLastLayoutMutationAt,
			now: now,
			quietPeriod: Self.bottomScrollOutcomeLayoutQuietPeriod
		)
	...
	private func applyScrollMetrics(_ newMetrics: AgentTranscriptScrollMetrics) -> AgentTranscriptScrollMetrics {
		...
		if #available(macOS 15.0, *) {
			...
			if nextIsNearBottom != bottomAffordance.isNearBottom {
				#if DEBUG
				AgentModeLayoutHotspotDiagnostics.increment("view.transcript.geometry.nearBottomWrite", tabID: currentTabID)
				#endif
	...
	private func recordUserScrollProgressIfNeeded(
		...
		let frameDirection: DetachedManualScrollDirection
		if isPinnedToLiveBottom, !userDetachedAutoFollow {
			frameDirection = AgentTranscriptUserScrollIntentResolver.resolvePinnedLiveBottomFollowIntent(
			...
			frameDirection = AgentTranscriptUserScrollIntentResolver.resolve(
				distanceDelta: distanceDelta,
				visibleMinYDelta: visibleMinYDelta,
				distanceThreshold: Self.manualScrollProgressDistanceThreshold,
				visibleMinYThreshold: Self.manualScrollProgressVisibleMinYThreshold
	...
	private func scheduleStagedSmoothPinnedSendLaunchIfNeeded(proxy: ScrollViewProxy) {
		...
		smoothPinnedSendLaunchGate.schedule(after: Self.stagedSmoothSendStabilizationDelay) {
			self.launchStagedSmoothPinnedSendIfStable(proxy: proxy)
		}
	}
	...
	private func cancelDeferredLiveBottomRestoreSettle() {
		deferredLiveBottomRestoreSettleTask?.cancel()
		deferredLiveBottomRestoreSettleTask = nil
	}

	private func resetLiveBottomRestoreDriveState() {
		liveBottomRestoreLastLayoutMutationAt = nil
		liveBottomRestoreDriveStartedAt = nil
		liveBottomRestoreCorrectionCount = 0
		cancelDeferredLiveBottomRestoreSettle()
	}

	private func markColdRestoreStart() {
		scrollEngine.rehydrate.coldRestoreStartedAt = Date()
		noteRehydrateRestoreTelemetry("start")
		#if DEBUG
		guard isStressHarnessEnabled else { return }
	...
	private func markColdRestoreBottomRequest(corrective: Bool) {
		if liveBottomRestoreDriveStartedAt == nil {
			liveBottomRestoreDriveStartedAt = Date()
		}
		noteRehydrateRestoreTelemetry(corrective ? "correct" : "scroll")
		#if DEBUG
		guard isStressHarnessEnabled else { return }
	...
	private func markColdRestoreCompletion() {
		...
		noteRehydrateRestoreTelemetry("complete", extra: "durationMS=\(Int(durationMS))")
		scrollEngine.rehydrate.coldRestoreStartedAt = nil
	...
	private func rehydrateRestoreIntent(
		...
		switch target {
		case .liveBottom:
			return .bottom(animated: false, reason: .sessionSwitchRestore)
		case .detached:
			return .bottom(animated: false, reason: .sessionSwitchRestore)
		}
	}
	...
	private func remountTranscriptForActivationRepaint(key: AgentTranscriptRehydrateRetryKey) {
		...
		currentRehydrateLayoutSampleKey = nil
		resetLiveBottomRestoreDriveState()
		switch rehydrateRestorePhase {
		case .awaitingLayout(let tabID, _, let target),
			.driving(let tabID, _, let target):
	...
	private func scheduleAwaitingLayoutRestore(
		...
		Task { @MainActor in
			...
			guard case .awaitingLayout(let phaseTabID, let phaseRevision, let target) = rehydrateRestorePhase else { return }
	...
	private func scheduleDeferredLiveBottomRestoreSettle(
		proxy: ScrollViewProxy,
		after delay: TimeInterval
	) {
		guard delay > 0 else {
			if continueRehydrateRestoreIfNeeded(proxy: proxy, source: .quietTimer) {
				return
			}
			finishRehydrateRestoreIfSettled(proxy: proxy)
			return
		...
		cancelDeferredLiveBottomRestoreSettle()
		let token = rehydrateLayoutPassToken
		let delayMS = Int(max(0, delay) * 1000)
		noteRehydrateRestoreTelemetry("settleDeferred", extra: "delayMS=\(delayMS)")
		deferredLiveBottomRestoreSettleTask = Task { @MainActor in
			let nanoseconds = UInt64(max(0, delay) * 1_000_000_000)
			try? await Task.sleep(nanoseconds: nanoseconds)
			guard !Task.isCancelled,
				token == rehydrateLayoutPassToken
			else {
				return
			}
			deferredLiveBottomRestoreSettleTask = nil
	...
	private func liveBottomRestoreWatchdogSettleDelay(now: Date) -> TimeInterval? {
		AgentTranscriptRehydrateRestoreLayoutPolicy.watchdogSettleDelay(
			startedAt: liveBottomRestoreDriveStartedAt,
			now: now,
			maxDuration: Self.rehydrateRestoreMaxDriveDuration,
			watchdogDelay: Self.rehydrateRestoreWatchdogSettleDelay
		)
	}

	private func canContinueLiveBottomRestore(now: Date) -> Bool {
		AgentTranscriptRehydrateRestoreLayoutPolicy.canContinueLiveBottomRestore(
			startedAt: liveBottomRestoreDriveStartedAt,
			correctionCount: liveBottomRestoreCorrectionCount,
			maxDuration: Self.rehydrateRestoreMaxDriveDuration,
			maxCorrectionCount: Self.rehydrateRestoreMaxCorrectiveScrollCount,
			now: now
		)
	}

	private func continueRehydrateRestoreIfNeeded(proxy: ScrollViewProxy, source: RehydrateRestoreContinuationSource) -> Bool {
		guard case .driving(let tabID, _, let target) = rehydrateRestorePhase else { return false }
		guard case .liveBottom = target else { return false }
		...
		let now = Date()
		guard liveBottomRestoreRemainingQuietDelay(now: now) == nil else { return false }
		guard scrollMetrics.distanceToBottom > Self.actualBottomRepinDistanceThreshold else { return false }
		guard canContinueLiveBottomRestore(now: now) else {
			noteRehydrateRestoreTelemetry("correctSkipped", extra: "source=\(source) maxed=true")
			return false
		}
		cancelDeferredLiveBottomRestoreSettle()
		let currentRevision = currentTranscriptPresentationRevision
		...
		liveBottomRestoreCorrectionCount += 1
		debugLog(
			"continueRehydrateRestoreIfNeeded reassert bottom",
			details: "source=\(source) presentationRevision=\(currentRevision) distance=\(Int(scrollMetrics.distanceToBottom)) correction=\(liveBottomRestoreCorrectionCount)"
		)
	...
	private func finishRehydrateRestoreIfSettled(proxy: ScrollViewProxy? = nil) {
		guard case .driving(let tabID, let presentationRevision, let target) = rehydrateRestorePhase else { return }
		...
		if case .liveBottom = target {
			let now = Date()
			if canCompleteLiveBottomRehydrateRestore(
				tabID: tabID,
				presentationRevision: presentationRevision,
				now: now
			) {
				...
				if let quietDelay = liveBottomRestoreRemainingQuietDelay(now: now),
					let proxy {
					scheduleDeferredLiveBottomRestoreSettle(proxy: proxy, after: quietDelay)
					return
				}
				if let proxy,
					continueRehydrateRestoreIfNeeded(proxy: proxy, source: .settled) {
					return
				}
				let shouldEndDegraded = !canContinueLiveBottomRestore(now: now)
				if shouldEndDegraded {
					noteRehydrateRestoreTelemetry("completeDegraded", extra: "presentationRevision=\(presentationRevision)")
				} else {
					debugLog(
						"finishRehydrateRestoreIfSettled awaiting strict layout",
						details: "sample=\(String(describing: currentRehydrateLayoutSampleKey)) presentationRevision=\(presentationRevision) distance=\(Int(scrollMetrics.distanceToBottom)) correction=\(liveBottomRestoreCorrectionCount)"
					)
					noteRehydrateRestoreTelemetry("settleAwaiting", extra: "presentationRevision=\(presentationRevision)")
					if let proxy,
						let watchdogDelay = liveBottomRestoreWatchdogSettleDelay(now: now) {
						scheduleDeferredLiveBottomRestoreSettle(proxy: proxy, after: watchdogDelay)
					}
					return
				}
			}
	...
	private func applyTranscriptAttachmentTarget(
		...
		case .detached(let userInitiated):
			clearPinnedTranscriptChangeSuppression()
			clearPinnedIdleTransitionManualDetach()
			if userInitiated {
				cancelPendingScrollWork()
	...
	private static let largeStreamingAssistantCharacterThreshold = 12_000
	...
	private func bottomScrollOutcomeSource(for reason: AgentTranscriptScrollReason) -> AgentTranscriptBottomScrollOutcomeSource? {
		...
		case .transcriptChangeWhilePinned,
			...
			return .pinnedFollowMaintenance
		default:
			return nil
		}
	}
	...
	private var hasPendingExplicitBottomScrollOutcome: Bool {
		guard let pendingBottomScrollOutcome,
			pendingBottomScrollOutcome.tabID == currentTabID else {
			return false
		}
	...
	private func resolvePendingBottomScrollOutcomeOnSettled(for settleReason: AgentTranscriptScrollReason) {
		guard let pendingBottomScrollOutcome,
			currentTabID == pendingBottomScrollOutcome.tabID else {
			return
		}
	...
	private func publishStressTelemetrySnapshot() {
		...
		let expandedApplyEditsCardCount = visibleRenderStates.filter { $0.toolName == "apply_edits" && $0.isExpanded }.count
		let expandedApplyEditsDiffPreviewCardCount = visibleRenderStates.filter {
			$0.toolName == "apply_edits" && $0.isExpanded && $0.renderMode == .diffPreview
		}.count
		let expandedApplyEditsMarkdownFallbackCardCount = visibleRenderStates.filter {
			$0.toolName == "apply_edits" && $0.isExpanded && $0.renderMode == .markdownFallback
		}.count
		let expandedApplyPatchCardCount = visibleRenderStates.filter { $0.toolName == "apply_patch" && $0.isExpanded }.count
		let expandedApplyPatchDiffPreviewCardCount = visibleRenderStates.filter {
			$0.toolName == "apply_patch" && $0.isExpanded && $0.renderMode == .diffPreview
		}.count
		let expandedApplyPatchMarkdownFallbackCardCount = visibleRenderStates.filter {
			$0.toolName == "apply_patch" && $0.isExpanded && $0.renderMode == .markdownFallback
		}.count
		let liveBashCardCount = visibleRenderStates.filter { $0.toolName == "bash" && $0.bashPhase == .live }.count
		let expandedLiveBashCardCount = visibleRenderStates.filter { $0.toolName == "bash" && $0.bashPhase == .live && $0.isExpanded }.count
		let completedBashCardCount = visibleRenderStates.filter { $0.toolName == "bash" && $0.bashPhase == .completed }.count
		let expandedCompletedBashCardCount = visibleRenderStates.filter { $0.toolName == "bash" && $0.bashPhase == .completed && $0.isExpanded }.count
