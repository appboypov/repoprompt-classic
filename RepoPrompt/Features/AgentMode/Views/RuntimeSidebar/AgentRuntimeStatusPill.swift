...
struct AgentRuntimeStatusPill: View {
	...
	private var isOracleStreaming: Bool {
		guard let tabID = currentTabID else { return false }
		let tabSessions = chatViewModel.sessions(forTabID: tabID)
	...
	private var pillBorderColor: Color {
		switch mode {
		case .contextBuilder: return Color.blue.opacity(0.3)
		case .oracle: return Color.purple.opacity(0.3)
		case .idle: return Color.secondary.opacity(isHovered ? 0.3 : 0.15)
		}
	}
}
