...
//  RepoPrompt
//
//  Input bar for Agent Mode that matches ChatInputBar styling and behavior.
...
struct AgentComposerActions {
	...
	let modelOptions: (_ agent: DiscoverAgentKind, _ includeClaudeEffortVariants: Bool) -> [AgentModelOption]
	let canSelectAgentInCurrentChat: (_ agent: DiscoverAgentKind) -> Bool
	let selectAgentModel: (_ agent: DiscoverAgentKind, _ rawModel: String) -> Void
...
struct AgentInputBar: View {
	...
	private var composerActions: AgentComposerActions {
		AgentComposerActions(
			...
			submit: { target, text in
				await agentModeVM.submitUserTurnCreatingSessionIfNeeded(text: text, target: target)
			},
...
struct AgentComposerView: View, Equatable {
	...
	@State private var modelMenuSnapshotByAgent: [DiscoverAgentKind: [AgentModelOption]]? = nil
	...
	private var composerChromeVerticalPadding: CGFloat { fontPreset.scaledClamped(8, max: 8) }
	private var composerChromeInnerSpacing: CGFloat { fontPreset.scaledClamped(2, max: 2) }
	private var composerControlStripHeight: CGFloat { fontPreset.scaledClamped(40, min: 40, max: 44) }
		...
		VStack(alignment: .leading, spacing: 8) {
			...
			ResizableTextField(
				...
				features: .agentInputBar(
					fileTagFileManager: promptManager.fileManager,
	...
	private func providerChipTooltip(isLocked: Bool, lockedMessage: String?) -> String {
		...
		if modelControlsDisabled {
			baseTooltip = modelControlsDisabledTooltip
		} else if isLocked {
			baseTooltip = lockedMessage ?? "This chat is limited to this agent family."
		} else {
			baseTooltip = "Choose agent and model"
		}
	...
	private var connectAgentProvidersButton: some View {
		...
		.buttonStyle(.plain)
		.hoverTooltip("Open Settings to connect Claude Code, Codex, Gemini, or OpenCode for Agent Mode")
		.fixedSize(horizontal: true, vertical: false)
	}
	...
	private func inputBarModelMenuItems(for agent: DiscoverAgentKind) -> [StableMenuItem] {
	...
	private func inputBarOpenCodeModelMenuItem(agent: DiscoverAgentKind, group: AgentModelCatalog.OpenCodeMenuGroup) -> StableMenuItem {
	...
	private func inputBarModelMenuItem(agent: DiscoverAgentKind, model: AgentModelOption, title: String? = nil) -> StableMenuItem {
		...
		let isLockedGeminiModelChange = props.isProviderPickerLockedForCurrentTab &&
			props.selectedAgent == .gemini &&
			agent == .gemini &&
			!isSelected
		return StableMenuItem.action(
			title ?? model.displayName,
			isEnabled: !isLockedGeminiModelChange,
	...
	private func inputBarModelOptions(for agent: DiscoverAgentKind) -> [AgentModelOption] {
	...
	private func captureModelMenuSnapshot() {
		modelMenuSnapshotReleaseTask?.cancel()
		var snapshot: [DiscoverAgentKind: [AgentModelOption]] = [:]
		for agent in props.availableAgents {
		...
		if let claudeTools = props.providerControls?.claudeTools {
			Form {
				...
				Section {
					...
					Text(claudeTools.mcpStrictModeEnabled
				...
				Section {
					...
					Text(claudeTools.toolSearchEnabled
...
struct AgentImageInputAdapter {
	...
	private static let maxPlainTextCandidateCharacters = 50_000
