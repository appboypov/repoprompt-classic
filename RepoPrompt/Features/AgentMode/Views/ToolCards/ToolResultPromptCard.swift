...
struct PromptResultCard: View {
	...
	init(item: AgentChatItem, promptManager: PromptViewModel? = nil) {
		...
		self._isExpanded = State(initialValue: Self.shouldAutoExpandInitially(item: item))
	}

	private static func shouldAutoExpandInitially(item: AgentChatItem) -> Bool {
		guard ToolJSON.payloadHasContent(item.toolResultJSON), item.toolIsError != true else { return false }
		let payloadSource = ToolJSON.resultPayloadSource(for: item, rawPayload: nil)
		guard let dto = ToolJSON.decodeResult(ToolResultDTOs.PromptToolEnvelope.self, from: payloadSource) else {
			return false
		}
	...
	private var dto: ToolResultDTOs.PromptToolEnvelope? {
		ToolJSON.decodeResult(ToolResultDTOs.PromptToolEnvelope.self, from: payloadSource)
	}
	...
	private var detailText: String? {
		...
		case "list_presets":
			...
			let visible = presets.prefix(2).map { $0.preset.name }
			var parts = visible
	...
	private var isExpandable: Bool {
		promptResultPayloadIsExpandable(
			isExportResult: isExportResult,
			payload: payloadSource.preferredPayload
		)
	}

	private func reconcileExpansion() {
		guard isExportResult else { return }
		let shouldExpand = isExpandable && item.toolIsError != true
	...
	var body: some View {
		...
		.onChange(of: rawPayloadRenderRevision) { _, _ in
			reconcileExpansion()
		}
	}
}

func promptResultPayloadIsExpandable(isExportResult: Bool, payload: String?) -> Bool {
	if isExportResult {
		return true
	}
	return ToolJSON.payloadHasContent(payload) && !ToolJSON.payloadIsSummaryOnly(payload)
}
