...
func normalizedToolCardName(_ name: String?) -> String? {
	...
	if lowered == "edit" || lowered == "edit_file" || lowered == "edit file" {
		return "edit"
	}
...
enum ToolCardRouter {
	...
	static func callView(
		...
		if key == "context_builder", let contextBuilder {
			return AnyView(ContextBuilderCallCard(item: item, context: contextBuilder))
		}
	...
	static func resultView(
		...
		case "context_builder":
			if let contextBuilder {
				return AnyView(ContextBuilderResultCard(item: item, context: contextBuilder))
			}
			return AnyView(UnknownToolResultCard(item: item, title: toolDisplayName(for: normalized ?? item.toolName)))
		case "agent_explore":
...
private enum ToolCardSubtitleBuilder {
	static func subtitle(for toolName: String?, argsJSON: String?) -> String? {
		...
		case "file_search":
			if let args = ToolJSON.decodeArgs(ToolArgsDTOs.FileSearchArgs.self, from: argsJSON) {
				...
				let parts = [patternPart, scopePart].compactMap { $0 }
		...
		case "agent_manage":
			if let args = ToolJSON.decodeArgs(ToolArgsDTOs.AgentManageArgs.self, from: argsJSON) {
				...
				case "list_sessions":
					...
					return (["list sessions"] + [suffix].compactMap { $0 }).joined(separator: " • ")
				case "get_log":
					var parts = ["get log"]
					if let sessionID = args.sessionID, !sessionID.isEmpty {
						parts.append(sessionID)
	...
	private static func fileTreeModeLabel(_ raw: String?) -> String {
		...
		case "auto", nil, "":
			return "auto tree"
		default:
			return raw?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() ?? "auto tree"
		}
	}
