...
typealias AgentRawToolResultPayloadResolver = @MainActor (UUID) -> String?

private struct AgentRawToolResultPayloadResolverKey: EnvironmentKey {
	static let defaultValue: AgentRawToolResultPayloadResolver? = nil
}

private struct AgentRawToolResultPayloadRenderRevisionKey: EnvironmentKey {
...
extension EnvironmentValues {
	...
	var agentRawToolResultPayloadResolver: AgentRawToolResultPayloadResolver? {
		get { self[AgentRawToolResultPayloadResolverKey.self] }
		set { self[AgentRawToolResultPayloadResolverKey.self] = newValue }
	}

	var agentRawToolResultPayloadRenderRevision: Int {
		get { self[AgentRawToolResultPayloadRenderRevisionKey.self] }
		set { self[AgentRawToolResultPayloadRenderRevisionKey.self] = newValue }
	}
}

enum AgentToolCardRenderedBashPhase: String, Equatable, Sendable {
	case live
	case completed
}

enum AgentToolCardRenderMode: String, Equatable, Sendable {
	case diffPreview
	case toolSpecificNoDiff
	case markdownFallback
}

struct AgentToolCardRenderState: Equatable, Sendable {
	let itemID: UUID
	let toolName: String
	let isExpanded: Bool
...
struct ToolCardContainer<Content: View>: View {
	...
	init(
		...
		self.managesOwnExpansion = false
		self.rawPayloadItemID = rawPayloadItemID ?? debugItemID
	...
	private var cardBody: some View {
		VStack(alignment: .leading, spacing: isExpanded ? 8 : 0) {
			...
			if isExpanded && effectiveIsExpandable {
				content()
			}
		}
		.padding(10)
	...
	private var effectiveIsExpandable: Bool {
		guard !isExpandable,
		      let rawPayloadItemID,
		      rawToolResultPayloadResolver?(rawPayloadItemID) != nil
		else {
			return isExpandable
		}
		return true
		...
		VStack(alignment: .leading, spacing: 2) {
			HStack(spacing: 6) {
				...
				if let headerTrailingView {
					headerTrailingView
				} else if showsTimestamp, let timestamp = timestamp {
	...
	private var backgroundColor: Color {
		switch status {
		case .running:
			return BubbleColors.toolCallBackground(colorScheme: colorScheme)
		case .failure:
			return BubbleColors.errorBackground(colorScheme: colorScheme)
		default:
			return BubbleColors.toolResultBackground(colorScheme: colorScheme)
		}
	}
	...
	private var debugRenderStatePreference: [UUID: AgentToolCardRenderState] {
		...
		return [
			debugItemID: AgentToolCardRenderState(
				...
				isExpanded: isExpanded && effectiveIsExpandable,
...
func toolIcon(for toolName: String?) -> String {
	...
	case "apply_edits", "mcp__RepoPrompt__apply_edits":
		return "pencil"
	case "apply_patch", "edit", "edit_file":
		return "pencil"
