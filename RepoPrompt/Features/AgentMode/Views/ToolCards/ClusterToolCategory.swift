...
enum ClusterToolCategory {
	...
	static func classification(forNormalizedToolName name: String) -> ToolClassification {
		...
		case let name where editTools.contains(name):
			return ToolClassification(family: .edit, summaryTitleSignal: .edit)
		case let name where execTools.contains(name):
			return ToolClassification(family: .execution, summaryTitleSignal: .execution)
		case let name where commTools.contains(name):
			return ToolClassification(family: .communication, summaryTitleSignal: .none)
		case let name where agentControlTools.contains(name):
			return ToolClassification(family: .agentControl, summaryTitleSignal: .agentControl)
		case let name where configTools.contains(name):
			return ToolClassification(family: .config, summaryTitleSignal: .none)
		default:
			return ToolClassification(family: .other, summaryTitleSignal: .none)
		}
	}
	...
	private static let configTools: Set<String> = [
		"manage_selection", "workspace_context", "prompt", "git", "bind_context", "manage_workspaces", "list_models", "context_builder",
		"app_settings"
	]
