...
struct BashResultCard: View {
	...
	var body: some View {
		...
		ToolCardContainer(
			...
			let expandedParsedResult = liveExecution?.parsedResult ?? BashToolResultParser.parse(
				raw: rawToolResultPayloadResolver?(item.id) ?? item.toolResultJSON,
				argsJSON: item.toolArgsJSON
			)
			BashTerminalOutputView(
				output: expandedParsedResult.output,
				isRunning: expandedParsedResult.isRunning
			)
		}
	}
}
...
private struct BashTerminalOutputView: View {
	...
	private func outputHeight(for text: String) -> CGFloat {
		...
		let lineCount = trimmed.utf8.lazy.filter { $0 == UInt8(ascii: "\n") }.count + 1
