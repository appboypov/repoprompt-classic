	...
	private func buildParts() -> [Text] {
		...
		if !toolNameCounts.isEmpty {
			...
			for name in toolNames {
				...
				if !seen.contains(key) { seen.insert(key); result.append(name) }
			}
			...
			unique = toolNames.filter { let k = $0.lowercased(); if seen.contains(k) { return false }; seen.insert(k); return true }
		}
