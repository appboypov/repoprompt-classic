	...
	struct Line: Equatable, Hashable {
		enum Kind: Equatable, Hashable {
			...
			func nsTextColor(colorScheme: ColorScheme) -> NSColor {
				...
				case .context:
					return colorScheme == .dark ? .textColor : .labelColor
				}
			}
			...
			func nsBackgroundColor(colorScheme: ColorScheme) -> NSColor? {
				switch self {
				case .addition:
					return NSColor.systemGreen.withAlphaComponent(colorScheme == .dark ? 0.15 : 0.1)
				case .deletion:
					return NSColor.systemRed.withAlphaComponent(colorScheme == .dark ? 0.15 : 0.1)
				case .fileHeader:
					return NSColor.secondaryLabelColor.withAlphaComponent(0.05)
				case .gap:
