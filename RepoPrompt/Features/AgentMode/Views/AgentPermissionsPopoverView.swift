...
struct AgentPermissionsPopoverView: View {
	...
	private func shortLabel(for policy: AgentSubagentPermissionPolicy) -> String {
		switch policy {
		case .safeManaged: return "Safe"
		case .inheritProviderSettings: return "Inherit"
	...
	private func summaryDetail(for policy: AgentSubagentPermissionPolicy) -> String {
		switch policy {
		case .safeManaged:
			return "Sub-agents run with Safe Managed sandbox overrides. Recommended."
		case .inheritProviderSettings:
			return "Sub-agents inherit your provider permissions, including permissive modes."
		case .custom:
			return "Sub-agents use the per-provider modes configured for each CLI."
		}
	}
	...
	private func deepLinkTitle(for policy: AgentSubagentPermissionPolicy) -> String {
		switch policy {
		case .safeManaged:
			return "Review Safe Managed profile"
		case .inheritProviderSettings:
			return "Open Sub-Agent Overrides"
		case .custom:
			return "Edit per-provider modes"
		}
	}
