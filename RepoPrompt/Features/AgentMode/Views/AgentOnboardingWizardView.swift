...
struct AgentOnboardingWizardView: View {
	...
	private var stepContent: some View {
		ScrollView {
			Group {
				...
				case .completion:
					CompletionStepView(viewModel: viewModel, windowID: windowID,
	...
	private var nextButtonLabel: String {
		...
		case .agentModeIntro:  return "Next"
		case .contextBuilder:  return "Next"
		case .mcpSetup:        return "Finish Setup"
		case .providers:       return "Next"
	...
	/// "Skip All" mid-wizard: jump straight into Agent Mode so the user lands
	/// somewhere useful instead of the workspace chooser. Falls back to the standard
	/// dismiss flow if the host did not provide an Agent Mode switch handler.
	private func skipAll() {
		viewModel.markOnboardingSeen()
		if let onSwitchToAgent {
			onSwitchToAgent()
		} else {
			onDismiss?()
		}
...
/// Welcome step shown to every user.
private struct WelcomeStepView: View {
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 28) {
			Text("Welcome! Here's what RepoPrompt can do — try Agent Mode to orchestrate your CLI agents with full codebase context, or set up the MCP server to supercharge tools like Claude Code, Codex, and Gemini CLI.")
				.font(.title3)
				.foregroundColor(.secondary)
			...
			HStack(spacing: 20) {
				ModeCard(
					...
					title: "IDE Mode",
					description: "Craft precise prompts with full control over file selection, codemaps, and context — get the best responses from reasoning models.",
					accentColor: .blue,
					delay: 0.1
				)
				...
				HStack(alignment: .top, spacing: 10) {
					...
					VStack(alignment: .leading, spacing: 4) {
						Text("Use in Any Coding Agent")
							.font(.headline)
						Text("Give Claude Code, Cursor, Codex, Gemini CLI, and other MCP-compatible tools access to RepoPrompt's token-efficient context tools.")
							.foregroundColor(.secondary)
							.fixedSize(horizontal: false, vertical: true)
					}
				}
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 18) {
			...
			HStack(spacing: 12) {
				...
				AgentIntegrationBadge(name: "Gemini CLI", icon: "terminal", status: "MCP", color: .green)
			...
			VStack(spacing: 10) {
				AnimatedFeatureRow(
					...
					description: "Agents trigger Context Builder directly — a discovery agent curates the right files, then an analysis model generates plans, reviews, or deep answers grounded in your actual code.",
					delay: 0.1,
					appeared: $showRows[0]
				)
		...
		VStack(alignment: .leading, spacing: 24) {
			...
			HStack(alignment: .top, spacing: 12) {
				...
				VStack(alignment: .leading, spacing: 4) {
					...
					Text("Agent Mode inside RepoPrompt already includes every tool below — no MCP setup required. Configure the MCP server only if you want to give external clients (Claude Code, Cursor, Codex CLI, Gemini CLI, OpenCode, Claude Desktop, VS Code) access to RepoPrompt's context tools.")
						.foregroundColor(.secondary)
						.fixedSize(horizontal: false, vertical: true)
				}
				Spacer(minLength: 0)
			...
			VStack(alignment: .leading, spacing: 12) {
				...
				HStack(spacing: 10) {
					Menu {
						...
						Button("Gemini CLI") { viewModel.installMCPServer(in: "Gemini CLI") }
					...
					Menu {
						...
						Section {
							...
							Menu("Per-project") {
								Button("MCP skills") {
									let count = MCPIntegrationHelper.installClaudeCodeCommands(workspacePaths: [], useCLIVariant: false)
									viewModel.showInstallFeedbackPublic(count > 0 ? "Commands installed" : "Open a folder first", isError: count == 0)
								}
								Button("rp-cli skills") {
									let count = MCPIntegrationHelper.installClaudeCodeCommands(workspacePaths: [], useCLIVariant: true)
									viewModel.showInstallFeedbackPublic(count > 0 ? "Commands installed" : "Open a folder first", isError: count == 0)
								}
								Divider()
								Button("Uninstall") {
									let count = MCPIntegrationHelper.uninstallClaudeCodeCommands(workspacePaths: [], useCLIVariant: false)
									let cliCount = MCPIntegrationHelper.uninstallClaudeCodeCommands(workspacePaths: [], useCLIVariant: true)
									viewModel.showInstallFeedbackPublic(count + cliCount > 0 ? "Commands removed" : "Nothing to remove")
								}
							}
						}
			...
			VStack(spacing: 12) {
				...
				MCPAdvantageRow(
					...
					description: "Two-stage pipeline: a discovery agent finds relevant files, then an analysis model produces deep plans, reviews, or answers.",
					color: .blue
				)
				...
				MCPAdvantageRow(
					...
					description: "Claude Code, Cursor, VS Code, Codex CLI, Gemini CLI, OpenCode, Claude Desktop \u{2014} install in any MCP client with one click.",
					color: .green
				)
			}
		}
...
private struct ProvidersStepView: View {
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 20) {
			...
			Text("Connect your CLI tools below — each one uses your existing subscription, no extra API keys needed. These providers power Agent Mode and Context Builder.")
				.foregroundColor(.secondary)
				.fixedSize(horizontal: false, vertical: true)
			...
			VStack(spacing: 10) {
				...
				CompactProviderRow(
					...
					icon: "terminal",
					description: "Headless CLI via MCP — context tools and editing, session management",
					isConnected: viewModel.geminiConnected,
					isLoading: viewModel.isLoadingGemini,
					errorText: viewModel.geminiError,
					setupHint: "Run 'gemini auth login' in terminal",
					isRecommended: false,
					onTest: { viewModel.testGemini() }
				)
			...
			VStack(spacing: 14) {
				if let onSwitchToAgent {
					QuickActionRow(
						...
						description: "Start using Agent Mode right now"
					) {
						onSwitchToAgent()
					}
				}
