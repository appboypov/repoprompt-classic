...
final class BenchmarkSettingsViewModel: ObservableObject {
	enum Tab: String, CaseIterable, Identifiable {
		...
		var title: String {
			...
			case .leaderboard: return "Local Leaderboard"
			}
		}
	}
	
	struct ProgressStep: Identifiable, Sendable {
		let id = UUID()
	...
	private struct SeedProgress: Sendable {
		var totalTasks: Int
		var completedTasks: Int
		var taskTypes: [BenchmarkCaseType]
	}
	
	struct DebugTaskRef: Identifiable, Sendable {
	...
	@Published private(set) var progressSummary: ProgressSummary = ProgressSummary()
	...
	init(
		...
		self.selectedModelRaw = promptViewModel.preferredModel
		self.coreSeedInput = String(BenchmarkSeedUtilities.canonicalCoreSeed)
		self.availableModels = apiSettingsViewModel.availableModels
		if let persistedModel = UserDefaults.standard.string(forKey: Self.selectedModelDefaultsKey) {
			self.selectedModelRaw = persistedModel
		}
	...
	private func handleProgressEvent(_ event: BenchmarkProgressEvent) {
		...
		case .seedStarted(let index, let seed, let totalSeeds, let taskCount, let taskTypes):
			...
			progressSummary.totalSeeds = totalSeeds
		case .taskCompleted(let seed, let completed, let total):
	...
	private func makeDebugLogContent(
		...
		if !sortedTypeStats.isEmpty {
			...
			for (type, stats) in sortedTypeStats {
				lines.append(String(format: "- %@ • count %d • pass %.2f%% • avg %.2f",
					type.rawValue,
					stats.count,
					stats.passRate * 100,
					stats.averageScore))
	...
	private func makeSingleTestDebugLogContent(
		...
		if let meta = result.meta {
			...
			if case .array(let virtualFilesArray)? = meta["virtualFiles"] {
				lines.append("## Virtual Files")
				for vf in virtualFilesArray {
					if case .object(let obj) = vf,
