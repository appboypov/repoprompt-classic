...
struct BenchmarkVerifierReasons {
...
struct BenchmarkVerifier: BenchmarkVerifying {
	...
	func verify(_ execution: BenchmarkTaskExecution) -> BenchmarkVerifyOutput {
		...
		let base: BenchmarkVerifyOutput
		switch task.type {
		case .removeXTs, .removeXGo, .removeXSwift:
			base = verifyRemoveX(task: task, baseline: baselineMap, final: finalMap)
		case .curlyFixTs, .curlyFixGo, .curlyFixSwift:
			base = verifyCurlyFix(task: task, baseline: baselineMap, final: finalMap)
		case .insertGuardTs, .insertGuardGo, .insertGuardSwift:
			base = verifyInsertGuard(task: task, baseline: baselineMap, final: finalMap)
		case .patchBlockTs, .patchBlockGo, .patchBlockSwift:
			base = verifyPatchBlock(task: task, baseline: baselineMap, final: finalMap)
		case .swapArgsInRegionTs, .swapArgsInRegionGo, .swapArgsInRegionSwift:
			base = verifySwapArgs(task: task, baseline: baselineMap, final: finalMap)
		case .indexOnlyAppsTs, .indexOnlyAppsGo, .indexOnlyAppsSwift:
			base = verifyIndexOnly(task: task, baseline: baselineMap, final: finalMap)
		case .renameExportImportsTs, .renameExportImportsGo, .renameExportImportsSwift:
			base = verifyRename(task: task, baseline: baselineMap, final: finalMap)
		case .moveFunctionTs, .moveFunctionGo, .moveFunctionSwift:
			base = verifyMoveFunction(task: task, baseline: baselineMap, final: finalMap)
		case .insertFunctionBottomTs, .insertFunctionBottomGo, .insertFunctionBottomSwift:
			base = verifyInsertFunctionBottom(task: task, baseline: baselineMap, final: finalMap)
		case .applyUnifiedPatchTs, .applyUnifiedPatchGo, .applyUnifiedPatchSwift:
			base = verifyApplyUnifiedPatch(task: task, baseline: baselineMap, final: finalMap)
		}
	...
	private func applyOutputPenalty(_ result: BenchmarkVerifyOutput, meta: [String: BenchmarkJSONValue]?) -> BenchmarkVerifyOutput {
		...
		let charOver = max(0, charCount - 10_000)
		let lineOver = max(0, lineCount - 200)
		if charOver == 0 && lineOver == 0 { return result }
		let charPenalty = 1.0 / (1.0 + Double(charOver) / 5_000.0)
	...
	private func verifyCurlyFix(task: BenchmarkTaskSpec, baseline: [String: String], final: [String: String]) -> BenchmarkVerifyOutput {
		if let files = task.params["files"]?.arrayValue?.compactMap({ $0.stringValue }), !files.isEmpty {
		...
		let printRegex: NSRegularExpression? = {
			switch task.language {
			case .go:
				return try? NSRegularExpression(pattern: #"fmt\.Print(ln|f)\s*\([^)]*\)"#, options: [])
			case .ts:
				return try? NSRegularExpression(pattern: #"console\.log\s*\([^)]*\)"#, options: [])
			case .swift:
				return try? NSRegularExpression(pattern: #"print\s*\([^)]*\)"#, options: [])
			}
		}()
		var printsInside = 0
		var printsOutside = 0
	...
	private func verifyApplyUnifiedPatch(task: BenchmarkTaskSpec, baseline: [String: String], final: [String: String]) -> BenchmarkVerifyOutput {
		...
		if discovery {
			...
			// Load candidates
			let candidatesParam = task.params["candidatePaths"]?.arrayValue?.compactMap { $0.stringValue } ?? []
	...
	private func verifyInsertGuardMarkerless(task: BenchmarkTaskSpec, baseline: [String: String], final: [String: String]) -> BenchmarkVerifyOutput {
		...
		let reason: String
		if !snippetFound {
			reason = "snippetMissing"
		} else if !afterPatternOk {
			reason = "snippetNotAfterPattern"
		} else if !unchangedOutside {
			reason = "collateralDamage"
		} else {
			reason = ""
		}
	...
	private func verifyInsertGuard(task: BenchmarkTaskSpec, baseline: [String: String], final: [String: String]) -> BenchmarkVerifyOutput {
		...
		let indentOptions = [baselineIndent, finalIndent].compactMap { $0 }.filter { !$0.isEmpty }
		...
		let withoutSnippet: String
		if let used = matchedSnippet {
			withoutSnippet = finalSegment.replacingOccurrences(of: used, with: "")
		} else {
			withoutSnippet = finalSegment
		}
		...
		let reason: String
		if snippetFound {
			reason = unchangedOutside ? "" : "collateralChangeInsideAnchors"
		} else {
			reason = "snippetMismatch"
		}
		let metrics: [String: BenchmarkJSONValue] = [
			"snippetFound": .boolean(snippetFound),
			"unchangedOutside": .boolean(unchangedOutside)
	...
	private func verifyIndexOnly(task: BenchmarkTaskSpec, baseline: [String: String], final: [String: String]) -> BenchmarkVerifyOutput {
		...
		let reason: String
		if doneInsideFunction && returnExact {
			reason = ""
		} else if !doneInsideFunction {
			reason = "doneMarkerMissingOrOutsideFunction"
		} else {
			reason = "returnChanged"
		}
		return makeScoredOutput(score: score, reason: reason, metrics: [
	...
	private func verifyRename(task: BenchmarkTaskSpec, baseline: [String: String], final: [String: String]) -> BenchmarkVerifyOutput {
		...
		let nearTokens = task.params["nearMissTokens"]?.arrayValue?.compactMap { $0.stringValue } ?? []
		...
		if let reexports = task.params["reexportPaths"]?.arrayValue?.compactMap({ $0.stringValue }), !reexports.isEmpty {
	...
	private func aggregate(_ parts: [(String, BenchmarkVerifyOutput)], label: String) -> BenchmarkVerifyOutput {
		...
		let allPass = parts.allSatisfy { $0.1.pass }
	...
	private func containsGuardSnippet(in segment: String, language: BenchmarkLanguage, expected: String) -> Bool {
		...
		switch language {
		case .ts:
			pattern = #"if\s*\(\s*n\s*<\s*0\s*\)\s*\{\s*return\s+0\s*;?\s*\}"#
		case .go:
			pattern = #"if\s+n\s*<\s*0\s*\{\s*return\s+0\s*\}"#
		case .swift:
			pattern = #"if\s+n\s*<\s*0\s*\{\s*return\s+0\s*\}"#
		}
