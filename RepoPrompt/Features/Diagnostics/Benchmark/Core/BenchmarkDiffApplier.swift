...
struct BenchmarkDiffApplier {
	...
	private static func minSearchLines(for caseType: BenchmarkCaseType) -> Int {
		...
		case .patchBlockTs, .patchBlockGo, .patchBlockSwift:
			return 3  // Blocks may have similar signatures; need surrounding context
		case .swapArgsInRegionTs, .swapArgsInRegionGo, .swapArgsInRegionSwift:
			return 3  // Multiple similar regions may exist; need distinct context
		case .applyUnifiedPatchTs, .applyUnifiedPatchGo, .applyUnifiedPatchSwift:
			return 3  // Patches should match sufficient context to avoid wrong location
		case .insertGuardTs, .insertGuardGo, .insertGuardSwift:
			return 2  // Anchors provide unique UIDs, reducing ambiguity risk
		case .renameExportImportsTs, .renameExportImportsGo, .renameExportImportsSwift:
			return 2  // Symbol names are typically unique within scope
		default:
			return 2  // Conservative minimum for general disambiguation
		}
	}
	...
	static func apply(
		...
		for parsedFile in parsedFiles {
			...
			do {
				...
				case .delegateEdit:
					// Delegate edits are skipped during parsing
					continue
				}
			...
			} catch BenchmarkDiffApplicationError.noChangesGenerated(let path) {
				errors.append(BenchmarkTaskError(code: "EDIT_APPLY_FAILED", path: path, detail: "noChangesGenerated"))
			} catch BenchmarkDiffApplicationError.searchBlockNotFound(let path) {
				errors.append(BenchmarkTaskError(code: "SEARCH_BLOCK_NOT_FOUND", path: path, detail: nil))
			} catch BenchmarkDiffApplicationError.editApplicationFailed(let path, let reason) {
	...
	private static func applyModifyChange(
		...
		let normalizedPath = relativePath(from: parsedFile.fileName)
		let (originalLines, detectedLineEnding) = String.splitContentPreservingLineEndings(originalText)
		let hadTrailingNewline = originalText.hasSuffix(detectedLineEnding)
		...
		for change in parsedFile.changes where change.isSelected {
			...
			for chunk in diffChunks {
				let decoded = chunk.getChunkWithDecodedIndentation()
		...
		for change in fileChanges {
			...
			if let err = result.error {
				throw BenchmarkDiffApplicationError.editApplicationFailed(path: parsedFile.fileName, reason: err.localizedDescription)
			}
		}
