...
final class BenchmarkTaskExecutor {
	...
	init(
		...
		maxDecoyPerFileChars: Int = 40_000
	) {
		...
		self.maxContextChars = max(10_000, maxContextChars)
		self.maxDecoyPerFileChars = max(2_000, maxDecoyPerFileChars)
	}
	...
	func runTask(
		...
		let benchmarkConfig = PromptConfig(
			...
			codeBlockFence: SystemPromptService.applyCodeFence,
	...
	private func fetchModelOutput(for message: AIMessage) async throws -> String {
		func performRequest() async throws -> String {
			...
			guard let aiQueriesService = aiQueriesService else {
	...
	private func composePackagedUserMessage(
		...
		let effectiveMaxContextChars: Int = {
			switch task.difficulty {
			case .hard:
				return Int(Double(self.maxContextChars) * 1.25)
			case .veryHard:
				return Int(Double(self.maxContextChars) * 1.5)
			default:
		...
		let fullDecoyPaths = task.params["fullDecoys"]?.arrayValue?.compactMap { $0.stringValue } ?? []
		...
		let decoyPaths = task.params["decoyPaths"]?.arrayValue?.compactMap { $0.stringValue } ?? []
...
"""
		...
		for path in otherDecoys {
			...
			let content: String
			if truncated {
				content = String(raw.prefix(allowed))
			} else {
				content = raw
			}
...
"""
			...
			case .renameExportImportsTs, .renameExportImportsGo, .renameExportImportsSwift:
				...
				let importerPaths = task.params["importPaths"]?.arrayValue?.compactMap { $0.stringValue } ?? []
				let allowed = ([task.selectFiles.first].compactMap { $0 } + importerPaths).filter { !$0.isEmpty }
...
private func shouldRetryAPIError(_ error: Error) -> Bool {
	...
	if let customError = error as? CustomOpenAIProviderError {
		...
		case .rateLimitExceeded, .serverError, .serviceUnavailable:
			return true
		case .requestFailed(statusCode: let status, _),
				.invalidResponse(statusCode: let status, _):
