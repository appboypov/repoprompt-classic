...
struct SimpleUnifiedPatchApplier {
	enum Op {
		case context(String)
		case minus(String)
		case plus(String)
...
struct UnifiedPatchGrader {
	...
	private static func isHunkApplied(hunk: SimpleUnifiedPatchApplier.Hunk, baseline: [String], final: [String]) -> Bool {
		...
		for minusLine in minusLines {
			let baselineCount = baseline.filter { $0 == minusLine }.count
			let finalCount = final.filter { $0 == minusLine }.count
