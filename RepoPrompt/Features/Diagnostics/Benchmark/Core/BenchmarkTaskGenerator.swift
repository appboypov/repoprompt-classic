...
struct BenchmarkTaskGenerator {
	...
	private func casePool(for language: BenchmarkLanguage, difficulty: BenchmarkDifficulty) -> [BenchmarkCaseType] {
		...
		case (.ts, .medium):
			return [.renameExportImportsTs, .removeXTs, .swapArgsInRegionTs, .indexOnlyAppsTs, .curlyFixTs]
		case (.ts, .hard):
			return [.patchBlockTs, .insertGuardTs, .insertFunctionBottomTs]
		case (.ts, .veryHard):
			return [.applyUnifiedPatchTs, .moveFunctionTs]
		case (.go, .medium):
			return [.renameExportImportsGo, .removeXGo, .swapArgsInRegionGo, .indexOnlyAppsGo, .curlyFixGo, .moveFunctionGo]
		case (.go, .hard):
			return [.insertFunctionBottomGo, .insertGuardGo, .patchBlockGo, .applyUnifiedPatchGo]
		case (.go, .veryHard):
			// Promote to VH using existing "VeryHard gauntlet" and multi-file exactness
			return [.applyUnifiedPatchGo, .patchBlockGo]
		case (.swift, .medium):
			return [.swapArgsInRegionSwift, .indexOnlyAppsSwift, .insertFunctionBottomSwift]
		case (.swift, .hard):
			return [.moveFunctionSwift, .insertGuardSwift, .patchBlockSwift]
		case (.swift, .veryHard):
			// Provide VH options: unified patch gauntlet, exact block patching, and strengthened move_function
			return [.applyUnifiedPatchSwift, .patchBlockSwift, .moveFunctionSwift]
		default:
			return []
		}
	}
	...
	private func scheduleCasePlan(for language: BenchmarkLanguage, count: Int, subseedIndex: Int = 0) -> [BenchmarkCaseType] {
		let canonical: [BenchmarkCaseType]
		switch language {
		case .ts:
	...
	private func generateTask(
		...
		case .curlyFixTs, .curlyFixGo, .curlyFixSwift:
			return generateCurlyFixTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .insertGuardTs, .insertGuardGo, .insertGuardSwift:
			return generateInsertGuardTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .patchBlockTs, .patchBlockGo, .patchBlockSwift:
			return generatePatchBlockTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .swapArgsInRegionTs, .swapArgsInRegionGo, .swapArgsInRegionSwift:
			return generateSwapArgsTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .indexOnlyAppsTs, .indexOnlyAppsGo, .indexOnlyAppsSwift:
			return generateIndexOnlyTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .renameExportImportsTs, .renameExportImportsGo, .renameExportImportsSwift:
			return generateRenameExportsTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .moveFunctionTs, .moveFunctionGo, .moveFunctionSwift:
			return generateMoveFunctionTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .insertFunctionBottomTs, .insertFunctionBottomGo, .insertFunctionBottomSwift:
			return generateInsertFunctionBottomTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
		...
		case .applyUnifiedPatchTs, .applyUnifiedPatchGo, .applyUnifiedPatchSwift:
			return generateUnifiedPatchTaskFor(
				language: language,
				rng: &rng,
				fileSystem: &fileSystem,
				config: config,
	...
	private func generateRemoveXTaskFor(
		...
		switch language {
		case .ts:
			return generateRemoveXTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .go:
			return generateRemoveXGoTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .swift:
			return generateRemoveXSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateCurlyFixTaskFor(
		...
		switch language {
		case .ts:
			return generateCurlyFixTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .go:
			return generateCurlyFixGoTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .swift:
			return generateCurlyFixSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateInsertGuardTaskFor(
		...
		switch language {
		case .ts:
			return generateInsertGuardTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .go:
			return generateInsertGuardGoTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .swift:
			return generateInsertGuardSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generatePatchBlockTaskFor(
		...
		switch language {
		case .ts:
			return generatePatchBlockTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .go:
			return generatePatchBlockGoTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .swift:
			return generatePatchBlockSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateSwapArgsTaskFor(
		...
		switch language {
		case .ts:
			return generateSwapArgsTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .go:
			return generateSwapArgsGoTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		case .swift:
			return generateSwapArgsSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, noise: noise, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateIndexOnlyTaskFor(
		...
		switch language {
		case .ts:
			return generateIndexOnlyTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .go:
			return generateIndexOnlyGoTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .swift:
			return generateIndexOnlySwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateRenameExportsTaskFor(
		...
		switch language {
		case .ts:
			return generateRenameExportsTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .go:
			return generateRenameExportsGoTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .swift:
			return generateRenameExportsSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateMoveFunctionTaskFor(
		...
		switch language {
		case .ts:
			return generateMoveFunctionTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .go:
			return generateMoveFunctionGoTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .swift:
			return generateMoveFunctionSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateInsertFunctionBottomTaskFor(
		...
		switch language {
		case .ts:
			return generateInsertFunctionBottomTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .go:
			return generateInsertFunctionBottomGoTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .swift:
			return generateInsertFunctionBottomSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateUnifiedPatchTaskFor(
		...
		switch language {
		case .ts:
			return generateUnifiedPatchTsTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .go:
			return generateUnifiedPatchGoTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		case .swift:
			return generateUnifiedPatchSwiftTask(rng: &rng, fileSystem: &fileSystem, config: config, difficulty: difficulty, layout: layout)
		}
	}
	...
	private func generateRenameExportsGoTask(
		...
		let apps: [String] = {
			...
			case .hard:
				return ["appA", "appB", "appC", "appD", "appE", "appF"]
			case .veryHard:
				return ["appA", "appB", "appC", "appD", "appE", "appF", "appG", "appH"]
			}
		}()
...
"""
	...
	private func generateMoveFunctionSwiftTask(
		...
		if difficulty == .veryHard {
			...
			let fnNames = ["alpha", "alphaHelper", "alphaUtil", "alphaPrime",
								"bravo", "bravoHelper", "charlie", "charlieX", "delta"]
			var lines: [String] = []
			for (index, name) in fnNames.enumerated() {
	...
	private func generateInsertFunctionBottomTask(
		...
		switch difficulty {
		case .medium:
			files = [self.tsWorkPath]
		case .hard:
			files = [self.tsWorkPath, "src/ts/work/WorkA.ts"]
		case .veryHard:
			files = [self.tsWorkPath, "src/ts/work/WorkA.ts", "src/ts/work/WorkB.ts"]
		case .simple:
			files = [self.tsWorkPath]
		}
...
"""
		...
		let fullDecoys = decoyBudget > 0 ? makeSimilarTsDecoys(rng: &rng, around: self.tsWorkPath) : []
		for decoy in fullDecoys {
			fileSystem.setFile(decoy.path, content: decoy.content)
		}
	...
	private func generateInsertFunctionBottomGoTask(
		...
		switch difficulty {
		case .medium, .simple:
			files = [self.goWorkPath]
		case .hard:
			files = [self.goWorkPath, "src/go/work/WorkA.go"]
		case .veryHard:
			files = [self.goWorkPath, "src/go/work/WorkA.go", "src/go/work/WorkB.go"]
		}
		let footer = "// END-OF-FILE (append new functions immediately above this line)"
		for path in files {
			var existing = fileSystem.content(for: path) ?? ""
...
"""
	...
	private func generateInsertFunctionBottomSwiftTask(
		...
		switch difficulty {
		case .medium, .simple:
			files = [self.swiftWorkPath]
		case .hard:
			files = [self.swiftWorkPath, "src/swift/work/WorkA.swift"]
		case .veryHard:
			files = [self.swiftWorkPath, "src/swift/work/WorkA.swift", "src/swift/work/WorkB.swift"]
		}
		let footer = "// END-OF-FILE (append new functions immediately above this line)"
		for path in files {
			var existing = fileSystem.content(for: path) ?? ""
...
"""
	...
	private func makeDeepClonePaths(token: String, ext: String, count: Int, language: BenchmarkLanguage) -> [String] {
		...
		switch language {
		case .ts:
			roots = ["src/ts/patchables", "apps/appA/src/patchables", "apps/appB/src/patchables", "packages/pkg1/src/patchables", "packages/pkg2/src/patchables"]
		case .go:
			roots = ["src/go/patchables", "apps/appA/patchables", "apps/appB/patchables", "packages/pkg1/patchables", "packages/pkg2/patchables"]
		case .swift:
			roots = ["src/swift/patchables", "Apps/appA/patchables", "Apps/appB/patchables", "Packages/Pkg1/patchables", "Packages/Pkg2/patchables"]
		@unknown default:
			roots = ["src/patchables"]
		}
		var out: [String] = []
		for i in 0..<count {
	...
	private func generateInsertGuardTask(
		...
		let fullDecoys = decoys > 0 ? makeSimilarTsDecoys(rng: &rng, around: self.tsWorkPath) : []
		for decoy in fullDecoys {
			fileSystem.setFile(decoy.path, content: decoy.content)
		}
...
"""
			...
			let instructions = [
				...
				"Use precise search-replace; do NOT use delegate edits or placeholders."
			]
			let acceptance = [
...
"""
	...
	private func generateSwapArgsTask(
		...
		case .hard:
			files = [self.tsWorkPath, "src/ts/work/WorkA.ts"]
		case .veryHard:
			files = [self.tsWorkPath, "src/ts/work/WorkA.ts", "src/ts/work/WorkB.ts", "src/ts/work/WorkC.ts"]
		case .simple:
			files = [self.tsWorkPath]
		}
...
"""
	...
	private func generateSwapArgsGoTask(
		...
		case .hard:
			files = [self.goWorkPath, "src/go/work/WorkA.go"]
		case .veryHard:
			files = [self.goWorkPath, "src/go/work/WorkA.go", "src/go/work/WorkB.go", "src/go/work/WorkC.go"]
		}
		let decoys = decoyCount(for: difficulty, config: config)
		_ = noise
		let isMarkerless = shouldUseMarkerlessMode(for: difficulty)
...
"""
	...
	private func generateSwapArgsSwiftTask(
		...
		case .hard:
			files = [self.swiftWorkPath, "src/swift/work/WorkA.swift"]
		case .veryHard:
			files = [self.swiftWorkPath, "src/swift/work/WorkA.swift", "src/swift/work/WorkB.swift", "src/swift/work/WorkC.swift"]
		}
		let decoys = decoyCount(for: difficulty, config: config)
		_ = noise
		let isMarkerless = shouldUseMarkerlessMode(for: difficulty)
...
"""
	...
	private func generateIndexOnlyGoTask(
		...
		let taskText: String = {
			switch difficulty {
			case .simple, .medium:
				return "Modify only \(targetPath) to mark completion for \(targetApp)."
			case .hard, .veryHard:
				return "Identify the primary app from apps/PRIMARY and add the DONE marker to that app's index file only."
			}
		}()
...
"""
	...
	private func scaledLines(_ base: Int, difficulty: BenchmarkDifficulty) -> Int {
		switch difficulty {
		case .medium:
			return max(20, Int(Double(base) * 0.35))
		case .hard:
			return max(30, Int(Double(base) * 0.6))
		case .veryHard:
			return max(40, base)
		case .simple:
			return max(20, Int(Double(base) * 0.35))
	...
	private func decoyCount(for difficulty: BenchmarkDifficulty, config: BenchConfig) -> Int {
		switch difficulty {
		case .medium:
			return config.decoysMedium
		case .hard:
			return config.decoysHard
		case .veryHard:
			return config.decoysVeryHard
		case .simple:
			return config.decoysMedium
		}
	}
...
"""
	...
	private func computeTsFunctionRange(_ base: [String], fnName: String) -> ClosedRange<Int>? {
		...
		var end = start
		while end < base.count && base[end] != "}" { end += 1 }
	...
	private func removedLineCount(before index: Int, in spans: [ClosedRange<Int>]) -> Int {
		return spans.filter { $0.upperBound < index }.map { $0.count }.reduce(0, +)
	}

	private func newStart(oldStart1Based: Int, bofAdded: Int, removedSpansBeforeTarget: Int) -> Int {
		return oldStart1Based + bofAdded - removedSpansBeforeTarget
	}
}
