...
enum DecoyPlanner {
    ...
    static func materialize(
        ...
        if shouldCreateMirrors && decoyBudget > 0 {
            ...
            let maxMirrorsPerTask: Int = {
                switch task.type {
                case .applyUnifiedPatchTs, .applyUnifiedPatchGo, .applyUnifiedPatchSwift:
                    return policy.style == .gauntlet ? 10 : 6
                default:
                    return 2  // Hard cap to prevent excessive proliferation
                }
            }()
...
extension DecoyPlanner {
    ...
    static func variants(for path: String, placement: DecoyPolicy.Placement, count: Int) -> [String] {
        ...
        switch placement {
        case .siblingDir:
            candidates = siblingDirCandidates()
        case .crossRoot:
            candidates = crossRootCandidates()
        case .mixed:
            candidates = siblingDirCandidates() + crossRootCandidates()
        }
...
extension DecoyPlanner {
    ...
    static func buildContextMirror(
        ...
        if let before = core.beforeLine {
            ...
            out.append(before.replacingOccurrences(of: "const", with: "let")
...
private extension DecoyPlanner {
    ...
    static func locateInsertGuardMarkerlessCore(in text: String, language: BenchmarkLanguage, functionName: String, insertAfterPattern: String) -> CoreRegion? {
        ...
        let signatureIndex: Int? = {
            switch language {
            case .ts:
                return lines.firstIndex(where: { $0.contains("function \(functionName)(") })
            case .go:
                return lines.firstIndex(where: { $0.contains("func \(functionName)(") })
            case .swift:
                return lines.firstIndex(where: { $0.contains("func \(functionName)(") })
            }
        }()
    ...
    static func locateFunctionCore(in text: String, language: BenchmarkLanguage, name: String) -> CoreRegion? {
        ...
        let signatureIndex: Int? = {
            ...
            case .ts:
                ...
                return lines.firstIndex(where: { $0.contains("function \(name)(") })
            case .go:
                // match: func [receiver] name(
                return lines.firstIndex(where: { $0.contains("func \(name)(") })
            case .swift:
