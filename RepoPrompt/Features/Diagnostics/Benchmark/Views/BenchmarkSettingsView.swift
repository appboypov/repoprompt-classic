...
struct BenchmarkSettingsView: View {
	...
	private var header: some View {
		VStack(alignment: .leading, spacing: 6) {
			Text("Repo Bench")
				.font(.title2).bold()
			Text("Run benchmarks against different models, track history, and compare scores on this Mac only.")
				.font(.subheadline)
				...
				HStack(spacing: 8) {
					...
					if let hadErrors = summary.hadErrors, hadErrors {
						Image(systemName: "exclamationmark.triangle.fill")
							.foregroundColor(.orange)
							.font(.caption)
							.hoverTooltip("This run encountered API errors and is excluded from local leaderboard rankings")
					}
				}
				ViewThatFits(in: .horizontal) {
		...
		VStack(alignment: .leading, spacing: 12) {
			HStack {
				...
				.confirmationDialog(
					...
					Text("This will permanently delete all benchmark run history and local leaderboard data. This action cannot be undone.")
				}
			}
		...
		VStack(alignment: .leading, spacing: 12) {
			...
			if viewModel.leaderboard.isEmpty {
				...
				VStack(spacing: 8) {
					...
					Text("No data yet")
						.font(.headline)
					Text("Run benchmarks to populate local model rankings.")
						.font(.caption)
						.foregroundColor(.secondary)
				}
				.frame(maxWidth: .infinity, maxHeight: .infinity)
