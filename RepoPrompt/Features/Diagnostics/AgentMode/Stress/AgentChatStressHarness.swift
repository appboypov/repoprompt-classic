...
struct AgentChatStressTelemetrySnapshot: Codable, Equatable, Sendable {
	var sampleIndex: Int
	var timestamp: Date
	var tabID: UUID?
...
struct AgentChatStressGroupingSnapshot: Codable, Equatable, Sendable {
	var sampleIndex: Int
...
final class AgentChatStressHarness: ObservableObject {
	...
	func start(currentTabID: UUID?) {
		...
		scenarioTask = Task { [weak self] in
			...
			if self.configuration.scenario == .persistedCodexReplayChurn {
				await self.bootstrapPersistedCodexReplayScenario(preferredTabID: currentTabID)
				return
			}
			if self.configuration.scenario == .persistedAgentSessionFixture {
				await self.bootstrapPersistedAgentSessionFixtureScenario(preferredTabID: currentTabID)
				return
			}
			let tabID = await self.resolveTargetTabID(preferred: currentTabID)
			guard !Task.isCancelled else { return }
			guard let tabID else {
				self.status = .failed("No compose tab available")
				self.note("Unable to acquire a compose tab")
				return
			}
			self.targetTabID = tabID
			await self.agentModeViewModel.testResetStressTranscript(tabID: tabID)
			await self.agentModeViewModel.testPrepareStressSession(tabID: tabID)
			await self.seedWarmup(on: tabID)
			self.beginLoop(on: tabID)
		}
	}
	...
	func reset(currentTabID: UUID?) {
		...
		Task { [weak self] in
			guard let self else { return }
			if let tabID = await self.resolveTargetTabID(preferred: currentTabID ?? self.targetTabID) {
				self.targetTabID = tabID
				await self.agentModeViewModel.testResetStressTranscript(tabID: tabID)
			}
			if self.configuration.autoStart {
				self.start(currentTabID: currentTabID)
			}
		}
	}
	...
	private func execute(step: ScenarioToolStep, on tabID: UUID, isWarmup: Bool) async {
		switch step {
		case .repoPromptTool(let toolName, let args, let resultJSON, let isError):
			if configuration.scenario == .richToolChurn {
				switch toolName {
				case "apply_edits":
		...
		case .liveBash(let command, let processID, let initialResultJSON, let outputChunks, let finalResultJSON, let isError):
			let invocationID = UUID()
	...
	private func richBashChunkDelayNanos(isWarmup: Bool) -> UInt64 {
		...
		if isWarmup {
			...
			seconds = max(0.11, min(0.14, configuration.insertionInterval * 1.35))
		}
	...
	private func makePersistedReplayMessage(
		...
		switch role {
		case .user:
			return .user(text, sequenceIndex: sequenceIndex)
		case .assistant:
			return .assistant(text, sequenceIndex: sequenceIndex)
		case .assistantInline:
			return .assistantInline(text, sequenceIndex: sequenceIndex)
		case .thinking:
			return .thinking(text, sequenceIndex: sequenceIndex)
		case .system:
			return .system(text, sequenceIndex: sequenceIndex)
		}
	}
	...
	private func appendPersistedReplayToolStep(
		...
		switch step {
		case .repoPromptTool(let toolName, _, let resultJSON, let isError):
			let invocationID = UUID()
			...
			nextSequenceIndex += 1
		case .liveBash(let command, _, _, _, let finalResultJSON, let isError):
			let invocationID = UUID()
	...
	private func loadPersistedCodexFixtureBashReplay(
		...
		guard let toolCall = events.first(where: {
			if case .toolCall(let name, _, _) = $0 {
				return name == "bash"
			}
			return false
		}) else {
		...
		guard case .toolCall(_, let invocationID, let argsJSON) = toolCall else {
			return nil
		}
	...
	private func assistantMarkdownChunkDelayNanos(isWarmup: Bool, usesMegaCadence: Bool) -> UInt64 {
		let seconds: TimeInterval
		if isWarmup {
			seconds = usesMegaCadence ? 0.018 : 0.02
		} else if usesMegaCadence {
			seconds = max(0.05, min(0.07, configuration.insertionInterval * 0.78))
		} else {
			seconds = max(0.06, min(0.09, configuration.insertionInterval * 0.85))
		}
		return UInt64(seconds * 1_000_000_000)
	}
