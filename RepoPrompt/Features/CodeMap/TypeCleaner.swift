...
struct TypeCleaner {
	...
	static func extractBaseTypes(from rawType: String, language: LanguageType) -> [String] {
		switch language {
		case .ts, .tsx:
			return extractBaseTypesTS(rawType, language: language)
		default:
			return extractBaseTypesNonTS(rawType, language: language)
		}
	}
	...
	private static func extractBaseTypesCached(
		...
		switch language {
		case .ts, .tsx:
			result = extractBaseTypesTSCached(rawType, language: language, cache: &cache, stats: activeStats)
		default:
			result = extractBaseTypesNonTSCached(rawType, language: language, cache: &cache, stats: activeStats)
		}
		if let activeStats {
	...
	static func extractBaseTypesNonTS(_ rawType: String, language: LanguageType) -> [String] {
		...
		let filtered = filterOutPrimitiveAndSpecialTypes(deduped, language: language)
		return filtered
	}
	...
	private static func extractBaseTypesNonTSCached(
		...
		let deduped = Array(Set(results.map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }))
		let filtered = filterOutPrimitiveAndSpecialTypesCached(deduped, language: language, cache: &cache, stats: stats)
		return filtered
	}
	...
	private static func extractBaseTypesTSCached(
		...
		for piece in splitted {
			...
			if trimmedPiece.hasPrefix("{") {
				...
				if shouldCacheSplitPieces {
					extracted = extractBaseTypesCached(from: trimmedPiece, language: language, cache: &cache, stats: stats)
				} else {
					extracted = extractTSObjectLiteralTypesCached(from: trimmedPiece, language: language, cache: &cache, stats: stats)
				}
				finalTypes.append(contentsOf: extracted)
			} else {
				let subTypes: [String]
				if shouldCacheSplitPieces {
					subTypes = extractBaseTypesCached(from: trimmedPiece, language: language, cache: &cache, stats: stats)
				} else {
					subTypes = extractBaseTypesNonTSCached(trimmedPiece, language: language, cache: &cache, stats: stats)
				}
				finalTypes.append(contentsOf: subTypes)
			}
		}
	...
	private static func removeMethodCalls(_ raw: String) -> String {
		...
		//  3) (optional) `<...>` generics
		//  4) (optional) `(...)` call
		return regexReplace(RegexCache.methodCall, in: raw)
	}
	...
	private static func removeInlineComments(from type: String, language: LanguageType) -> String {
		...
		case .php:
			...
			if let cutIndex = [slashIndex, hashIndex].compactMap({ $0 }).min() {
	...
	private static func findTopLevelChar(in type: String, char: Character) -> String.Index? {
		return TopLevelScanner.firstTopLevelIndex(of: char, in: type, track: .all)
	}
	...
	static func isGenericPlaceholderTypeName(_ typeName: String, language: LanguageType) -> Bool {
		return isGenericPlaceholder(typeName, language: language)
	}
	...
	static func isSwiftSpecialTypeName(_ typeName: String) -> Bool {
		return isSwiftSpecialType(typeName)
	}
