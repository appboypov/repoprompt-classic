...
struct TypeScriptCodeMapStrategy {
	...
	static func useRangeContainment(_ context: Context) -> Bool {
		return !context.containerBoundaries.isEmpty
	}
	...
	static func handleCapture(
		...
		case "call_signature", "construct_signature", "index_signature":
			...
			case "index_signature":
				fallbackName = "index"
			default:
				fallbackName = nsContent.substring(with: cap.range)
			}
	...
	private static func lineNumber(for location: Int, using boundaries: [Int]) -> Int {
		return CodeMapGenerator.lineNumber(for: location, using: boundaries)
	}
	...
	private static func extractTSHeritageTypes(from declLine: String) -> [String] {
		...
		let head: String
		if let braceIndex = TopLevelScanner.firstTopLevelIndex(of: "{", in: trimmed, track: .all) {
			head = String(trimmed[..<braceIndex])
		} else {
			head = trimmed
		}
		var results: [String] = []
	...
	private static func findTopLevelChar(in input: String, char: Character) -> String.Index? {
		return TopLevelScanner.firstTopLevelIndex(of: char, in: input, track: .all)
	}
}
