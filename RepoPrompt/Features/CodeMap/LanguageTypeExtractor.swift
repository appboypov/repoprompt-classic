	...
	case .ts, .tsx:
		...
		case .accessor:
			if let m = tsAccessorRegex.firstMatch(in: line) {
				...
				let paramTypes = paramList.map { LanguageTypeExtractor.parseTSParameterList($0) }
				let fullName = [prefix, name].compactMap { $0 }.joined(separator: " ")
				return FunctionLineMatch(
...
extension LanguageTypeExtractor {
	enum TS {
		static func extractTypeAnnotation(from line: String) -> String? {
			return LanguageTypeExtractor.extractTSTypeAnnotation(from: line)
		}
		
		static func extractTypeAliasRHS(from line: String) -> String? {
			return LanguageTypeExtractor.extractTSTypeAliasRHS(from: line)
		}
		
		static func extractReturnType(from signature: String, stats: CodeMapPerfStats? = nil) -> String? {
			return LanguageTypeExtractor.extractTSReturnType(from: signature, stats: stats)
		}
		
		static func parseParameterTypes(from paramList: String) -> [String] {
			return LanguageTypeExtractor.parseTSParameterList(paramList)
		}
	}
}
