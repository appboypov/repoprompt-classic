...
struct CodeMapGenerator {
	...
	private static func shouldSkipSyntheticMainClass(for language: LanguageType) -> Bool {
		...
		case .swift, .ts, .tsx, .js, .c, .cpp, .go, .rust, .python, .php, .ruby, .dart:
			return true  // These languages should populate functions[] and globalVars[] directly
		default:
			return false
		}
	}
	...
	static func generateCodeMap(from namedRanges: [NamedRange],
								content: String,
								fullPath: String,
								perfOptions: CodeMapPerfOptions = .disabled,
								perfStats: CodeMapPerfStats? = nil) -> FileAPI? {
		...
		// ------------------------------------------------------------------
		struct ClassBoundary { let startLine: Int; let name: String; let range: NSRange? }
		...
		struct InterfaceBoundary { let startLine: Int; let name: String; let range: NSRange? }
		...
		struct RustImplBoundary { let range: NSRange; let targetTypeName: String; let startLine: Int }
		...
		func classKey(forTypeName name: String) -> Int? {
			return classesByLine.first(where: { $0.value.name == name })?.key
		}
		...
		for cap in sortedCaps {
			...
			case "import", "import.module", "import.namespace":
				...
				if !seenImportExport.contains(fullLine) {
					imports.append(fullLine); seenImportExport.insert(fullLine)
					if debugLogging {
			...
			case "export", "export.source":
				...
				if !seenImportExport.contains(fullLine) {
					exports.append(fullLine); seenImportExport.insert(fullLine)
					if debugLogging {
					...
					if let match = lightweightMatch {
						...
						if let pt = match["parameterTypes"], !pt.isEmpty {
							...
							params = types.enumerated().map {
								ParameterInfo(externalName: nil,
			...
			case "variable.global", "variable.field", "field",
				...
				if isLightweightLang {
					...
					if isJSTS {
						...
						if isFunctionValued && cap.name == "variable.global" {
							...
							let fnName: String
							if let match = firstRegexGroup(RegexCache.jstsVarNameExtract, in: trimmedLine) {
								fnName = match
							} else {
								fnName = fullDecl
							}
				...
				let variableName: String
				if isJSTS, cap.name == "variable.global" {
					variableName = extractJSTSVariableName(from: trimmedLine)
						?? extractJSTSVariableName(from: fullDecl)
						?? simpleName
						?? matchedName
					...
					variableName = Self.extractGoVariableName(captureText: capturedText, decl: fullDecl)
						?? matchedName
						?? simpleName
						?? fullDecl
				} else if supportedLanguage == .c {
					variableName = Self.extractCVariableName(captureText: capturedText, decl: fullDecl)
						?? matchedName
						?? simpleName
						?? fullDecl
				} else {
					variableName = matchedName ?? simpleName ?? fullDecl
				}
		...
		for (_, cls) in classesByLine where cls.name == mainClassName { mainClassFound = true }
		...
		if !skipSyntheticClass {
			if mainClassFound {
				...
				if shouldCreateSynthetic {
					classesByLine[-1] = ClassInfo(name: mainClassName,
		...
		let api = FileAPI(filePath:      fullPath,
							imports:       imports,
							exports:       exports,
	...
	static func lineNumber(for location: Int, using boundaries: [Int]) -> Int {
		return lineIndex(for: location, using: boundaries) + 1
	}
