...
private actor CodeScanAsyncLimiter {
	...
	private func cancelWaiter(id: UUID) {
		if case .waiting(let continuation)? = waiterStates.removeValue(forKey: id) {
			continuation.resume(throwing: CancellationError())
...
actor CodeScanActor {
	...
	private var progressCoalescingTask: Task<Void, Never>? = nil

	// Changed to 2 seconds
	private let progressCoalescingDelay: UInt64 = 2_000_000_000

	private var lastKnownProgress: (Int, Int)? = nil

	// New: store a pending progress that we’ll flush in one shot
	private var pendingProgress: (Int, Int)? = nil
	...
	private var resultsCoalescingTask: Task<Void, Never>? = nil
	...
	// -------------------------------------
	enum ScanBatchPurpose: Sendable {
	...
	struct ScanResult: @unchecked Sendable {
		...
		init(request: ScanRequest, fileAPI: FileAPI?) {
			self.fileID = request.fileID
			self.modificationDate = request.modificationDate
			self.fileExtension = request.fileExtension
	...
	#if DEBUG
	struct CodemapMemoryCounters: Sendable {
	...
	// -------------------------------------------------------
	public nonisolated func subscribeToScanResults() -> AsyncStream<[ScanResult]> {
		let id = UUID()
	...
	// -------------------------------------------------------
	public nonisolated func subscribeToProgress() -> AsyncStream<(Int, Int)> {
		let id = UUID()
		return AsyncStream { continuation in
	...
	private func cancelRootCacheLoads<S: Sequence>(forRootKeys rootKeys: S) where S.Element == String {
	...
	private func advanceRootGenerations<S: Sequence>(forRootKeys rootKeys: S) where S.Element == String {
		for rootKey in rootKeys {
	...
	@discardableResult
	private func removeBufferedResults<S: Sequence>(forRootKeys rootKeys: S) -> Int where S.Element == String {
	...
	// -------------------------------------------------------
	public func cancelAndUnloadScans(forRootFolder rootFolderPath: String) async {
		let rootKey = canonicalRoot(rootFolderPath)
		advanceRootGenerations(forRootKeys: [rootKey])

		// Remove any queued scan requests
		let queuedToRemove = queue.filter { canonicalRoot($0.rootFolderPath) == rootKey }.count
	...
	// -------------------------------------------------------
	public func cancelAndUnloadScans(forRootFolders rootFolderPaths: [String]) async {
	...
	private func checkCacheAndHandleResult(
		...
		if initialRootLookupFiles != nil {
			rootCache = rootCaches[rootKey]
		} else {
			rootCache = await ensureRootCacheLoaded(forRootKey: rootKey)
		}
		guard rootGenerationMatches(rootKey: rootKey, expected: expectedRootGeneration) else {
			return .staleRoot
		}
	...
	private func removeCacheEntry(relativePath: String, rootKey: String) {
		guard var rootEntry = rootCaches[rootKey],
			rootEntry.files.removeValue(forKey: relativePath) != nil else {
			return
		}
	...
	// -------------------------------------------------------
	public func requestScan(_ request: ScanRequest) async {
	...
	public func requestScans(
		...
		let newFilesCount = requestsToQueue.filter { !acceptedAPIFileIDs.contains($0.fileID) }.count
	...
	// -------------------------------------------------------
	public nonisolated func requestScan(_ request: ScanRequest) {
		Task { await self.requestScan(request) }
	}

	public nonisolated func requestScans(
		_ requests: [ScanRequest],
		purpose: ScanBatchPurpose = .adhoc,
		rootFolderPaths: [String] = [],
		purgeCachesOnEmptyInitialRequests: Bool = false
	...
	public func clearAllCaches(rootFolders: [String]) {
	...
	public func purgeStaleRootCaches(keepingRootPaths: [String]) {
	...
	private func removeTrackedFiles<S: Sequence>(forRootKeys rootKeys: S) where S.Element == String {
		for rootKey in rootKeys {
	...
	struct RootCacheDiskLoadCounters: Sendable {
		let count: Int
		let duration: TimeInterval
		let loadedFileEntryCount: Int
	}

	struct RootCacheDiskSaveCounters: Sendable {
		let count: Int
		let duration: TimeInterval
