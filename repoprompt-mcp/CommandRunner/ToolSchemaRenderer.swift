...
enum AllowedValuesScope: Sendable {
...
struct ToolParameterVariantDoc: Sendable {
...
struct ToolParameterDoc: Sendable {
    let name: String
    let typeDescription: String
    let required: Bool
    let description: String?
...
struct ToolSchemaVariantDoc: Sendable {
    let title: String?
    let description: String?
...
enum ToolSchemaIntrospector {
    ...
    private static func extractRootVariants(_ obj: [String: Value]) -> [ToolSchemaVariantDoc] {
        // Check for oneOf
        if let oneOf = obj["oneOf"], case .array(let options) = oneOf {
            return options.compactMap { extractVariant(from: $0) }
        }
        // Check for anyOf
        if let anyOf = obj["anyOf"], case .array(let options) = anyOf {
            return options.compactMap { extractVariant(from: $0) }
        }
        return []
    }
    ...
    private static func extractParameter(name: String, value: Value?, required: Bool) -> ToolParameterDoc {
        ...
        if allowedValues == nil,
           let typeValue = obj["type"], case .string(let t) = typeValue, t == "array",
           let items = obj["items"], case .object(let itemObj) = items {
    ...
    private static func extractParameterVariants(_ obj: [String: Value]) -> [ToolParameterVariantDoc] {
        var options: [Value] = []
        if let oneOf = obj["oneOf"], case .array(let arr) = oneOf {
            options = arr
        } else if let anyOf = obj["anyOf"], case .array(let arr) = anyOf {
            options = arr
        }
        ...
        return options.compactMap { opt -> ToolParameterVariantDoc? in
            guard case .object(let optObj) = opt else { return nil }
    ...
    private static func formatType(_ type: String, _ obj: [String: Value]) -> String {
        switch type {
        case "array":
            if let items = obj["items"], case .object(let itemObj) = items {
                let itemType = extractTypeDescription(itemObj)
                return "array<\(itemType)>"
            }
            return "array"
        case "object":
            if let additionalProps = obj["additionalProperties"], case .object(let addObj) = additionalProps {
    ...
    private static func extractChildParameters(_ obj: [String: Value]) -> [ToolParameterDoc] {
        ...
        if let typeValue = obj["type"], case .string(let t) = typeValue, t == "object" {
            let requiredNames = extractStringArray(obj["required"])
            let properties = extractObject(obj["properties"])
        ...
        if let typeValue = obj["type"], case .string(let t) = typeValue, t == "array" {
            if let items = obj["items"], case .object(let itemObj) = items {
                // If items is an object type with properties, show those
                if let itemType = itemObj["type"], case .string(let it) = itemType, it == "object" {
