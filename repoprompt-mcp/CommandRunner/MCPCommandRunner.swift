...
actor MCPCommandRunner {
    ...
    init(
        ...
        self.parseContext = CommandParseContext(currentDirectory: initialDirectory)
    ...
    func runLine(_ line: String) async -> LineExecutionSummary {
        ...
        for segment in parsed.segments {
            ...
            do {
                ...
                if case .unknownCommand(let cmd) = error,
        ...
        let succeeded: Bool
        switch settings.exitCodeMode {
        case .anyFailure:
            succeeded = failedCount == 0
        case .lastSegment:
            succeeded = lastSegmentSucceeded
        }

        return LineExecutionSummary(
            line: line,
            succeeded: succeeded,
    ...
    func runLines(_ lines: [String]) async -> (allSucceeded: Bool, summaries: [LineExecutionSummary]) {
    ...
    private func printToolList(mode: ToolListMode) async throws {
        ...
        case .groups(let groups):
            ...
            let groupNames = groups.map { $0.rawValue }
    ...
    private func callTool(name: String, jsonPayload: String?) async throws {
        ...
        if name == "context_builder", var argsDict = args {
            ...
            if case .string(let value) = argsDict["instructions"] {
                hasInstructions = !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            } else {
    ...
    private func callToolWithArgs(name: String, args: [String: AnyCodableSendable]) async throws {
    ...
    private func snapshotTools(to path: String) async throws {
        ...
        let snapshot = ToolSnapshot(
            generatedAt: Date(),
            serverName: await session.serverName ?? "unknown",
            serverVersion: await session.serverVersion ?? "unknown",
    ...
    private static func isRawJSONResult(_ result: CallTool.Result) -> Bool {
        ...
        guard case .text(let text, _, _) = result.content[0] else { return false }
    ...
    private func printStatus() async {
        ...
        // Connection
        let serverInfo = "\(await session.serverName ?? "?") v\(await session.serverVersion ?? "?")"
    ...
    private func printHelp() async {
        ...
        Conversation:
          oracle_send                    Send oracle conversation turn
            ...
            mode         (string)        chat|plan|edit|review
          ...
          ask_user {"questions":[...]}    Ask structured clarifying questions (answers keyed by id)
        ...
        Agent Control:
          agent_run                       Control Agent Mode runs
            ...
            op=respond session_id="..." interaction_id="..." response="..."
                                          Resolve scalar approvals / legacy single-response prompts
            op=respond session_id="..." interaction_id="..." answers={"qid":{"answers":["..."]}}
                                          Resolve structured question input; use skip=true to skip
          ...
          agent_manage                    Discover agents, manage sessions, browse workflows
        ...
        ═══════════════════════════════════════════════════════════════
          ...
          tools <groups>                  Filter by groups (binding,context,explore,edit,conversation,settings)
        ...
        ═══════════════════════════════════════════════════════════════
          ...
          agent_run op=respond session_id="<session-uuid>" interaction_id="<id>" answers={"scope":{"answers":["UI"]}}
          agent_run op=respond session_id="<session-uuid>" interaction_id="<id>" skip=true
    ...
    private func multiWindowRoutingSection() -> String {
        ...
        rp-cli --context-id <uuid> -e 'context'     Bind a specific compose context
...
struct ToolSnapshot: Codable {
    ...
    struct ToolEntry: Codable {
        ...
        init(from tool: MCP.Tool) {
            ...
            if hasAnnotations {
                self.annotations = AnnotationsEntry(from: ann)
            } else {
    ...
    struct AnnotationsEntry: Codable {
        ...
        init(from ann: MCP.Tool.Annotations) {
            ...
            self.readOnlyHint = ann.readOnlyHint
            self.destructiveHint = ann.destructiveHint
            self.idempotentHint = ann.idempotentHint
            self.openWorldHint = ann.openWorldHint
        }
    }
}
