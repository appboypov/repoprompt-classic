...
// Enable via: defaults write com.repoprompt.mcp enableSocketDebugLog -bool true
private let enableSocketDebugLog: Bool = {
	ProcessInfo.processInfo.environment["MCP_SOCKET_DEBUG"] == "1" ||
	UserDefaults.standard.bool(forKey: "enableSocketDebugLog")
}()

private let debugLogURL: URL = {
	let url = FileManager.default.homeDirectoryForCurrentUser
		.appendingPathComponent("Library/Application Support/RepoPrompt/socket-proxy-debug.log")
...
enum MCPCLIRuntimeError: Swift.Error {
...
enum CLIEventLogger {
	...
	static func shouldPersistEvent(for err: MCPCLIRuntimeError) -> Bool {
		switch err {
	...
	static func logRuntimeError(_ err: MCPCLIRuntimeError) {
	...
	private static func mapRuntimeError(_ err: MCPCLIRuntimeError) -> (Event.Code, String, [String: String]?) {
		switch err {
		case .connectionFailed(let underlying):
			...
			if let socketError = underlying as? SocketProxyError {
				...
				case .connectFailed(let errno),
					 .socketCreationFailed(let errno),
					 .bindFailed(let errno),
					 .listenFailed(let errno),
					 .acceptFailed(let errno),
					 .writeFailed(let errno),
					 .readFailed(let errno),
					 .pollFailed(let errno):
					details["errno"] = String(errno)
				case .connectionRefused:
					details["errno"] = String(ECONNREFUSED)
				case .stdoutWriteTimeout(let bytesWritten, let totalBytes, let stallTimeout):
				...
				case .stdoutBrokenPipe(let bytesWritten, let totalBytes):
					details["layer"] = "stdout_bridge"
		...
		case .terminatedByServer(let reason):
...
enum CLIKillSignal {
	...
	/// Termination reasons (mirrors app-side TerminationReason)
	...
	static func messageForReason(_ reason: Reason) -> String {
		...
		case .userBootFromDashboard:
			return "Connection terminated by user from MCP dashboard"
		case .runCompleted:
		...
		case .idleTimeout:
			return "Connection closed due to idle timeout"
		case .approvalDenied:
			return "Connection approval was denied"
		case .connectionReplaced:
			return "Connection replaced by a newer connection"
		}
	}
}
...
enum SocketProxyError: Swift.Error, LocalizedError {
	...
	var errorDescription: String? {
		...
		case .handshakeRejected(let errorCode, let reason):
...
actor BootstrapSocketProxy {
	...
	init(sessionToken: String, clientName: String?, identityCache: ClientIdentityCache) {
		self.socketURL = MCPFilesystemConstants.bootstrapSocketURL()
...
private enum BridgeTaskExit: Sendable {
	case stdinClosed
	case socketClosed
}
...
private extension BootstrapSocketProxy {
	...
	static func extractProgressMessage(from jsonLine: Data) -> String? {
		...
		guard let str = String(data: jsonLine, encoding: .utf8),
			str.contains("repoprompt/control/progress") else {
			return nil
		}
	...
	private static func mapStdoutWriteError(_ error: NonBlockingFDWriteError) -> SocketProxyError {
		...
		case .brokenPipe(let bytesWritten, let totalBytes):
			return .stdoutBrokenPipe(bytesWritten: bytesWritten, totalBytes: totalBytes)
		case .localTimeout(let stallTimeout, let bytesWritten, let totalBytes):
			return .stdoutWriteTimeout(
				bytesWritten: bytesWritten,
				totalBytes: totalBytes,
				stallTimeout: stallTimeout
			)
		case .pollFailed(let errno), .fcntlFailed(let errno):
			return .pollFailed(errno: errno)
		case .writeFailed(let errno, _, _):
...
actor MCPService: Service {
	...
	func run() async throws {
		...
		try await withThrowingTaskGroup(of: Void.self) { group in
			...
			group.addTask {
				...
				throw MCPCLIRuntimeError.terminatedByServer(
	...
	private func runPPIDWatchdog(initialPPID: pid_t) async throws {
		...
		while !Task.isCancelled {
			...
			if currentPPID != initialPPID {
				...
				throw MCPCLIRuntimeError.hostDisconnected
			}
		}
	}
	...
	private func shouldRetry(after error: MCPCLIRuntimeError) -> Bool {
		switch error {
		case .approvalDenied,
			 .hostDisconnected,
			 .terminatedByServer:
		...
		case .connectionFailed(let underlying):
			...
			case .handshakeRejected(let errorCode, _):
	...
	private func runTransport() async throws {
		...
		let aggressiveDelay: Double = 0.5
		let aggressivePhaseDuration: Double = 60.0

		// Phase 2: Exponential backoff
		let backoffBase: Double = 1.0
		...
		while !Task.isCancelled {
			do {
				...
				return
			} catch let err as MCPCLIRuntimeError {
				...
				throw MCPCLIRuntimeError.connectionFailed(underlying: error)
			}
		}
	...
	private func runSocketLoop() async throws {
		...
		do {
			...
			switch err {
			case .approvalDenied:
				throw MCPCLIRuntimeError.approvalDenied
			case .hostDisconnected:
			...
			case .stdoutWriteTimeout:
				throw MCPCLIRuntimeError.connectionFailed(underlying: err)
			...
			case .terminatedByServer(let reason):
				...
				throw MCPCLIRuntimeError.terminatedByServer(reason: reason)
			case .handshakeFailed(let reason):
...
func handleRuntimeError(_ err: MCPCLIRuntimeError) -> Never {
...
func parseCLIMode() -> CLIMode {
    ...
    while i < args.endIndex {
        ...
        default:
            ...
            // This allows: rp-cli tree, rp-cli search "pattern", etc.
...
func printUsage() {
    ...
    MCP TOOLS WITH EXAMPLES (use -d <tool> for full parameter docs):
        ...
        ask_oracle (chat, plan, review) - Send/continue an oracle conversation
          ...
          ask_oracle message="Fix bug" mode=edit new_chat=true
          ask_user {"questions":[{"id":"scope","question":"Scope?"}]}
        ...
        agent_run - Session-based agent run control (advanced, policy-gated)
          ...
          agent_run op=respond session_id="<uuid>" interaction_id="<id>" answers={"scope":{"answers":["UI"]}}
          agent_run op=respond session_id="<uuid>" interaction_id="<id>" skip=true
    ...
    MULTI-WINDOW ROUTING:
        ...
        rp-cli -e 'windows'                          List windows, tabs, and context_id values
        rp-cli -w <id> -e 'context'                  Bind a window for this invocation
        rp-cli --context-id <uuid> -e 'context'      Bind a compose context directly
        rp-cli -w <id> -t <tab-or-uuid> -e 'context' Resolve/bind a tab in one step
    ...
    TOOL SCHEMAS - Use -d <tool> to see complete parameter documentation:
        rp-cli -d manage_selection    Show selection params
        rp-cli -d file_search         See filter options and valid values
        rp-cli -d ask_oracle          See mode options and chat workflow
        rp-cli -d app_settings        App-wide preferences operations
        rp-cli -d agent_run           Agent run control operations
        rp-cli -d agent_manage        Session/workflow management operations
    ...
    MACHINE-READABLE SCHEMAS:
        ...
        rp-cli --tools-schema=settings        Only app_settings
        rp-cli -e 'tools --schema'            Same via exec mode
        rp-cli -e 'tools explore --schema'    Filter by group via exec mode
        rp-cli -e 'tools settings --schema'   app_settings schema via exec mode
    """
    print(usage)
}
...
func printInteractiveUsage() {
    ...
    USAGE:
        rp-cli -i                  Start REPL
        rp-cli -i -w <id>          Start with window pre-selected
    ...
    IMPORTANT: When multiple windows are open, you must select a window:
        rp-cli -i -w <id>          Pre-select window at startup
    ...
    ONE-SHOT FLAGS (run and exit):
        ...
        --list-tools=<groups>      List tools in specific groups (e.g., explore,edit,settings)
        --list-tools=groups        List available tool group names
        --tools-schema             Print all tools as MCP tools/list JSON (machine-readable)
        --tools-schema=<groups>    Print tools in specific groups as JSON (e.g., settings)
    ...
    JSON ARGUMENTS (-j / --json):
        ...
        From stdin:      echo '{"path":"..."}' | rp-cli -c tool -j @-
        Auto-repair:     Multiline strings with raw newlines are auto-escaped

    EXAMPLES:
        # Explore available tools
        rp-cli -l
        rp-cli --list-tools=explore
        rp-cli --tools-schema > tools.json
        rp-cli --tools-schema=settings
        rp-cli -d file_search
        rp-cli -d agent_run                    # Agent run control schemas
        rp-cli -d agent_manage                 # Session/workflow schemas

        # Call a tool directly
        rp-cli -c read_file -j '{"path":"/tmp/test.txt"}'
...
func printScriptingUsage() {
    ...
    SCRIPT FILES (.rp):
        rp-cli --exec-file ~/scripts/export.rp
    ...
    READING FROM STDIN:
        echo 'tree' | rp-cli --exec-stdin
        cat commands.txt | rp-cli --exec-stdin

    MULTIPLE EXEC FLAGS:
        Multiple -e flags run in order (each is a separate command):

        rp-cli -e 'workspace MyProject' \\
                       -e 'select set src/' \\
                       -e 'context > out.md'
    ...
    WORKFLOW SHORTHAND FLAGS:
        ...
        Is equivalent to:
        rp-cli -e 'workspace switch "MyProject" && select set src/ && context --all > "~/out.md"'
    ...
    MULTI-WINDOW NOTE:
        ...
        Example:
        rp-cli -w 1 -t MyTab -e 'select set src/ && context > out.md'
...
func printAdvancedUsage() {
    ...
    TAB TARGETING:
        ...
        rp-cli -w 1 -t MyTab -e 'context'
        rp-cli --context-id <uuid> -e 'chat Hello'

        # Or pass hidden routing params directly to any tool call
        rp-cli -e 'call get_file_tree {"_windowID":1,"context_id":"<uuid>"}'
    ...
    HIDDEN ROUTING PARAMETERS:
        ...
        Example:
        rp-cli -e 'call read_file {"path":"src/main.swift","_windowID":2,"context_id":"<uuid>"}'
    ...
    CONNECTION BINDING VS PARAMETER PASSING:
        ...
        CRITICAL FOR AI AGENTS:
          ...
          Example workflow:
          rp-cli -e 'windows'                    # Discover windows and context_id values
          rp-cli --context-id <uuid> -e 'context' # Get context from a specific tab
...
func printVersion() {
    print("rp-cli (repoprompt-mcp) \(CLI_VERSION)")
}

/// Launches the RepoPrompt app from /Applications.
func launchRepoPromptApp() {
    let process = Process()
    process.executableURL = URL(fileURLWithPath: "/usr/bin/open")
    process.arguments = ["/Applications/Repo Prompt.app"]
    do {
        try process.run()
        process.waitUntilExit()
    } catch {
        fputs("Error: Failed to launch Repo Prompt.app: \(error)\n", stderr)
        exit(1)
    }
}
...
#if DEBUG
/// Recursively unwraps AnyCodableSendable values for JSON serialization.
...
fileprivate func unwrapForSerialization(_ value: Any) -> Any {
    ...
    case let array as [AnyCodableSendable]:
...
func testParseCommand(_ input: String) {
    ...
    do {
        ...
        case .toolsSchema(let mode):
            result["command"] = "toolsSchema"
            result["mode"] = String(describing: mode)
        case .describe(let toolName):
            result["command"] = "describe"
            result["toolName"] = toolName
        case .call(let toolName, let jsonPayload):
            result["command"] = "call"
            result["toolName"] = toolName
            result["jsonPayload"] = jsonPayload as Any
        case .aliasCall(let toolName, let args):
            result["command"] = "aliasCall"
            result["toolName"] = toolName
            // Convert args to serializable format (recursively unwrap AnyCodableSendable)
...
case .proxy:
    ...
    if stdinIsTTY || stdoutIsTTY || (hasNoUserArgs && (!stdinLooksLikeMCPTransport() || stdinHasImmediateDisconnect())) {
        ...
        Quick start:
          rp-cli -l                    # List available tools
          rp-cli -e 'tree'             # Execute a command
          rp-cli -i                    # Interactive REPL
          rp-cli --help                # Full help
...
let lifecycleLogger: Logger = {
    var logger = Logger(label: "ServiceLifecycle") { _ in
...
do {
    ...
    case .scriptReadError(let underlying):
