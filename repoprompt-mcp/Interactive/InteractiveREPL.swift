...
struct InteractiveOptions: Sendable {
...
struct REPLSettings {
    ...
    var outputFile: String? = nil  // Temporary redirect for single command
}
...
actor InteractiveREPL {
    ...
    private var cachedWorkspaceName: String? = nil
    ...
    init(session: InteractiveMCPClientSession, options: InteractiveOptions) {
        ...
        self.settings = REPLSettings(prettyJSON: options.prettyJSON, verbose: options.verbose)
    ...
    private func refreshStatusCache() async {
        ...
        do {
            ...
            if let first = result.content.first, case .text(let text, _, _) = first {
    ...
    private func printHelp() async {
        ...
        \u{001B}[4mEditing:\u{001B}[0m
          \u{001B}[32mcall apply_edits\u{001B}[0m {...}         Find/replace (JSON args required)
                                          ...
                                          Stdin:   echo '...' | rp-cli -c apply_edits -j @-
        ...
        \u{001B}[4mConversation:\u{001B}[0m
          ...
          \u{001B}[32mask_user\u{001B}[0m {"questions":[...]}       Ask structured clarifying questions
        ...
        \u{001B}[4mAgent Control (advanced, policy-gated):\u{001B}[0m
          ...
          \u{001B}[32magent_run\u{001B}[0m op=respond session_id="..." interaction_id="..." response="..."
                                          Respond to scalar approval / legacy prompt
          \u{001B}[32magent_run\u{001B}[0m op=respond session_id="..." interaction_id="..." answers={"qid":{"answers":["..."]}}
                                          Respond to structured questions; use skip=true to skip
    ...
    private func multiWindowRoutingHelpSection() -> String {
        ...
        rp-cli -e 'windows'                            \u{001B}[2mDiscover windows and context_id values\u{001B}[0m
        rp-cli \u{001B}[33m-w <id>\u{001B}[0m -e 'context'                    \u{001B}[2mBind a window for this invocation\u{001B}[0m
        rp-cli \u{001B}[33m--context-id <uuid>\u{001B}[0m -e 'context'        \u{001B}[2mBind a compose context directly\u{001B}[0m
        rp-cli \u{001B}[33m-w <id> -t <tab-or-uuid>\u{001B}[0m -e 'context'   \u{001B}[2mResolve/bind a tab in one step\u{001B}[0m
    ...
    private func endOutputRedirect() {
        if case .file(_, let handle) = outputSink {
            try? handle.close()
        }
        outputSink = .stdout
    }
